...
WHAT THE RUNNER DOES FOR YOU, each verifiable in `oc_runipd.py` by symbol: the queue is sorted with DEPENDENCY DEPTH as the FIRST sort key (`queue_sort_key`, `dependency_depth`), so a dependent never precedes its dependency; dependencies are RE-CHECKED AT DISPATCH, not only at queue build, and an unmet edge marks that one item `dependency-blocked` and continues instead of failing the run; an ORCHESTRATOR is not agent-executed but self-finalizes once every child of its Set reached `executed` (`action_for` returns `orchestrate`, and the dispatcher calls `_set_children_all_executed` then `finalize_orchestrator`), so an Order-0 parent in the queue is correct and needs no human step; and each execute item gets its own ISOLATED WORKTREE by default (`isolate_worktree`, opt out with `--no-isolate-worktree`) whose changes return through the merge-and-revalidate gate, which is why two plans naming the same file is not a hazard even in PARALLEL, let alone serially.
SO: to answer "what can I run unattended?", report the plans whose `- Status:` is `approved` and say `aw oc run all` (or a setid). Legitimate things to raise instead: a plan whose SCOPE the human may not want executed yet, an `- Item-Dependencies:` edge pointing at something that will never execute, a genuinely OPEN `- Blocking: yes` question, or a capability `aw host capabilities` reports as REFUSED for the action class. Do NOT raise file overlap, queue order, or orchestrator finalization: those are solved, and asserting otherwise is a claim about code you have not read.
