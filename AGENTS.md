...
SO: to answer "what can I run unattended?", report the plans whose `- Status:` is `approved` and say `aw oc run all` (or a setid). Legitimate things to raise instead: a plan whose SCOPE the human may not want executed yet, an `- Item-Dependencies:` edge pointing at something that will never execute, a genuinely OPEN `- Blocking: yes` question, a capability `aw host capabilities` reports as REFUSED for the action class, or a REFUSED orchestrator retirement with the reason the run recorded. Do NOT raise file overlap, queue order, or an Order-0 parent's PLACEMENT in the queue: those are solved, and asserting otherwise is a claim about code you have not read.
