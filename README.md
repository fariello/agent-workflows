...
| `/ipd-lifecycle` | Authoritative gate for **beginning execution** of an approved IPD and its **terminal transition**: runs `aw ipd lint` at pre-execution/pre-transition/post-transition and fails closed; defines the terminal transaction (history, Status, `git mv`, path-scoped commit) as a post-gate step. The in-between sibling of `/plan-review` and `/verify-execution`. | Executes the approved plan; never approves or tags/releases |
