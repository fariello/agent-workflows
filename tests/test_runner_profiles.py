...
class SourceAuditTests(unittest.TestCase):
    ...
    #: (the forbidden construct, its regex, why this row exists)
    FORBIDDEN_CONSTRUCTS = (
        (
            "shell=True",
            r"shell\s*=\s*True",
            "a shell invocation makes every stored string a potential command fragment, which is "
            "the exact risk the narrow `MODEL_RE` character class exists to contain. With a shell "
            "in the module, that containment stops being sufficient",
        ),
        (
            "eval(",
            r"\beval\s*\(",
            "`eval` on any stored value turns the config file into code, so a store an attacker can "
            "write becomes a store an attacker can run",
        ),
        (
            "exec(",
            r"\bexec\s*\(",
            "the same for statements rather than expressions",
        ),
        (
            "subprocess",
            r"\bsubprocess\b",
            "THE SCOPE FENCE: this module DECIDES a launch identity and never performs a launch. A "
            "subprocess call here would put process spawning behind a validator, where a future "
            "reader would reasonably assume the validation covered the spawn",
        ),
        (
            "os.system",
            r"\bos\.system\b",
            "the shell route that bypasses argv entirely, so a single string reaches `/bin/sh`",
        ),
        (
            "os.popen",
            r"\bos\.popen\b",
            "the same via a pipe, and the one most likely to be reached for by someone adding a "
            "'quick' model-catalog probe to this module rather than to the wizard where it belongs",
        ),
        (
            "os.exec*",
            r"\bos\.exec",
            "the direct exec family, which replaces the process image and so escapes every later "
            "check in this module",
        ),
        (
            "__import__(",
            r"\b__import__\s*\(",
            "dynamic import by name would let a stored string select code to run, which is the "
            "indirect form of the same defect `eval` is the direct form of",
        ),
        (
            "os.environ",
            r"os\.environ",
            "THE CREDENTIAL FENCE, and it is a different claim from the ones above: the module must "
            "not READ a credential from the environment either, because a value it never reads is a "
            "value it can never accidentally persist into the store it writes",
        ),
        (
            "getenv",
            r"getenv",
            "the second spelling of the same access, so the fence is not bypassed by style. NOTE the "
            "path accessors legitimately need XDG, which is why they live in `config.py` and are "
            "IMPORTED rather than reimplemented here; that structure is what makes this row hold",
        ),
    )

    def test_no_prohibited_construct_appears_in_executable_code(self):
        """One table over every construct V-04 forbids, replacing two regex-sweep tests.

        The two tests it replaces (`no_shell_true_no_eval_no_exec_no_subprocess` and
        `the_module_does_not_read_credentials_from_the_environment`) ran the SAME sweep over the same
        `code_lines` and differed only in the pattern list, so the second was really two more rows.

        THE TWO CLASSES OF CONSTRUCT ARE ROWS IN ONE TABLE because the audit is one mechanism, and
        keeping them apart hid that: the environment rows were a second sweep whose vacuity depended
        on the same `setUp` guard while stating a different fence (reading a credential, versus
        running a command). Each row now records WHICH fence it is, so a failure says what became
        possible rather than only which regex matched.
        """

        wrong = []
        for construct, pattern, why in self.FORBIDDEN_CONSTRUCTS:
            hits = [
                line.strip() for line in self.code_lines if re.search(pattern, line)
            ]
            if hits:
                wrong.append(
                    f"  {construct} appears in executable code:\n"
                    + "".join(f"    - {h}\n" for h in hits)
                    + f"    this row exists because: {why}"
                )
        self.assertEqual(
            wrong,
            [],
            f"{len(wrong)} of {len(self.FORBIDDEN_CONSTRUCTS)} prohibited constructs are now present "
            "in `runner_profiles.py`. These rows are NOT stylistic: this module validates strings that "
            "name a model and a profile, and its whole safety argument is that a stored string can "
            "never become a command or a credential. FIX: if a construct is genuinely needed, it "
            "belongs in the module that PERFORMS the launch, not in the one that decides it; adding "
            "it here means a future reader will assume this module's validation covers the execution. "
            "If a match is in a COMMENT or DOCSTRING, `setUp`'s AST-based exclusion has broken rather "
            f"than the module, and the fix is there and not a prose edit.\n"
            + "\n".join(wrong),
        )
