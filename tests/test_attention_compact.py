...
class AttentionCompactTests(unittest.TestCase):
    ...
    def test_common_prefix_in_header_and_bare_names(self):
        ...
        self.assertRegex(out, r"open\s+backlog\s+-\s+-\s+-\s+0\s+0\s+a")
        self.assertRegex(out, r"open\s+backlog\s+-\s+-\s+-\s+0\s+0\s+b")
    ...
    def test_unknown_and_gate_markers(self):
        ...
        self.assertRegex(
            out,
            r"blocked\s+backlog\s+-\s+-\s+-\s+0\s+0\s+a\s+\[gate artifact: TODO.md\]",
        )
