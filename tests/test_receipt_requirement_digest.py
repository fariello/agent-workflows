...
class FrozenRegionComparisonTests(_WideningFixture):
    ...
    def test_ONE_payload_builder_is_shared_with_frozen_region_digest(self):
        ...
        digest_src = inspect.getsource(LC.frozen_region_digest)
        compare_src = inspect.getsource(LC.frozen_region_comparison)
        for name, src in (
            ("frozen_region_digest", digest_src),
            ("frozen_region_comparison", compare_src),
        ):
            self.assertIn(
                "_frozen_region_payload(",
                src,
                f"{name} must build its payload through the shared builder",
            )
            self.assertNotIn(
                '"requirements":',
                src,
                f"{name} carries its OWN copy of the payload literal, which can drift",
            )
