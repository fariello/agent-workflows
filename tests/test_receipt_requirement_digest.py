...
class FrozenRegionComparisonTests(_WideningFixture):
    ...
    def test_ONE_payload_builder_is_shared_with_frozen_region_digest(self):
        """E-03: a SECOND copy of the payload literal is a silent-drift hazard on a gate input.

        Asserted as behavior rather than described: the digest must be reproducible by calling the
        shared builder and hashing it the same way, and neither function may carry its own copy of the
        `{"scope_paths": ..., "requirements": ...}` literal. A drifted copy would make the substitution
        never match, so the accept would never fire and the bug would look like "it does not work".

        THE SOURCE-TEXT HALF IS REPLACED BY A SPY PLUS A SENTINEL. It read
        `inspect.getsource(...)` for both functions and asserted `"_frozen_region_payload("`
        present and `'"requirements":'` absent. Both halves were unsound: the presence half is
        satisfied by a COMMENT or a docstring naming the builder, and both docstrings here do name
        it at length, so a function that mentioned it and then inlined the literal passed; the
        absence half is a claim about one exact spelling, so a second copy assembled with a
        variable key, a dict literal built in pieces, or `dict(requirements=...)` contributed
        nothing and passed too.

        Replaced by driving the builder: it is patched to a SENTINEL payload and both consumers
        must produce the answer that sentinel implies. A consumer with a private copy is
        unaffected by the patch and therefore produces the ORIGINAL answer, which fails. That is
        the sharing claim as behavior: not that the name appears, but that the builder is the
        thing whose output reaches the digest and the substitution test.
        """
        import hashlib
        import json
        ...
        expected = hashlib.sha256(
            json.dumps(
                LC._frozen_region_payload(text), sort_keys=True, ensure_ascii=True
            ).encode("utf-8")
        ).hexdigest()
        self.assertEqual(LC.frozen_region_digest(text), expected)

        real_builder = LC._frozen_region_payload
        sentinel_payload = {
            "scope_paths": ["<sentinel>"],
            "requirements": {"probe": ["one"]},
        }
        sentinel_digest = hashlib.sha256(
            json.dumps(sentinel_payload, sort_keys=True, ensure_ascii=True).encode(
                "utf-8"
            )
        ).hexdigest()

        # (1) `frozen_region_digest` must hash the SHARED BUILDER'S output. With the builder
        # returning a sentinel, the digest must be the sentinel's hash; a private copy of the
        # payload literal would keep producing the real plan's digest.
        LC._frozen_region_payload = lambda text, **kwargs: dict(sentinel_payload)  # type: ignore[assignment]
        try:
            observed = LC.frozen_region_digest(text)
        finally:
            LC._frozen_region_payload = real_builder  # type: ignore[assignment]
        self.assertEqual(
            observed,
            sentinel_digest,
            "frozen_region_digest did not hash the shared builder's output: it carries its own copy "
            "of the payload shape, which can drift from the one the substitution test reproduces",
        )

        # (2) `frozen_region_comparison` must reproduce THAT SAME payload with the receipt's stored
        # scope substituted. A spy records the calls: the comparison must invoke the shared builder
        # WITH a `scope_paths_override`, since substituting the stored scope is the entire mechanism.
        calls = []

        def spy(text_arg, **kwargs):
            calls.append(kwargs)
            return real_builder(text_arg, **kwargs)

        LC._frozen_region_payload = spy  # type: ignore[assignment]
        try:
            cmp_result = self._compare(self._widened("tests/test_extra.py"))
        finally:
            LC._frozen_region_payload = real_builder  # type: ignore[assignment]
        self.assertTrue(
            calls,
            "frozen_region_comparison never called the shared builder, so it is reproducing the "
            "payload from its own copy and the two can silently drift",
        )
        self.assertTrue(
            any("scope_paths_override" in kwargs for kwargs in calls),
            "the comparison called the shared builder but never with `scope_paths_override`; the "
            f"substitution test is what decides eligibility, so this cannot be right (saw: {calls!r})",
        )
        self.assertTrue(
            cmp_result.non_scope_identical,
            "sanity: with only a scope path added, the substitution must reproduce the stored digest",
        )

        # (3) THE DRIFT ITSELF, made observable. With the builder returning a payload that ignores
        # the override, the substitution can no longer reproduce the stored digest, so the accept
        # silently stops firing - which is exactly how a drifted second copy presents ("the feature
        # does not work") and exactly what no source scan reports.
        LC._frozen_region_payload = lambda text_arg, **kwargs: real_builder(text_arg)  # type: ignore[assignment]
        try:
            drifted = self._compare(self._widened("tests/test_extra.py"))
        finally:
            LC._frozen_region_payload = real_builder  # type: ignore[assignment]
        self.assertFalse(
            drifted.non_scope_identical,
            "a builder that IGNORES the scope override still reproduced the stored digest, so this "
            "test cannot observe drift at all and its other assertions prove nothing",
        )

        # The override substitutes BOTH scope inputs, which is the property the substitution needs.
        payload = LC._frozen_region_payload(
            self._widened("tests/test_extra.py"),
            scope_paths_override=["agent_workflows/demo.py", "tests/test_demo.py"],
        )
        self.assertEqual(
            payload["scope_paths"], ["agent_workflows/demo.py", "tests/test_demo.py"]
        )
        self.assertEqual(
            payload["requirements"]["scope"],
            ["agent_workflows/demo.py", "tests/test_demo.py"],
            "the `scope` requirement category must be substituted in lockstep with the allowlist",
        )
