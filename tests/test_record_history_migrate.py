...
class MigrateInlineHistoryTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        # a spec with 3 inline history records in newest-first order, with a --by-human attestation as newest
        (self.specs / "20260101-1200-01-x.spec.md").write_text(
            "# Spec\n\n- Id: aaa111\n- Status: approved\n\n## Workflow history\n"
            "- 2026-01-03 approved (aw specs, --by-human): approved by human\n"
    ...
    def test_preview_writes_nothing(self) -> None:
        ...
        self.assertIn(
            "- 2026-01-03 approved (aw specs, --by-human)",
    ...
    def test_apply_folds_and_slims_excluding_plans(self) -> None:
        ...
        self.assertEqual([r["message"] for r in recs], ["approved by human", "b", "a"])
        # spec inline slimmed to latest-one (newest round, preserving the --by-human attestation)
    ...
    def test_human_approval_attestation_survives_slimming(self) -> None:
        """Regression test for 8pcdoa / F-1: newest-first retention preserves --by-human attestation."""
        ...
        self.assertNotIn("- 2026-01-01 draft (t): a", spec_text)
