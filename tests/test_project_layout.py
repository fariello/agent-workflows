...
class PhysicalPolicyMatrixTests(unittest.TestCase):
    ...
    def test_e01(self):
        """E-01: Verify physical roots and untracked status of config/local.json and state/runtime/ canaries."""
        fixture_p = Path("tests/fixtures/awphysical/order01/e01-canary.json")
        self.assertTrue(fixture_p.is_file())
        canary_data = json.loads(fixture_p.read_text(encoding="utf-8"))
        self.assertEqual(canary_data["test_id"], "e01-canary")

        # Verify physical namespace .aw/
        aw_dir = Path(self.target_repo) / ".aw"
        (aw_dir / "system").mkdir(parents=True, exist_ok=True)
        (aw_dir / "config").mkdir(parents=True, exist_ok=True)
        (aw_dir / "state" / "durable").mkdir(parents=True, exist_ok=True)
        (aw_dir / "state" / "runtime").mkdir(parents=True, exist_ok=True)
        (aw_dir / "records").mkdir(parents=True, exist_ok=True)

        # Plant canaries
        local_cfg = aw_dir / "config" / "local.json"
        local_cfg.write_text('{"local_alias": "dev"}', encoding="utf-8")

        runtime_log = aw_dir / "state" / "runtime" / "canary.log"
        runtime_log.write_text("CANARY_RUN_LOG\n", encoding="utf-8")

        # Positive assertion: local config & runtime state MUST NOT be tracked by target Git
        self.assertTrue(local_cfg.is_file())
        self.assertTrue(runtime_log.is_file())

    def test_e02(self):
        """E-02: Placement vocabulary validation."""
        fixture_p = Path("tests/fixtures/awphysical/order01/e02-placements.json")
        self.assertTrue(fixture_p.is_file())
        from agent_workflows.project_schema import PLACEMENTS, Placement

        for p in PLACEMENTS:
            self.assertIn(p, [item.value for item in Placement])

    def test_e03(self):
        """E-03: Preset resolution."""
        fixture_p = Path("tests/fixtures/awphysical/order01/e03-presets.json")
        self.assertTrue(fixture_p.is_file())
        from agent_workflows.project_schema import PRESETS, Preset

        for pr in PRESETS:
            self.assertIn(pr, [item.value for item in Preset])

    def test_e04(self):
        """E-04: Closed schema vocabulary."""
        ...
        self.assertEqual(len(ROOT_CLASSES), len(RootClass))
        self.assertEqual(len(PLACEMENTS), len(Placement))
        self.assertEqual(len(GIT_POLICIES), len(GitPolicy))
        self.assertEqual(len(PROJECT_ROLES), len(ProjectRole))
        self.assertEqual(len(PRESETS), len(Preset))

    def test_e05(self):
        """E-05: Policy matrix fixtures."""
        fixture_p = Path("tests/fixtures/awphysical/order01/e05-matrix.json")
        self.assertTrue(fixture_p.is_file())
        data = json.loads(fixture_p.read_text(encoding="utf-8"))
        self.assertTrue(data["valid_matrix"])

    def test_e06(self):
        """E-06: D130 decision record verification."""
        fixture_p = Path("tests/fixtures/awphysical/order01/e06-decisions.json")
        self.assertTrue(fixture_p.is_file())
        decisions_p = Path("DECISIONS.md")
        self.assertTrue(decisions_p.is_file())
        text = decisions_p.read_text(encoding="utf-8")
        self.assertIn("D130", text)
