...
from agent_workflows import agy_runipd, git_commit_helper, ipd_lifecycle, oc_runipd
...
class OneChangeCoversBothHosts(unittest.TestCase):
    def test_the_commit_path_is_the_same_object_in_both_drivers(self):
        """OBJECT IDENTITY, not two runner invocations.

        This is the plan's central economy: `commit_backlog_close` holds the ONLY `offer_commit` call
        in either runner and `agy_runipd` reaches it by IMPORT, so passing trailers at that one site
        serves both hosts. If a second commit path ever appears, this fails and the claim is void.
        """

        self.assertIs(
            agy_runipd.commit_backlog_close,
            oc_runipd.commit_backlog_close,
            "`commit_backlog_close` must be the SAME object in both drivers; a second copy means the "
            "trailer wiring covers one host only",
        )
