...
class CriterionA17NoSecondLifecycleTableTests(unittest.TestCase):
    ...
    def test_no_module_holds_a_second_lifecycle_color_or_glyph_table(self):
        violations: list[str] = []
        for path in _module_files():
            ...
            for name, keys, values in _color_or_glyph_dicts(tree):
                if name in allowed:
                    continue
                lifecycle_keys = sorted(
                    (set(keys) & _NATIVE_STATUSES) - _WORD_COLLISIONS
                )
                if len(lifecycle_keys) < _TABLE_THRESHOLD:
                    continue
                maps_color = any(isinstance(v, int) for v in values)
                maps_glyph = any(
                    isinstance(v, str) and v in _LIFECYCLE_GLYPHS for v in values
                )
                if maps_color or maps_glyph:
                    kind = "color index" if maps_color else "glyph"
                    violations.append(
                        f"{module}.{name} maps {len(lifecycle_keys)} lifecycle statuses to a "
                        f"{kind}: {lifecycle_keys[:8]}"
                    )
        self.assertEqual(
            violations,
            [],
            "A SECOND LIFECYCLE PALETTE EXISTS, which spec `uonrjg` criterion A17 forbids ('No "
            "second lifecycle color or glyph table remains'). Lifecycle color, glyphs and ASCII "
            "fallbacks belong to `lifecycle_style` and reach a terminal through `term`'s rendering "
            "helpers. FIX: delete the table and resolve through `term.resolve_lifecycle` plus "
            "`style_lifecycle_text`/`format_lifecycle_marker`. If the table is genuinely NOT "
            "lifecycle (a severity axis, a tool-event axis, an attention CLASS axis), add it to "
            "`_ALLOWED` with the vocabulary it belongs to.\n  "
            + "\n  ".join(violations),
        )

    def test_the_guard_can_actually_fail(self):
        """GUARD THE GUARD: a probe that cannot fail is not evidence.

        Feeds the detector a deliberately reintroduced local lifecycle palette and asserts it is
        caught. Without this, every assertion above would pass against a broken detector.
        """
        ...
        dicts = _color_or_glyph_dicts(tree)
        self.assertEqual(len(dicts), 1)
        name, keys, values = dicts[0]
        lifecycle_keys = sorted((set(keys) & _NATIVE_STATUSES) - _WORD_COLLISIONS)
        self.assertGreaterEqual(
            len(lifecycle_keys),
            _TABLE_THRESHOLD,
            f"the detector did not recognize {name} as a lifecycle table: {lifecycle_keys}",
        )
        self.assertTrue(any(isinstance(v, int) for v in values))

    def test_a_reintroduced_glyph_table_is_also_caught(self):
        """The glyph half of A17, which a color-only probe would miss entirely."""
        ...
        tree = ast.parse(source)
        _name, keys, values = _color_or_glyph_dicts(tree)[0]
        lifecycle_keys = sorted((set(keys) & _NATIVE_STATUSES) - _WORD_COLLISIONS)
        self.assertGreaterEqual(len(lifecycle_keys), _TABLE_THRESHOLD)
        self.assertTrue(
            any(isinstance(v, str) and v in _LIFECYCLE_GLYPHS for v in values),
            "the glyph detector did not match Section 5 glyphs",
        )

    def test_the_retained_generic_role_table_PASSES(self):
        """THE FALSE-FAIL HALF: the generic roles R10.3 keeps must not trip the guard.

        This is why A17 is asserted by content. `term.ROLE_COLOR_256` legitimately survives the split
        and legitimately maps words to color indices; a name-based guard would have had to special-case
        it, and a stricter content guard would have flagged it. It passes because its keys are generic
        command-outcome and formatting roles, not lifecycle statuses.
        """

        tree = ast.parse(
            pathlib.Path(term.__file__).read_text(encoding="utf-8"),
        )
        role_dicts = [
            (n, k, v)
            for (n, k, v) in _color_or_glyph_dicts(tree)
            if n == "ROLE_COLOR_256"
        ]
        self.assertEqual(len(role_dicts), 1, "ROLE_COLOR_256 not found in term.py")
        _name, keys, _values = role_dicts[0]
        lifecycle_keys = sorted((set(keys) & _NATIVE_STATUSES) - _WORD_COLLISIONS)
        self.assertEqual(
            lifecycle_keys,
            [],
            f"the retained GENERIC role table carries lifecycle status(es) {lifecycle_keys}, so the "
            "split in E-04 was incomplete: those keys belong to `lifecycle_style`",
        )
