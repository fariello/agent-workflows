...
`YYYYMMDD-HHMM-NN-<slug>.prompt.md` name dated before this repository's `prompt_id6` cutover is still
valid and converts on demand with `aw rename prompts <legacy-name> --to-id6`. When a prompt has been
run and its results filed under
`.aw/records/research/<topic>/`, move it to `executed/`. If a prompt is dropped instead of run, retire
