...
It writes a `Kind: session-handoff` draft to the gitignored `.aw/records/prompts/untracked/` lane, applies a
sensitivity/privacy gate + `aw check-local-leaks`, and NEVER auto-commits - the human promotes it. Run
`/handoff [focus]`, or from any agent: "read and execute `.aw/system/workflows/handoff/handoff.md`". See
`.aw/system/workflows/index.md` for the catalog and `/whatnext` for the short next-action ordering.
