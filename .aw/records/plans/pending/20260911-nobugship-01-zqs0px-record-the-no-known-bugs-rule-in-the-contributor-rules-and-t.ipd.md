...
- Scope: State the rule where it is discoverable and where the enforcing code can cite it: the existing release-gates section of the contributor rules, and a numbered entry in the decisions log. Also state its two honest limits. Does NOT change any code, does NOT gate any other work kind, and does NOT backfill any item.
...
- [ ] V-02 validates E-02
  - Required evidence: paste the `engine.py` diff and the regenerated `AGENTS.md` diff side by side, showing the rule and BOTH stated limits (author-classification leak, live-items-only). Paste a grep proving the wording contains none of `never`, `no exception`, `always must`, per the no-absolutes instruction. State the diff line count.
  - Observed evidence:
  - Result: pending
