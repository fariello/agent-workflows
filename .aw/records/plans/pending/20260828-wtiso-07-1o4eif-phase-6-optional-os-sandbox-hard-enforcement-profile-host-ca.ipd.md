...
- Scope-Paths: agent_workflows/host_sandbox_profile.py, agent_workflows/oc_runipd.py, agent_workflows/host_adapters.py, tests/test_host_sandbox_profile.py, .aw/records/plans/pending/20260828-wtiso-07-1o4eif-phase-6-optional-os-sandbox-hard-enforcement-profile-host-ca.ipd.md
...
- [ ] E-02 Add `detect_host_capabilities(host: str, platform_name: str | None = None) -> HostSandboxCapabilities` to `host_sandbox_profile.py` that returns the fail-closed default (all `False`) for any host/platform NOT explicitly proven, and populates `supports_os_sandbox=True` (plus the process-tree-kill and read-only-phase fields it can back) ONLY for the single launch platform this phase certifies (Linux, using an unprivileged mount/user-namespace jail); wire it so `host_adapters` exposes the capability snapshot without editing the default launch path.
  - Depends on: E-01
  - Expected outcome: `detect_host_capabilities("opencode", "windows").supports_os_sandbox is False` and `detect_host_capabilities("opencode", "darwin").supports_os_sandbox is False` (unproven platforms), while the certified Linux path reports the fields it has proven; no capability is asserted without a corresponding probe.
  - Execution state: pending
...
- [ ] E-05 Add `enter_sandbox(argv, sandbox_plan, capabilities) -> list[str]` to `host_sandbox_profile.py` that, on the certified Linux platform, wraps the worker `argv` in the unprivileged-namespace jail enforcing `sandbox_plan` (writable lane roots, read-only toolchain + git-common-dir, inaccessible control/main/sibling/credentials) and attaches the child tree to a killable group; the DRIVER (not the sandboxed worker) performs `git add`/commit/refs on the lane branch after the worker exits, so the sandboxed worker never needs common-dir write access. This is invoked ONLY when the hardened profile is explicitly selected AND `capabilities.supports_os_sandbox` is True.
  ...
  - Expected outcome: `enter_sandbox` returns an argv whose leading tokens are the jail launcher configured from `sandbox_plan` (asserted by string inspection in tests); when the hardened profile is NOT selected the default `run_opencode` launch at `oc_runipd.py:1692` is byte-for-byte unchanged (regression test on the default path passes).
  - Execution state: pending
...
- [ ] E-08 Add adversarial test `test_hardened_worker_write_to_control_main_sibling_denied_by_os` to `tests/test_host_sandbox_profile.py` (marked `@pytest.mark.skipif(not _linux_userns_available(), reason="requires the certified Linux namespace sandbox")`): under the hardened profile, launch a worker command that attempts to write a file into (a) the control root, (b) the main worktree, and (c) a sibling lane root, and assert each write is DENIED BY THE OS (the target file does not exist afterward and the attempt returns a permission error), NOT merely flagged by policy.
  - Depends on: E-05
...
- [ ] E-10 Add fail-closed test `test_hard_mode_requested_without_capability_fails_closed` to `tests/test_host_sandbox_profile.py` WITHOUT any skipif (it MUST run on every platform, because it is pure dispatch logic): assert `select_execution_profile("hardened", HostSandboxCapabilities(supports_os_sandbox=False))` raises `HardModeUnavailableError` and that the default launch path is never entered in that case (no unsandboxed worker is spawned); and write `.aw/records/walkthroughs/` guarantees text is out of scope but publish the hardened-profile guarantee summary as a module docstring in `host_sandbox_profile.py` (what it prevents, on which platform, that the common dir is read-only, that the driver commits).
  - Depends on: E-06
...
- Worker launch seam: `run_opencode` in `agent_workflows/oc_runipd.py:1606` builds `argv` (`:1621`), sets the lane worktree via `--dir agent_dir` (`:1640`, `agent_dir = work_dir or state["repo"]` at `:1624`), and `subprocess.Popen(argv, **popen_kwargs)` at `:1692` with `cwd=agent_dir` (`:1680`) and `start_new_session=True` on POSIX (`:1687`). There is NO filesystem write boundary today; `start_new_session` exists only so `terminate_process` (`:1559`) can `os.killpg` the tree (`:1570`). The hardened profile wraps this argv; the default path stays untouched.
...
- Receipt handling: the current default copies the begin receipt into the worktree (`sync_receipt_into_worktree`, `oc_runipd.py:462`); wtiso-03..05 move that authority out of the lane. This phase does not reintroduce a lane-local authority copy.
...
| F3 | A linked worktree's `.git` points into the shared common dir, so a writable common dir lets the worker mutate shared refs/hooks/config; hardened mode MUST keep the common dir read-only and have the driver own staging/commits. | x03wgn Section 4 ("A linked worktree adds one subtlety ... keep the common directory read-only to the worker and make the driver own staging and commits"); real substrate: `worktree_lease.py:70`/`:95` (`git worktree add`), `project_registry.py:182` (`--git-common-dir`). |
...
| F8 | The current launch has no write boundary; it only groups the process for killing. | `oc_runipd.py:1640` (`--dir agent_dir`), `:1680` (`cwd=agent_dir`), `:1687` (`start_new_session=True`), `:1692` (`Popen`), `:1559`/`:1570` (`os.killpg` reap). |
...
3. `agent_workflows/oc_runipd.py`: call `select_execution_profile` before launch and, only when `"hardened"` is selected and supported, wrap the worker `argv` via `enter_sandbox`; the default path at `:1692` is unchanged (E-05, E-06).
...
- [ ] V-02 validates E-02
  - Required evidence: ACTUAL pasted output of `python3 -c "from agent_workflows.host_sandbox_profile import detect_host_capabilities as d; print(d('opencode','windows').supports_os_sandbox, d('opencode','darwin').supports_os_sandbox)"` printing `False False` (unproven platforms report no sandbox capability - fail-closed).
  - Observed evidence:
  - Result: pending
...
- [ ] V-06 validates E-06
  - Required evidence: ACTUAL pasted stdout of `python3 -m pytest -p no:randomly -q tests/test_host_sandbox_profile.py::test_hard_mode_requested_without_capability_fails_closed` reporting `1 passed` on this platform (fail-closed dispatch); this V-item overlaps E-10's test which is the canonical fail-closed acceptance.
  - Observed evidence:
  - Result: pending
...
- [ ] V-08 validates E-08
  - Required evidence: ACTUAL pasted stdout of `python3 -m pytest -p no:randomly -q tests/test_host_sandbox_profile.py::test_hardened_worker_write_to_control_main_sibling_denied_by_os` reporting `1 passed` on the certified Linux host (or `1 skipped` on hosts lacking the namespace sandbox); the test asserts none of the three target files (control-root write, main-worktree write, sibling-lane write) exist after the run AND each attempt returned an OS permission error - proving OS denial, not policy detection.
  - Observed evidence:
  - Result: pending

- [ ] V-09 validates E-09
  - Required evidence: ACTUAL pasted stdout of `python3 -m pytest -p no:randomly -q tests/test_host_sandbox_profile.py::test_hardened_worker_git_common_dir_mutation_denied` reporting `1 passed` (or `1 skipped` where unavailable); the test asserts the target ref/hook/config bytes are byte-identical before and after the worker's mutation attempt through the read-only common dir, AND that a subsequent driver-performed lane-branch commit is reachable (`git cat-file -e <oid>` succeeds) - proving the common dir is read-only to the worker and the driver owns git mutation.
  - Observed evidence:
  - Result: pending
...
This child implements x03wgn Section 8 "Phase 6" (with Sections 1, 4, and 6 Layer 4). It is optional/hardening and parkable per orchestrator OQ-01. Post-gate lifecycle move: after every V-item above carries pasted passing/skipped-as-specified evidence and `aw ipd lint --phase pre-transition` conforms, run `aw ipd finalize` on this plan; do not move it to executed/ otherwise.
