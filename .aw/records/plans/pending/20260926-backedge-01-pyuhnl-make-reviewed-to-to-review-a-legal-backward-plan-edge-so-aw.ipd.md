...
- [ ] E-03 ADD `("reviewed", "to-review")` to `ipd_lifecycle._LEGAL_BACKWARD_EDGES`, and update the comment above it: it currently reads "Permits recovery of an approved or auto-approved plan back to reviewed ... All other backwards transitions fail closed". Name the new edge, cite the maintainer ruling of 2026-09-26 and backlog `qzo6dn`, and keep the fail-closed sentence. Do NOT touch the rank comparison in `validate_transition`: spec `2vev8j` 4.8 point 2 says an implementation that removes it "would permit EVERY backward edge, which is NOT what was decided", and E-02 case (4) pins that.
  - Depends on: E-02
  - Expected outcome: `validate_transition("reviewed", "to-review")` returns `TransitionCheck(ok=True, reason='')`; the E-02 file passes in full.
  - Execution state: pending

### Task group 3: contract and docs

- [ ] E-04 AMEND THE IPD SPEC `.aw/records/specs/implemented/20260726-1340-01-ipd-spec.spec.md`, in the `## Workflow history` bullet whose sentence reads "The only legal backward lifecycle transitions are `approved -> reviewed` and `auto-approved -> reviewed` (spec `2vev8j` 4.8 / spec `25kzda` 4.5); every other backwards move fails closed." Add `reviewed -> to-review` (re-review after revision, maintainer ruling 2026-09-26) to that enumeration and keep the fail-closed clause verbatim. Record the amendment on the spec through the tool, `aw specs note <spec path> --message "..."` naming this plan `pyuhnl` and the ruling, rather than hand-writing a history line; if `aw specs note` refuses on an `implemented` spec, paste the refusal and add no history line (the content edit alone is the contract change, declared here).
  - Depends on: E-03
  - Expected outcome: the spec sentence enumerates exactly three legal backward edges; `aw specs check` (or `aw check specs`) reports no new finding on the spec.
  - Execution state: pending
...
- Over-scope: none.
- Under-scope: `agent_workflows/check_engine.py` and `agent_workflows/work_cmd.py` are deliberately NOT declared: both consume `validate_transition` unchanged and need no edit.
...
- [ ] V-04 validates E-04
  - Required evidence: paste the spec diff showing three enumerated edges and the unchanged fail-closed clause; paste the `aw specs note` output (or its refusal); paste `aw check specs` output showing no new finding for the spec.
  - Observed evidence:
  - Result: pending
