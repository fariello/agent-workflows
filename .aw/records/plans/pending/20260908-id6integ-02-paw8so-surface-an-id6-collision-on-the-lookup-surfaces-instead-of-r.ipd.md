...
| F-3 | HIGH | the defect is still live, on a three-cause example | `aw find uyeko5` prints THREE rows with exit 0, from THREE different causes: a genuine owner (`kind=id6`), a parser artifact (research prompt `27rjro` quoting `- Id: uyeko5` at `:60`), and a correct review reference (`kind=substring`). One generic warning cannot serve all three. | command run at HEAD; `resolve` kinds measured per type |
