...
**THIS PLAN CANNOT PROCEED ON DOCUMENTATION ALONE: SEE OQ-04.** Spec `25kzda` §2.9 owns the dependency wait/release semantics that `edge_satisfied` implements, it is `approved`, and it defines an `executed:` edge as satisfied only when the target "is in `executed/` with status `executed`". It does not describe a review-action relaxation at all. So the change here is not "make the runner read the state correctly" in a spec-neutral sense: the three-state tuple the code carries is itself unsanctioned, and this plan makes it LIVE. The spec must be amended to state the review-action relaxation (and that readiness is read from the `- Status:` field, not the directory) before or alongside the code change, per the maintainer's direction in OQ-04.
...
- Owner: maintainer
- Finding: PR-001
- Resolution or deferral rationale: UNRESOLVED, AND ONLY THE MAINTAINER CAN RESOLVE IT. The decision needed: whether the `("executed", "reviewed", "approved")` acceptance for a review-action edge is the intended contract, and if so whether spec `25kzda` is amended BEFORE this code ships or in the same change.
