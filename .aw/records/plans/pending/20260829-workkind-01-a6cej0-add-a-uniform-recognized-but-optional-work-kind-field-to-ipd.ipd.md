...
- `agent_workflows/backlog.py` is deliberately NOT declared, which is the machine-readable expression of the OQ-03 deferral: E-01 only IMPORTS `backlog.KINDS` and must not modify it. If the executor finds itself editing `backlog.py`, the rename deferral has been breached and it must STOP and report.
- Under-scope: NONE OUTSTANDING, but one file needs checking BEFORE work starts rather than assumed: adding a CLI option may require an entry in whatever surface manifest the CLI-conformance tests consult (locate `test_command_surface_declarations` and `test_cli_conformance_matrix`). These have been observed failing in this repo for undeclared parser leaves during concurrent CLI work. Establish whether an option (as opposed to a new subcommand) triggers that requirement, and if satisfying it means editing an undeclared file, STOP and report rather than widening the fence.
...
- The CLI-surface conformance tests must pass, or, if they were already failing at your baseline for unrelated undeclared leaves, ATTRIBUTE those by NAME and prove your change is not among them.
...
- The `ipd-spec` document under `.aw/records/specs/` defines the IPD's structural contract including its metadata fields. CHECK whether it enumerates recognized metadata fields, and if it does, whether `Priority` was added there when `xprio` landed. Follow whatever that precedent did: if `Priority` is listed, list `Work-Kind`; if it is not, do not. Do NOT invent a new documentation obligation, and do NOT edit a spec outside this plan's declared scope without reporting first.
...
- [ ] V-04 validates E-04
  - Required evidence: paste `aw ipd set ... --work-kind bug` writing the line, `--work-kind -` clearing it, and an out-of-vocab value REJECTED with a nonzero exit and no write. Same three for `aw specs set`. State whether `aw ipd scaffold` already has `--priority` and therefore whether you added `--work-kind` there, citing what you found. Paste the CLI-conformance result, or the attributed baseline failures if they pre-existed.
  - Observed evidence:
  - Result: pending
