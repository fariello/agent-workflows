...
- Resolution: RESOLVED BY THE MAINTAINER 2026-09-19, AS READING A, AND THE RULING IS WIDER THAN THE QUESTION ASKED. `FORCE_COLOR` KEEPS its escape hatch over `NO_COLOR`, so the top rung is `NO_COLOR AND NOT FORCE_COLOR`, `tests/test_term.py:48` keeps passing, and R9.3a.2's "unconditional" is read as unconditional with respect to the DEPTH PIN only. This plan's E-01 is therefore written against current behavior and changes none of it.
  ...
  WHAT THIS PLAN MUST DO WITH THAT: E-01 consumes whatever single `should_color` exists when it runs, and MUST NOT reimplement the precedence locally. It does not fix `nyz8dt`; that item is sequenced after `yaxr4i` (which owns the `--color` flag twin the maintainer ruled for: two flags, mutually exclusive) so the resolver is not written twice. If `nyz8dt` has not landed when this executes, E-01 still conforms by consuming `term.should_color` as the single definition, which is the implementation the ruling declares correct.
