...
- Scope: Wire `offer_commit` into each records-mutating verb so that after it mutates files + regenerates any INDEX, it collects the EXACT set of paths it touched (moved/renamed/deleted paths + regenerated index files) and calls `offer_commit(...)` with a good per-verb default message. The verb set (with VERIFIED backends, plan-review PR-003/PR-004):
  ...
  - `aw research set-assign`/`mv`: `research_refs.run_set_assign` (research_refs.py:285) / `research_refs.run_mv` (research_refs.py:310), dispatched from the `research` command branch (cli.py:7180).
...
- [ ] E-03 Make the `group`/`rename` backends surface their touched-path set (e.g. return a small `MutationResult(touched_paths, index_paths)`) across `plans_refs`, `research_refs`, and `artifact_rename`, without yet committing.
  - Depends on: none
  - Expected outcome: each group/rename backend returns the exact set of paths it moved/renamed + the regenerated index paths; a test reads those return values for plans, research, and one artifact_rename type.
  - Execution state: pending
- [ ] E-07 Wire `offer_commit` ONCE at the `_run_noun_verb` group/rename dispatch (cli.py:5601) using the backend-returned touched paths, with a `refactor(<type>): regroup/rename <selector> and rewrite refs` message (`on_unrelated_staged="scope"`), covering ALL types. Where a backend cannot cleanly return its touched paths, wire that backend directly - but no group/rename type is left uncovered.
  - Depends on: E-01, E-03
  - Expected outcome: `aw group <type>`/`aw rename <type>` for every supported type offer to commit exactly their moved paths + rewritten index; flags honored; verified for at least plans, research, and one artifact_rename type (e.g. specs).
  - Execution state: pending

### Task group 3: adopt in the shared status-set engine and the specs dual path

- [ ] E-04 Wire `offer_commit` into `research_refs.run_set_assign`/`run_mv` (research_refs.py:285/310) for their file moves + rewritten refs/index with a `refactor(research): ...` message (`on_unrelated_staged="scope"`), if not already covered by the E-07 dispatch-site wiring (the `aw research` command branch at cli.py:7180 is a SEPARATE dispatch from `_run_noun_verb`, so confirm which path `set-assign`/`mv` take and wire exactly once).
  - Depends on: E-01, E-07
  - Expected outcome: `aw research set-assign`/`mv` offer to commit exactly their touched paths; flags honored; no double-offer with E-07.
  - Execution state: pending
...
Adoption is mechanical per integration point: collect the touched paths (already known at the mutation site) and call the child-01 helper with `on_unrelated_staged="scope"`. The two real risks are (1) COVERAGE - group/rename fan out over ~8 artifact types across 3 backend modules, so a naive "edit plans_refs.py" misses most types; the fix is to wire once at the type-parameterized dispatch site (E-03) or cover every backend; and (2) EXACTLY-ONCE - the shared `status_set` engine and the `specs.py` dual path must not double-offer or miss. Tests assert exactly which paths get committed and that each invocation offers exactly once.
...
- [ ] V-03 validates E-03
  ...
  - Result: pending
- [ ] V-04 validates E-04
  - Required evidence: A test drives `aw research set-assign`/`mv`, asserting it offers to commit exactly its touched paths and that no double-offer occurs with the E-07 dispatch wiring (exactly one commit). Paste pytest output.
  - Observed evidence:
  - Result: pending
- [ ] V-05 validates E-05
