...
- [ ] E-01 BUILD THE REVERSE INDEX from source id6 to the plans that cite it, reusing the enumeration the forward dangling checks already use rather than walking the tree afresh. `check.from-backlog-dangling` and `check.from-spec-dangling` already parse `From-Backlog:` and `From-Spec:` out of every plan to validate the forward direction, so the parse exists; this item inverts it.
  ...
  - Depends on: none
  - Expected outcome: a reverse index mapping each source id6 to its plans with each plan's id6, status and Set, built by reusing the existing field readers; terminal directories included; no second parser added.
...
- THE CORPUS GROWS DURING WORK. The item measured 71 source-linked plans; it is 125 five days later, and an executed plan in this repository's history was reviewed with a note that its own corpus count "GREW during this Set's own operation, so the count must be derived not asserted". Derive, never pin.
- Suite bare: `python3 -m pytest`. Measure the baseline in the executing worktree and compare failing NODE IDS, never totals; a bare run on main is `1 failed, 5648 passed` (the known `tests/test_orchestrator_retirement.py` failure, which reads live plan statuses and is itself an example of pinning to a mutable corpus).
...
- THE ALREADY-IMPLEMENTED VERDICT. Backlog `f1sw71` (open). The item explicitly permits shipping without it.
...
- [ ] V-01 validates E-01
  - Required evidence: paste the reverse index's construction, showing it REUSES the existing `From-Backlog:`/`From-Spec:` readers (show the call, not a similar regex). Paste the index entry for `25kzda` listing all nine plans with their statuses and Sets, and confirm terminal directories are included by showing the eight `executed` members. Paste a grep proving no second parser for either field was added.
  - Observed evidence:
  - Result: pending
