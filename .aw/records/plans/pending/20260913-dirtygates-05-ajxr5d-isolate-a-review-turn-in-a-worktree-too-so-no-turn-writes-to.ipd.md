...
- Scope: Give a review turn the same worktree isolation an execute turn gets, so its plan edit and review record land via one merge. THE CHANGE IS LARGER THAN "FLIP THREE GUARDS", which review established by reading the control flow rather than the guard list: worktree ALLOCATION, lane-submission COLLECTION, the MERGE-BACK and the worktree TEARDOWN are each nested inside a `not is_review` block this plan classifies as leave-alone, and the review disposition reads the plan's status from MAIN. Six additional call sites therefore need work, on each host. Excludes the review SESSION sharing (which must stay shared), the auto-approve step's own semantics, and the validation gates a review legitimately skips. Excludes Orders 01 to 04.
...
- [ ] E-02 Allocate a lane for a review turn and run the turn inside it, reusing the existing `wt_handle` / `work_dir` machinery rather than adding a second isolation path. `work_dir = str(wt_handle.path)` at `:6199` is the existing seam. The lane-relative plan resolution at `:6279` exists because of a MEASURED incident (run `run-20260831T153226Z-3424176`, plan `y6mfgo`): the prompt carried MAIN's absolute plan path and no statement of isolation, and the agent read `../../../DECISIONS.md` and committed 18 files into MAIN while the lane stayed at zero commits. A review turn must get the same lane-relative plan path and the same explicit statement that it is in a lane, or it will reproduce that incident.
  - THE ALLOCATION IS NOT REACHABLE WITHOUT E-01'S SPLIT OF `:6151`, which is the single most important correction review made to this plan. The allocation call sits INSIDE the `self_finalize and not is_review` block, so this item's first real task is that split, and it must hoist the allocation WITHOUT letting a review reach `driver_begin`. A review has no execution authority to be granted and must not mint a receipt.
  - LOCALIZE THE ATTACHMENTS TOO, per E-01's added `:5467` row. The prompt TEXT and the `--file` attachments are separate surfaces and were measured to disagree once already; fixing only the prompt reproduces half of the F-4 incident.
  - Depends on: E-01
...
- [ ] E-03 Integrate a review lane back to main as one merge, by WIDENING `integrate_lane_branch` to take an explicit action kind and SKIP the revalidation gate for a review (OQ-01, resolved). A review's output is two files, both scoped to one plan: the plan itself (revisions applied) and the review record, verified at review on commit `a9510164`, which holds exactly `M .aw/records/plans/pending/...tgop8e...ipd.md` and `A .aw/records/reviews/...tgop8e...review.md`. Keep the merge steps of that function fully shared, because they carry the `--ff-only`-then-`--no-ff` sequence whose ff failure is the expected "main advanced" case, the capture-conflicted-paths-BEFORE-abort ordering, and the `host_label` merge subject. THE HARD CONSTRAINT: the action kind must be an EXPLICIT parameter and the docstring must state that a review is merged without revalidation because it produces nothing to revalidate. Do NOT pass a synthetic "validation passed" value into the gate; that is a forged attestation, not a skip.
  ...
  - Execution state: pending
- [ ] E-04 Resolve the SHARED-SESSION versus ISOLATION conflict the way OQ-03 rules, and record the reasoning wherever the promise lives. Both the CLI help (`:7688`, `:7699`) and the sweep (`:7728`) state that every review in a run shares one OpenCode session "for continuity", and `run_opencode` drops `--session` when a turn is isolated (`:5405`, `session = None if (fresh_session or isolated_turn or is_rotation)`).
  - THIS IS A DIRECT CONTRADICTION, NOT AN INTERACTION TO WORK AROUND, and the plan's original framing ("preserve the shared session ... record the reason isolation does not break it") presumes an outcome that the code's own recorded reasoning says is unsafe. The `:5383-5390` comment documents incident `lanesess xd9sll` in detail: sessions were keyed per SET while worktrees are per ITEM, an opencode session carries its own project/`directory` binding which then OVERRIDES `--dir`, turns silently ran in the PREVIOUS lane's worktree, every main-repo path became "external", and FOUR CONSECUTIVE LANES WERE LOST. The conclusion drawn there is categorical: "an isolated turn (work_dir set) is ALWAYS a fresh session". So a shared session across per-review lanes is precisely the configuration that incident forbids.
  - THEREFORE ONE OF THE TWO MUST YIELD, and which one is OQ-03's second half. If isolation wins, the CLI help's promise at `:7699` and the sweep text at `:7728` MUST be corrected in this plan, since leaving a false promise in the help is worse than the loss of continuity itself. If sharing wins, this plan cannot isolate per-review and OQ-02's answer changes with it. Do NOT attempt to keep both by re-binding a shared session into successive lanes: that is the exact mechanism `xd9sll` measured losing work.
  - Depends on: E-02
...
- [ ] V-01 validates E-01
  ...
  - Result: pending
- [ ] V-02 validates E-02
  - Required evidence: paste MAIN's `git status --porcelain` sampled DURING a review turn (not only after) and compare it to the pre-turn value; they must be EQUAL. Paste the review prompt showing the lane-relative plan path and the explicit in-lane statement, AND the constructed argv showing every `--file` attachment resolving inside the lane, which together are the F-4 regression. Also paste proof that a review turn performed no `driver_begin` (no receipt written), which is the property the `:6151` split must preserve.
  - Observed evidence:
  - Result: pending
- [ ] V-03 validates E-03
  ...
  - Result: pending
- [ ] V-04 validates E-04
  - Required evidence: paste the OQ-03 ruling, then evidence that the code matches it. If isolation won: paste three reviews each running in their own lane with their own session id, AND the diff correcting the help text at `:7699` and the sweep text at `:7728`, so no surface still promises continuity that is gone. If sharing won: paste the shared session id across three reviews and the code path showing no session is re-bound across two trees, with an explanation of why `xd9sll`'s failure mode cannot recur. DO NOT paste a shared session id across three separate lanes as a success: that is the exact configuration the incident measured losing four lanes.
  - Observed evidence:
  - Result: pending
- [ ] V-05 validates E-05
