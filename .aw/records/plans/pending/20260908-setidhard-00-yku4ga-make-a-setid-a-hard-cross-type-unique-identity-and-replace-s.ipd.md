...
- Owner: maintainer
- Finding: PR-701
- Resolution or deferral rationale: NOT AGENT-RESOLVABLE, and it is genuinely blocking rather than procedural. The backlog item's own instruction is to implement the spec "once the spec is reviewed", and the spec's header says "REVISABLE before implementation".
  STILL OPEN AT REVIEW: `/plan-review` on 2026-09-08 attempted to ask and found no interactive channel in that session, so this stays explicitly OPEN per the non-interactive exception. VERIFIED at review rather than trusted: the spec really is `- Status: draft` with `- Id: 4w7d6s`, and all three of its open questions really are open. TWO CORRECTIONS THE ANSWER SHOULD BE GIVEN. FIRST, the spec's own three questions each carry `- Blocking: no`, so the spec is not itself blocked; what blocks is this plan's own two `Blocking: yes` questions, and `aw ipd lint` refuses this plan at every checkpoint until they are resolved (measured: exit 1 with two `IPD-Q501` diagnostics). SECOND, the spec's filename carries NO id6 (it is the legacy `20260827-1514-01-setid-uniqueness-...` form), so anyone looking for it by id6 glob will not find it; that is a lookup hazard, not a missing artifact. Two of its three open questions change what the children build: OQ-02 asks whether I1 over-constrains legitimate WITHIN-type clustering, which is the shape of every multi-child Set here (including this one), so if the predicate needs correcting then the thing this Set hardens is wrong; OQ-03 asks whether a fresh-setid mint is compatible with the same-name mental model, which is Order 03's entire user-visible behavior. An agent cannot answer either, and cannot set the spec `approved` without forging a human attestation. E-02 holds this gate; nothing in the Set may execute until a human answers.
...
- Owner: maintainer
- Finding: PR-701
- Resolution or deferral rationale: THIS IS THE QUESTION THE MEASUREMENT FORCES, and it is bigger than the spec's own OQ-02. All live findings are a backlog item sharing its setid with the plan Set it graduated into, and that is not sloppiness: it is how a reader currently sees at a glance that item `dk16dx` and plan `st5klo` are the same work.
  STILL OPEN AT REVIEW, AND NOT FOR WANT OF TRYING. `/plan-review` on 2026-09-08 attempted to put this to the maintainer and found NO interactive channel available in that session, so under the workflow's non-interactive exception it is left explicitly OPEN with the required decision stated rather than guessed. Nothing here has been decided by an agent.
  WHAT REVIEW ADDED TO THE DECISION, since the numbers moved and they bear on the answer: the population is 40 findings across 30 setids, not 29 across 21, and it GREW by eleven findings and nine setids in the single day between authoring and review. ALL 40 sit on the backlog side. That cuts both ways and the maintainer should have both directions. It STRENGTHENS the case for acting (the convention reproduces collisions at a measurable rate, so the sweep's cost only rises), and it also RAISES the cost of the answer that says the name match matters more (the exemption would have to cover 30 setids and counting). If the answer is to NARROW I1 rather than sweep, the natural shape is an exemption for a backlog-to-plan pair joined by a resolvable `From-Backlog`, which is measurable today and would take all 40 findings to zero without renaming anything. The spec's answer (G1 plus typed bidirectional links) removes the collision and preserves TRACEABILITY, but it does trade away the at-a-glance name match, and the spec's own OQ-03 admits the tension. So the maintainer is being asked to accept a convention change affecting every future graduation, in exchange for a setid that means one thing. An agent must not decide that: it is a workflow-ergonomics judgement about how the maintainer reads their own tree. If the answer is that the name match matters more, the correct outcome is to NARROW I1 (for example, exempt a backlog-to-plan pair joined by a resolvable `From-Backlog`) rather than to sweep 21 setids, and this Set would need re-authoring.
