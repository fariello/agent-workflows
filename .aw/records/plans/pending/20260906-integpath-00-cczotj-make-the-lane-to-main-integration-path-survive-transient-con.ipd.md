...
## Workflow history
- 2026-09-07 reviewed (opencode its_direct/pt3-claude-opus-5-1m-us): plan-review complete: PR-001..PR-009; PR-002 BLOCKER left OPEN and escalated to OQ-04 (Blocking: yes); Readiness no-go
...
- [ ] E-02 RECORD WHAT THE SET CLOSED AND WHAT IT DID NOT, because this Set deliberately leaves several named things open and a reader six months from now must not have to infer which. THE ARTIFACT IS A WALKTHROUGH: write `.aw/records/walkthroughs/<YYYYMMDD>-integpath-00-cczotj-<slug>-walkthrough.md` per AGENTS.md's narrative-walkthrough convention. Do NOT invent a new record type and do NOT leave the artifact's type or path to the executor's judgement.
  ...
  - Depends on: E-01
  - Expected outcome: a walkthrough at the named path listing every residual with its reason and every sibling Set with its boundary, with `76gsmv` recorded as ANSWERED and `phawyy` recorded as RETRACTED; no residual is left as an unstated assumption.
  - Execution state: pending
...
WHICH V-ITEM OWNS EACH CRITERION, stated because a criterion no `V-*` demands evidence for is an aspiration rather than a gate. V-01 owns the two incident criteria, the no-`--no-verify` criterion, and the bare-suite delta. V-02 owns the E-02 record AND, through the CID-1 through CID-5 evidence it requires, the one-implementation, no-relaxed-refusal, and revalidate-gate criteria. The per-child mechanism criteria (children 01 through 04) are owned by those children's OWN `V-*` items and are NOT re-verified here; this orchestrator's job is the INTERACTION, not a second copy of each child's suite. Criteria 3 through 5 above are therefore satisfied by citing the owning child's pasted evidence, not by re-deriving it.

THE STARTUP GATE IS NOT A COMPLETION CRITERION. `p8ni63` is not a child of this Set (see the child table's exclusion note), so its absence never blocks Set completion; E-01 determines its presence and records the answer either way.
...
- Owner: maintainer
- Finding: PR-002
- Resolution or deferral rationale: RAISED AT REVIEW, NOT AGENT-RESOLVABLE, because the answer is a risk-appetite decision that belongs to the maintainer. THE DEFECT IS MEASURED, NOT PREDICTED. The runner-owned rollup deliberately OMITS the `pre-transition` E/V checkpoint (`ipd_lifecycle.ROLLUP_OMITTED_GATES['pre-transition-ev-checkpoint']`) on the premise that an orchestrator's items are "performed by NOBODY". For this plan that premise is FALSE: E-01 reproduces both incidents and E-02 records the residuals, and they are the ONLY evidence the Set's central claim holds. The immediately preceding orchestrator proves the outcome: `84j8d7` is in `executed/` right now with `Execution state: pending` on E-01 and `Result: pending` on V-01, retired by run `run-20260906T222606Z-2987341` (commit `52c2872a`) with its whole-Set verification never performed. Exposure begins at APPROVAL, since `action_for('orchestrator','approved')` is `orchestrate` and the queue admits `approved`.
...
- [ ] V-01 validates E-01
  - Required evidence: paste BOTH incidents reproduced and survived, for BOTH hosts. Incident one: a lane refused on dirty overlap, DEFERRED, then INTEGRATED after the dirt cleared, with the merge commit on main and NO agent turn spent. Incident two: an item with a verified lane resumed, showing the FIRST lane integrated and `git branch --list 'aw/lane/<id6>*'` containing NO `_attempt2`. For both, paste evidence that NO `--no-verify` was used anywhere. State whether `p8ni63` had landed (name the check you ran) and, if so, paste the startup gate refusing a dirty base and honoring its override; if it had not, say so and treat this item as complete without it, since it is not a child of this Set. If a host could not be demonstrated, SAY SO PLAINLY; an inferred agy result is a FAILED validation.
    Paste the BARE `python3 -m pytest` summary line (the `N passed` line), with the BEFORE baseline measured yourself and the AFTER-minus-BEFORE failure set shown EMPTY. `Required tests / validation` demands this and V-01 previously did not ask for it, so no `V-*` item carried the suite evidence the gate's honesty rule requires.
  - Observed evidence:
  - Result: pending

- [ ] V-02 validates E-02
  - Required evidence: paste the walkthrough's path (under `.aw/records/walkthroughs/`, per E-02's named form) and its residuals section, confirming all four residuals appear with their reasons (content narrowing declined, category allowlists rejected, `h1ksy6` unreconciled, `a8eufb` insufficient-for-uncommitted-dirt) AND that why `76gsmv` passed the hook is recorded as ANSWERED, citing child 01's resolved OQ-01 and the journal-lifetime cause. A record still calling it unknown FAILS this item. Confirm the three sibling Sets are named with their boundaries (`integearn`, `scopeattr`, `depreview`) and that `phawyy` is shown as RETRACTED rather than as live work. Paste the CID-1 through CID-5 evidence, each demonstrated rather than asserted, including the object-identity check for the two extracted functions and proof the anti-re-fork guard is SYMMETRIC over both runners.
  - Observed evidence:
  - Result: pending
...
Execution requires explicit human approval (`- Status: approved`). OQ-01 through OQ-03 are resolved; OQ-04 is OPEN and `Blocking: yes`, so this plan is NOT approvable until the maintainer answers it. That is deliberate: OQ-04 is the mechanism protecting this plan's own E-01/E-02 verification from being discharged by the runner's rollup, and `aw set approved` refuses over an unresolved blocking question (`plan_readiness.has_unresolved_blocking_question`). Do not clear it with `--allow-open-questions` without recording the maintainer's answer, and note that the CHILDREN are independently approvable and executable; only this orchestrator is gated.
...
BUT THAT RETIREMENT DOES NOT PERFORM E-01/E-02, AND WILL DISCHARGE THIS PLAN WITHOUT THEM. The rollup deliberately OMITS the `pre-transition` E/V checkpoint (`ipd_lifecycle.ROLLUP_OMITTED_GATES['pre-transition-ev-checkpoint']`, spec `77tr3o` R-5 shape (b)), on the general premise that an orchestrator's items are "performed by NOBODY". That premise does NOT hold for this plan: E-01 is the whole-Set incident reproduction and E-02 is the residuals record, and they are the ONLY evidence the Set's central claim is true. THIS IS NOT HYPOTHETICAL. It is MEASURED on the immediately preceding orchestrator: `84j8d7` sits in `executed/` today carrying `Execution state: pending` on its E-01 and `Result: pending` on its V-01, retired by run `run-20260906T222606Z-2987341` (`orchestrator-finalized`, commit `52c2872a`) with its whole-Set verification never performed. Exposure begins the MOMENT a human approves: `action_for('orchestrator','approved')` is `orchestrate` and the queue admits `approved` (verified directly), so the very next `aw oc run all` can retire this plan before E-01 has run.

SO THE ORDER IS FIXED, NOT A PREFERENCE: perform E-01 and E-02 and record V-01/V-02's evidence FIRST, then transition. If a runner retires this plan automatically before then, that is a PREMATURE DISCHARGE to REPORT to the maintainer, and the Set's central claim remains UNVERIFIED; do NOT treat the `executed` status as evidence the incidents were reproduced. The honest recovery is a new corrective IPD carrying E-01/E-02, never an in-place edit of a plan already in `executed/`. Whoever wants the verification to survive unattended execution should keep OQ-04 answered as it stands (do not run this Set with `--full-auto` while `Blocking: yes` is unresolved) rather than relying on this paragraph being read.
