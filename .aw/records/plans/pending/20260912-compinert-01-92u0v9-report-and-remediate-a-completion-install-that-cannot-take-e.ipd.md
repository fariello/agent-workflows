...
- [ ] E-03 OFFER THE ONE-LINE REMEDIATION, AND DO NOT APPLY IT WITHOUT CONSENT.
  ...
  APPENDING IS NOT IDEMPOTENT BY NATURE, so detect your own prior stanza before writing and refuse to duplicate it. Match on a stable marker line you emit (the same `installed-by: agent-workflows` convention the completion files already use), never on a fuzzy match of the snippet body, since a user may reformat it. A second consenting run must be a no-op that says so.
  ...
  - Depends on: E-02
  - Expected outcome: the guarded snippet printed verbatim in the non-reachable case, PLUS an opt-in `[y/N]` offer on a TTY that appends it atomically, refuses under `--yes` and on a non-TTY, is a reported no-op on a second run via its own marker line, and reports rather than creates an absent `~/.bashrc`.
  - Execution state: pending
