...
- [x] E-01 In `agent_workflows/check_engine.py`, add a COMMIT-SCOPED rule (e.g. `check.status-untooled`) that, given the set of PLAN files whose `- Status:` changed in the commit (compare staged content vs HEAD; ignore files not in the commit and files under `executed/`), flags each whose new status has NO matching tool-authored `## Workflow history` transition line for that status value (predicate A per OQ-01: a plain textual "is there a matching `- <date> <status> (...)` line" check - catches the careless omission, does NOT catch a hand-edit that also adds a plausible line; that limit is accepted). Reuse `ipd_lint`'s `_HISTORY_LINE_RE`/`doc.history_lines` parsing (`ipd_lint.py:168`) rather than a second parser. Fire fail-closed with the standard `Drift`/exit convention. NO grandfathering and NO whole-tree scan (only commit-changed files are examined, so historical records are never touched). Restrict to history-bearing plan records; do not examine prompts/releases (no `## Workflow history`).
  ...
  - Evidence (E-01): Added `check_engine.check_status_untooled(repo_root)`: runs `git diff --cached --name-status -M -- .aw/records/plans/`, and for each plan `.ipd.md` whose staged `- Status:` differs from HEAD's, with no matching tool-authored `## Workflow history` line for the new status, emits a `check.status-untooled` Drift. Reuses `ipd_lint.parse().history_lines` + `_HISTORY_LINE_RE` (no second parser). `executed/` paths skipped; deletions skipped; only plan IPDs examined. Fast no-op when nothing staged. Wired into `check_types` in the `collisions`/`["all"]` branch so it rides `aw check`/`aw check all`. DECISION 16-79li67-D1 records the integration-point choice.
  - Execution state: performed

### Task group 2: Wire it as a local pre-commit hook + doctor remediation

- [x] E-02 Register the E-01 check as a LOCAL `repo: local` pre-commit hook in `.pre-commit-config.yaml` (mirroring ipdgates Order 08's pattern and the existing local-leaks guard), scoped to the staged plan files, running once per commit and a fast no-op when no plan status change is staged; and map the rule into `agent_workflows/doctor.py` so `aw doctor` (over changed files) reports it with a self-documenting remediation naming the exact fix ("apply this status change via `aw set <status> <id6>` so it carries an attributed history entry"). Do NOT add any GitHub Actions / CI (local only, per Order 08's human decision).
  - Depends on: E-01
  ...
  - Evidence (E-02): Added `agent_workflows/hooks/status_untooled_gate.py` (wraps the E-01 rule; exit 0 ok/no-op, 1 refused) + the `aw ipd-status-untooled-gate` CLI subcommand (parser + description + dispatch in cli.py) + a `command_surface.py` CommandDeclaration (check-class, mutation_gate none). Registered a `repo: local` hook `ipd-status-untooled-gate` in `.pre-commit-config.yaml` AND in BOTH engine.py installer templates (`_LOCAL_LEAKS_PRECOMMIT_TEMPLATE` + `_LOCAL_LEAKS_PRECOMMIT_BLOCK`), so a fresh `aw install`/`setup-repo` writes it. Mapped `check.status-untooled` into `doctor.build_remediation` (title + `aw set <status> <id6>` command, status word parsed from the detail) AND invoked `check_status_untooled` in the doctor artifact probe alongside `check_collisions`. NO GitHub Actions/CI file added (the three existing `.github/workflows/*.yml` are untouched). Verified e2e in a temp repo with the installed `.git/hooks/pre-commit`: a raw hand-edit commit is REFUSED naming `aw set approved`, the tooled path (matching line added) PASSES.
  - Execution state: performed

### Task group 3: Prove detection + no false positives + no whole-tree scan

- [x] E-03 Add tests: a fixture staging a hand-edited `- Status:` change (no matching tool-authored line) is FLAGGED; a fixture whose status change was made via `aw set` (matching attributed line) is CLEAN; an UNCHANGED plan with a historically hand-set status is NOT examined (proving commit-scoping, not a tree scan - no grandfathering needed); a plan moved OUT of `executed/` IS checked; a plan inside `executed/` is NOT; a prompt/release (no `## Workflow history`) is not falsely flagged; an ordinary commit with no plan status change is a fast no-op. Confirm `pytest -n auto` is green. No network/CI test.
  ...
  - Evidence (E-03): Added `tests/test_status_untooled_gate.py` (11 tests): hand-edited flip FLAGGED; freshly-added plan with no matching line FLAGGED; tooled change (matching `aw set` line) CLEAN; UNCHANGED historical hand-set status NOT examined (incl. after an unrelated body edit) - commit-scoping proven; plan INSIDE executed/ NOT examined; plan MOVED OUT of executed/ IS checked+flagged; prompt status change NOT examined; release status change NOT examined; ordinary commit fast no-op; body-only edit (status unchanged) CLEAN; plus an end-to-end installed-hook test that a raw hand-edit commit is BLOCKED naming `aw set`. `pytest -n auto` = 2315 passed, 1 skipped. No network/CI test.
  - Execution state: performed
...
- Document the new `check.status-untooled` rule + local hook in the check-rule list / `aw check`/`aw doctor` help and the lifecycle/installer docs (via managed verbs): it is a COMMIT-SCOPED DETECTOR for intermediate hand-edited status (the terminal gate is ipdgates Order 08; the preventive path is `aw set`); local only, no CI. Note `--no-verify` bypasses the hook (honest limit). Else N/A with reason.
  - DONE: added a paragraph to `.aw/system/workflows/ipd-lifecycle/ipd-lifecycle.md` (right after the dulzpy terminal-gate paragraph) describing the proclint intermediate-transition detector, its two surfaces (`aw check`/`aw doctor` + the `ipd-status-untooled-gate` local hook), commit-scoping (no whole-tree scan / no grandfathering / executed/ + history-less types excluded), and the honest limits (predicate A evadable; `--no-verify` bypasses; no CI). Appended a matching spec history note via `aw specs note` to `20260802-1904-01-ipd-structure-and-linting.spec.md`. The CLI subcommand carries a self-documenting `--help` description (cli.py DESCRIPTIONS).
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: `check_engine.check_status_untooled` (check_engine.py) reads `git diff --cached --name-status -M -- .aw/records/plans/`, compares the staged `:0:` blob's `- Status:` against HEAD's, and emits `check.status-untooled` when the staged status changed with no matching `## Workflow history` line (reusing `ipd_lint.parse().history_lines` + `_HISTORY_LINE_RE`). `tests/test_status_untooled_gate.py::UntooledStatusDetectorTests` drives it in real temp git repos: `test_hand_edited_status_change_without_history_line_is_flagged` asserts exactly one `check.status-untooled` Drift naming the new status + `aw set`; `test_new_plan_added_with_status_but_no_matching_history_is_flagged` (added-plan case) flagged; `test_tooled_status_change_with_matching_history_line_is_clean` yields none; `test_unchanged_plan_with_historically_handset_status_is_not_examined` yields none even after an unrelated body edit (commit-scoping, no grandfather); `test_plan_inside_executed_is_not_examined` yields none; `test_plan_moved_out_of_executed_is_checked` flags the moved-out plan; `test_prompt_status_change_is_not_examined` + `test_release_status_change_is_not_examined` yield none. Ran `pytest tests/test_status_untooled_gate.py -q` -> 11 passed.
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: `.pre-commit-config.yaml` carries a `repo: local` hook `ipd-status-untooled-gate` (`entry: python3 -m agent_workflows ipd-status-untooled-gate`, `language: system`, `pass_filenames: false`, `always_run: true`), mirroring the local-leaks/dulzpy pattern; `engine._LOCAL_LEAKS_PRECOMMIT_TEMPLATE` AND `_LOCAL_LEAKS_PRECOMMIT_BLOCK` both include it (verified `'ipd-status-untooled-gate' in ...` == True for both), so a fresh `aw install`/`setup-repo` writes it. E2E in a temp repo with the installed `.git/hooks/pre-commit`: a raw hand-edit `git commit` was REFUSED (exit 1, message named `aw set approved`), and the tooled path (matching history line added) PASSED (exit 0, commit landed). `aw doctor` in a temp repo staging a hand-edited status renders "Plan status changed without an attributed history entry (looks hand-edited)" with a fix naming `aw set reviewed <id6>` (build_remediation `check.status-untooled` branch + the doctor probe now calls `check_status_untooled` alongside `check_collisions`). NO `.github/workflows/` file added or changed (the 3 existing yml are untouched). `test_ordinary_commit_no_plan_status_change_is_noop` proves the fast no-op.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: `pytest tests/test_status_untooled_gate.py -q` -> `11 passed` (all cases above). Full suite `python3 -m pytest -n auto` -> `2315 passed, 1 skipped in 122.87s (0:02:02)` (up from 2304 before this plan, i.e. the 11 new tests). `aw specs check` -> "all specs conform"; `aw ipd lint --all` -> conforming=8, error=0; `check-local-leaks --agent` -> clean (exit 0). No network/CI test introduced; no `.github/` change.
  - Result: pass
