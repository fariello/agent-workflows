...
- [ ] E-03 Enable spec name-checking with a grandfather cutover: add `"names"` to `check_engine.SUPPORTED["specs"]`, and make `check_names` pass `require_id6=True` ONLY for specs whose date (or recorded creation) is at/after the repo dependency/naming cutover marker; pre-cutover specs stay conformant (advisory `grandfathered` at most, never mass-fail). Reuse the existing cutover-policy mechanism if one exists; otherwise use the spec's filename date against a single configured cutover date.
  - Depends on: E-02
  - Expected outcome: `aw check specs` still reports 0 name failures for all 21 existing (pre-cutover) specs; a synthetic post-cutover legacy-named spec is flagged `check.name-nonconformant` with an `aw rename specs ... --to-id6` recovery command.
  - Execution state: pending

### Task group 3: On-demand id6-minting rename

- [ ] E-04 Extend the rename engine: add an id6-minting conversion mode (e.g. `aw rename specs <legacy> --to-id6`) so `compute_target_name` (artifact_rename.py:67) recognizes the legacy `_LEGACY_TIMESTAMP_RE` spec form and, in this mode, mints an id6 (`artifact_core.generate_id6` over existing spec ids), returns the clustered `build_clustered_name(..., artifact_type="spec")` name, AND signals the caller to inject `- Id: <id6>` into the file metadata during the rename transaction. Keep the default (non-`--to-id6`) rename behavior unchanged (HHMM-preserving). A spec that already has an `- Id:` reuses it rather than minting.
  - Depends on: none
...
- [ ] E-05 Ensure the `--to-id6` rename rewrites repo-wide references to the old spec path/stem (reusing `plan_reference_rewrites`), covering full-path citations; where a full-path citation cannot be auto-rewritten, report it fail-loud so the operator can fix it. Pilot on the 2 already-id6 specs (25kzda, research-lifecycle) as a preview-only demonstration in the test/validation, NOT an in-run mass rename.
  - Depends on: E-04
  - Expected outcome: a preview of `aw rename specs 25kzda --to-id6` lists the new name and every reference that would be rewritten; a test asserts references to a renamed fixture spec are updated and any unrewritable full-path citation is surfaced.
  - Execution state: pending
...
- 19 of 21 specs have no id6 at all; 2 (`25kzda`, the research-lifecycle spec) already carry `- Id:` but still use legacy filenames - the cheap pilots.
...
`python -m pytest tests/ -p no:randomly` green including new tests for: the spec producer (mints id6, clustered filename, `- Id:` present, lints clean); `is_conformant` require_id6 matrix; `aw check specs` grandfathering (all 21 existing specs pass; a synthetic post-cutover legacy spec fails with the rename recovery); the `--to-id6` minting rename (previews name + `- Id:` injection, `--apply` git-mv + metadata write, idempotent on an id6 spec, reference rewrite + fail-loud on unrewritable citation). `aw ipd lint` on this plan clean; `aw check all --agent` adds no new findings to the existing baseline for the 21 legacy specs; `aw sanitize --agent` clean.
...
- Status: open
- Owner: none
- Resolution or deferral rationale: E-03 needs a cutover boundary to decide which specs must be id6-clustered. Recommended: reuse the repository's existing dependency/naming schema-cutover policy marker if one is already recorded (the run-and-verify spec 25kzda references a "dependency-schema cutover commit"); if no reusable marker exists at execution time, fall back to a single configured cutover date compared against the spec filename date, and record that choice as a DECISION. Non-blocking: both approaches are deterministic and testable; prefer the existing policy marker for consistency.
...
- [ ] V-03 validates E-03
  - Required evidence: `aw check specs --agent` showing ZERO name failures across all 21 existing (pre-cutover) specs; a test with a synthetic post-cutover legacy-named spec showing `check.name-nonconformant` with an `aw rename specs ... --to-id6` recovery command.
  - Observed evidence:
  - Result: pending
...
- [ ] V-05 validates E-05
  - Required evidence: a test showing references to a renamed fixture spec are rewritten and an unrewritable full-path citation is surfaced fail-loud; a preview-only `aw rename specs 25kzda --to-id6` listing the references it would rewrite (NOT applied in-run).
  - Observed evidence:
  - Result: pending
