...
- Status: open
- Owner: maintainer
- Resolution or deferral rationale: BLOCKING for the SPEC's content, and it is the item's own question 3 sharpened by a fact the item did not have: `97df1z` is now EXECUTED (F-1), so `--full-auto` is live and performing automated advances today. A fail-closed default matches this repository's posture everywhere else, but applied naively it would refuse the transitions a shipped feature already performs, breaking it on the day the policy lands. THREE SHAPES. (a) Fail-closed with an explicit GRANDFATHER list naming exactly the transitions `--full-auto` performs today: safest and most honest, at the cost of a list that must be maintained. (b) Permissive default matching today's behavior, with fail-closed available per transition: nothing breaks, but the repository's default posture inverts, which is a real change of character. (c) Fail-closed with no grandfathering and a coordinated `--full-auto` change in the same release: cleanest end state, largest blast radius. RECOMMENDED: (a), because it is the only one that is both fail-closed and non-breaking, and the grandfather list doubles as the inventory E-01 produces anyway.
...
- Status: open
- Owner: maintainer
- Resolution or deferral rationale: DEFERRED, leaning BOTH, and recorded because it is the item's question 2. Loosen-only is easier to reason about and is the safer default for a local invocation. But tighten-also is genuinely useful in CI, where a job may want to be STRICTER than the repository policy (for example refusing any automated advance at all on a release branch), and a policy that cannot be tightened forces CI to reimplement the check, which is exactly the divergence E-04's single-predicate rule exists to prevent. RECOMMENDATION: allow both, and require that a TIGHTENING override never needs justification while a LOOSENING one is recorded in the artifact, mirroring how `--allow-open-questions` is folded into the actor string today so an override is auditable rather than silent.
