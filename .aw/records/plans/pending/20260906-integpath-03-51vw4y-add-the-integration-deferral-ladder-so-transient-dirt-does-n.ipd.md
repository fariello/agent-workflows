...
- [ ] E-01 Add `integration-deferred` as a NON-TERMINAL status and keep it out of `TERMINAL_STATES` in BOTH runners (`oc_runipd.py:301-313`, `agy_runipd.py:355`). This is the single change that makes re-attempt possible: today's `integration-blocked` is terminal, which is why nothing retried.
  AUDIT EVERY TERMINAL_STATES CONSUMER before adding the status, because a new status that some readers treat as unknown is worse than no new status. Both runners read the set at least at `oc_runipd.py:3451` and `agy_runipd.py:3052`/`:4246`, and `agy_runipd.py:3052` computes `TERMINAL_STATES - {"dependency-blocked", "not-attempted"}`, so a set-difference expression can silently include or exclude a new member. Enumerate the consumers, state what each does with a deferred item, and make each choice deliberate.
...
- [ ] E-02 Add a SEPARATE `--integration-retry-limit` (default 10) plus its config default, and do NOT reuse `DEFAULT_RETRY_LIMIT`. Two different quantities are being counted and conflating them is a category error the backlog item names explicitly.
  ...
  DO NOT CLAMP IT TO SPEC 2.1's 0..10 RANGE, which bounds the CORRECTION budget specifically. And note there is nothing to reuse even if reuse were wanted: `plan_retry`/`retry_budget_remaining` have zero production callers (backlog `trjfyy`), so this is a new counter either way. Register the flag through the shared run-policy flag table (`runner_shared.RUN_POLICY_FLAGS`) so both hosts get it from one declaration and it is frozen at queue build like every other policy value.
  - Depends on: E-01
  - Expected outcome: a distinct integration-retry budget on both hosts, default 10, frozen at queue build, provably independent of `DEFAULT_RETRY_LIMIT` (changing one does not move the other).
  - Execution state: pending

### Task group 2: the three rungs

- [ ] E-03 RUNG 1, DEFER AND RE-ATTEMPT while other work exists. On a dirty-overlap refusal, mark the item `integration-deferred` rather than `integration-blocked`, and re-attempt integration at the top of the existing dispatch loop (`oc_runipd.py:6970`-region), which already reloads state and already runs `cascade_dependency_blocked` each iteration. Zero waiting, zero tokens, nothing blocked: the next item's completion is the natural retry trigger.
...
- [ ] E-04 RUNG 2, a BOUNDED POLL when nothing else is dispatchable. THE TRIGGER IS NOT "is this the last item" but "is there any item I could dispatch instead", which the loop ALREADY computes as `runnable is None` (`oc_runipd.py:7024`). That one condition covers both the last-item case and the case where five items remain and ALL are deferred, which a last-item test would miss.
...
- `runnable is None` IS ALREADY COMPUTED (`oc_runipd.py:7024`) and is exactly rung 2's trigger. Do not invent a last-item test.
...
- `is_interactive_run` (`runner_shared.py:1790`) ALREADY encodes the correct prompt predicate (real TTY on stdin and stderr, AND no `--unattended`, the latter winning over a TTY that happens to exist). Reuse it.
...
| # | Finding | Evidence |
|---|---|---|
| F-1 | `integration-blocked` is TERMINAL, so a refused integration is never re-attempted for the rest of the run. Still true at HEAD `a4279302`. | `oc_runipd.py:301-313`, `agy_runipd.py:355` |
| F-2 | `integration-deferred` does NOT exist as a status in either runner. | measured grep 2026-09-06 |
| F-3 | **A PARTIAL-LOOKING ARTIFACT EXISTS AND IS NOT THE LADDER.** Both runners write an `integration_deferred` reason string into the attempt record and `integration_deferral` onto the item, then set the status to a TERMINAL value anyway. An executor grepping for the name could wrongly conclude the work is done. | `oc_runipd.py:6473`, `agy_runipd.py:3671` |
| F-4 | THE MEASURED INCIDENT: run `run-20260905T050043Z-639569`, 34 items, 7h40m, $165.90. Four items refused on dirty overlap (`76gsmv` 08:06:32, `eyh1fu` 08:51:26, `txc9l1` 10:48:33, `uyeko5` 11:47:04); three more cascaded to `dependency-blocked`. Seven of 34 lost to transient dirt. | backlog `5wdoze` event excerpts |
...
| F-8 | `runnable is None` is already the loop's own condition and covers both the last-item case and the all-deferred case that a last-item test would miss. | `oc_runipd.py:7024` |
...
| F-10 | `TERMINAL_STATES` is consumed via SET DIFFERENCE in at least one place (`TERMINAL_STATES - {"dependency-blocked", "not-attempted"}`), so adding a member changes that expression's result implicitly; every consumer needs an audited decision. | `agy_runipd.py:3052`; also `oc_runipd.py:3451`, `agy_runipd.py:4246` |
...
- `python3 -m pytest` BARE, before and after, with the `N passed` summary line pasted and counts stated. Baseline at authoring: `5536 passed, 3 skipped, 2 xfailed` at HEAD `3d239cfa`; re-measure at execution since children 01 and 02 land first.
...
Spec `25kzda` (deterministic run-and-verify) governs the run flag surface and the retry budget's 0..10 range. The new `--integration-retry-limit` is a DIFFERENT quantity from the correction budget that range bounds (F-6), so it must NOT be described as, or clamped to, that budget. If the executor finds the spec's flag table or its retry-budget section stating a single retry budget, NOTE IT for a spec amendment rather than editing the spec here: this plan has no spec-edit authority, and the spec's finding-code table in particular is transcribed verbatim into `run_evidence.RUN_FINDING_CODES` with a byte-equality test, so editing a cell IS a code change.
...
- [ ] V-01 validates E-01
  - Required evidence: paste both runners' `TERMINAL_STATES` showing `integration-deferred` ABSENT and `integration-blocked` still present. ENUMERATE every `TERMINAL_STATES` consumer you found (at minimum `oc_runipd.py:3451`, `agy_runipd.py:3052`, `agy_runipd.py:4246`) and state for each what a deferred item does there, calling out the set-difference expression explicitly. Paste a probe showing a dependent item is NOT dispatched while its prerequisite is `integration-deferred`.
  - Observed evidence:
  - Result: pending
...
DEPENDENCY: this child declares `- Item-Dependencies: executed:6sb3yu` and MUST NOT run before child 02. The ladder belongs in the SHARED integration module; written before the extraction it would be written twice, into two copies already measured as drifted (0.65 similarity), which is the outcome three of the four source backlog items explicitly ask to avoid. The runner re-checks dependencies at dispatch, so a queued-together Set is safe.
