...
- [ ] E-01 In `agent_workflows/cli.py`, replace the body of `_run_show` (currently cli.py:3514-3525) so it FIRST tries the records selector, THEN falls back to the action ledger. Use this exact new body (keep the function name + signature `def _run_show(args, term)`):
  ```python
