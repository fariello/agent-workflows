...
- [ ] E-04 Bounds-check the retry budget at the spec's 0 through 10 inclusive, on the SHIPPED helper rather than a new one. MEASURED (citations CORRECTED at review, PR-003, re-verified at HEAD `381dbd5c`; re-measure again before use, since HEAD moves hourly here): `run_recovery.retry_budget_remaining` is at `agent_workflows/run_recovery.py:355` and `DEFAULT_RETRY_LIMIT: int = 2` is at `:62`. The earlier draft cited `:340` and `:47`, which had drifted by roughly 15 lines; the symbols were correct, only the line numbers were stale. THE DISCREPANCY IS ALREADY RESOLVED, so do not re-litigate it: spec 2.1 said 2 while the code said 3, and the maintainer decided (2026-08-31) that the SPEC is authoritative. `DEFAULT_RETRY_LIMIT` is now 2 (see OQ-03). Your job here is ONLY the range validation; do NOT change the default again, and do not 'restore' 3. Add only the RANGE validation the spec fixes (reject below 0 and above 10, at the boundary where the value enters), plus the spec's rule that the frozen value cannot change on resume if that is cheap to assert where the value is read.
  - Depends on: none
  - Expected outcome: a budget outside 0..10 is refused at entry with a clear message; 0 and 10 are both accepted (boundaries tested, not just the middle); `DEFAULT_RETRY_LIMIT` remains 2 (already aligned to the spec; see OQ-03) and no test hard-codes it.
  - Execution state: pending

- [ ] E-05 Extend the SHIPPED test modules (`tests/test_run_evidence_completion.py`, `tests/test_run_recovery_cli.py`) rather than creating a new one. The retired plan proposed `tests/test_deterministic_checker.py`, which its review rejected because these modules plus `tests/test_run_viewer.py` already cover these surfaces. Cases MUST include: the 13 codes enumerated and each message asserted against the SPEC TEXT (so rewording fails the test); every code's recorded state matching a live re-measurement (so the mapping cannot silently rot); `--unverifiable-ok` changing ONLY the aggregate and provably not the item label, in both directions; and the retry-budget boundaries 0, 10, -1, and 11. Do NOT weaken, remove, or alter any existing assertion in either file. NOTE two tests in `tests/test_run_recovery_cli.py` were already updated (2026-08-31) to DERIVE their expectations from `DEFAULT_RETRY_LIMIT` instead of hard-coding 3; keep that property.
...
- [ ] V-04 validates E-04
  - Required evidence: paste the refusals for -1 and 11 and the acceptances for 0 and 10 (boundaries, not a middle value). Paste `DEFAULT_RETRY_LIMIT` showing it is 2, matching spec 2.1 (already aligned 2026-08-31 per OQ-03; do NOT change it again). Paste evidence no test hard-codes the number.
  - Observed evidence:
  - Result: pending
