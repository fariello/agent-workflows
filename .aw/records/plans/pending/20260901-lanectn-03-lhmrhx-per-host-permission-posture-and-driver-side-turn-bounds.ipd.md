...
- [ ] E-04 IMPLEMENTS R4.4, R4.4a. Add the two driver-side bounds that do not trust the host, with the defaults spec R4.4 makes NORMATIVE, and arm them for EVERY unattended turn whether or not it is isolated (maintainer ruling 2026-09-01, "make it uniform"). (a) PERMISSION DEADLINE, default 30 SECONDS, armed when the driver OBSERVES a permission request including a nested child-session one; seconds rather than minutes because an unattended turn has NO answerer, so the ask is dead rather than slow. (b) ABSOLUTE TURN DEADLINE, default 4 HOURS, which output CANNOT extend; that is what distinguishes it from the coarse no-progress watchdog, which a chatty-but-wedged turn resets forever. Both overridable per run. On expiry, terminate through the ONE shared reaper (spec `c4gd2h` R5 forbids a second; do not call a lower-level terminator directly and do not add a bare kill) and record the safe-failure disposition naming WHICH bound fired.
  - Depends on: E-03
  - Expected outcome: the permission deadline defaults to 30s and the absolute deadline to 4h, both overridable; a synthetic unanswered permission request terminates the turn within the permission deadline and demonstrably NOT at the coarse no-progress bound; both bounds are armed for a NON-isolated turn as well as an isolated one; the recorded disposition names the firing bound; and the termination is attributable to the shared reaper.
  - Execution state: pending
...
- [ ] V-03 validates E-03 (proves R4.3, R4.1a; spec A9, A8b)
  ...
  - Observed evidence:
  - Result: pending
- [ ] V-04 validates E-04 (proves R4.4, R4.4a; spec A10, A10b)
