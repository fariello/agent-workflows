...
- Status: open
- Owner: maintainer
- Resolution or deferral rationale: RAISED AT AUTHORING because the item's own analysis cuts both ways and the answer changes what the predicate accepts. AGAINST: the item argues a spec is not sufficient, because there is no per-requirement tracking, no notion of partial implementation, and no join from a requirement to an implementing plan, so "a spec can sit `approved` for weeks with half its requirements unbuilt and nothing asks which half". FOR: `aw attention` DOES map spec `approved` -> `ready`, so a spec is not invisible the way an executed plan is, and the repository already treats a spec as a legitimate release-gate carrier (`- From-Spec:` is a recognized field and a spec may carry `Blocks-Release:`). MY READING, offered not adopted: accept a spec as a carrier ONLY when it is non-terminal, and treat that as strictly better than the status quo of prose, while recording that it is weaker than a backlog item. E-02 is written so the accepted-carrier set is one list in one predicate, so widening or narrowing it later is a one-line change plus a test. NOT BLOCKING: the backlog-item and pending-plan carriers alone close the defect this plan exists to close.
