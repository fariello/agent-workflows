...
| F-10 | MED (added at review) | `oc_runipd.py:6863`; `runner_shared.py:3258`; `evaluate_backlog_close` | THIS PLAN CANNOT CLOSE `6h7y2y` BY EXECUTING, AND MUST NOT CLAIM IT WILL. `process_backlog_close` fires only at an agent-executed IPD's finalize, and `evaluate_backlog_close` requires EVERY IPD carrier terminal `executed`; the orchestrator `y9s4vm` also carries `- From-Backlog: 6h7y2y`, and orchestrator retirement (`dispatch_orchestrator_item`, `retire_orchestrator`) contains no backlog-close call at all. Measured: the verdict is `close=False, 'IPD carrier(s) not executed: ...y9s4vm..., ...jxxec8..., ...iuxtjy...'`. So the close is a DELIBERATE act after the whole Set, exactly as the three `done/` precedents with orchestrator carriers (`kjzlgw`, `1ap48y`, `kxkc04`) were each closed by hand. | `evaluate_backlog_close(repo,'6h7y2y',[])` run live; `'backlog' in getsource(dispatch_orchestrator_item)` is False |
...
- [ ] V-05 validates E-05
  - Required evidence: paste a `python3 -c` showing any new shared symbol resolves to the SAME object from `oc_runipd`, `agy_runipd` and `runner_shared`. Paste both hosts resolving the same spec selector identically. Paste the RE-MEASURED `--full-auto` default for BOTH hosts (not agy alone, and not this plan's stale claim: F-9), then show that a widened `ACTION_IMPLEMENTED` cannot execute a plan the operator asked to merely graduate under whatever the defaults actually are. Paste the AST-measured oc-to-agy import count before and after, showing it did not increase from 47.
  - Observed evidence:
  - Result: pending
