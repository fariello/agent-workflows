...
- [ ] E-01 RE-SCOPED BY THE MAINTAINER'S 2026-09-26 OQ-03 RULING: in `check_engine._question_obligations`, skip a question whose heading is still the scaffold's untouched placeholder (`### OQ-01: TODO a question`), matched through `ipd_authoring._AUTHORING_PLACEHOLDERS` rather than a new literal, so a fresh scaffold no longer fails the gate on the scaffold's OWN example. Do NOT change `build_skeleton` and do NOT emit any carrier line; the templates therefore stay byte-equal with no regeneration. The bullets below that discuss choosing placeholder TEXT are SUPERSEDED by this ruling and kept only as history.
  - Depends on: none
