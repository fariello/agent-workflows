...
- [ ] E-03 SWITCH THE WRITERS IN BOTH HOSTS TO EMIT THE CANONICAL TOKENS, which is the only item that changes what a new run records. `reconcile_disposition` in each host is the primary site; `runner_stop.STOPPED_DISPOSITION` already emits `interrupted` and needs no change, only promotion into the terminal set (E-04). Emit `fail-verify` where the verifier refused, `fail-gate` where a lifecycle gate refused, `fail-merge` where integration refused, `fail-depend` for an unmet prerequisite, `not-run` for never-reached, and `failed` only where none of the above applies. THE LEGACY `blocked` TOKEN SPLITS ACROSS TWO NEW LABELS AND IS NOT A SINGLE ALIAS ROW (OQ-01, PR-008): its four PRE-FLIGHT producers (`clean_base_refusal`, `begin_refusal`, `worktree_error`, and the sweep-refresh arm) become `fail-gate`, while `_VERDICT_TABLE`'s `VERDICT_BLOCKED`/`VERDICT_NOT_CONFORMING` arms become `fail-verify`, because the verifier is the authority that refused there. E-01's alias map must therefore map the legacy READ of `blocked` to a single canonical token for back-compatibility (`fail-gate`, the 193-of-209 majority) while the WRITERS pick per producer; state that asymmetry in the map's own comment so a later reader does not "simplify" it away.
  - Depends on: E-02
...
- [x] OQ-01 Should `blocked` map to `fail-gate`, or to its own label? Blocking: no. RESOLVED: map to `fail-gate`, and the ORIGINAL JUSTIFICATION IN THIS PLAN WAS FACTUALLY WRONG (maintainer challenge, 2026-09-24; corrected at review, PR-008). The draft claimed `blocked` "means the agent stopped and preserved its work" and argued from operator indistinguishability. THE AGENT NEVER SETS IT. Every producer is a PRE-FLIGHT REFUSAL BY THE RUNNER, raised BEFORE an agent session starts, and each records its own reason field. Measured over all 209 `blocked` items in `.aw/records/runs/`: 179 `clean_base_refusal` (the dirty-base gate), 9 `begin_refusal` (`aw ipd begin` declining execution authority), 5 `worktree_error` (lane allocation failed), 16 with no reason field, those being the verifier's `VERDICT_BLOCKED`/`VERDICT_NOT_CONFORMING` arms via `_VERDICT_TABLE`. So `fail-gate` is correct ON THE EVIDENCE rather than by analogy: four of the five producers ARE gates, which is the authority the label names.
  ...
  ONE GAP THIS ANSWER EXPOSES, recorded rather than folded in: the 16 verifier-verdict cases come from a DIFFERENT authority than the four pre-flight gates and, unlike them, leave NO reason field on the item. E-03 must therefore emit `fail-verify` (not `fail-gate`) for the `VERDICT_BLOCKED`/`VERDICT_NOT_CONFORMING` arms, since the verifier is what refused; and the missing-reason case is a separate defect this plan does not fix (it renames outcomes, it does not add records).
...
- [ ] V-09 validates E-09
  - Required evidence: paste `aw runs` output showing the `Landed` column, and paste the agreement check between that column and `.aw/records/plans/executed/` membership for every item of a replayed historical run. THE BAR IS DIRECTIONAL, NOT EQUALITY (corrected at review, PR-004, on F-02's re-measurement): `Landed` is computed FROM the directory, so it must agree with the directory exactly, and it must NOT be required to agree with the STATUS token. On the cited run three items (`yeh7gc`, `m7gvuz`, `xdvglg`) carry a non-`executed` status while sitting in `executed/`; a `Landed` column that reported `no` for those three would be WRONG, and the original wording of this item would have accepted that error as success. Assert: every item whose plan is in a terminal `executed/` reads landed, every other reads not-landed, and report the status-versus-landed disagreements as EXPECTED output rather than as a defect.
  - Observed evidence:
  - Result: pending
