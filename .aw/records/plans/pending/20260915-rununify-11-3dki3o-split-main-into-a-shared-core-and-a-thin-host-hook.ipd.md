...
- Scope-Paths: agent_workflows/runner_shared.py, agent_workflows/oc_runipd.py, agent_workflows/agy_runipd.py, tests/test_rununify_main.py
...
- [ ] E-02 Extract the host-neutral core into `runner_shared.py`, taking the oc logic, and define the hook boundary at the points named in Findings rather than wherever the code happens to divide. Each host keeps a thin function that supplies its own pieces and calls the core. Do NOT change behavior: this is a relocation with a parameter, and any place the two hosts genuinely differ becomes a hook input, not an `if host == ...` branch inside the core.
  - Depends on: E-01
  - Expected outcome: one core in `runner_shared`; each host's `main` is a thin caller; no host-name conditional inside the shared core.
  - Execution state: pending
...
- [ ] E-04 Add `tests/test_rununify_main.py`: the core resolves to the SAME OBJECT from both hosts; an AST scan proves neither runner still holds a second implementation (repo-wide, per the parent's F10, not a pairwise check); each host's hook still supplies its OWN host-specific pieces; and the hazard named in Findings is asserted directly.
...
| # | Sev | Where | Finding |
|---|---|---|---|
| F-1 | HIGH | measured at HEAD | `main` is the process entry point: argument dispatch, subcommand routing, and the top-level error translation. Only 8 of 46 changed lines carry a host token, so most of the difference is drift in shared plumbing rather than genuine host specificity. |
| F-2 | HIGH | `main`, both hosts | THE HAZARD THIS SPLIT CARRIES: `main` owns EXIT CODES. Spec `25kzda` pins exit-code meanings (0 ok, 1 findings, 2 cannot-run), and `tests/test_exit_codes.py`-shaped assertions depend on them. A shared `main` must not renumber a single one. |
| F-3 | MED | `main` | DIFFERENCE, subcommand routing: each host routes its own verbs; the ROUTING TABLE is host data, the dispatch loop is not. |
| F-4 | MED | `main` | DIFFERENCE, error translation: the `DriverError` to exit-code mapping is shared logic that has drifted; unify it and keep each host's message prefix from the descriptor. |
| F-5 | MED | `main` | DIFFERENCE, host binary resolution: oc resolves `opencode`, agy resolves via `resolve_agy`. Genuinely host-specific; stays behind the hook. |
...
- Under-scope: this plan does not attempt to shrink `main` itself. Making the shared core smaller is a
...
2. `tests/test_rununify_main.py` (new): shared-core object identity across hosts; AST scan proving no
...
- [ ] V-02 validates E-02
  - Required evidence: pasted proof that the shared core is ONE object reachable identically from both hosts, that each host's `main` is now a thin caller, and a grep/AST result showing NO host-name conditional inside the shared core.
  - Observed evidence:
  - Result: pending
