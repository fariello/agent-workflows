...
- [ ] E-03 In `agent_workflows/attention_contract.py`, add a `_RELEASES_MAP` mapping the release statuses to attention classes: `{"planned": "ready", "blocked": "blocked", "shipped": "done"}` and register it in `CLASS_MAPS` (attention_contract.py:249) under key `"releases"`. Add `releases` to the tracked-tree policy (`TREE_POLICY`, attention_contract.py:83) so the attention scanner reads `.aw/records/releases/`. Add a `_release_record` reader in `attention.py` mirroring `_spec_record`/`_backlog_record` (read the release file's `- Status:` via the same bullet regex and call `A.class_of("releases", status)`).
  - Depends on: E-01
