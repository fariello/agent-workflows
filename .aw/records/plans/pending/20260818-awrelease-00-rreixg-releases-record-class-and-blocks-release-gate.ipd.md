...
- `aw check` flags an item whose `Blocks-Release` points at a nonexistent release (AC3), via validation
...
round-trips through the setter and parser (AC4), `aw check` flags a dangling release reference (AC3),
