...
- [ ] E-01 Create `agent_workflows/doctor.py` with a `run_doctor(repo_root: Path) -> List[Drift]` that COMPOSES existing signals into one drift list. Header + imports + function:
  ```python
...
- [ ] E-02 In `doctor.py` add the three composing probes, each reusing an EXISTING signal and returning `List[core.Drift]`. Reimplement nothing:
  ```python
...
- [ ] E-03 Add the `aw doctor` parser + dispatch + a `doctor.run(args)` entrypoint. In `doctor.py` add:
  ```python
  import sys
