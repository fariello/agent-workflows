...
RETIRED 2026-07-12: both findings are now resolved elsewhere, so this corrective IPD is no longer
needed. VE-1 (the order-dependent test_setup_artifacts error + the 2 red installer tests) is FIXED -
the isolation error was resolved by Gemini's 1043-01 execution and the 2 installer tests by commit
2707274; the full suite is green (205, OK). VE-2 (the systemic "execution must not exceed approved
scope / validate-before-executed" rule) is now carried by AGENTS.md item 6 ("Validation Requirement")
and DECISIONS D64, with the residual scope-guard clause tracked by IPD 20260712-auto-approved-00-7w8b8u-auto-approved-status-for-mechanical-correctives. Nothing left
for this IPD to do; not executed, retired per D45/D47.
...
- Id: 852jpc
- Set: verify-1041 (verify 1041 split ok but suite error)
- Order: 0
...
2. **Handle 1041-01 OQ2 (systemic rule).** Either implement it here or confirm it is carried by the
   ...
   IPD 20260712-auto-approved-00-7w8b8u-auto-approved-status-for-mechanical-correctives (auto-approved) and 20260712-falsely-marked-00-y6kuar-falsely-marked-executed-tests-still-red (validate-before-executed) are the
