RETIRED 2026-09-23: superseded by executed plan `1f7xno` (runnerlayer Order 02) for the part of its
scope that is now done, and REFUSED for the remainder because its central move is one `main`
deliberately reverted. Superseded by `1f7xno` plus the corrective work tracked by backlog `zt2b16`.
The lane `aw/lane/nmlx47` is DELETED; this header and the record below are the surviving evidence.

MEASURED AT `main` `50a820a6` WITH THE REPOSITORY'S OWN COMMITTED SCANNER, `tools/runner_fork_scan.py`,
not by reading either plan's prose. Re-measured rather than inherited: an earlier retirement of this
plan was drafted at HEAD `22cf67d9`, and a retirement that cites a stale measurement is the exact
failure this Set exists to stop.

WHAT IS GENUINELY DONE (4 of the 12). `reclaim_lanes_on_interrupt`, `reconcile_interrupted`,
`reconcile_disposition` and `enforce_dependency_preflight` no longer appear as forks: the scanner
reports them BOTH-DELEGATE, i.e. both hosts reach one body in `runner_shared`. Note this is
DELEGATION, not object identity - a naive `oc.X is agy.X` probe reports False for all four and is the
wrong instrument, which is worth recording because that probe is what first made this plan look
entirely unstarted.

WHAT REMAINS FORKED (3 of the 12), and it is NOT lost. `expand_selectors` (oc 97 / agy unparse lines),
`_lane_reclaim_prompt` (29) and `_add_output_mode_flags` (6) are still DIVERGENT FORKS. Backlog
`xw4rb7` ("output-mode flags orphaned between rununify children") and `ga2dz1` already carry this
work on `main`, so retiring this plan drops no obligation; the remainder has a live carrier that is
not this plan.

WHY THE REST IS REFUSED RATHER THAN RE-PLANNED, which is the decisive finding. This plan's central
move is group (b): re-point the three agy stubs (`classify_recovery_disposition`, `route_recovery_turn`,
`build_verify_and_continue_notice`) at `runner_shared`. `1f7xno` ATTEMPTED exactly that consolidation
and REVERTED it, recording why: the shared `classify_recovery_disposition` reads `st.path` and
`st.base_commit` off a `worktree_lease.LaneState` whose real fields are `worktree_path` and `base_sha`,
so it raises `AttributeError` on any lane that exists, and four `tests/test_resumedupe.py` tests failed
with that error the moment the move made them reach it. Executing this plan would adopt the body `main`
rejected. Backlog `zt2b16` (high, blocking, open) owns the real remedy, which is a RECONCILIATION -
pick the authoritative body, delete the other, bind both hosts - and a pure-move plan cannot perform it.
Backlog `yo0ooe`, landed on `main` in `50a820a6`, records the general hazard that a dead shared copy
hides defects indefinitely.

AND THIS PLAN NEVER PASSED ITS OWN GUARDS, verified independently on the lane ALONE with no merge
involved. `python3 -m pytest` on `aw/lane/nmlx47` at `7465977f`: `3 failed, 8751 passed`. Two failures
are `tests/test_runner_refork_guard.py` (`SymmetricReForkGuardTests::test_every_runner_attribute_is_the_owning_modules_object`
and `FrontMatterReaderBehaviorTests::test_both_runners_still_accept_every_spelling_they_did_before`),
because the lane DELETED `agy_runipd._read_status` while that guard requires the re-export in BOTH
runners; the third is `tests/test_runner_item_dependencies.py::ExternalTargetReadinessMatrixTests`.
Neither failing file is in this plan's `Scope-Paths`, which is why its own run reported the failures as
belonging to the merge rather than to the lane. A plan whose lane is red on its own is not a plan whose
integration merely went badly.

A TRIAL MERGE WAS PERFORMED AND ABANDONED: 20 conflict hunks across 9 files including all three runner
modules, and `main` and the lane had added 64 and 68 `runner_shared` definitions respectively since
their merge base with exactly ONE in common. That is divergence, not a rebase.

WHAT THE RECORD BELOW IS STILL GOOD FOR. Its E/V evidence is unusually thorough and it CORRECTS its own
plan in several places (the authored `488 oc lines` did not reproduce; the shape partition was 6/3/2/1,
not the authored 7/3/2; `_record_forced_stop`'s only difference is a QUOTED type annotation). It also
found and fixed four live defects in dead `runner_shared` copies, and the four backlog items it filed
(`39jkux`, `6ckjze`, `e8i8dz`, `yo0ooe`) were landed on `main` in `50a820a6` INDEPENDENTLY of this
retirement, because a defect report is true on its own evidence regardless of whether its plan ships.
Read this record as measurement, not as an instruction to execute.

HTTP-STYLE SUMMARY OF THE DISPOSITION: superseded in part, refused in part, evidence preserved, remainder
carried by `xw4rb7`/`ga2dz1`/`zt2b16`, lane deleted.
