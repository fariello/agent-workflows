RETIRED 2026-08-30: split into the `wkindname` Set because it bundled a mechanical 88-file migration with a two-contract feature; superseded by .aw/records/plans/pending/20260829-wkindname-00-c2tvmm-one-work-nature-field-name-across-backlog-plans-and-specs.ipd.md (orchestrator), child 01 `9trlc3` (rename + migrate backlog) and child 02 `ng2blv` (add the field to IPDs and specs). All maintainer decisions and review findings carried forward.
...
- Scope-Paths: agent_workflows/backlog.py, .aw/records/backlog, agent_workflows/ipd_schema.py, agent_workflows/specs.py, agent_workflows/check_engine.py, agent_workflows/cli.py, agent_workflows/status_set.py, tests/test_work_kind.py
...
## Workflow history
- 2026-08-30 superseded (aw set): Split into the wkindname Set at the maintainer's direction. Not a size split (8 leaves / 3 groups against limits of 18 / 5): the spec's concern-mixing and independently-executable-phases criterion. After the rename-first decision this plan bundled a mechanical 88-file migration with a two-contract feature, whose failure modes and evidence differ, and whose phases were already independent (the field-addition items declared Depends on: none). Superseded by orchestrator c2tvmm plus children 9trlc3 (rename + migrate) and ng2blv (add the field). Also fixes a real defect in this plan: its cohesion rationale still claimed 6 leaves across 2 task groups after the rename items took it to 8 across 3.
...
### Task group 0: rename backlog's field first, so there is ONE field name

- [ ] E-07 Rename the on-disk work-nature field from `- Kind:` to `- Work-Kind:` in `backlog.py` only, covering its field regex, its parser mapping, its validator, and its `new` renderer (locate each by symbol). Constraints: anchor every change on the full-line field pattern and never on the bare token `Kind`, because `backlog.py` also parses a DISTINCT `- Gate-Kind:` field that 2 items carry and a substring rename would corrupt it; leave the on-disk item files to E-08 and the CLI flag spelling to E-08.
  - Depends on: none
  - Expected outcome: `backlog.py` reads and writes `- Work-Kind:`; `Gate-Kind` is untouched and still parses; a newly created item carries `- Work-Kind:`.
  - Execution state: pending

- [ ] E-08 Migrate the 88 existing backlog items on disk, and accept the OLD spelling on read so nothing breaks mid-migration. Measured: 88 items carry `- Kind:`. Add a dual-read window (accept `- Kind:` and `- Work-Kind:`, preferring the new one) BEFORE rewriting any file, so an item not yet migrated still parses and `aw backlog check` stays clean throughout. Then rewrite the field in every item. Do the rewrite with a script anchored on the full-line regex from E-07, not a bare substring, and re-verify the `Gate-Kind` count is still 2 afterwards. Decide and record whether the `--kind` CLI flag gains a `--work-kind` spelling with `--kind` kept as an accepted alias; the house pattern for a renamed surface is to accept both rather than break a caller.
  - Depends on: E-07
  - Expected outcome: all 88 items carry `- Work-Kind:`; `aw backlog check` is clean before, during (with a partially migrated tree), and after; `Gate-Kind` still appears on exactly 2 items and still validates; the CLI flag decision is recorded.
  - Execution state: pending

### Task group 1: one shared vocabulary, recognized in two contracts

- [ ] E-01 Export backlog's work-nature vocabulary as the ONE shared source and do not fork it. `backlog.KINDS` already exists as a frozenset of `bug`, `feature`, `chore`, `security`, `followup`. Consume THAT from the plan and spec contracts exactly as `backlog.PRIORITIES` is consumed today by `ipd_schema`, `specs`, `check_engine`, `releases`, and `research_cmd` (locate those consumers by grepping for `PRIORITIES` to copy the established import pattern). Do NOT define a second tuple and do NOT define a per-type subset. The vocabulary SYMBOL `backlog.KINDS` keeps its name (only the on-disk FIELD is renamed, by task group 0), so do not rename the symbol either. If the executor believes a shared SUPERSET is needed (see OQ-01's resolution, which rejects this), that is a scope question to report rather than a silent addition.
  - Depends on: none
...
| F7 | MED | `backlog.py`; `.aw/records/backlog/` | THE RENAME IS IN SCOPE (maintainer decision, OQ-03 reversed) and is smaller than first assumed, but carries one corruption hazard. MEASURED: `backlog.py` is the ONLY module that reads this field, so no other consumer needs migrating; 88 items on disk carry `- Kind:`. THE HAZARD: `backlog.py` also parses a DISTINCT `- Gate-Kind:` field and 2 items carry it, so a substring rename turns `Gate-Kind` into `Gate-Work-Kind` and silently breaks the gate contract. Anchor on the full-line field regex. Task group 0 owns this. | `grep -rl '^- Kind:' .aw/records/backlog/` = 88; `grep -rl '^- Gate-Kind:'` = 2; the only `.kind` reads live in `backlog.py` |
...
- RENAMING backlog's own `Kind` is NO LONGER DEFERRED. The maintainer chose one consistent name (OQ-03, reversed 2026-08-30), so it is task group 0 and runs BEFORE the field is added to plans and specs. The accepted-asymmetry position this section previously held is withdrawn.
...
- `agent_workflows/backlog.py` IS NOW DECLARED, reversing this plan's earlier fence. It is no longer import-only: task group 0 renames the field inside it. `.aw/records/backlog` is declared too, because E-08 rewrites 88 tracked items. A reviewer comparing against `xprio` child `1b45el`, which declared `backlog.py` for an import-only relationship and then had to ack it as in-scope-unmodified at finalize, should note the situation now differs: here the file is genuinely modified.
...
- The backlog README and any doc naming the `- Kind:` field must be updated to `- Work-Kind:` as part of task group 0; grep the docs tree for the field name and fix every hit, since leaving a doc describing the old spelling reintroduces the confusion the rename removes. Record in the terminal history that ONE field name now spans backlog, plans, and specs.
...
- Status: resolved
- Owner: none
- Resolution or deferral rationale: RENAME IT, FIRST. RESOLVED 2026-08-30 by maintainer decision, recorded via the graduation sweep's interactive prompt, and this REVERSES the plan's earlier position. The plan previously deferred the rename and accepted two names for one concept (backlog `Kind`, plans and specs `Work-Kind`) on the grounds that the migration was larger than the blocking need. The maintainer chose the consistent name instead, so task group 0 (E-07, E-08) now performs the rename BEFORE the field is added anywhere else, and the accepted-asymmetry language is withdrawn. Measurement done for this decision, which makes the migration smaller than the earlier text implied: `backlog.py` is the ONLY consumer of the field (no other module reads it), and 88 items on disk carry it. Two hazards are recorded and owned: a substring rename would corrupt the DISTINCT `- Gate-Kind:` field that 2 items carry, so the rename must anchor on the full-line regex (E-07); and the tree must stay parseable mid-migration, so a dual-read window lands before any file is rewritten (E-08). The honest cost the maintainer accepted: this makes a release blocker bigger, and a migration touching 88 tracked files in a shared checkout carries more conflict risk than adding an optional field.
...
- [ ] V-07 validates E-07
  - Required evidence: paste the changed field regex, parser mapping, validator, and `new` renderer in `backlog.py`, showing each anchors on the full-line field pattern and not on the bare token `Kind`. Paste a newly created item carrying `- Work-Kind:`. THEN paste the corruption guard: `grep -rlc '^- Gate-Kind:' .aw/records/backlog/` still returning 2 items, and one of those items parsing cleanly, proving `Gate-Kind` was not mangled into `Gate-Work-Kind`.
  - Observed evidence:
  - Result: pending

- [ ] V-08 validates E-08
  - Required evidence: paste the dual-read window landing FIRST, with a transcript showing a tree containing BOTH spellings parsing cleanly and `aw backlog check` clean in that mid-migration state; a migration that rewrites files before dual-read exists FAILS this item. Then paste the item count carrying `- Work-Kind:` reaching 88 and the count carrying `- Kind:` reaching 0, `aw backlog check` clean afterwards, and the `Gate-Kind` count still 2. State the `--kind` versus `--work-kind` CLI flag decision and paste the flag working.
  - Observed evidence:
  - Result: pending
