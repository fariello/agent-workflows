...
RETIRED 2026-09-10: this orchestrator's own title states the goal the maintainer REVERSED, so it cannot be re-scoped into truth. A setid is a SHARED cross-type TOPIC label, not a hard unique identity, and the shared-setid graduation coupling this plan set out to REPLACE is the behavior now deliberately PRESERVED. Superseded by `.aw/records/specs/20260910-2lcqno-01-2lcqno-setid-shared-topic-label-and-type-scoped-resolution.spec.md` (`2lcqno`).

WHY RETIRE RATHER THAN RE-SCOPE, since the checklist offered both. Two of its three children are retired (`drzbs9` and `dw7i3m`, superseded in the same pass) and the survivor, `bwgyum`, declares `- Item-Dependencies: none` and needs no sequencing from anyone. An orchestrator exists to order a Set and to verify criteria no single child can demonstrate; with one independent child there is no order to hold, and its three E-items are now each either moot or wrong: E-01 sequences children that no longer exist, E-02 holds a spec-approval gate on a spec that is now `superseded`, and E-03 verifies six acceptance criteria that the new spec has replaced. Re-scoping would have meant rewriting the title, the goal, the child table and every E-item, which produces a new plan wearing an old plan's identity.

WHAT SURVIVES AND WHERE. `bwgyum` (Order 02) is untouched and remains `pending`/`reviewed`: it builds the `Graduated-To` forward link and `check.graduated-to-dangling`, which `2lcqno` Section 2 carries forward as still wanted. It now stands alone rather than as a Set member, which is correct because it always was independent. The replacement work (type-scoped resolution, and re-scoping the collision check to its within-type half) is NOT inherited by this plan; it will graduate from `2lcqno` as fresh plans, so nothing lands under a Set whose premise was reversed.

NOT FILED AS EXECUTED, because none of its items were performed. Its two `Blocking: yes` open questions were answered on 2026-09-10 immediately before this retirement (`cf922221`) and both answers point here: OQ-01 resolved as "the Set as designed must not execute at all", and OQ-02 resolved as "yes, hard uniqueness breaks the legitimate backlog-to-plan naming convention, and that is why the design is reversed". Those answers are preserved in the body below with the measurements that produced them.
...
## Workflow history
- 2026-09-10 superseded (aw set): Retired per the maintainer's 2026-09-10 reversal; superseded by spec 2lcqno. Its title states the reversed goal, so it cannot be re-scoped into truth: a setid is a SHARED cross-type topic label, and the shared-setid graduation coupling this plan set out to REPLACE is now deliberately PRESERVED. Retired rather than re-scoped because two of its three children are retired and the survivor bwgyum declares Item-Dependencies: none, so there is no order left to hold and all three E-items are moot or wrong (E-01 sequences retired children, E-02 gates on a now-superseded spec, E-03 verifies replaced acceptance criteria). bwgyum is untouched and stands alone, which is correct because it always was independent. The replacement work graduates fresh from 2lcqno rather than being inherited here. Not filed as executed: nothing was performed.
