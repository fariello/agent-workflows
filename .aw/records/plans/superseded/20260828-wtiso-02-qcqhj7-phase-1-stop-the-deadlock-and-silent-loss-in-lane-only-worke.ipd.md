RETIRED 2026-09-02: superseded by the `lanectn` Set (7 plans, `reviewed`), which is the PORT of this
phase's lane-containment design onto current `main`; see orchestrator `h0zljh` and spec `7ckptx`
(APPROVED, 41 requirements). The `qyaime` deadlock this phase targeted is already BOUNDED on `main`
independently of it: `StallWatchdog` (`oc_runipd.py:413`, wired at `:4449`) kills a turn with no
stream events inside `--stall-timeout`, and isolated turns are always fresh sessions
(`oc_runipd.py:4297`).

WHY PORTED, NOT MERGED (backlog `vqv9im`, maintainer ruling): while this lane sat unmerged, `main`
independently solved the same problem the OPPOSITE way. `main` keeps absolute driver-owned control
paths as DECLARED EXCEPTIONS in the prompt; this lane forbids absolute paths outright. The lane-side
design measured better (zero absolute out-of-lane paths versus main's five) and `lanectn` carries it
forward, but the two cannot be merged mechanically. Lane `aw/lane/qcqhj7` holds 3 unique commits.

Do NOT read this retirement as "the containment work was wrong". It was right, and it is being
delivered by `lanectn`.
...
## Workflow history
- 2026-09-02 superseded (aw set): superseded by the lanectn Set (port, not merge; backlog vqv9im): main solved the absolute-control-path problem the opposite way while this lane sat unmerged. qyaime is separately bounded on main by StallWatchdog + always-fresh lane sessions.
