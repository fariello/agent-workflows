RETIRED 2026-08-30: superseded by `.aw/records/plans/pending/20260830-runtrail-01-m73aet-immutable-aw-run-and-aw-item-commit-trailers-on-the-shipped.ipd.md` (`runtrail-01`, id `m73aet`). This plan was `REJECT - NEEDS REPLAN` twice by its own reviews, and it is retired rather than re-scoped because its second review had already reduced it from three items to ONE and named the correct shape for that one: a `trailers=` parameter on the SHIPPED `git_commit_helper.offer_commit` (`:133`), not a new module. The successor does exactly that. What its residue turned into: the `AW-Run:`/`AW-Item:` commit trailers (re-verified at ZERO hits at HEAD `738980ec`), carried forward in full, including the three message-composition cases the review identified (multiline body, body already ending in a trailer block, body with no trailing newline). What did NOT survive, and why: E-01's worktree allocator, lease table, and scope assertion all ship as `worktree_lease.py` (note the first review cited `orchestrate_isolation.py` in error; the second corrected it, and the correct citation rejects the item MORE completely); the proposed `commit_gateway.py` would have FORKED the one commit path AGENTS.md names as immune to index pollution by construction, which is the highest-risk duplication possible in this repo; and the quarantine bundle plus `contained: true` receipt may only extend `wtiso-03`'s `lane_status.harvest_and_teardown_gate`, owned by an approved plan in a stack whose five lanes hold verified work reachable only from those branches. This plan's one remaining BLOCKING question (OQ-03, `rununify` sequencing) is resolved WITHOUT a maintainer decision: the successor defers the runner wiring entirely and touches neither runner, dissolving the conflict. Release gate PRESERVED, not dropped: the successor carries `- Blocks-Release: next` and `- From-Spec: 25kzda`. Retired, not deleted; NOT filed under `executed/`, because nothing here was ever implemented.
...
## Workflow history
- 2026-08-31 superseded (aw set): RETIRED: superseded by runtrail-01 (m73aet). REJECT - NEEDS REPLAN twice; its own second review had already cut it from three items to ONE and named the correct shape: a trailers= parameter on the SHIPPED git_commit_helper.offer_commit (:133), not a new module. Residue CARRIED FORWARD: the AW-Run:/AW-Item: commit trailers (re-verified ZERO hits at HEAD 738980ec) including the three message-composition cases the review identified. NOT carried: E-01's allocator/lease/scope-assertion ship as worktree_lease.py (the first review cited orchestrate_isolation.py in ERROR; the second corrected it and the correct citation rejects the item more completely); the proposed commit_gateway.py would FORK the one commit path AGENTS.md names immune to index pollution by construction; the quarantine bundle may only EXTEND wtiso-03's harvest_and_teardown_gate, owned by an approved plan whose lanes hold irreplaceable work. Its one remaining BLOCKING OQ (rununify sequencing) is dissolved: the successor touches neither runner. Release gate PRESERVED on the successor (Blocks-Release: next, From-Spec: 25kzda), not dropped.
...
- Implements spec `25kzda` (`20260826-25kzda-01-25kzda-aw-run-deterministic-run-and-verify.spec.md`) Sections 4.1, 4.2, 5.1, and 5.7.
