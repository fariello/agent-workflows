RETIRED 2026-08-30: the whole `detrun` Set is retired, its residue re-homed into four narrower successors; superseded by `hostcap-01` (`mjx7ne`), `runmixed-01` (`6lu3rq`), `runtrail-01` (`m73aet`), and `runcodes-01` (`wlxkoz`). This orchestrator is retired LAST, after every child, exactly as its own REPLAN NOTICE directed. The root cause, recorded so it is not repeated: this Set was authored at `453673b6`/`647bfd32` (00:08/00:14) against a paragraph of spec `25kzda` that declared the whole design net-new; the maintainer corrected that paragraph 21 minutes later at `a59f2c53` (00:35), and the corrected spec says a graduating Set "must CONSUME, not rebuild" the shipped machinery. Every child inherited the false premise, which is why all six plans reviewed as `REJECT - NEEDS REPLAN` rather than as flawed craft. Disposition of the five children, each verified individually rather than assumed: `bmh754` retired because its residue LANDED (commit `8c437188`, merged `b0eb74e6`); `a54m79` retired 2026-08-30 into `hostcap-01`; `kaygwo` into `runmixed-01`; `k7o7el` into `runtrail-01`; `7f7782` into `runcodes-01`. The Set's own two BLOCKING questions are both resolved WITHOUT a maintainer decision: OQ-02 (who owns the runner-safety capability contract) was answered from repository evidence when `hostcap-01` assigned it to the shipped `host_sandbox_profile.py`, and OQ-03 (must this wait for `rununify`) is DISSOLVED rather than answered, because all four successors defer the runner wiring and touch neither `oc_runipd.py` nor `agy_runipd.py`. Release gate PRESERVED, not dropped: all four live successors carry `- Blocks-Release: next`, and `bmh754`'s gate is discharged by its landed commit. Retired, not deleted; NOT filed under `executed/`, because this Set was never implemented as written.
...
- Scope: Orchestrates the 5 child implementation plans of Set `detrun` implementing approved spec `25kzda` (`20260826-25kzda-01-25kzda-aw-run-deterministic-run-and-verify.spec.md`). Defines the child plan sequence, dependency graph, integration validation, and Set completion criteria.
...
## Workflow history
- 2026-08-31 superseded (aw set): RETIRED: the whole detrun Set is retired, residue re-homed into four narrower successors (hostcap-01 mjx7ne, runmixed-01 6lu3rq, runtrail-01 m73aet, runcodes-01 wlxkoz). Orchestrator retired LAST, after every child, as its own REPLAN NOTICE directed. Root cause recorded: the Set was authored at 453673b6/647bfd32 against a spec 25kzda paragraph that called the design net-new; the maintainer corrected it 21 minutes later at a59f2c53, and every child inherited the false premise. Child disposition verified individually: bmh754 residue LANDED (8c437188, merged b0eb74e6); a54m79 -> hostcap-01; kaygwo -> runmixed-01; k7o7el -> runtrail-01; 7f7782 -> runcodes-01. Both Set-level BLOCKING OQs resolved WITHOUT a maintainer decision: OQ-02 answered from repository evidence by hostcap-01 assigning the capability contract to the shipped host_sandbox_profile.py; OQ-03 DISSOLVED because all four successors defer the runner wiring and touch neither runner. Release gate PRESERVED: all four live successors carry Blocks-Release: next; bmh754's is discharged by its landed commit.
...
- Spec `25kzda` (`20260826-25kzda-01-25kzda-aw-run-deterministic-run-and-verify.spec.md`) is linked and cited across all child plans.
