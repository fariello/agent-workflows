...
Spec `25kzda`'s amended Section 1.3 already records that skipping the independent verifier turn is a RETAINED, host-configurable choice rather than a prohibited bypass, and names `kgpptv` plus `f2mrsw` E-03 as the mechanism that makes it configuration (verified in review at `.aw/records/specs/approved/20260826-25kzda-01-25kzda-aw-run-deterministic-run-and-verify.spec.md:105`, which says in as many words that "until per-role model routing exists ... a host-level control is the only available mechanism"). This plan implements that mechanism FOR OPENCODE ONLY, so no spec change is required and no spec edit is authorized here.
