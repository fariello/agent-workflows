RETIRED 2026-08-30: superseded by .aw/records/plans/pending/20260830-hostcap-01-mjx7ne-extend-the-shipped-sandbox-capability-contract-with-the-runn.ipd.md. This plan was already marked 'REPLAN - DO NOT EXECUTE' by its own second review, and it blocked four dependents by refusing at pre-execution on two unresolved maintainer questions. Its SUBSTANCE is preserved in the replacement (capability vocabulary, action-requirement map, fail-closed RUN-HOST-CAPABILITY preflight); only the OWNER changed, from a net-new host_capabilities.py to the already-shipped host_sandbox_profile.py contract. Both of its blocking OQs are resolved there: OQ-02 (ownership) from repository evidence, OQ-03 (wait for rununify) dissolved by deferring the runner wiring instead of answering it.
...
## Workflow history
- 2026-08-31 superseded (aw set): Superseded by hostcap-01 (mjx7ne) at the maintainer's direction. Already self-marked REPLAN - DO NOT EXECUTE; blocked 4 dependents at pre-execution on two maintainer OQs. Substance preserved in the replacement; only the owner changed, from a net-new host_capabilities.py to the shipped host_sandbox_profile.py. Both OQs resolved there.
...
- Implements spec `25kzda` (`20260826-25kzda-01-25kzda-aw-run-deterministic-run-and-verify.spec.md`) Section 5.2.
