...
- Both manifests are GENERATED, GITIGNORED, LOCAL views. They are not committed, a fresh clone has
  ...
  They are not committed because they are regenerated byte deterministically from the plan files
  themselves, so every diff is derived, and because a tracked auto regenerated file conflicts on any
  concurrent branch or worktree by construction: on 2026-09-06 a conflict in this very file was the
  sole reason one lane's merge back aborted, stranding 2477 lines of correct, tested code while the
  real code auto merged cleanly.
  Accepted losses, decided deliberately: no manifest in a fresh clone, no `INDEX.md` browsing on a git
  host, no manifest diffs in review. Two consequences worth stating so they do not surprise you: a NEW
  WORKTREE has no manifest until `aw index plans` runs there, and switching branches no longer changes
  the manifest, since it is now one local file shared across every branch.
