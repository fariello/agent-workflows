...
## Workflow history
- 2026-09-18 executed (aw oc run): RETIRED as the orchestrator rollup step of a runner Set completion, not executed by an agent: every child of Set nobugship reached executed, so the runner (run run-20260918T205349Z-3540382) retired this Order-0 plan as bookkeeping. Its own E-*/V-* items were NOT performed; the runner superseded them by enforcing the ordering, the isolation and the per-child merge gate. Justifying children: zqs0px, di08i9, rgaasb.
...
- Resolution or deferral rationale: RAISED SO THE OMISSION IS DELIBERATE RATHER THAN AN OVERSIGHT. The maintainer's stated rule is about bugs; a `security` item is arguably a stronger case for never shipping, and treating it as ungated while a low-priority bug is gated would be hard to defend. But extending a rule the maintainer did not state is exactly the kind of inference that produced the misrecorded optionality this session already had to correct, so it is asked rather than assumed. Non-blocking because the Set is coherent for bugs alone and adding a second kind later is a one-line change to whatever predicate child 03 builds. Measured at authoring: 1 live `security` item exists, so the immediate impact is small either way.
  ANSWERED BY THE MAINTAINER 2026-09-12: MAKE IT CONFIGURABLE PER REPOSITORY, defaulting to `bug` ALONE, rather than deciding `security` globally. The `security` auto-gate does NOT land in this Set; child 01's written rule names `bug` and states that the gating set is configurable.
    THE DIAGNOSIS IS THE MAINTAINER'S AND IS WHY A GLOBAL ANSWER WOULD HAVE BEEN WRONG, so it is recorded verbatim in substance: a `security` designation is only as good as the judgement behind it, and they have measured AGENT security classifications in THIS repository to be overstated, because an agent defaults to assuming an adversarial actor when "an adversarial agent can in fact overcome ANY security we put in place". For MOST of their other projects the opposite holds and every security finding genuinely must block. That is a PROJECT-DEPENDENT property, so encoding it in a shipped rule would be wrong for one side of their own estate no matter which way it went.
    MEASURED CONTEXT: exactly ONE live `security` item exists today, `754txs` (`graduated`, ungated), whose subject is that the auto-approve predicate trusts a `- Readiness:` field without verifying a review produced it. So the concrete case is real and is already carried by a plan; gating the LABEL was never what protected it.
    THE PRECEDENT TO FOLLOW IS ALREADY SHIPPED, WHICH IS WHY THIS IS A SMALL CHANGE RATHER THAN A NEW SUBSYSTEM: `review_findings_gate` in `.aw/config/project.json` (`config.py:1085-1104`) is the same problem shape solved once already. It takes `{"block_at": "high"}`, tolerates a bare string, is DELIBERATELY NOT registered in `CONFIG_SCHEMA` because `project_schema.parse_portable_policy` preserves unknown keys in `unknown_fields` and writes them back on serialization, and it documents its default as fail-CLOSED so a repo gets protection without opting in. Copy that shape, including the reasoning about why the key is absent from the schema.
    THE DEFAULT IS `bug` ONLY, AND THAT IS A DELIBERATE DEPARTURE FROM STRICT FAIL-CLOSED. Including `security` by default would be the stricter reading, and it was offered and DECLINED for a specific reason: it would be wrong for the very repository doing the configuring, forcing an immediate opt-out of a label the maintainer does not trust here. A default that the reference repo must override is a bad default. `bug` alone is also exactly the maintainer's standing rule, so a repo that configures nothing behaves as the rule already says.
    DO NOT WIDEN THE KEY BEYOND WORK-KINDS. A broader "all auto-gates" object was offered and declined: it would design a surface before a second real case exists, and this Set exists to state ONE rule plainly. The inefficiency perceptibility test stays prose in the written rule, not config.
    SCOPE: this ruling does NOT authorize implementing the key inside this Set. It is carried by its own backlog item so child 01 can state "the gating work-kinds are configurable, defaulting to `bug`" without also shipping the reader. If the executor finds that sentence cannot be written honestly before the key exists, RAISE it rather than expanding scope.
...
- Resolution or deferral rationale: RESOLVE BY MEASURING, THEN REPORT. The gate keys on `Work-Kind`, which is an author's judgement, so a genuine defect filed as `chore` is invisible to it. That is a real limit and must be stated in child 01's written rule rather than discovered later. The executor should sample the live `chore` and `followup` items for ones whose summary describes a defect and report the count; if it is small the limit is acceptable and the written rule simply names it, and if it is large the classification itself needs attention, which is a different piece of work. Non-blocking because the gate is a strict improvement over nothing regardless of the leak's size.
  RESOLVED BY EVIDENCE THE SAME DAY THE QUESTION WAS ASKED, so no maintainer decision is needed: THE LEAK IS REAL, IT WAS DEMONSTRATED ON A LIVE ITEM, and the mitigation is a stated test rather than a mechanism.
    THE MEASURED INSTANCE. On 2026-09-12 an agent filed the `aw find` display-layer double read as backlog `59t9x5` with `Work-Kind: chore`, on the reasoning that the command returns entirely CORRECT answers. The maintainer reclassified it to `bug` and gated it on `next`. So a genuine defect was filed as `chore` and escaped the gate within hours of this question being raised, by exactly the mechanism the question predicted: the gate keys on `Work-Kind`, which is an author's judgement.
    THE MITIGATION IS THE MAINTAINER'S PERCEPTIBILITY TEST, ruled the same day: inefficiency that impacts the USER EXPERIENCE is a defect, inefficiency users cannot notice is not. That converts the judgement from a vibe into something a filer must MEASURE and a reviewer can DISAGREE with, which is the most a classification-keyed gate can offer. `59t9x5` qualifies on ~128ms of a ~530ms interactive command; at 3ms it would correctly have stayed a `chore`.
    SO THE ANSWER TO "IS THAT ACCEPTABLE" IS YES, WITH THE LIMIT NAMED IN WRITING, which is what this OQ asked for. Child 01's written rule must state that the gate keys on an author's classification and therefore cannot catch a misfiled defect, and should cite the perceptibility test as the standard a filer is held to. It must NOT claim completeness.
    NO MECHANISM IS PROPOSED, deliberately. A reviewer-side audit of every `chore` would be a second classification pass with the same judgement problem one layer down; the honest control is a stated test plus review, and `zqs0px` already owns writing the rule where a reader will find it.
...
- Resolution or deferral rationale: MEASURED BY EXECUTION, NOT PREDICTED, WHICH IS WHY IT IS BLOCKING. `check.from-backlog-gate-mismatch` ships today at ERROR severity in the exit-blocking sweep (`check_engine.py:2226-2241`, registered `:105-124` under `I-07`) and fires whenever a plan or spec carrying `- From-Backlog: <item>` has a `- Blocks-Release:` that DIFFERS from that item's. Child 03 cites this very rule family as the precedent for its own registration and does not notice that its backfill is what trips it. VERIFIED IN A THROWAWAY COPY of the repo at review: adding `- Blocks-Release: next` to graduated item `t156g1` and calling the shipped predicate immediately produced one `check.from-backlog-gate-mismatch` naming plan `5wtzqv`, which sits in `executed/`. Enumerated across the whole population, gating the 11 graduated gateless bugs would newly flag 13 carrier plans: 11 in `pending/` (`1f7xno`, `9kmbr0`, `lyo1tz` all from item `cnwy8g`, plus `76w6mq`, `akzy45`, `i1hlgx`, `k9awrq`, `m867ox`, `vdabn5`, `yeh7gc`, `zexed1`) and 2 in `executed/` (`5wtzqv` from `t156g1`, `h9cn0y` from `hyx1dg`).
  ...
  ANSWERED BY THE MAINTAINER 2026-09-12, AND THE ANSWER REFRAMES THE RULE RATHER THAN CARVING AN EXEMPTION. Their question was the pivot: "So any gate should be that no plan be non-blocking if it graduated from a blocking backlog item, but why would we care if a plan is blocking but the backlog is not?" Chosen: RESTATE `check.from-backlog-gate-mismatch` AS THE ONE-WAY OBLIGATION IT ACTUALLY MEANS, and skip a TERMINAL carrier.
    BOTH DIRECTIONS WERE MEASURED IN A THROWAWAY CLONE BEFORE DECIDING, because the reframing only holds if the asymmetry is real. (1) THE DIRECTION THAT FIRES: gating graduated item `t156g1` produced exactly ONE finding, naming carrier `5wtzqv` in `executed/` (delta +1 from a zero baseline). (2) THE DIRECTION THE MAINTAINER ASKED ABOUT: adding `- Blocks-Release: next` to pending plan `yeh7gc`, whose item `5ev6lh` has NO gate, produced ZERO findings (delta 0). The cause is structural, not incidental: the comparison loop only populates `item_gate` for an item that HAS a gate (`check_engine.py:2207-2216`), so a gated plan under an ungated item is unreachable by construction.
    SO THE ASYMMETRY IS ALREADY THE SHIPPED BEHAVIOR, and it is correct on the merits: a plan can discover DURING execution that it gates a release for reasons its originating item never knew, and forbidding that would punish the plan for being better informed than its own provenance. What the rule protects is the opposite and narrower thing: a plan must not be NON-blocking when it graduated from a BLOCKING item, because that is a DROPPED HANDOFF, the gate silently vanishing between item and plan. That is the failure `evaluate_blocking_close` and the whole handoff contract exist to prevent.
    READ THAT WAY, THE TERMINAL CARRIER IS NOT A COLLISION TO ROUTE AROUND; IT IS A CASE THE RULE SHOULD NEVER HAVE FLAGGED. `5wtzqv` and `h9cn0y` are in `executed/`: their work is DONE, so they cannot drop a future obligation, and there is no future release for them to gate. Flagging them demands an edit `AGENTS.md` forbids ("Do NOT add commits to a plan already in `.aw/records/plans/executed/`") in order to assert a live claim on an artifact with no future. The rule was over-reaching, and the Set merely happened to be the first thing to notice.
    THIS IS THEREFORE NOT A SPECIAL CASE CARVED FOR THIS SET, which was the test I applied before recommending it: `check_engine.py` already ships `_EXECUTED_SEGMENT` (`:999`), an `is_retired` predicate documented for exactly this purpose (`:483`), 31 references to terminal exclusion, and a NEIGHBOURING rule that excludes `executed/` records for the identical stated reason (`:1069-1070`, "terminal; a move OUT of executed/ is itself a staged change and IS checked"). The narrowing follows that precedent instead of inventing an exception.
    WHAT THE EXECUTOR MUST DO, and what it must NOT. DO: narrow the rule so it fires only when a LIVE (non-terminal) carrier lacks or contradicts a gate its item carries, reusing the shipped terminal predicate rather than a new path test; co-update the 11 PENDING carriers so their gates match; and add a test pinning BOTH measured directions, including the zero-finding case, so a future change cannot silently make the rule symmetric again. DO NOT: edit `5wtzqv` or `h9cn0y`, weaken the rule to flag only a both-gated CONFLICT (the maintainer was offered that and declined it, because it would drop the dropped-handoff detection that is the rule's entire purpose), or leave the two terminal findings standing as known noise in an exit-blocking sweep.
    SCOPE CONSEQUENCE: child 03's `Scope-Paths` must grow the plans tree (for the 11 co-updates) and `agent_workflows/check_engine.py` (for the narrowing). That widening is authorized by this ruling; note it is a SHIPPED RULE CHANGE, so the amended behavior belongs in the child's spec-sync section and its test must fail against today's code.
