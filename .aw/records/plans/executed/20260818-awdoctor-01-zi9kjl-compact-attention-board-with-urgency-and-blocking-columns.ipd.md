...
- 2026-08-18 executed (opencode Opus 4.8): E-01..E-03 performed, V pass; compact colored attention board; full serial suite 1111 passed 1 skipped.
...
- [x] E-01 In `render_board` (attention.py:436), when `colored` is true, compute each class group's common directory prefix and print it in the section header, then print BARE filenames. Add a helper ABOVE `render_board` (after `_colorize_tree_segment`, near attention.py:434):
  ```python
  import posixpath
...
- [x] E-02 In the COLORED per-item builder only (attention.py:500-507), add two compact leading markers derived from EXISTING Item fields, so the line becomes `- <blk><age> <bare> (<status>){inline_gate}`. Add a small helper ABOVE `render_board`:
  ```python
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: Verified: colored board folds common prefix into header + bare names + age/gate markers; plain board unchanged; test_attention_compact 4 pass; suite 1111p/1s.
  - Result: pass
