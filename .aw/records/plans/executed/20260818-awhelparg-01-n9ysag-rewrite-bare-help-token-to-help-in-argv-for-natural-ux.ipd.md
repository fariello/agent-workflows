...
- 2026-08-18 executed (opencode Opus 4.8): E-01..E-03 performed, V pass; bare help token -> --help in argv; full serial suite 1144 passed 1 skipped.
...
- [x] E-02 In `_dispatch` (cli.py:4018), immediately AFTER the existing plans-shim rewrite block that produces `argv_list`/`argv` (cli.py:4023-4030) and BEFORE `args = parser.parse_args(argv)` (cli.py:4031), apply the helper so both rewrites compose. Insert:
  ```python
...
- [x] E-03 Add `tests/test_help_token.py` with a `unittest.TestCase` `HelpTokenTests`. Cover BOTH the pure-function guard and the end-to-end dispatch:
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: Verified: _rewrite_help_token pure cases + aw help / aw ipd help show help (exit 0) + option-value help preserved; test_help_token 4 pass; suite 1144p/1s.
  - Result: pass
