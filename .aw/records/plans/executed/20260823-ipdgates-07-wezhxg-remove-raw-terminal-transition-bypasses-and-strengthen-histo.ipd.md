...
## Workflow history
- 2026-08-24 executed (opencode/its_direct/pt3-claude-opus-4.8): Removed the raw plan-terminal bypass; delegated aw set executed into finalize + strengthened attribution lint. Dogfooded.
- 2026-08-24 executed (opencode its_direct/pt3-claude-opus-4.8-1m-us, ipdrunner run-20260824T150827Z-2301181): E-01..E-03 performed, V-01..V-03 verified. LOCKOUT GATE verified first (v7e88a/qmt3yk/3xh53a executed; finalize proven end-to-end via 3 dogfooded finalizes with correct path-scoped commits; injected-failure rollback + corrected-retry proven; tests green; every plan retains a terminal path). E-01: status_set now delegates plan->executed (record_type==plans + target executed) into the gated aw ipd finalize (added --actor/--scope-reason/--scope-ack to set + ipd set; missing actor fails closed naming the command); retirement (superseded/not-executed), non-plan terminal, and nonterminal plan transitions preserved. E-02: post-transition attribution lint (ipd_lint IPD-S406) rejects generic (aw set)/empty actor + empty summary on the newest terminal entry, forward-only via Order oorry1's Scope-Paths cutoff. E-03: 8 DelegationAndBypassRemovalTests + 10 attribution tests + updated status_set test + 2 retargeted auto-index tests. Documented in the ipd-lifecycle workflow doc + plans README + spec Section 11 (aw specs note). Decisions 13-wezhxg-D1/D2/D3. pytest -n auto = 2294 passed, 1 skipped. Finalized via aw ipd finalize (dogfood - the now-only path).
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: `status_set.run_set_command` now detects `record_type == "plans"` AND normalized target `executed` and routes those records into `_delegate_plan_executed_to_finalize` (which calls `ipd_lifecycle.finalize`) BEFORE the raw apply loop; all other records use the unchanged raw path. Per OQ-03 (DELEGATE, DECISION 13-wezhxg-D1) it requires an attributed `--actor <agent/model>` (added to both `set` and `ipd set` parsers); a missing actor fails closed (exit 2) naming the exact `aw set executed <plan> --actor ...` command (never a fabricated generic actor). `tests/test_ipd_lifecycle_cli.py::DelegationAndBypassRemovalTests` (8 tests): `aw set executed`/`aw ipd set executed`/`set done` all delegate into finalize with `--actor` (plan moved to executed/ with a real actor, no `executed (aw set)`); without `--actor` fails closed naming `--actor`; a PROMPT `executed` transition is NOT diverted (raw path, moved to prompts/executed); plan `superseded` retirement is NOT diverted (raw RETIRED+git-mv); a nonterminal plan `approved` transition is NOT diverted; missing-receipt delegation refuses (exit 1) with no fabrication and no move. `tests/test_status_set.py::...test_set_plan_executed_delegates_and_refuses_ungated` confirms the ungated raw move is gone. The discriminator is `record_type=="plans"` (not the shared `executed`/`done` token), proven by the prompt test.
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: `ipd_lint.py` adds `C_EXEC_ATTRIBUTION` (`IPD-S406`), `_HISTORY_ATTRIB_RE` (parses `- <date> <status> (<actor>): <msg>`), `_GENERIC_ACTORS = {"aw set", "aw set, --by-human"}`, and `_check_terminal_attribution` (called at post-transition) which validates the NEWEST `executed` entry's actor+summary and is SKIPPED for grandfathered plans via `_is_grandfathered_for_attribution` (consuming Order oorry1's `Scope-Paths` cutoff marker: absent or `grandfathered` => pre-cutoff; DECISION 13-wezhxg-D2). `tests/test_ipd_lint.py::PostTransitionExecutedHistoryTests`: rejects generic `aw set` actor (post-cutoff), rejects empty summary (post-cutoff), accepts a real actor/model+summary, does NOT reject bare names (`Antigravity`/`maintainer`/`codex/gpt-5`), and grandfathers a pre-cutoff `(aw set)` record both when `Scope-Paths: grandfathered` and when there is no `Scope-Paths` field. Forward-only (OQ-01): the existing executed tree (no real `Scope-Paths`) is untouched.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: `pytest tests/test_ipd_lifecycle_cli.py::DelegationAndBypassRemovalTests` = 8 passed (delegation, ipd-set delegation, done-alias, prompt-not-diverted, retirement-not-diverted, nonterminal-not-diverted, missing-actor fail-closed, missing-receipt no-fabrication). `pytest tests/test_ipd_lint.py::PostTransitionExecutedHistoryTests` = 10 passed (attribution forward-only + grandfather). `pytest tests/test_status_set.py` green (delegation replaces the old raw-move test). The legitimate out-of-scope light-touch + grandfathered-advisory recovery paths are the Order 05/04 reconciliation behavior (finalize's --scope-reason + grandfathered empty-allowlist), exercised by ReconciliationTests/FinalizeTests. Two out-of-scope auto-index tests were retargeted executed->superseded (DECISION 13-wezhxg-D3, a mechanical consequence of removing the raw plan->executed move). Full suite `pytest -n auto` -> `2294 passed, 1 skipped in 122.56s (0:02:02)`. Docs updated: ipd-lifecycle workflow doc (no-ungated-bypass + delegation + recovery paths + honest-limits), plans README (delegation + retirement preserved), spec Section 11 note. `aw ipd lint --all` exits 0; `aw specs check` conforms; `check-local-leaks` clean.
  - Result: pass
