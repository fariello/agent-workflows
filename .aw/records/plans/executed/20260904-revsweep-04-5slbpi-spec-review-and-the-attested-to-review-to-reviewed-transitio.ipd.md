...
## Workflow history
- 2026-09-06 executed (aw oc run): aw oc run self-finalize: 5slbpi verified (set revsweep, attempt 1). [Scope reconciliation - out-of-scope .claude/commands/spec-review.md: changed by the plan's approved execution (auto-reconciled by aw oc run); out-of-scope .claude/commands/spec.md: changed by the plan's approved execution (auto-reconciled by aw oc run); out-of-scope .opencode/commands/spec-review.md: changed by the plan's approved execution (auto-reconciled by aw oc run); out-of-scope .opencode/commands/spec.md: changed by the plan's approved execution (auto-reconciled by aw oc run); out-of-scope agent_workflows/migration_inventory.py: changed by the plan's approved execution (auto-reconciled by aw oc run); out-of-scope agent_workflows/status_set.py: changed by the plan's approved execution (auto-reconciled by aw oc run); out-of-scope tests/test_reporting_contract.py: changed by the plan's approved execution (auto-reconciled by aw oc run); out-of-scope tests/test_spec_review_attestation.py: changed by the plan's approved execution (auto-reconciled by aw oc run); out-of-scope tests/test_status_set.py: changed by the plan's approved execution (auto-reconciled by aw oc run); in-scope-unmodified agent_workflows/run_selection_policy.py: declared-but-unmodified (auto-acknowledged by aw oc run); in-scope-unmodified tests/test_review_findings.py: declared-but-unmodified (auto-acknowledged by aw oc run); in-scope-unmodified tests/test_specs.py: declared-but-unmodified (auto-acknowledged by aw oc run)]
...
- 2026-09-04 reviewed (opencode its_direct/pt3-claude-opus-5-1m-us): POST-REVIEW EDIT, recorded because the `reviewed` verdict above predates it. MAINTAINER RULING on OQ-01: the spec reviewer is a SEPARATE `spec-review/` package, NOT a generalization of `plan-review/`. THIS NARROWS THE PLAN rather than widening it: E-01 changed from deciding the shape to RECORDING the ruling, E-02 names the separate package, and `plan-review`/`plan-review-long` moved OUT of scope except where E-03 corrects a factually false claim about them. The deciding evidence, recorded so it is not re-derived: all three plan-only obligations in the 601-line body are mandatory requirements (`plan-review.md:113-133`, `:377-398`, `:487-496`), so generalizing meant six conditional sites across two files held in parity. The `exception` size assessment is RETAINED and re-justified: E-02 authoring a workflow body and E-04 adding a lifecycle gate over 26 existing specs are still two high-blast-radius surfaces, and splitting them is still wrong because the gate is meaningless without the review and the review is unattested without the gate. The single constraint the ruling does not relax is restated in E-01: a separate BODY is authorized, a separate RECORD FORMAT is not. `aw ipd lint` conforming after the edits.
...
- CHANGING PLAN-REVIEW'S RUBRIC, SEVERITY, OR VERDICT VOCABULARY, or its body at all. The OQ-01 ruling makes the spec reviewer a SEPARATE package, so `plan-review`/`plan-review-long` are now OUT OF SCOPE for this plan except where E-03 corrects a factually false claim about them in the manifest and command shim.
...
- Status: resolved
- Owner: maintainer
- Resolution or deferral rationale: RESOLVED 2026-09-04 by the maintainer, asked interactively: a SEPARATE `spec-review/` workflow, NOT a generalization of `plan-review/`. Ownership moved from executor to maintainer because the decision has now been made by the maintainer rather than delegated. E-01 is therefore no longer a decision item: it must RECORD this ruling and its evidence in the new workflow's README, and MUST NOT re-litigate the choice. The deciding evidence is the measurement added at review and restated below: all three plan-only obligations are mandatory REQUIREMENTS, not optional prose, so generalizing would mean three conditional branches in a 601-line body duplicated across two files held in parity, i.e. six sites an agent could misread under load, with a wrong branch writing a wrong lifecycle value to a real artifact. THE CONSTRAINT THAT SURVIVES THIS ANSWER UNCHANGED, and the fork's one real cost: the findings/verdict/record machinery stays shared EXACTLY ONCE, so `review_findings` remains the single writer and parser and the verdict vocabulary remains `plan_readiness.VERDICTS`. A separate workflow body is authorized; a separate record format is NOT, and a design that forks the record is rejected. The executor must also state how the new body will be kept from drifting from `plan-review`'s shared rubric, since drift is the acknowledged cost of this option. ORIGINAL ANALYSIS: NOT BLOCKING because E-01 is exactly the item that decides it, with cited evidence, and either answer yields a working capability. Deliberately left open by spec `6m4kow` R-10 and its Section 5 because it needs the code in front of it: the cost of generalizing is real but only countable by reading the body (the `aw ipd lint` preflight, the required `Readiness` write, the E/V and Scope-Paths rubric items, all becoming conditionals in a body an agent reads under load, in TWO files held in parity), and the cost of forking is a second rubric drifting from the first. The constraint that makes either answer safe is fixed and not open: the findings/verdict/record machinery stays shared exactly once, so a design that forks the record is rejected either way. Owner is the executor rather than the maintainer because the decision is technical and evidence-driven; escalate only if the counted evidence is genuinely balanced.
...
- Cohesion rationale: 6 E-leaves across 2 task groups, one concern: a spec review that exists and a transition that proves it happened. Assessed `exception` rather than `standard`, honestly and with the reason stated: E-02 authors a workflow body and E-04 adds a lifecycle gate bearing on 26 existing specs, which are two independent high-blast-radius surfaces in one plan. They are nonetheless one plan because the gate is meaningless without the review that satisfies it, and the review is unattested without the gate: shipping either alone leaves the integrity hole this plan exists to close. E-01 is separate and first because the shape decision determines E-02's entire form. E-05 and E-06 are separate because enumeration and end-to-end proof have different failure modes: a discovery bug returns the wrong set, an end-to-end bug corrupts a lifecycle. RE-CHECKED 2026-09-04 after the OQ-01 ruling, which NARROWED this plan rather than widening it: choosing a separate `spec-review/` package took `plan-review`/`plan-review-long` out of scope entirely (no conditional branches in two 601-line bodies held in parity), and E-01 became a recording step instead of an open design decision. The `exception` assessment is RETAINED nonetheless, because the two high-blast-radius surfaces that earned it are unchanged: E-02 still authors a workflow body and E-04 still adds a lifecycle gate bearing on 26 existing specs. Splitting them was considered and rejected on the same ground as before: the gate is meaningless without the review that satisfies it, and the review is unattested without the gate, so shipping either alone leaves the integrity hole open.

Open questions: BOTH RESOLVED, so no decision is outstanding. OQ-01 (workflow shape) was ruled by the maintainer on 2026-09-04 as a SEPARATE `spec-review/` package, and E-01 is now the item that RECORDS that ruling rather than the item that decides it. OQ-02 (record versus history receipt) is RESOLVED as requiring a record, with the reason recorded so the weaker option is not adopted later. No blocking question remains.
...
Execution contract: RE-READ the plan-review body (as a REFERENCE for the new package, not a file to edit), `specs.py`, and `attention_contract.py` immediately before editing, and locate every site BY SYMBOL, never by the line numbers in this plan. The lockstep-parity warning that stood here is now MOOT and was removed: the OQ-01 ruling makes the spec reviewer a separate package, so `plan-review` and `plan-review-long` are not edited at all and cannot be forked by this plan. Commit ONLY the files changed, path-scoped (`git commit -m msg -- <paths>`), never `git add -A`, never push. Verify with `git diff --cached --name-only` before every commit and re-verify after any hook interruption, since a failed hook invalidates the check. If a co-worker's in-flight change cannot be safely combined with an edit, STOP and report rather than overwriting.
