...
## Workflow history
- 2026-08-24 executed (opencode/its_direct/pt3-claude-opus-4.8): Added the local pre-commit executed-transition gate; dogfooded - this finalize commit passes the very hook it installs.
- 2026-08-24 executed (opencode its_direct/pt3-claude-opus-4.8-1m-us, ipdrunner run-20260824T150827Z-2301181): E-01..E-03 performed, V-01..V-03 verified. Verified OQ-01 resolved (option B) before implementing. E-01: agent_workflows/hooks/executed_transition_gate.py detects, in the staged diff, a PLAN gaining `- Status: executed`/`done` or moved into executed/, and requires finalize evidence in .aw/state/ (the finalize transaction JOURNAL - DECISION 14-dulzpy-D1, since the as-built finalize leaves a begin-digest receipt consumed only after the commit; the journal is the present-at-commit-time proof, and editing finalize is out of this fence); missing -> refuse naming `aw ipd finalize`. E-02: registered a repo:local hook `ipd-executed-transition-gate` (entry `python3 -m agent_workflows ipd-executed-gate`) in .pre-commit-config.yaml + both installer templates (engine.py) + a cli subcommand + command_surface decl; NO CI. E-03: tests/test_executed_transition_gate.py (10 tests). Documented in the ipd-lifecycle workflow doc + spec Section 11 (aw specs note): LOCAL best-effort only (--no-verify bypasses; proclint via aw check/doctor is the backstop). One decision (14-dulzpy-D1) recorded. pytest -n auto = 2304 passed, 1 skipped. Finalized via aw ipd finalize (dogfood - its own commit passes the very hook it installs).
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: `agent_workflows/hooks/executed_transition_gate.py::check` reads `git diff --cached --name-status -M` scoped to `.aw/records/plans/`, and for each PLAN that gained `- Status: executed`/`done` (staged blob vs HEAD) OR was moved into `executed/`, verifies finalize evidence via `_finalize_evidence_ok`. Per DECISION 14-dulzpy-D1, the receipt-consumed predicate is realized against the artifact finalize actually leaves at commit time: the finalize TRANSACTION JOURNAL (`.aw/state/runtime/transactions/ipd_finalize_<id6>.json`) whose phase is a finalize-transaction phase (ready-to-commit/committed-incomplete/complete), plan_id matches, and dest_path matches the staged executed path (instrumentation confirmed the begin receipt carries the pending-time digest and is consumed only AFTER the commit, so the journal is the present-at-commit-time proof; editing finalize to add a receipt flag is out of this plan's fence). `tests/test_executed_transition_gate.py::PreCommitExecutedGateTests` (10 tests): hand-edited status-executed w/o evidence REFUSED naming `aw ipd finalize`; git-mv-into-executed w/o evidence REFUSED; a ready-to-commit journal with matching dest PASSES; a stale journal (wrong dest) REFUSED; a wrong-phase (prepared) journal REFUSED; a PROMPT executed transition NOT gated (record_type discriminator = plan paths only); nonterminal plan change NOT gated; ordinary commit fast no-op; a real begin+finalize's OWN commit PASSES via the installed hook; a grandfathered plan w/o finalize REFUSED (OQ-01 option B).
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: `.pre-commit-config.yaml` now carries a `repo: local` hook `ipd-executed-transition-gate` (`entry: python3 -m agent_workflows ipd-executed-gate`, `language: system`, `pass_filenames: false`, `always_run: true`), mirroring the local-leaks pattern. `agent_workflows/engine.py` adds the same hook to BOTH installer templates (`_LOCAL_LEAKS_PRECOMMIT_TEMPLATE` for a fresh config and `_LOCAL_LEAKS_PRECOMMIT_BLOCK` for merging into an existing one), so a fresh `aw install`/`setup-repo` writes it. The cli subcommand `aw ipd-executed-gate` is registered + declared in command_surface. NO GitHub Actions/CI file was added (the plan forbids it; `git status` shows no `.github/workflows/` change). Manual e2e in a temp repo with the real `.git/hooks/pre-commit -> python3 -m agent_workflows ipd-executed-gate`: a raw hand-edit+`git mv`+`git commit` was REFUSED (commit blocked, change left staged, message named `aw ipd finalize raw999`), while `aw ipd finalize`'s own commit PASSED (plan reached executed/). `test_real_finalize_own_commit_passes_via_installed_hook` re-proves finalize's commit passes the installed hook; `test_ordinary_commit_no_plan_transition_is_noop` proves the fast no-op.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: `pytest tests/test_executed_transition_gate.py` -> `10 passed` (raw hand-edit refused, git-mv-into-executed refused, ready-to-commit-journal passes, stale-journal-wrong-dest refused, wrong-phase-journal refused, prompt not gated, nonterminal not gated, ordinary-commit no-op, real-finalize's-own-commit passes via installed hook, grandfathered-without-finalize refused = OQ-01 option B). No network/CI test introduced. Full suite `pytest -n auto` -> `2304 passed, 1 skipped in 121.20s (0:02:01)`. The new `ipd-executed-gate` leaf is declared in command_surface + has a fuller CLI description (test_cli SubcommandDescriptionTests green). `aw ipd lint --all` exits 0; `aw specs check` conforms; `check-local-leaks` clean. Note: OQ-01's literal receipt-flag predicate was realized via the finalize JOURNAL (DECISION 14-dulzpy-D1) since finalize (as built in Orders 03/04/06, out of this fence) leaves a begin-digest receipt consumed only after the commit, and the journal is the present-at-commit-time proof.
  - Result: pass
