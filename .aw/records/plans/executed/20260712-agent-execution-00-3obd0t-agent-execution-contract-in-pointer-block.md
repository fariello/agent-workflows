...
- Id: 3obd0t
- Set: agent-execution (agent execution contract in pointer block)
- Order: 0
...
- Reach: `AGENTS.md`-only does NOT reach Claude Code or default Gemini (survey
  ...
  only reach those agents once IPD `20260712-mirror-workflow-00-ndxj3i-mirror-workflow-pointer-into-native-agent-files` mirrors the block into an existing
  `CLAUDE.md`/`GEMINI.md`. SOFT dependency on 0030-01 for full reach (the rules are correct in
  AGENTS.md meanwhile).
- Brevity budget: IPD `20260712-agent-brain-00-lhmlzo-agent-brain-dir-plan-and-walkthrough-mirroring` (PB4-2) sets a combined ~6-8 line budget for ADDED contract
...
- REACH (already satisfied): IPD `20260712-mirror-workflow-00-ndxj3i-mirror-workflow-pointer-into-native-agent-files` (mirror the block into existing CLAUDE.md/GEMINI.md)
...
- SHARED HOT SPOT: this and `20260712-agent-brain-00-lhmlzo-agent-brain-dir-plan-and-walkthrough-mirroring` edit the single `agents_pointer_block()` under the
  ~6-8 line brevity budget (0014-04 PB4-2); `20260712-release-consent-00-l9q4gs-release-consent-decision-tree-and-candidate-tier` change #4 adds one more line to it.
