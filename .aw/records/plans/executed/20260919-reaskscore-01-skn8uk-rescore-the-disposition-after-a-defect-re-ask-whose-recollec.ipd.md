...
## Workflow history
- 2026-09-22 executed (aw oc run model=uri/its_direct/pt3-claude-opus-5-1m-us variant=high profile=opus): aw oc run self-finalize: skn8uk verified (set reaskscore, attempt 1).
...
- [x] E-01 ADD A PURE, HOST-NEUTRAL PREDICATE `rescore_is_an_improvement(before, after)` TO `runner_shared.py`, ANSWERING ONE QUESTION: may this rescored disposition replace the one already computed? It MUST be a pure function of two status strings, defined by an explicit RANK over dispositions rather than by a membership test, because the question is comparative ("is `after` better than `before`") and a set test cannot answer it. Required behavior, each of which is a case `V-01` pins: an improvement is permitted (`partial` -> `substantially-complete`, `partial` -> `executed`, `failed-safely` -> `substantially-complete`); an equal score is NOT an improvement (so nothing is rewritten and no event is emitted when the re-ask changed nothing); a DOWNGRADE is REFUSED (`substantially-complete` -> `partial`, `executed` -> anything lower), which is the safety property that makes this change unable to harm a turn that already succeeded; and `merge-retry` (renamed from `integration-deferred` on 2026-09-21, `l2mzxn`; prefer the constant `runner_shared.INTEGRATION_DEFERRED_STATUS`), `runner_stop.STOPPED_DISPOSITION` and `runner_stop.FORCED_DISPOSITION` are NEVER replaceable in either direction, because each is a deliberate driver or operator decision that an agent's outcome file has no standing to overrule. An unknown status on either side MUST refuse (fail closed), so a future disposition added elsewhere cannot silently acquire replace-ability here.
  WHAT THE REFUSAL LIST BUYS, STATED HONESTLY AFTER MEASUREMENT, so nobody mistakes belt for braces. `runner_stop.STOPPED_DISPOSITION` is `"interrupted"` and `runner_stop.FORCED_DISPOSITION` is `"unknown_outcome"`, and BOTH are already members of `runner_shared.DEFECT_REASK_SKIPPED_STATUSES`, so `defect_reask_is_warranted` refuses a re-ask on either and neither can ever be the `before` value at the rescore point. Their entries here are therefore DEFENCE IN DEPTH against a future widening of that skip set, not a live path - keep them, and do not describe them as the safety property that matters. `merge-retry` (formerly `"integration-deferred"`) is the opposite case and is the one that genuinely matters: it is NOT in the skip set, so a re-ask CAN fire on a deferred item and reach the rescore, which is exactly why E-03 exists and why it is load-bearing rather than theoretical.
  AS PERFORMED: `rescore_is_an_improvement(before, after)` sits immediately above `reconcile_disposition` in `runner_shared.py`, with the rank in a module-level `RESCORE_DISPOSITION_RANK` mapping the four dispositions an agent outcome can legitimately produce (`failed-safely` 0, `partial` 1, `substantially-complete` 2, `executed` 3). ONE DEVIATION, recorded because it is visible in the body: the two stop constants are reached through a FUNCTION-LOCAL `from agent_workflows import runner_stop` rather than a module-level import, because `tests/test_orchestrator_probe_cache.py::test_no_new_module_level_first_party_import_in_runner_shared` permits `runner_shared` exactly two module-level first-party imports. The alternative (re-spelling `"interrupted"` and `"unknown_outcome"` as literals) was rejected as a second definition of two statuses. The local import is the module's own established convention (used in 20-plus places) and leaves the function pure in the sense V-01 measures.
  - Depends on: none
...
- [x] E-02 ADD THE RESCORE CALL TO `execute_item_core`, AT THE ONE POSITION WHERE IT HAS EFFECT AND CANNOT RACE A GATE: after the defect record is persisted onto `attempt`/`item` and its `defect-report-recorded` event is appended, and BEFORE `integration_gate_relevant` is computed. ANCHOR BY SYMBOL, NOT BY LINE: the insertion point is the blank line between the `append_jsonl(... "event": "defect-report-recorded" ...)` call that closes the `if not is_review:` block and the `suite_result: Any = None` / `integration_gate_relevant = (` pair that follows it. Verified at review: that window is 2 lines wide, the rescore sits at function top level (4-space indent, NOT inside `if not is_review:`), and the existing top-level `save_state(run_dir, state)` after the `integration_gate_relevant` block already covers it, which is what lets E-04 add no new call site.
  ...
  AS PERFORMED, WITH ONE DELIBERATE DEPARTURE FROM THE ITEM'S LITERAL WORDING, disclosed because it changes the shape the expected outcome describes. The item asks for "a second `disposition, outcome = reconcile_disposition(...)` tuple assignment". Written that way the second call would OVERWRITE `disposition` BEFORE `rescore_is_an_improvement` could compare against it, destroying the monotonicity this plan exists to provide. So the second call assigns into temporaries (`rescored, rescored_outcome = reconcile_disposition(...)`) and the ADOPTION is a separate tuple assignment `disposition, outcome = rescored, rescored_outcome` inside the improvement branch. Both properties the item actually needs are preserved and measured: a FOURTH `reconcile_disposition` call exists in `execute_item_core` (AST: lines 17464, 17553, 17565, 17737), and a SECOND tuple-to-`disposition` assignment exists for the AST ordering pins to resolve (AST: 17464 and 17753). The gate reads `"outcome" in (receipt.get("collected") or ())` after `lane_containment.read_collection_receipt`, passes the ORIGINAL `exit_code`, and passes `plan_repo=Path(work_dir) if work_dir else None` exactly as the first call does. Placement is at the same 8-space indent as `suite_result: Any = None`, between the `defect-report-recorded` append and `integration_gate_relevant`.
  - Depends on: E-01
...
- [x] E-03 PRESERVE THE `merge-retry` (formerly `integration-deferred`) PASSTHROUGH ACROSS THE SECOND CALL, WHICH IS THE ONE WAY THIS CHANGE COULD DESTROY DATA RATHER THAN MERELY MIS-SCORE. `reconcile_disposition` READS `item["status"]` for its deferral passthrough (`oc_runipd.py:6133`, twin `agy_runipd.py:2986`), and `execute_item_core` has ALREADY overwritten `item["status"]` with the first disposition by this point (currently `:13193`). So the second call runs with an input the first call did not have. E-01's refusal list already prevents replacing a deferral, and this item makes the property explicit and tested rather than emergent: assert that an item whose status is `merge-retry` at the rescore point retains it, and that the rescore neither rewrites it nor emits an event. The comment at `oc_runipd.py:6120-6132` explains why relabelling a deferral `partial` destroys it; do not re-derive that reasoning, cite it.
  AS PERFORMED, AND ONE OF THIS ITEM'S PREMISES IS FALSE, measured rather than argued (filed as backlog `ddzc4h`). The premise "`execute_item_core` has ALREADY overwritten `item["status"]` with the first disposition by this point" is true, but the conclusion that a deferral can therefore BE that first disposition is not: `execute_item_core` sets `item["status"] = "running"` at dispatch, so `reconcile_disposition`'s deferral passthrough (which reads `item["status"]`) cannot fire on a live turn's FIRST score, and `merge-retry` can never be the `before` value on a real run today. The refusal is KEPT anyway, exactly as E-01's reasoning requires (`merge-retry` is not in `DEFECT_REASK_SKIPPED_STATUSES`, so the day a deferral does reach here, the refusal is what stops it being destroyed), and the property is tested by SCRIPTING `reconcile_disposition` to return `merge-retry` then `substantially-complete` - the only way to reach the refusal, and one that reproduces precisely the hazard this item names. No new code was needed in `execute_item_core`: E-01's never-replaceable list already refuses, and this item's contribution is the test plus its negative control.
  - Depends on: E-02
...
- [x] E-04 CARRY THE IMPROVED SCORE ONTO ALL FOUR STALE CARRIERS, not only the local. The measured defect leaves `item["last_outcome"] = null` beside a fully collected outcome file, which is the defect in one line and is itself a reporting bug: `run_viewer.py` reads `item.get("last_outcome")` for the disposition and summary it displays (verified at review; unlike the `events.jsonl` claim in E-05, THIS reader is real), so a stale `None` there means the run report cannot describe a turn that succeeded. On an accepted rescore, update the local `disposition`, `attempt["disposition"]`, `item["status"]`, `item["last_outcome"]` (to the re-collected outcome dict) and `item["verification_status"]` handling, then persist.
  ...
  AS PERFORMED: the accepted branch sets the local `disposition`, `outcome`, `attempt["disposition"]`, `item["status"]` and `item["last_outcome"]`, and NOTHING else. `verification_status` is left untouched on both objects, per OQ-02, with the reason and the consequence written as a comment at the site rather than left for a reader to infer. No `save_state` call site was added (the diff's only two `save_state` additions are comment prose), and `test_no_call_site_was_rewritten` passes with its counts unchanged; V-04 carries the evidence, including a re-read of the persisted `state.json` proving the existing downstream `save_state` really does persist these mutations.
  - Depends on: E-02
...
- [x] E-05 EMIT A DURABLE `ipd-rescored` EVENT ON EVERY ACCEPTED RESCORE, naming `id6`, `attempt`, `before`, `after`, and the reason the rescore was permitted. A silent status rewrite is exactly the class of change a human must be able to audit afterwards, and scrollback is not a record. Use the same `append_jsonl(run_dir / "events.jsonl", {...})` shape the adjacent `defect-report-recorded` event uses, so the new record is not a novel channel. Emit NOTHING when the rescore is refused or is not an improvement, so the event's presence means "a re-ask changed this item's fate" and never "the code ran".
  ...
  AS PERFORMED: one `append_jsonl(run_dir / "events.jsonl", {...})` inside the accepted branch, carrying `at`, `event` (`ipd-rescored`), `id6`, `attempt`, `before`, `after` and `reason`, in the same shape as the adjacent `defect-report-recorded` append. A one-line operator print accompanies it (`↺ IPD <id6> rescored <before> -> <after> ...`), which is scrollback and is explicitly NOT the record; the durable event is. `run_viewer.py` is not edited and appears nowhere in the diff.
  - Depends on: E-02
...
- [x] E-06 PIN THE ORDERING ON THE CALL GRAPH IN `tests/test_rununify_execute_item_gates.py` AND RE-BASE THE ONE DOCSTRING THIS CHANGE FALSIFIES. Two existing AST pins are involved: `test_submissions_are_collected_before_the_disposition_is_reconciled` and `test_the_disposition_is_reconciled_before_integration`. Both resolve their target through `_execute_item_ast`, which FOLLOWS each host's `execute_item` delegation into `runner_shared.execute_item_core`, so they read the shared body this plan edits.
  ...
  AS PERFORMED, AND THE REVIEW'S PREDICTION HELD IN FULL: both existing pins pass UNWEAKENED with no assertion touched (the only removed line in the whole file diff is docstring prose), the docstring's count is corrected three -> four with the two drifted `oc_runipd.py` offsets re-anchored by symbol, and the new pin `test_the_post_reask_rescore_precedes_the_integration_GATE` asserts four things on the call graph: exactly TWO disposition-assigning tuple assignments exist, the receipt is READ before the rescore is judged, `max(disposition_assign) < first(integration_is_earned)`, and `first(validate_defect_report) < first(rescore_is_an_improvement)`. The deliberate break (the rescore block moved after `integration_is_earned`) fails exactly that pin and NOT the two older ones, which is the measurement showing the new pin is strictly stronger on this property. V-06 carries all of it.
  - Depends on: E-02
...
- [x] V-01 validates E-01
  - Required evidence: pasted `pytest` output for the `rescore_is_an_improvement` cases showing ALL of: three improving pairs return True (`partial`->`substantially-complete`, `partial`->`executed`, `failed-safely`->`substantially-complete`); equal pairs return False; every downgrade returns False including `substantially-complete`->`partial` and `executed`->`partial`; `merge-retry`, the stopped and the forced dispositions return False in BOTH directions; and an unrecognized status on either side returns False. Plus a demonstration that the function performs no I/O and mutates neither argument.
  - Observed evidence: `python3 -m pytest tests/test_defect_report.py -o addopts="" -p no:cacheprovider -v -k RescorePredicateTests`:

    ```text
    tests/test_defect_report.py::RescorePredicateTests::test_an_improvement_is_permitted PASSED [ 16%]
    tests/test_defect_report.py::RescorePredicateTests::test_an_EQUAL_score_is_not_an_improvement PASSED [ 33%]
    tests/test_defect_report.py::RescorePredicateTests::test_the_three_never_replaceable_statuses_refuse_in_BOTH_directions PASSED [ 50%]
    tests/test_defect_report.py::RescorePredicateTests::test_every_DOWNGRADE_is_refused PASSED [ 66%]
    tests/test_defect_report.py::RescorePredicateTests::test_it_performs_no_IO_and_mutates_neither_argument PASSED [ 83%]
    tests/test_defect_report.py::RescorePredicateTests::test_an_UNRECOGNIZED_status_on_either_side_refuses PASSED [100%]
    ======================= 6 passed, 64 deselected in 0.68s =======================
    ```

    EVERY CASE THE ITEM NAMES IS A SUBTEST OF ONE OF THOSE SIX, and each is enumerated in the test body rather than implied: the improving pairs are `partial`->`substantially-complete`, `partial`->`executed`, `failed-safely`->`substantially-complete` (plus `failed-safely`->`executed` and `substantially-complete`->`executed`); equality is tested for all four ranked statuses; the downgrades include `substantially-complete`->`partial`, `executed`->`partial`, `executed`->`substantially-complete`, `executed`->`failed-safely` and `partial`->`failed-safely`; the never-replaceable set is the product of `{merge-retry, interrupted, unknown_outcome}` with all four ranked statuses in BOTH directions; and the unknown-status cases cover an unranked string on either side plus `None` and `""`.
    PURITY IS SHOWN TWO WAYS in `test_it_performs_no_IO_and_mutates_neither_argument`: by AST over the function body (no `open(`, `read_text`, `write_text`, `append_jsonl`, `save_state`, `unlink`, `mkdir`, `subprocess`, and no subscript assignment), and by a control that the module-level `RESCORE_DISPOSITION_RANK` table is byte-identical after a call. Its ONE impurity-adjacent construct is the function-local `from agent_workflows import runner_stop`, which is deliberate and required: `runner_shared` may keep only two module-level first-party imports (pinned by `tests/test_orchestrator_probe_cache.py::test_no_new_module_level_first_party_import_in_runner_shared`, which passes unchanged), so reaching `STOPPED_DISPOSITION`/`FORCED_DISPOSITION` by their constants rather than by re-spelling the strings requires the local import.
  - Result: pass
...
- [x] V-02 validates E-02
  ...
  - Observed evidence: `python3 -m pytest tests/test_defect_report.py -o addopts="" -p no:cacheprovider -v -k "MEASURED_case or control_a or control_b or control_c"`:

    ```text
    tests/test_defect_report.py::RescoreAfterAReaskTests::test_the_MEASURED_case_is_rescued_instead_of_recorded_partial PASSED [ 25%]
    tests/test_defect_report.py::RescoreAfterAReaskTests::test_control_c_an_ABSENT_outcome_with_a_complete_receipt_refuses PASSED [ 50%]
    tests/test_defect_report.py::RescoreAfterAReaskTests::test_control_a_a_receipt_recording_the_outcome_FAILED_refuses PASSED [ 75%]
    tests/test_defect_report.py::RescoreAfterAReaskTests::test_control_b_NO_receipt_at_all_refuses PASSED [100%]
    ======================= 4 passed, 66 deselected in 1.53s =======================
    ```

    THE FIXTURE IS STRONGER THAN THE ITEM ASKED FOR and the difference is worth stating, because it is what makes the negative controls mean anything. Rather than hand-writing a receipt and an outcome file, `RescoreAfterAReaskTests._drive` runs the REAL `oc_runipd.execute_item` against a REAL git repository with a REAL isolated lane, so the receipt and the driver-side outcome are produced by the shipped `collect_lane_submissions` and read by the shipped `read_collection_receipt`. Only the model turn, `driver_begin`/`driver_finalize`, the tool-identity assertion, the suite check and the suite baseline are stubbed; nothing about the collection or the scoring is.
    THE MEASURED CASE, observed on that fixture: the first launch writes NOTHING (so the turn scores from the empty-outcome fallback `partial`), the re-ask launch writes `{"disposition": "executed", ...}` into the lane, the receipt comes back `collection_runs: 2` / `collected: ["outcome"]`, and the item ends `substantially-complete`, which the test also asserts is a member of `oc_runipd.EXECUTION_SUCCESS_STATES` - the property that unblocks the siblings. Two launches are asserted, so a test that silently failed to spend the re-ask cannot pass.
    EACH CONTROL PRODUCES ITS RECEIPT SHAPE BY WRAPPING THE COLLECTOR, never by patching the READER under test. (a) wraps `collect_lane_submissions` to move `"outcome"` from `collected` to `failed` AFTER the real copy, so the driver-side outcome file EXISTS and only the receipt disagrees; (b) deletes the receipt file after the real collection, again leaving the outcome file present, which is exactly the shape spec `7ckptx` R2.5 forbids inferring from; (c) needs no wrapper at all, because a re-ask that writes nothing naturally yields `status: "complete"`, `collected: []`, `failed: []`, and that triple is asserted explicitly in the test so the control cannot rot into vacuity. All three leave the item `partial` with zero `ipd-rescored` events.
  - Result: pass

- [x] V-03 validates E-03
  - Required evidence: pasted output showing an item whose status is `merge-retry` at the rescore point still reads `merge-retry` afterwards, with no `ipd-rescored` event emitted, AND pasted output of the negative control proving the test can fail: with the deferral refusal removed, the same test fails (name the assertion that fires).
  - Observed evidence: `python3 -m pytest tests/test_defect_report.py -o addopts="" -p no:cacheprovider -v -k deferr`:

    ```text
    tests/test_defect_report.py::RescoreAfterAReaskTests::test_the_deferral_refusal_is_what_holds_that_line PASSED [ 50%]
    tests/test_defect_report.py::RescoreAfterAReaskTests::test_a_deferred_item_passes_through_the_rescore_unchanged PASSED [100%]
    ======================= 2 passed, 68 deselected in 1.39s =======================
    ```

    THE NEGATIVE CONTROL EXISTS IN TWO FORMS and both were run. The SHIPPED one, `test_the_deferral_refusal_is_what_holds_that_line`, substitutes a rank-only predicate that has lost the never-replaceable list and asserts the SAME fixture then rewrites the deferral to `substantially-complete` with one `ipd-rescored` event; so the pair passes only while the refusal is what makes the difference. The LIVE one was performed against the real product code by temporarily deleting the `never_replaceable` block from `rescore_is_an_improvement` and giving `merge-retry` a rank of 0, then restoring the file byte-identically (verified by diff):

    ```text
    >           self.assertEqual(
    E           AssertionError: 'merge-retry' != 'substantially-complete'
    E           - merge-retry
    E           + substantially-complete
    E            : a deferred item must keep its non-terminal status: relabelling it destroys the automatic re-attempt and resurrects the cascade it exists to prevent
    tests/test_defect_report.py:1552: AssertionError
    ======================= 1 failed, 69 deselected in 1.27s =======================
    ```

    THE ASSERTION THAT FIRES is `test_a_deferred_item_passes_through_the_rescore_unchanged`'s `assertEqual(R.INTEGRATION_DEFERRED_STATUS, item["status"])`.
    AND ONE OF E-03's STATED PREMISES IS FALSE, measured rather than assumed, so the refusal is defence in depth here too rather than a live path (reported as a finding, backlog `ddzc4h`). E-03 says a deferred item's status can be the `before` value at the rescore point. It cannot, on a live turn: `execute_item_core` sets `item["status"] = "running"` at dispatch, so `reconcile_disposition`'s deferral passthrough (which reads `item["status"]`) cannot fire on the first score, and the first disposition therefore cannot be `merge-retry`. That is exactly why the test scripts `reconcile_disposition` to return `merge-retry` then `substantially-complete`: it is the only way to reach the refusal at all, and it reproduces precisely the hazard the item names (the second call running with `item["status"]` already overwritten by the first). The REFUSAL IS STILL CORRECT AND IS KEPT: the plan's own E-01 reasoning holds that `merge-retry` is not in `DEFECT_REASK_SKIPPED_STATUSES`, so the day a deferral does reach this point the refusal is what prevents destroying it.
  - Result: pass
...
- [x] V-04 validates E-04
  ...
  - Observed evidence: `python3 -m pytest tests/test_defect_report.py tests/test_runner_shared.py::WrapperTests::test_no_call_site_was_rewritten -o addopts="" -p no:cacheprovider -v -k "carriers_agree or no_call_site"`:

    ```text
    tests/test_runner_shared.py::WrapperTests::test_no_call_site_was_rewritten PASSED [ 50%]
    tests/test_defect_report.py::RescoreAfterAReaskTests::test_all_four_fate_deciding_carriers_agree_after_a_rescore PASSED [100%]
    ======================= 2 passed, 69 deselected in 2.92s =======================
    ```

    WHAT `test_no_call_site_was_rewritten` REQUIRED: NOTHING. It passes UNWEAKENED with its `save_state: 22` relocation count untouched, because no `save_state` call site was added. Verified directly on the diff: `git diff agent_workflows/runner_shared.py | grep "^+.*save_state"` returns exactly two lines and BOTH are comment prose ("`save_state(run_dir, state)` after the `integration_gate_relevant` block persists these" and "mutations, so no new `save_state` call site is added (see"), not calls. The rescore's mutations are persisted by the pre-existing top-level `save_state(run_dir, state)` that follows the `integration_gate_relevant` block, which sits downstream of the insertion point at the same indent level.
    THE CARRIERS ARE CHECKED IN MEMORY AND ON DISK, which is more than the item asked for and is the half that matters for the measured symptom: `test_all_four_fate_deciding_carriers_agree_after_a_rescore` asserts `item["status"]`, `attempt["disposition"]`, `item["last_outcome"] is not None` and `item["last_outcome"] == <the re-collected outcome file's parsed contents>`, and then re-reads `<run_dir>/state.json` and asserts the PERSISTED queue entry carries the same `status` and the same `last_outcome`. A rescore that mutated only the in-memory dicts would pass the first four assertions and fail the last two.
    `verification_status` IS DELIBERATELY NOT CARRIED, per OQ-02, and the consequence is pinned separately in V-06 rather than left implicit.
  - Result: pass
...
- [x] V-05 validates E-05
  ...
  - Observed evidence: `python3 -m pytest tests/test_defect_report.py -o addopts="" -p no:cacheprovider -v -k "durable_event or no_event_is_emitted or control_a or control_b or control_c"` (the FIVE tests that together cover the accepted path and all four non-acceptance paths):

    ```text
    tests/test_defect_report.py::RescoreAfterAReaskTests::test_no_event_is_emitted_when_NO_reask_was_performed PASSED [ 16%]
    tests/test_defect_report.py::RescoreAfterAReaskTests::test_no_event_is_emitted_when_the_rescore_is_NOT_an_improvement PASSED [ 33%]
    tests/test_defect_report.py::RescoreAfterAReaskTests::test_an_accepted_rescore_emits_exactly_one_durable_event PASSED [ 50%]
    tests/test_defect_report.py::RescoreAfterAReaskTests::test_control_a_a_receipt_recording_the_outcome_FAILED_refuses PASSED [ 66%]
    tests/test_defect_report.py::RescoreAfterAReaskTests::test_control_c_an_ABSENT_outcome_with_a_complete_receipt_refuses PASSED [ 83%]
    tests/test_defect_report.py::RescoreAfterAReaskTests::test_control_b_NO_receipt_at_all_refuses PASSED [100%]
    ======================= 6 passed, 64 deselected in 3.95s =======================
    ```

    READ FROM THE FILE, NEVER FROM A MOCKED APPENDER: `_drive` returns the parsed lines of `<run_dir>/events.jsonl` as written by the real `append_jsonl`, and every assertion here reads that list. `test_an_accepted_rescore_emits_exactly_one_durable_event` asserts exactly one `ipd-rescored` record and checks its `id6`, `attempt` (1), `before` (`partial`), `after` (`substantially-complete`), a non-empty `reason` and a non-empty `at`; it also asserts the record's index FOLLOWS `defect-report-recorded`'s, so the two facts read in the order they happened.
    THE FOUR NON-ACCEPTANCE PATHS each assert `[] == [e for e in events if e["event"] == "ipd-rescored"]`: no re-ask performed (also asserting exactly ONE launch, so the absence is not because the re-ask silently failed), the recollection recording the outcome `failed`, no receipt at all, and a re-ask whose answer is still `partial` (also asserting TWO launches, so the absence is not because the re-ask never ran). So the event's presence means "a re-ask changed this item's fate" and never "the code ran".
    NO `aw runs` CLAIM IS MADE OR RELIED ON, and `run_viewer.py` is not edited (it is absent from `- Scope-Paths:` and from the diff). The honest property claimed is that the record is durable and greppable in the run directory; OQ-03 records the pre-existing, plan-independent gap that no driver event is rendered anywhere in the run report.
  - Result: pass
...
- [x] V-06 validates E-06
  ...
  - Observed evidence: THE FULL BARE SUITE, `python3 -m pytest` (no added flags, so `pyproject.toml` `addopts` supplies `-q -n auto --dist=worksteal -m 'not slow'`):

    ```text
    8085 passed, 3 skipped, 2 xfailed, 3 warnings in 109.08s (0:01:49)
    ```

    ONE ENVIRONMENTAL CAVEAT, disclosed rather than hidden, and it is NOT a failure of this change. Run bare in THIS turn's shell the suite also reports `FAILED tests/test_turn_bounds.py::TestArmedForEveryUnattendedTurn::test_the_permission_policy_by_contrast_IS_isolation_scoped`, with `assert 'OPENCODE_CONFIG_CONTENT' not in {...}`. That test reads the AMBIENT environment, and this executing turn itself runs with `OPENCODE_CONFIG_CONTENT` exported by its own host; `env -u OPENCODE_CONFIG_CONTENT python3 -m pytest` makes it pass, and the summary above is from that run. It reproduces at HEAD without any of this plan's changes, touches no file in `- Scope-Paths:`, and is filed as backlog `nnv3bz` rather than worked around.

    THE PINS FILE, `python3 -m pytest tests/test_rununify_execute_item_gates.py -o addopts="" -q`:
    ...
    BOTH PRE-EXISTING ORDERING PINS PASS UNWEAKENED, and the claim is checkable on the diff rather than asserted. They are `test_submissions_are_collected_before_the_disposition_is_reconciled` and `test_the_disposition_is_reconciled_before_integration`. `git diff -U2 tests/test_rununify_execute_item_gates.py | grep -E "^[-+].*(assert|self\._first_line|min\(|max\()"` shows exactly ONE removed line, and it is DOCSTRING PROSE ("therefore assert a false ordering and fail on correct code. The existing pin resolves this"); every added `assert*`/`_first_line`/`max(` line belongs to the NEW pin. No assertion in either pre-existing pin was removed, relaxed, or re-anchored. Measured arithmetically too: `min(collect_lane_submissions)=17428 < max(disposition_assign)=17753` and `max(disposition_assign)=17753 < first(integrate_lane_branch)=18265`, so both inequalities still hold with the rescore having become `max(disposition_assign)`.

    THE DOCSTRING COUNT IS CORRECTED, quoted from the file: "calls `reconcile_disposition` FOUR times on each host, and only TWO of those four are tuple assignments to `disposition` that this pin can see". Measured by AST on the current tree: four `reconcile_disposition` calls in `execute_item_core` (lines 17464, 17553, 17565, 17737) and two tuple assignments to `disposition` (17464 the score, 17753 the rescore's adoption). THE TWO DRIFTED OFFSETS ARE RE-ANCHORED BY SYMBOL rather than carried forward: the docstring cited `oc_runipd.py:7344` and `:7372` for the two early-recovery calls, and on the current tree those lines read `sub_parser.set_defaults(output_mode="clean")` and `def print_status(run_dir: Path) -> None:`, neither of which is a `reconcile_disposition` call; the docstring now names "the two `except` handlers around the verifier spawn inside `execute_item_core`" instead.

    THE DELIBERATE BREAK, performed by moving the whole 92-line rescore block to immediately AFTER the `integration_is_earned(...)` call and then restoring `runner_shared.py` byte-identically (verified with `diff -q`, reported `IDENTICAL`):

    ```text
    E           AssertionError: 17815 not less than 17754 : oc_runipd: the rescore must be adopted BEFORE `integration_is_earned` reads the disposition; a rescore after it leaves the measured defect live
    FAILED tests/test_rununify_execute_item_gates.py::TheGateOrderingIsPinnedOnTheCallGraph::test_the_post_reask_rescore_precedes_the_integration_GATE
    1 failed, 2 passed, 17 deselected in 2.42s
    ```

    NOTE THE TWO THAT STILL PASSED in that break: `test_submissions_are_collected_before_the_disposition_is_reconciled` and `test_the_disposition_is_reconciled_before_integration`. That is precisely why the new pin is a separate, STRICTLY STRONGER assertion rather than an edit to either: anchored on the MERGE, both older pins are satisfied by a rescore that lands after the GATE, while the measured defect stays live.

    OQ-02's CONSEQUENCE, `python3 -m pytest ... -k "rescued_item"` (contained in the 13-test run pasted under V-02/V-04/V-05; the case is `test_a_rescued_item_reaches_the_gate_with_no_verifier_verdict_and_a_DRIVER_suite`, PASSED). It captures the kwargs `integration_is_earned` is actually handed on a rescued turn and asserts `validate is False`, `verify_disp is None`, and `suite_result.passing` with that result also persisted on `attempt["suite_check"]`, i.e. a DRIVER-observed suite run and never an agent claim. It then drives the real predicate both ways: `integration_is_earned(validate=False, verify_disp=None, suite_result=<passing>)` returns earned with signal `driver-run-suite`, and with `suite_result=None` returns NOT earned with signal `no-trust-signal`. So a rescued item is integrated on a driver-run suite result or not at all, which is the fail-closed direction.
  - Result: pass
