...
- 2026-08-19 executed (agy_run.py Gemini 3.7 + opencode independent-validate): E-01..E-06 performed; agy made aw ipd scaffold require --set/--order, enforce the clustering-grammar --path (--legacy-name escape), always emit Set/Order, + tests + closed 7vd36f. opencode INDEPENDENTLY verified live: scaffold errors without --set/--order, rejects nonconforming --path with the grammar message, conforming path works; test_awnaming_grammar_and_producers passes; full serial suite 1189 passed 1 skipped; 7vd36f done. (agy status ERROR = stale-path view_file exploration, false-ERROR uhbdt1; work committed c132c06.)
...
- 2026-08-19 executed (antigravity): implemented required Set/Order in scaffold CLI parser, enforced Set/Order and clustering grammar on explicit --path in ipd_authoring, updated templates and tests, synced specs, closed backlog 7vd36f.
...
- [x] E-01 In `agent_workflows/cli.py` make the scaffold parser require `--set` and `--order` (change `p_ipd_scaffold.add_argument("--set", ...)` and `--order` at cli.py:713-721 to `required=True`), and update their help text to say they are required. Add a new `--legacy-name` flag (default False, `action="store_true"`) to the scaffold parser as the escape hatch used by E-03.
  - Depends on: none
...
- [x] E-06 Close backlog item 7vd36f to done (`aw backlog set .aw/records/backlog/open/20260815-7vd36f-01-7vd36f-ipd-scaffold-clustering-grammar.backlog.md --status done --message "scaffold enforces Set metadata + clustering grammar" --dir .`) and run the full serial suite - canonical `make test-serial` (`python3 -m unittest discover -s tests -t .`); `python3 -m pytest -p no:xdist` is equivalent only with the `.[test]` extra installed.
  - Depends on: E-05
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: `aw ipd scaffold --kind child --title X` exits 2 ("error: the following arguments are required: --set, --order"); `aw ipd scaffold --help` lists `--set` and `--order` as required, documents grammar on `--path`, and lists `--legacy-name`.
  - Result: pass
...
- [x] V-02 validates E-02
  ...
  - Observed evidence: `aw ipd scaffold --kind child --title "Demo Plan" --set demo --order 1` outputs `- Set: demo` and `- Order: 1`; grep confirms `if set_name is not None:` conditional and fallback removed from `agent_workflows/ipd_authoring.py`.
  - Result: pass
...
- [x] V-03 validates E-03
  ...
  - Observed evidence: `--path .../explicit.md` exits 2 ("error: --path must follow clustering grammar YYYYMMDD-<setid>-NN-<id6>-<slug>.ipd.md"); same with `--legacy-name` exits 0; conforming `--path 20260819-demo-01-v1rj3p-conforming-plan.ipd.md` writes `- Id: v1rj3p`.
  - Result: pass
...
- [x] V-04 validates E-04
  ...
  - Observed evidence: `python3 -m pytest tests/test_awnaming_grammar_and_producers.py -p no:xdist` -> 13 passed in 1.60s.
  - Result: pass
...
- [x] V-05 validates E-05
  ...
  - Observed evidence: Updated help strings in `cli.py`; updated authoring section in `20260726-1340-01-ipd-spec.spec.md` and Section 6.2 in `20260802-1904-01-ipd-structure-and-linting.spec.md`.
  - Result: pass
...
- [x] V-06 validates E-06
  ...
  - Observed evidence: `git status` shows 7vd36f in `.aw/records/backlog/done/`; full test suite `python3 -m unittest discover -s tests -t .` -> Ran 1190 tests in 206.492s OK (skipped=1).
  - Result: pass
