...
## Workflow history
- 2026-09-17 executed (opencode/its_direct-pt3-claude-opus-5-1m-us): Give the two runners ONE HostLabels descriptor and lift all 8 host-label symbols into runner_shared, each reached through a one-line per-host wrapper at the original name and signature so no call site was rewritten. Three changes are not refactors: agy's agents now receive oc's safety instructions (preserve partial work, leave others' checkouts safe, never over-claim executed) with oc's own prompt byte-identical; agy's verifier prompt gained the Never push prohibition it entirely lacked while instructing the agent to commit; and agy's backticked report verify cell is now bare, so run_viewer's badge finally matches (no agy run had ever rendered it). Work landed in 2cd4e7f3 and e5fbe9c4, integrated to main as d62fbca9 after resolving a runner_shared conflict whose merged symbol set was verified to be exactly union(HEAD, lane) = 191 with no duplicates and nothing lost. FINALIZED OUT OF BAND on 2026-09-17: the run left this substantially-complete because the begin receipt went STALE when the agent honestly ADDED three test files to Scope-Paths mid-execution. Reconciled against three proven guards (receipt digest genuinely describes the plan at its base_head; requirement categories differ ONLY in scope; the scope change is purely additive), base_head unchanged. Defect filed as plan 63425h. The four acked paths were verified genuinely unmodified by this plan's own commits (git diff 4b68a786..e5fbe9c4 returns nothing for each). [Scope reconciliation - in-scope-unmodified tests/test_agy_runipd_cli.py: verified-unmodified-by-this-plans-commits; in-scope-unmodified tests/test_lane_prompt_purity.py: verified-unmodified-by-this-plans-commits; in-scope-unmodified tests/test_oc_runipd.py: verified-unmodified-by-this-plans-commits; in-scope-unmodified tests/test_run_viewer.py: verified-unmodified-by-this-plans-commits]
