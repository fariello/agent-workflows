...
- 2026-08-21 executed (opencode (its_direct/pt3-claude-opus-4.8-1m-us)): executed via /ipd-lifecycle in two phases. E-01..E-07 performed (workflow_schema/source/loader/compiler/profile + workflow_cli + cli wiring + example package + 40 tests). V-01..V-07 pass with concrete evidence. Full serial suite green (`python3 -m pytest tests/ -n auto` -> 1300 passed, 1 skipped in 47.12s). aw ipd lint --phase pre-execution and --phase pre-transition both conforming (exit 0). Independent fresh-context verification returned MATCHES/GO (re-ran the tests + CLI itself, confirmed no stubs, reproduced determinism/drift/exit-codes and the D139 no-runtime-YAML boundary). Human authorized the terminal transition. Commits (path-scoped, not pushed): 712f826, c73c921, 8f1abbd, aeefff4, d4c02b8, 3035961, e2ac4b4, ebf02e9.
...
- [x] E-07 Add focused property and golden tests for malformed packages, dependency cycles, stable ordering, semantic-digest parity, source mapping, path safety, generated drift, and round-trip preservation of every schema field.
  - Depends on: E-06
  ...
  - Execution state: performed
  - Execution note: added `tests/test_workflow_schema_compiler.py` (stdlib unittest, 40 tests, all passing in 0.28s). Coverage: schema positive + every named invalid invariant (bad id, unknown intent/field, dup requirement, unknown req ref, self-dep, cycle, forbidden terminal action, read-only/allowed-path contradiction, missing validation, unknown evidence kind); source layout (parse+validate, digest stable, ignores generated/cruft, changes on authoritative edit, symlink refused); loader (full IR, DELETING A REQUIRED RESOURCE fails closed with no IR, traversal rejected, provenance in findings, missing entry); compiler (all 6 projections, manifest binds digest, every schema field reachable, BYTE-IDENTICAL across two clean runs, sorted+compact JSON); profile parity (transport-only preserves; dropping a validation / widening scope fence / changing step shape rejected; invalid profile rejected); CLI drift (validate clean=0 / bad path=2, compile dry-run writes nothing, apply then check-generated clean=0, HAND-EDITING a generated file -> exit 1 'changed', unexpected file -> exit 1 'unexpected', agent JSONL valid + no ANSI). Formal V-07 evidence in the validation pass.
