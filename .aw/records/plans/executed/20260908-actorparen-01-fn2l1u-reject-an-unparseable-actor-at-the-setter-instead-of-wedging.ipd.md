...
## Workflow history
- 2026-09-14 executed (opencode/recovery): Recovered stranded lane and finalized: all eight E-items performed and eight V-items verified with pasted evidence in run-20260914T020813Z-2543555; the lane was RUNNING when the reboot killed the driver at 06:30Z. Attribution via run-record-exact (gys47u) after reconstructing the outcome record the reboot prevented the driver from writing. [Scope reconciliation - out-of-scope agent_workflows/agy_runipd.py: E-08 twin of the oc change, applied to keep the two hosts from diverging on actor validation; a rule present in one driver only is a defect this repo has measured before.; out-of-scope agent_workflows/attention_contract.py: E-08: one of the two additional history READERS found by grepping the character class rather than the symptom (F-12). Leaving a reader on the old bound is the drift the one-definition rule exists to prevent.; out-of-scope agent_workflows/oc_runipd.py: review F-18/E-07: finalize_orchestrator passed --actor 'aw oc run (orchestrator rollup)', which the new guard refuses. The plan requires resolving that call site deliberately rather than leaving a string the guard would reject, even though the function is dead by AST scan.; out-of-scope agent_workflows/status_set.py: review PR-501/new-F-12 and E-07, the plan's most important item: apply_status_change is the SINGLE writer every --actor surface funnels into, and a parenthesized actor there silently converts the one un-overridable approval refusal into a no-op (reproduced end to end at review). Guarding here covers aw set, aw ipd set, the finalize transaction and specs.run_set at once.]
