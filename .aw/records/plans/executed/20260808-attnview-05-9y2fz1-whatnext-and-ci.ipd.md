...
- 2026-08-08 executed (opencode its_direct/pt3-claude-opus-4.8-1m-us): rewired /whatnext to consume aw attention --format json first (stop on invalid, no silent raw rescan, comms headers-only preserved) + README; added the CI attention-check job (aw specs check + aw attention --check, fail-closed); added the AGENTS 'What needs attention' pointer via agents_pointer_prose (empty-diff invariant holds); DECISIONS D125; TODO deferred-adopters note; specs README status/verb docs. Also refined the contract to exclude non-artifacts (STATUS/templates/index/READMEs + the docs/prompts library) so --check is clean on the real repo. E-01..E-06 performed; V-01..V-06 pass. Full suite Ran 716 tests OK (skipped=1); aw attention --check + aw specs check exit 0; leak-clean; no em/en dashes. Commits c7a2f86, 56f6a70, 0beb7f6, f8e150c.
...
- [x] E-02 update the `/whatnext` README so its described sources match the rewired body (registry-view-first).
  - Depends on: E-01
...
- [x] E-06 run the full suite; run `aw attention --check` and `aw specs check` on the repo; if AGENTS.md is installer-generated, verify the empty-diff invariant after regeneration; confirm leak-clean and no em/en dashes. Paste actual output.
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: `whatnext.md` Step 1 now opens with "Attention view (PRIMARY; run FIRST). Run `aw attention --format json`"; on nonzero/`"valid": false` it "present ALL of its `violations` ... and STOP normal prioritization"; the quoted no-fallback line: "Do NOT silently fall back to re-scanning every raw artifact (that recreates the cost and nondeterminism this view removes); report the failure and the remediation instead." It prioritizes `active` then `ready`, shows `blocked` WITH gate detail, omits done/parked unless asked, and keeps comms headers-only (the memory-kernel invariant is untouched) + staged prompts as bounded secondary.
  - Result: pass
- [x] V-02 validates E-02
  - Required evidence: the whatnext README's source list matches the rewired body (no stale "scour raw files first").
  - Observed evidence: `.agents/workflows/whatnext/README.md` now says it consumes "the deterministic cross-tree attention view (`aw attention --format json`) FIRST as the primary source (stopping if the view is invalid), then the bounded secondary sources" - matching the rewired body; no stale raw-scan-first language.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: `.github/workflows/tests.yml` gained a new `attention-check` GitHub Actions job (chosen surface: a dedicated job, matching the secret-scan/local-leaks single-job precedent, since it needs the full `.agents/` tree once) running `python -m agent_workflows specs check` and `python -m agent_workflows attention --check --agent`. The E-03 text states the ground truth that no prior `ipd lint`/`plans index --check` CI gate existed, so this is the first such gate. Both commands run clean locally after the exclusion fix.
  - Result: pass
- [x] V-04 validates E-04
  ...
  - Observed evidence: `agents_pointer_prose()` (engine.py) gained a "What needs attention (cross-tree view)" section naming `aw attention` (with `--format json`/`--check`) and `aw specs` (set/note/check, the human-only approved, evidence-gated implemented, typed gates); it was regenerated into AGENTS.md and the empty-diff invariant holds (a second `merge_aw_block` yields no change; AGENT-PLANS byte-identical). `.agents/docs/specs/README.md` now documents the closed status enum, the attention-class mapping, typed gates/Canonical, and the `aw specs` owner verbs.
  - Result: pass
- [x] V-05 validates E-05
  - Required evidence: the DECISIONS entry records the model (cite spec + review set); TODO notes the deferred adopters.
  - Observed evidence: `DECISIONS.md` D125 records the attention-view + owner-written-spec-status model, citing the approved spec `20260808-1945-01` and the executed `attnview` Set; `TODO.md` "Planned next" gained a note that the attnview Set shipped and enumerates the deferred future work (prompts/comms Phase-3 adopters, walkthroughs/roadmaps excluded, optional snapshot, the two deferred specs' gates, plans `executing`).
  - Result: pass
- [x] V-06 validates E-06
  ...
  - Observed evidence: full suite `python3 -m unittest discover -s tests -t .` -> `Ran 716 tests in 154.088s / OK (skipped=1)`. `aw attention --check --agent` exit 0 (clean on the real repo after the non-artifact exclusions); `aw specs check` -> "all specs conform." (exit 0). AGENTS.md empty-diff invariant confirmed (merge_aw_block second run: no change). `aw sanitize --agent` exit 0. No em/en dashes in authored files.
  - Result: pass
