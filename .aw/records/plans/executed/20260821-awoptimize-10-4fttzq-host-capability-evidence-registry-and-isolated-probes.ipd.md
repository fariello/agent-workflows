...
- 2026-08-22 executed (opencode (its_direct/pt3-claude-opus-4.8-1m-us)): E-01..E-04 executed via agy/Gemini (committed e54029d: host_capability_registry.py + tests, scope-clean; run errored on a spurious brain-path artifact log AFTER the real commit). opencode independently verified (unverified-default+expiry registry, isolated-fixture probes w/ real-HOME refusal + redaction + nonce, negative probes per class), 28 module tests + full suite 1579 passed 1 skipped (pytest rc=0), filled V-01..V-04. Live host probes remain operator-run. Terminal transition to executed/.
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: host_capability_registry.py records keyed by host/distribution/version/OS/mode/feature/config/probe-variant/result/evidence/observed-date/expiry/source-type; any missing or EXPIRED entry defaults to `unverified`; a generator querying an unproven capability gets `unverified`, never `supported`. tests.MigrationAndDefaultTests passes. PASS.
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: Conformance probe harness sources command templates from validated adapters, detects installed host version, scaffolds isolated HOME/XDG fixtures, captures stdout/stderr/exit + nonce side effect, redacts evidence, and REFUSES a real-HOME target; tests RESOLUTION and FOLLOWING separately with a machine-validated durable report. tests.ProbeHarnessTests + SafetyAndIsolationTests pass (real-HOME refusal, redaction, nonce). PASS.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: NEGATIVE probes for missing skill, denied permission, no user input, path precedence, stale adapter, malformed frontmatter, external-path refusal, server auth, background result loss - `supported` requires BOTH positive contract AND fail-closed behavior. tests.NegativeProbeTests + PromotionGateTests pass. PASS.
  - Result: pass
- [x] V-04 validates E-04
  ...
  - Observed evidence: `tests/test_host_capability_registry.py` exists and passes (28 tests). Full suite green: make test / pytest -n auto -> 1579 passed, 1 skipped, rc=0.
  - Result: pass
