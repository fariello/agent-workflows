...
- 2026-08-18 executed (opencode Opus 4.8): E-01..E-03 performed, V pass; top-level when/why+examples epilog + ipd/show/storage EXAMPLES; full serial suite 1128 passed 1 skipped.
...
- [x] E-01 In `agent_workflows/cli.py`, add an `epilog=` to the top-level parser (cli.py:378-383) containing a "When and why to use aw" block plus common worked examples, and switch that parser's `formatter_class` to a raw-preserving alphabetical formatter so the epilog renders verbatim. Add the formatter class near `_AlphaHelpFormatter` (cli.py:347):
  ```python
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: Verified: aw --help shows WHEN AND WHY + COMMON EXAMPLES (raw line breaks), ipd/show/storage --help show EXAMPLES, missing-arg still nonzero, subcommand list alphabetical; test_help_verbose 4 pass; suite 1128p/1s.
  - Result: pass
