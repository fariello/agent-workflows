...
- Declared-but-conditional: if the executor judges that `25kzda` (`.aw/records/specs/approved/20260826-0718-01-aw-run-deterministic-run-and-verify.spec.md`, Sections 2.5/2.5a) also needs amending, ADD that path to `Scope-Paths` before editing it, so the pre-run announcement names it; it is deliberately not pre-declared, since V-07 permits deciding it is unnecessary.
