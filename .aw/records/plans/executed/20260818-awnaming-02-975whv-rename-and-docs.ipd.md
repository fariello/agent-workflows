...
- 2026-08-18 executed (opencode Opus 4.8): E-01..E-07 performed, V-01..V-07 pass; renames + docs committed f0ddd40 (184 plans + 65 other records to .type.md; comms + research documented exceptions; AGENTS.md reconciled to one grammar; vf03z3 done + u9cicx follow-up filed); full serial suite 1021 passed, 1 skipped; sanitize + all --check clean; conforms at aw ipd lint --phase pre-transition.
...
- [x] E-02 Rename the non-plan durable records: specs (16) -> `.spec.md`, walkthroughs (11) -> `.walkthrough.md`, roadmaps (1) -> `.roadmap.md`, backlog (33) -> `.backlog.md`, comms (22) -> `.comms.md`, prompts (16) + prompt-library (4) -> `.prompt.md`, via `git mv`. Leave research (86) as `.<model>.<kind>.md`.
  - Depends on: none
...
- [x] E-03 Regenerate every manifest so it points at the new names: `aw plans index`, `aw research index` (unchanged names but re-verify), `aw specs check`, `aw backlog check`, plus any STATUS/INDEX boards. Confirm each `--check` passes.
  ...
  - Execution state: performed
- [x] E-04 Fix internal path citations that reference the old bare-`.md` names (grep the repo for `.aw/records/**/*-*.md` string references in docs/specs/plans/READMEs and update to the `.type.md` names where the citation is to a renamed file). Skip research citations.
  - Depends on: E-01,E-02
...
- Blocking: no
- Status: resolved
- Owner: opencode (resolved at E-02, 2026-08-18)
- Resolution or deferral rationale: RESOLVED = comms is a SECOND DOCUMENTED EXCEPTION (like research). Comms filenames are ROUTING-named (`<date>-<time>-NN-<sender>--to--<recipient>-<kind>-<slug>.md`), not the clustered `YYYYMMDD-<setid>-NN-<id6>-<slug>` grammar, and already contain dots (`agent-workflows.agent`). The `.type.md` grammar does not apply to them; forcing a `.comms.md` facet onto routing-named files adds no value and risks the envelope/ack parser. So the 22 comms files were LEFT AS-IS and comms is treated as an exception alongside research. Not blocking; no AGENTS.md grammar change needed (the grammar governs clustered artifacts).
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: `plans: 184/184 on .ipd.md` (0 non-`.ipd.md` plan files remain excluding README/INDEX/STATUS); `git status --short | grep -c '^R'` = 184 (all renames R, not delete+add). date/set/order/id6/slug unchanged (facet-append only).
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: specs 16/16 `.spec.md`, walkthroughs 11/11 `.walkthrough.md`, roadmaps 1/1 `.roadmap.md`, backlog 33/34 `.backlog.md` (the 1 non-faceted is the persistent UNTRACKED stray `xd78mr`, correctly skipped by `git mv`), prompts 16/16 + prompt-library 4/4 `.prompt.md`. EXCEPTIONS kept unchanged: comms 22 (routing-named, OQ-01 resolved as a documented exception) and research 86 (`.<model>.<kind>.md`).
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: `plans index --check: clean`; `index --check: clean` (research); `aw specs check: all specs conform.`; `aw backlog check: all backlog items conform.`; `aw attention --check: the view is valid.`
  - Result: pass
- [x] V-04 validates E-04
  ...
  - Observed evidence: two FUNCTIONAL citations fixed - `tests/test_ipd_templates.py:22` (`...ipd-spec.md` -> `...ipd-spec.spec.md`) and `.aw/system/workflows/handoff/handoff.md:124` (corrected to `.aw/records/prompts/superseded/...-session-handoff-resume-here.prompt.md`). All other filename mentions are justified historical prose (workflow-history/narrative inside already-executed plans and walkthroughs, plus `DECISIONS.md`/`TODO.md` logs that use legacy `.agents/docs/...` paths describing past work) or synthetic test-string literals; plans cross-reference by immutable `- Id:`/`PLAN-xxxxxx`, so no live path resolution broke. Full suite green confirms no functional citation remained stale.
  - Result: pass
- [x] V-05 validates E-05
  - Required evidence: paste the reconciled AGENTS.md grammar line(s) + `rg "YYYYMMDD-HHMM-NN" AGENTS.md` returning nothing.
  - Observed evidence: `rg "YYYYMMDD-HHMM-NN" AGENTS.md` returns nothing (stale grammar gone). Both mentions now reference the one grammar: line 26 `...-<slug>.ipd.md`, the uniform `YYYYMMDD-<setid>-NN-<id6>-<slug>.<type>.md` with facet `.ipd.md`; line 51 the same grammar with the `.ipd.md` facet. The engine generator source (engine.py:922) was updated to match so a regen stays consistent. No em/en dashes in the new prose.
  - Result: pass
- [x] V-06 validates E-06
  ...
  - Observed evidence: `aw backlog set ... vf03z3 -> done` (now at `.aw/records/backlog/done/20260817-awretrofit-01-vf03z3-plan-name-grammar-tooling.backlog.md`); the follow-up `aw backlog new` wrote `.aw/records/backlog/open/20260818-awnaming-01-u9cicx-aw-migrate-layout-optional-rename-on-migrate-to-th.backlog.md` (the OQ-02 optional rename-on-migrate nicety); `aw backlog check: all backlog items conform.`
  - Result: pass
- [x] V-07 validates E-07
  ...
  - Observed evidence: `1021 passed, 1 skipped in 194.75s`; `aw sanitize --agent` exit 0 (no leaks). One fix surfaced during rename: the legacy `_NEW_RE` (`YYYYMMDD-HHMM-NN-<slug>`) in normalize_plan_names.py also needed the optional `.<type>` facet (4 old-style plans like `20260808-0004-00-...` and `20260815-2156-01-...` carry `.ipd.md`); added, and `test_all_plan_files_conform` passes. No test references a renamed real record by old basename (remaining test literals are synthetic fixtures).
  - Result: pass
