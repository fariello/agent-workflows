...
## Workflow history
- 2026-09-13 executed (aw oc run): aw oc run self-finalize: 6ltz1y verified (set auditshare, attempt 1).
- 2026-09-13 executed-pending-transition (opencode its_direct/pt3-claude-opus-5-1m-us): All six E-items PERFORMED and all six V-items VERIFIED with pasted evidence; `aw ipd lint --phase pre-transition` reports `conforming`. Commits `5f159173` (implementation + tests) and `56887ec2` (V-evidence), both path-scoped, neither pushed. THE TERMINAL TRANSITION IS NOT MINE TO MAKE: `aw ipd finalize` refused with `AW-LIFECYCLE-ROLE-001` ("the runner owns begin/finalize for managed lanes; a worker-role process must not run them"), so this plan stays `approved` in `pending/` and the driver performs the move. WHAT LANDED: the audit predicate and its lookup now live in the new `agent_workflows/artifact_audit.py`, which BOTH `aw runs` and `aw doctor` call; `run_viewer` keeps only a `StepSummary`-shaped adapter and its audit attribute IS the shared object (`assertIs`, the refork-guard pattern). The lookup consumes `selectors` rather than the private nine-directory list, fixing the TYPE SET defect (a `backlog` record returned `None` at HEAD) and the first-match-wins defect (an id6 matching several records now yields the collision verdict). `aw doctor` gained the audit by E-04 route (b), TRACKED-ONLY, at `info` severity chosen from a MEASURED count of ZERO findings on the live tree; route (a) was rejected because run records are gitignored and absent from a clone or a lane (this lane has zero run dirs), the objection OQ-03 accepts against `aw check`. It is not duplication: measured, `IPD-M105` covers only the `pending/` direction, while a plan in `executed/` declaring `- Status: superseded` lints `legacy/not evaluated` with ZERO diagnostics. `aw runs` output byte-identical across five flag shapes. THREE CORRECTIONS TO THE PLAN'S OWN ASSUMPTIONS, all measured: (1) an EXACT-ONLY lookup would have REGRESSED 268 of 1202 records into "missing", because `selectors` reads a bounded 4096-byte header and 268 records (including THIS PLAN'S OWN FILE, byte 6485) declare `- Id:` below it, so the lookup is two-tier with a clustered-filename id6 FIELD fallback that still cannot match a review record; (2) consuming the resolver per step cost 15x on `tests/test_run_viewer.py` (2.41s -> 35.41s), so `build_index` memoizes one traversal per (root, type vocabulary), invalidated by directory mtimes, restoring 4.00s; (3) the suite baseline in a lane is NOT what the plan predicted - 17 failures come from the runner's own `AW_EXECUTION_ROLE=worker` marker (proved by toggling only that variable), and the predicted `test_reporting_contract` failure does not occur here because the gitignored `opencode-recovery/` tree is absent. Suite measured identically on both sides with the marker unset: BEFORE `5971 passed, 3 skipped, 2 xfailed`, AFTER `5998 passed, 3 skipped, 2 xfailed`, failure delta by node id EMPTY. `selectors.py` NOT modified. The falsified monthly-shard premise was NOT acted on; that case is a no-regression assertion only.
