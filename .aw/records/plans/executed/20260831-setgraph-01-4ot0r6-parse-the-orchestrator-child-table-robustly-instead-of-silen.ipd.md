...
## Workflow history
- 2026-08-31 executed (opencode/its_direct/pt3-claude-opus-5-1m-us): Fixed the orchestrator child-table parser: 18/47 -> 37/47 tables now parse, and every remaining refusal states a reason so the conservative serial fallback is never silent again. Four measured causes fixed (an inline pipe inside backticks shifting the hard-coded cells[3]; column indices hard-coded against 17 real header shapes; trailing prose in the Depends-on cell; and the canonical executed:<id6> grammar being rejected by the tool that defines it). Fallback ORDER and existing cross_edges_source values deliberately unchanged; new fields additive and defaulted. Ambiguity still fails closed, because a wrong edge is worse than a reported failure. Product change is commit 17871a1e. Validated in the PRIMARY checkout: tests/test_ipd_set_plan.py 34 passed (was 19, 15 new cases, no existing assertion weakened), bare pytest 3832 passed / 3 skipped / 4 xfailed (baseline 3816), all 18 previously-parsing Sets byte-identical, deliberate sabotage runs prove the new tests are not vacuous, aw sanitize clean. HONEST SCOPE: aw ipd execute-set cannot execute anything, so this fixes a PREVIEW, not execution; and it does not settle whether the hand-written table or the machine-readable Item-Dependencies field should be authoritative. Two follow-ups deliberately NOT done: a lint rule for the column layout (would put 28 existing plans, some immutable, into violation at once) and the misleading verb name, which belongs to the command-surface owners. [Scope reconciliation - in-scope-unmodified agent_workflows/ipd_set_plan.py: changed in commit 17871a1e, which precedes this receipt's baseline; in-scope-unmodified tests/test_ipd_set_plan.py: changed in commit 17871a1e, which precedes this receipt's baseline]
