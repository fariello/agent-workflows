...
## Workflow history
- 2026-09-01 executed (opencode/its_direct/pt3-claude-opus-5-1m-us): runstop whole-Set verification performed at 33dc8264; record at .aw/records/walkthroughs/20260901-runstop-00-zpbx7o-graceful-quit-whole-set-verification.walkthrough.md. A1-A10 hold (260 tests re-run, 0 failures; A8 verified directly since no child owns it; A10 recorded UNVERIFIED on Windows per OQ-02). CID-1..CID-5 pass, CID-1/CID-5 by repo-wide AST. Found and fixed one real defect en route (mzy2so, a fixture fault, sabotage-verified twice). [Scope reconciliation - in-scope-unmodified .aw/records/plans/pending/20260829-runstop-00-zpbx7o-runner-graceful-quit-protocol-adopt-spec-c4gd2h.ipd.md: the E-01/V-01 evidence was committed in 82c69d33 immediately before this begin, so it is unmodified under this receipt; in-scope-unmodified .aw/records/walkthroughs/: the required verification record was written and committed in 82c69d33, before this receipt was minted]
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: PERFORMED 2026-09-01 at HEAD `33dc8264`. The full verification record is written at the declared `Scope-Paths` location: `.aw/records/walkthroughs/20260901-runstop-00-zpbx7o-graceful-quit-whole-set-verification.walkthrough.md`. Summary of what it contains, with the load-bearing numbers inline so this V-item is auditable without opening it:
    (0) A DEFECT WAS FOUND AND FIXED BY THIS VERIFICATION, which is why the Set did not finalize on the first attempt. `make test-all` failed `tests/test_runner_stop_levels12.py::Level2Tests::test_level_2_leaves_another_sets_runnable_item_queued_when_this_set_is_blocked`. Diagnosed as a FIXTURE fault, not a product defect (no product file changed; fixed in `33dc8264`, filed and closed as backlog `mzy2so`): `_PLAN_TEMPLATE` put the metadata bullets BEFORE the H1 title, which the shared structural reader cannot see, AND the test injected a LEGACY `- Dependencies:` field that `8guhs0` deliberately deleted. Both faults yielded a dependency-FREE item, so the driver correctly treated it as runnable. Measured: template shape -> `([], None)`; canonical order -> the declared edge. IMPORTANT PROCESS NOTE: my first fix PASSED but passed VACUOUSLY (sabotaging the dependency gate outright still left it green); the final fixture is sabotage-verified TWICE - forcing the gate to always pass FAILS it, and disabling the level-2 boundary decline FAILS it.
    (1) A1-A10 WITH COMMAND EVIDENCE. A1-A7 and A9 are exercised by the Set's `slow`-inclusive suites, RE-RUN by me at this HEAD rather than quoted from the children: `test_runner_stop.py` 54 passed, `test_runner_stop_levels12.py` 20 passed, `test_runner_stop_level3.py` 49 passed, `test_runner_stop_level4.py` 41 passed, `test_runner_stop_triggers.py` 52 passed, `test_runner_shutdown.py` 27 passed, `test_run_viewer_liveness.py` 17 passed = 260 tests, 0 failures. R1-R4 were also observed in LIVE driver output during this session: `clean shutdown: all invariants satisfied`, `children_reaped (R1): ok`, `lock_released (R2): ok - lock file removed; lock free=True`, `ledger_coherent (R3): ok`, `tree_observed (R4): ok - 4 dirty path(s) left exactly as found`.
    A8 VERIFIED HERE (not silently dropped), since no child claims it: `python3 -m pytest tests/test_runner_shutdown.py -m '' -k pins_hard_abort` -> `1 passed`. The honest finding is NARROWER than A8's wording: a SIGKILL'd driver DOES leave `driver.lock` holding a dead pid, but that file does NOT block a later run because the kernel drops an `flock` on holder death (`runner_shutdown.py:27`); detection is by ACQUIRABILITY via `run_viewer.driver_holder_state`, whose side passes 17/17. So A8's substance holds (crash is detected and reported by the next run's reconciliation, proving the shared routine covers crash as well as stop) while "stale lock" must be read as "stale lock FILE with a dead pid", not "a lock that blocks".
    A10 RECORDED WITH ITS LIMITATION rather than passed - see (5).
    (2) PER-CHILD TABLE: all six children (`2ouj70`, `gq6m2u`, `1qxuke`, `foi1b3`, `m0z0ti`, `71vjbn`) are in `executed/` with every V-item checked, every `Result: pass`, and ZERO empty `Observed evidence:` lines, so none is prose-only.
    (3) CID-1..CID-5 ALL PASS, with CID-1 and CID-5 checked by AST over `agent_workflows/` REPO-WIDE (not a text grep, and not per-file - CID-3 warns a per-file check would pass while two byte-identical copies exist). CID-1: `terminate_process` -> `agy_runipd.py:2580 DELEGATES`, `oc_runipd.py:3994 DELEGATES`, `runner_shutdown.py:126 IMPLEMENTS`; `clean_shutdown` -> `runner_shutdown.py:434 IMPLEMENTS`; `_signal` -> `runner_shutdown.py:153 IMPLEMENTS`. So exactly ONE implementation and the byte-identical duplication CID-3 measured is GONE. Corroborating: `os.killpg` appears once as a call (`runner_shutdown.py:158`) and the SIGKILL escalation once (`:181`). CID-2: no `.aw/state` construction outside the shared accessors (remaining matches are docstrings/comments/a declared constant). CID-3: both drivers delegate to the one implementation, so no level can exist in one driver only. CID-4: the `unknown_outcome` resume refusal is exercised through the real entry point by `test_runner_stop_level4.py` (41 passed). CID-5: `exclusive_file_lock` and `_kill_process_tree` are ABSENT on main (correctly deferred to `wtiso` Phase 5 `2c122z` per P8) and `RunLockHandle` exists exactly once (`runner_shutdown.py:193`).
    (4) `make test-all` (required because a bare `pytest` deselects the `slow` tests this Set depends on): `4 failed, 4394 passed, 3 skipped, 4 xfailed in 155.51s (0:02:35)`. Before the `mzy2so` fix this was `5 failed`; the delta is exactly the level-2 test. THE 4 REMAINING ARE PRE-EXISTING AND OUT OF THIS SET'S SCOPE, named rather than waved through: `test_command_surface_declarations.py::test_zero_undeclared_parser_leaves` (reports `AssertionError: 65 != 0 : Found undeclared parser leaves`), `test_cli.py::test_every_subparser_has_fuller_description`, and both `test_cli_conformance_matrix.py::UndeclaredLeafGuardTests` cases. All four are CLI-surface DECLARATION checks, not runner behavior, and all four were reproduced at `5e5da9a0` before this session's commits. `command_surface.py`/`cli.py` are in approved `0soncw`'s Scope-Paths, so it is the plausible owner; I did NOT confirm `0soncw` intends to close these specific assertions, so ownership is probable, not established.
    (5) THE A10 PLATFORM STATEMENT REQUIRED BY OQ-02: NO ROW WAS VERIFIED ON WINDOWS. I had no Windows host, so zero A10 rows were executed there. What is verifiable, and all the code itself claims (`runner_stop.py:28-34`): the module and both drivers now IMPORT on a non-POSIX host, which removes an import-time barrier and nothing else; the SIGINT/SIGTERM ladder still REQUIRES POSIX signal semantics and the process-tree kill (`os.killpg`/`getpgid`) still has NO Windows equivalent (`runner_shutdown.py:181` degrades via `getattr(signal, "SIGKILL", signal.SIGTERM)`); and A10's second half - unsupported triggers must fail LOUDLY rather than silently no-op - IS implemented, via `install_stop_signal_handlers` returning a per-trigger status and `render_trigger_support` rendering what could not be installed. Per the module's own standard, no text here promises a working Windows subset.
  - Result: pass
