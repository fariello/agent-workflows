...
- [x] V-01 validates E-01
  ...
  - Observed evidence: nonterminal inventory (pending/; no reusable/ plans): the 8 research-org `20260730-2201-*` plans were QUARANTINED (each given the metadata trio `Quarantine:`/`Quarantine owner:`/`Quarantine follow-up:` + a `2026-08-03 quarantined` workflow-history line recording the maintainer's IPD-system-first sequencing authorization; reason = old-shape draft superseded by this convention, follow-up = re-author to the E-*/V-* shape after the ipd-structure Set); NONE were migrated (they are deferred by decision, not re-authored here). The 2 remaining nonterminal plans (`20260802-ipdstruct-00-by245s-ipd-structure-orchestrator` orchestrator + `-06` this plan) are already conforming and are the ipd-structure Set's own in-flight plans (they transition to executed/ as the Set completes). `aw ipd lint` on a quarantined file reports `quarantined` (confirmed on `20260730-researchorg-00-jwbo2u-research-org-orchestrator`).
  - Result: pass
