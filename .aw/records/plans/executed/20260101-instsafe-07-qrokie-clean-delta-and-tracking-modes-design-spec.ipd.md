...
- Status: executed
- Set: instsafe (install safety and ownership)
- Order: 7
...
- 2026-07-26 executed (opencode its_direct/pt3-claude-opus-4.8-1m-us): document-only execution. Wrote the design spec `.agents/docs/specs/20260726-1239-01-clean-delta-and-tracking-modes.spec.md` (two modes; merge-base-diff clean-PR definition; host-native skills primary delivery with a Claude `.claude/skills` adapter and no universal out-of-repo pointer; sibling companion repo; user-global per-repo config + separate global ownership manifest; `context` resolver + conditional-commit rule; visible same-version no-op state table + downgrade-preservation record referencing D103/backups; local-only feature; capability + explicit-harness skill taxonomy; migration both directions; build decomposed into gated per-phase IPDs). Added DECISIONS D109 + CHANGELOG. IPD 05's open questions resolved here; executed IPD 05 left untouched. Documents only; internal consistency checked; dash/leak clean; full suite unchanged at 425 passed, 1 skipped. Path-scoped commits, no push. Status: approved -> executed; moved to `.agents/plans/executed/`. NEXT: author the Phase 0 conformance-harness IPD as the first buildable step.
- 2026-07-26 /plan-review, incl. an explicit SPLIT assessment (opencode its_direct/pt3-claude-opus-4.8-1m-us): APPROVE WITH REVISIONS APPLIED; PR-001..PR-003. Verified claims against evidence: the two reconciliation reports exist at the cited bundle paths; the "producing runbooks that hard-instruct commit (never push)" count is 7 (not ~8) across assess/assess-all/incident/migrate/spec/plan-review/release-review; plan-lifecycle moves are convention-only (the only engine `_git_mv` at `engine.py:1795`/`:1869` migrates the legacy release-review artifacts dir, not plans); IPD 05 is in executed/. SPLIT VERDICT: do NOT split the SPEC (it is one coherent, low-risk, document-only narrative; splitting adds lifecycle overhead for no safety gain); the large BUILD is already correctly decomposed into separate, gated per-phase IPDs, and this review made that structural and hard-ruled (new "Build decomposition" section; Step 5 states no IPD spans a phase and no delivery-tier phase runs before Phase 0 reproduces it). PR-001 fixed the ~8->7 count; PR-002 added the Build-decomposition section + hard rules; PR-003 cited the `_git_mv` evidence. Author was reviewer, so claims verified from code + the bundle. No open questions (OQ1-OQ3 resolved; OQ4 open-by-design as the harness's job, not a spec blocker); no unfixed BLOCKER/HIGH. Readiness: GO - PENDING HUMAN APPROVAL.
...
| C5 | MEDIUM | maintainer | lifecycle prose | Seven producing runbooks (assess, assess-all, incident, migrate, spec, plan-review, release-review; verified) hard-instruct "commit this (never push)"; those must become conditional on the resolved artifact root via a SINGLE resolver (P8), not per-runbook edits. A deterministic `context` resolver returns target root, mode, companion root, per-class routes, and whether the current command may commit in target/companion/neither/both. Note: plan-lifecycle MOVES are convention-only (no engine code moves plan files; the only `_git_mv` at `engine.py:1795`/`:1869` migrates the legacy release-review artifacts dir, not plans), so untracking does not break a code mechanism, only the documented prose + git-history-as-provenance. | clean-delta reconciliation Sec 9.3; `engine.py:1795`,`:1869` |
...
| 3 | C5 | Document the resolver contract (`context --repo <path> --json` returning target root/mode/companion/routes/commit-permissions) and the rule that producing workflows call the resolver instead of parsing config, so the seven "commit (never push)" runbooks become conditional via ONE source (P8). No runbook edits in this IPD. | the spec doc | Low | resolver contract + single-source rule documented |
...
| 5 | C7, C8 | Document the PHASED PLAN and explicitly DEFER the build as a MULTI-IPD sequence (see "Build decomposition" below). Each phase is its OWN separate IPD with its own /plan-review + human approval; NO single build IPD may span more than one phase, and NO delivery-tier phase may execute before Phase 0 reproduces that tier's documented host behavior. Label the initial feature LOCAL clean-delta; remote is separate. | the spec doc | Low | phased plan documented; each phase named as a separate gated future IPD; "no phase spans; Phase 0 gates tiers" rule stated; build explicitly deferred |
...
## Build decomposition (why this stays a spec, and how the build is split)

This IPD is deliberately a SINGLE, document-only spec: the design is one coherent narrative and splitting the SPEC would fragment it for no safety gain (it is low-risk prose). The large, invasive work is the BUILD, and this spec mandates that the build be executed as a sequence of SEPARATE, individually-reviewed, gated IPDs, not one lump:

- Phase 0 - Conformance harness (its own IPD; the FIRST buildable step). A live per-host/version probe (clean temp home + temp repo, external content, a unique nonce side-effect, host diagnostics, precedence + permission runs, local vs cloud). Nothing downstream may claim a host tier works until this reproduces it.
- Phase 1 - Resolver + state/ownership (its own IPD). The `context` resolver, the user-global per-repo config section, the separate global ownership manifest, companion mapping, per-class routes; makes the seven producing runbooks conditional via the single resolver (P8).
- Phase 2 - Packaged user skills (its own IPD; GATED on Phase 0). The C9 subset (capability skills + explicit harness skills), at the documented host paths, per-host, only for tiers Phase 0 reproduced.
...
Hard rules for the build phases: each phase is its own IPD + /plan-review + human approval; no IPD spans more than one phase; no delivery-tier phase (2, 3, 4) executes before Phase 0 reproduces that tier for the exact host/version; the feature is LOCAL clean-delta only until a separate remote design exists.
