...
- 2026-08-18 executed (opencode Opus 4.8): E-01..E-05 performed, V pass; artifact_types vocab + TYPE_BACKENDS + six noun-verb parsers/router; existing verbs unaffected; full serial suite 1087 passed 1 skipped.
...
- [x] E-04 In `agent_workflows/cli.py` `_build_parser` (cli.py:369-1747), register SIX NEW top-level subparsers - `check`, `find`, `search`, `index`, `rename`, `group` - each taking a positional `type` (validated via `artifact_types.normalize_type`), the verb-appropriate selectors/flags (a minimal `selector` positional `nargs="*"` for now; the full selector grammar is Set E awselect), and the shared `--json`/`--agent`. DO NOT touch the existing top-level `archive` verb in this Order: it currently has a `target` positional (cli.py:1600, research-only) and generalizing it to a `type`-first signature would BREAK `aw archive <id6>` for the whole window between this Order and Order 03. The `archive` verb is generalized by Order 03 (awcmdsurf-03), which owns archive and can flip its signature + wire `aw archive research`/`aw archive plans` in one atomic step. Add each of the six new verbs to the dispatch chain (cli.py:4018-4241) routing to a thin `_run_<verb>(args, term)` that resolves the backend via `TYPE_BACKENDS` and either delegates (where a backend exists) or prints "not yet wired / not supported for <type>" (exit 2) for the parts later Orders fill.
  ...
  - Execution state: performed
- [x] E-05 Add `tests/test_awcmdsurf_vocab_and_parsers.py` covering: `normalize_type`/`expand_types` (happy + unknown + `all`), `TYPE_BACKENDS` lookups (supported + None), the exit-code helper (0/1/2), and that each of the six new verbs parses `--help` and errors on an unknown type. Run the FULL serial suite (`python3 -m pytest -p no:xdist`) and paste the tail (the new parsers must not break existing dispatch).
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: Verified: normalize/expand/backend map + no eager imports; six verbs parse --help + reject unknown type (exit 2); archive <id6> unchanged; test_awcmdsurf_vocab_and_parsers 8 pass; suite 1087p/1s.
  - Result: pass
