...
## Workflow history
- 2026-08-30 executed (opencode its_direct/pt3-claude-opus-5-1m-us): Levels 1-2 (stop-after-call, stop-after-set) in BOTH drivers over one shared boundary helper; deliberate stop recorded as a non-failure on events.jsonl; honest exit-0 contract that never rewrites a queued status. 20 new tests drive the real drivers over a fake child; zero net-new full-suite failures (19 == 19 vs base). [Scope reconciliation - out-of-scope tests/test_runner_stop.py: Phase 1's placeholder scope-fence test (test_this_child_wires_no_level_behavior) explicitly reserved room for Phases 2-4 and asserted no driver consumes the poll result, which levels 1-2 must do. Consciously NARROWED per orchestrator CID-4 (updated, never deleted): the superseded poll assertions are dropped, the still-live 'no signal.signal( before Phase 5' guard is kept, and the behavior is now asserted positively in tests/test_runner_stop_levels12.py. See DECISION 04-1qxuke-D2.; in-scope-unmodified agent_workflows/agy_runipd.py: MODIFIED by this execution in commit dc6b0a80; appears unmodified only because 'aw ipd begin' had to be re-run after that commit (stale-digest gate) which re-froze base_head to it. NOT 'not-needed' - the suggested wording would be false.; in-scope-unmodified agent_workflows/oc_runipd.py: MODIFIED by this execution in commit dc6b0a80; appears unmodified only because 'aw ipd begin' had to be re-run after that commit (stale-digest gate) which re-froze base_head to it. NOT 'not-needed' - the suggested wording would be false.; in-scope-unmodified agent_workflows/runner_stop.py: MODIFIED by this execution in commit dc6b0a80; appears unmodified only because 'aw ipd begin' had to be re-run after that commit (stale-digest gate) which re-froze base_head to it. NOT 'not-needed' - the suggested wording would be false.; in-scope-unmodified tests/test_runner_stop_levels12.py: MODIFIED by this execution in commit dc6b0a80; appears unmodified only because 'aw ipd begin' had to be re-run after that commit (stale-digest gate) which re-froze base_head to it. NOT 'not-needed' - the suggested wording would be false.]
...
- [x] E-02 In BOTH drivers, branch on a level-2 request: continue dequeuing items whose `setid` matches the CURRENT set (the driver already tracks `item['setid']`, see `oc_runipd.py:1722-1727`), then stop before the first item of any OTHER set, run `clean_shutdown`, exit 0. DEFINE "the current set" precisely as the `setid` of the item that was in flight when the level-2 request was OBSERVED, captured once and held for the rest of the wind-down. This matters because the runtime dequeue is DEPENDENCY-ordered, not set-ordered: `run_queue` picks the first `queued` item whose dependencies are satisfied, scanning the entire queue (`oc_runipd.py:2500-2505`), so sets can INTERLEAVE (proven in Findings) and a naive "stop when `setid` changes" rule would stop at the wrong place or resume the original set later. During a level-2 wind-down, an item of any other set is treated as out-of-scope and left `queued`, even if it is the only runnable item; in that case the wind-down ends immediately rather than running it.
  ...
  - Execution state: performed
- [x] E-03 Handle the final-set boundary explicitly (OQ-01): a level-2 request while on the last set completes that set and exits 0 with nothing skipped, still running `clean_shutdown` so the invariant is uniform (spec R5/R6).
  - Depends on: E-02
...
- `make test-all` (`python -m pytest tests/ -m ''`): the FULL suite, since a bare `python -m pytest -q` deselects `-m 'not slow'` per `pyproject.toml:122`.

  EXECUTION CORRECTION (2026-08-30), recorded rather than quietly satisfied: as authored this line said the full suite "remains green". It is NOT green on this base and was not green before this child either. Measured: `make test-all` reports `19 failed, 3340 passed, 3 skipped, 4 xfailed` at my HEAD, and the SAME 19 fail at my base commit `c1cbb2e` when run in a separate clean worktree (`19 failed, 3320 passed`). The honest and checkable criterion is therefore ZERO NET-NEW failures, proven by diffing the sorted FAILED sets rather than asserting it:

  ```
  head=19 base=19
  === diff base vs head ===
  IDENTICAL -> zero net-new failures
  ```

  The 19 pre-existing failures are 15 in `tests/test_run_viewer.py` plus 4 CLI-surface/description guards (`test_cli.py`, `test_cli_conformance_matrix.py`, `test_command_surface_declarations.py`); none is in a file this child touches. The passed count rising 3320 -> 3340 is exactly this child's 20 new tests.

  Also recorded: an intermediate full-suite run showed 6 EXTRA failures (`test_leak_sanitizer`, `test_local_leaks`, two `test_release_readiness` leak/lint gates). Those were REAL and caused by this child, not flakes: (a) my own V-01 evidence had pasted an absolute home path into a tracked file, which the leak sanitizer correctly flagged (now redacted; `aw sanitize --agent` -> `outcome=clean, findings=0`), and (b) `aw ipd lint` exited 1 because the V-item evidence was written as an indented block under a bare `- Observed evidence:` line, which `ipd_lint._SUBFIELD_RE` cannot see (it reads only the SAME-line value), so all five V-items linted as empty. Both are fixed and those 77 tests now pass.
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: 20/20 tests in tests/test_runner_stop_levels12.py pass (pasted below, with the actual driver artifacts). Level 1 ran ONLY item 1 of 3 (read from `ipd-started` events + `outcomes/*.json`, not a mock count); exit 0; all four Phase-0 invariants observed, and the R1 orphan check is proven to have teeth by a planted-process mutation.

    `python3 -m pytest tests/test_runner_stop_levels12.py -m '' -p no:randomly -o addopts="" -v` (level-1 rows):

    ```
    tests/test_runner_stop_levels12.py::Level1Tests::test_level_1_completes_only_the_in_flight_item PASSED [ 52%]
    tests/test_runner_stop_levels12.py::Level1Tests::test_level_1_records_the_deliberate_stop_as_a_non_failure PASSED [ 57%]
    ```

    The items that ran are read from the run's OWN artifacts (`ipd-started` events cross-checked against `outcomes/*.json`), never a mock call count. Direct artifact dump of a real driver run, 3-item queue, level-1 request written during item 1:

    ```
    LEVEL 1 (oc): A1 - stop-after-call
    process exit code: 0
    --- state.json queue (id6/setid/status) ---
      aa0001  set=aaa   status=executed
      aa0002  set=aaa   status=queued
      aa0003  set=aaa   status=queued
    --- unknown_outcome occurrences in the whole ledger: 0
    ```

    The four Phase-0 invariants are asserted inside `assert_phase0_invariants` on every behavioral test above: R1 by reading the real process table (`ps -eo pid,ppid,args`), R2 by RE-ACQUIRING `driver.lock` with a fresh non-blocking `flock`, R3 by calling Phase 0's own `runner_shutdown.observe_ledger(run_dir)` plus checking every item's status against `runner_shutdown.KNOWN_ITEM_STATUSES`, and R4 by comparing `git status --porcelain` (UNCHANGED-not-clean per `2ouj70`'s observe-and-report semantics) and asserting `git stash list` is empty. The driver's own shutdown report, captured from a real run's stderr, agrees:
    ...
    clean shutdown: all invariants satisfied
      ...
      ledger_coherent (R3): ok - 3 item(s), all in a defined state
      tree_observed (R4): ok - 4 dirty path(s) left exactly as found (nothing stashed, reset, or moved)
    ```

    The R1 check is proven to HAVE TEETH rather than being vacuously true, by planting a live process whose command line contains the run's repo path and confirming the assertion fires:

    ```
    clean case: PASSED (no orphan)
    orphan case: correctly DETECTED -> Lists differ: ['3524742 3524549 <redacted-interpreter-path> ...repo'] != []
    ```

    (the interpreter path in that line is redacted; the leak sanitizer correctly flags a raw home path in a tracked file, and the meaningful content is that the planted PID was matched and the assertion fired)
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: Level 2 completed BOTH set-A items and started NEITHER set-B item; exit 0; four invariants observed. The INTERLEAVED case is demonstrated twice: first that the real `dependency_status` would pick a set-B item next, then that the shipped wind-down declines it and leaves it `queued` (resolved OQ-02). Details and a measured test-authoring correction below.

    ```
    tests/test_runner_stop_levels12.py::Level2Tests::test_level_2_finishes_the_current_set_and_starts_no_other PASSED [ 63%]
    tests/test_runner_stop_levels12.py::Level2Tests::test_level_2_leaves_another_sets_runnable_item_queued_when_this_set_is_blocked PASSED [ 68%]
    ```

    Real driver run, set A (2 items) then set B (2 items), level-2 request written during A's first item:

    ```
    LEVEL 2 (oc): A4 - stop-after-set
    process exit code: 0
    --- events.jsonl (deliberate-stop event) ---
    {
      "at": "2026-08-30T07:07:44+00:00",
      "boundary": "next-set",
      "current_setid": "saa",
      "deliberate": true,
      ...
      "failure": false,
      "level": 2,
      "level_name": "after-set",
      "not_started": ["bb0001", "bb0002"],
      "requester": "test-operator"
    }
    --- state.json queue (id6/setid/status) ---
      ba0001  set=saa   status=executed
      ba0002  set=saa   status=executed
      bb0001  set=sbb   status=queued
      bb0002  set=sbb   status=queued
    --- unknown_outcome occurrences in the whole ledger: 0
    ```

    Both set-A items completed; NEITHER set-B item started; exit 0; the four invariants asserted as in V-01.

    THE INTERLEAVED CASE, which is the whole reason the current set is CAPTURED rather than re-derived. First, the naive expectation is disproved against the real `dependency_status`: with set A's next item blocked on an unmet dependency, the driver's normal next pick is a set-B item, so the in-flight set would jump A -> B while A still holds queued work:

    ```
    === FIRST: prove the naive expectation is wrong (sets DO interleave) ===
      ca0002 set=sca satisfied=False missing=['zzzz99']
      cb0001 set=scb satisfied=True missing=[]
      -> WITHOUT a wind-down the driver's next pick is cb0001 (set scb): the in-flight set jumps sca -> scb
    ```

    Then the shipped code is shown to DECLINE that item during a level-2 wind-down, ending with runnable work outstanding (resolved OQ-02):

    ```
    === THEN: with a level-2 wind-down the driver DECLINES that set-B item ===
      exit code: 0
      items that RAN: ['ca0001']
        ca0001 set=sca   status=executed
        ca0002 set=sca   status=queued
        cb0001 set=scb   status=queued
      -> set-B item cb0001 was runnable and was LEFT queued (resolved OQ-02)
    ```

    Note a MEASURED test-authoring correction worth recording: the first version of the interleave test injected the dependency as `- Item-Dependencies:`, which the driver's queue builder does NOT read (it reads `- Dependencies:` / `- Depends-on:`, `oc_runipd._DEPS_RE`). The test therefore built a dependency-FREE item and observed it running; the failure is what surfaced the wrong key. The shipped test uses the key the driver actually parses, which is why the blocked item now genuinely stays blocked.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: Final-set case passes and CAUGHT A REAL DEFECT: the first implementation recorded no deliberate stop when the queue drained (measured: `deliberate-stop events recorded: 0`), which is exactly OQ-01's predicted mis-special-casing. Fixed at all three loop-exit boundaries with exactly-once recording; a third path (relabelling the remainder `dependency-blocked`) was found and fixed with it. Full before/after evidence below.

    ```
    tests/test_runner_stop_levels12.py::Level2Tests::test_level_2_on_the_final_set_skips_nothing_and_still_cleans_up PASSED [ 75%]
    tests/test_runner_stop_levels12.py::Level2Tests::test_a_wind_down_does_not_relabel_the_remainder_dependency_blocked PASSED [ 60%]
    ```

    A REAL DEFECT WAS FOUND HERE, and this V-item is the reason. My first implementation recorded the deliberate stop only at the "declined to start an item" boundary. A level-2 stop on the FINAL set has no item to decline: it drains the queue and leaves the loop by the `if not queued: break` path, so it recorded NOTHING. Measured before the fix, by dumping the real run's ledger rather than reading the code:

    ```
    exit: 0 ran: ['da0001', 'da0002']
    deliberate-stop events recorded: 0
    ```

    That is precisely the mis-special-casing OQ-01 predicted: an operator-requested wind-down would have been indistinguishable from an ordinary finish, dropping the intent spec R21 requires. Fixed by recording the stop at whichever boundary the loop actually exits, guarded by a `stop_recorded` flag so it is recorded EXACTLY once. A third exit path was found by the same reasoning and fixed with it: under a wind-down the `runnable is None` path would have relabelled the untouched remainder `dependency-blocked`, a fabricated reason (those items were never offered to the dependency check; they are `queued` because the operator stopped), so a wind-down now leaves them `queued`. Both paths now have their own test, including the newly added `test_a_wind_down_does_not_relabel_the_remainder_dependency_blocked`.

    After the fix, the same real run:

    ```
    LEVEL 2 on the FINAL set (OQ-01)
    process exit code: 0
    items that RAN: ['da0001', 'da0002'] (nothing skipped)
    --- deliberate-stop event STILL recorded ---
    {
      "at": "2026-08-30T07:20:48+00:00",
      "boundary": "next-set",
      "current_setid": "sda",
      "deliberate": true,
      "event": "deliberate-stop",
      ...
      da0001 set=sda status=executed
      da0002 set=sda status=executed
    ```

    Evidence that `clean_shutdown` was NOT bypassed by a special case is both structural and observational. Structurally there is no early return around it: the level-1/2 branch `break`s out of the dequeue loop, and `clean_shutdown` runs in `locked_run`'s `finally` at the LOCK-HOLDING layer (added by Phase 0), so every exit from `run_queue` (normal end, deliberate stop, or exception) passes through it. Observationally, the routine's own per-invariant report from that same run:

    ```
    --- clean_shutdown report from the real run ---
    clean shutdown: all invariants satisfied
      children_reaped (R1): ok - no live child agent process among 0 tracked
      lock_released (R2): ok - lock file removed; lock free=True
    ...
    OQ-01 is therefore confirmed as resolved AS STATED: nothing is skipped, exit is 0, and the ONLY thing distinguishing this from an ordinary finish is the recorded deliberate stop (`not_started: []`).
  - Result: pass
- [x] V-04 validates E-04
  ...
  - Observed evidence: Actual `events.jsonl` records after a level-1 AND a level-2 stop are pasted below: each names the level and requester, is explicitly non-failure, un-run items remain `queued`, and `unknown_outcome` occurrences across the whole ledger are 0 (counted, not eyeballed). Coherence is checked via Phase 0's own `observe_ledger` + `KNOWN_ITEM_STATUSES`, and a test asserts no new ledger substrate.

    AFTER A LEVEL 1 STOP (actual `events.jsonl` record, real driver run):

    ```
    {
      "at": "2026-08-30T07:07:42+00:00",
      "boundary": "next-item",
      "current_setid": "aaa",
      "deliberate": true,
      "event": "deliberate-stop",
      "failure": false,
      "level": 1,
      "level_name": "after-call",
      "not_started": ["aa0002", "aa0003"],
      "requester": "test-operator"
    }
    --- state.json queue (id6/setid/status) ---
      aa0001  set=aaa   status=executed
    ...
    AFTER A LEVEL 2 STOP (actual `events.jsonl` record):
    ...
    Both records carry the LEVEL (`level`/`level_name`) and the REQUESTER, are explicitly non-failure (`"deliberate": true`, `"failure": false`), and every un-run item is still `queued`, so no completed item is reported as failed and no per-item status was invented. `unknown_outcome` occurrences across the ENTIRE ledger are 0 in both cases (spec R20), counted programmatically rather than eyeballed.

    Ledger coherence is not asserted as prose: the test calls Phase 0's own `runner_shutdown.observe_ledger(run_dir)` and additionally checks every item's status against `runner_shutdown.KNOWN_ITEM_STATUSES`. Using Phase 0's vocabulary is deliberate, because it means a level that invented a new per-item status would FAIL here instead of passing a test-local allowlist. The driver's own report on the same runs: `ledger_coherent (R3): ok - 3 item(s), all in a defined state`.

    NO NEW SUBSTRATE: the event rides the established append-only `events.jsonl` channel via the drivers' existing `append_jsonl`. Enforced as a test (`test_no_new_ledger_substrate_was_introduced`), which asserts neither driver mentions `run_ledger_store`:

    ```
    tests/test_runner_stop_levels12.py::BothDriversWireTheSameLevelsTests::test_no_new_ledger_substrate_was_introduced PASSED [ 45%]
    ```
  - Result: pass
- [x] V-05 validates E-05
  ...
  - Observed evidence: Both required halves are pasted below, plus a third guard. (a) exit 0 after level-1 and level-2 stops WHILE `state.json` still shows un-run items `queued`. (b) a stop whose run item genuinely failed exits 1 with that item `failed-safely`. (c) with NO stop, `queued` still fails, so the normal exit contract is unchanged. The pre-change predicate returning 1 is re-verified independently.

    First, the PROBLEM re-verified independently on this base, so the change is not taken on faith. The unmodified predicate returns 1 for a deliberate stop:

    ```
    SUCCESS_STATES = {'reviewed', 'executed', 'approved'}
    exit code for [executed,queued,queued] = 1
    ```

    (a) EXIT 0 WITH THE QUEUE STILL SHOWING `queued`. Both halves in one observation, from real driver processes:

    ```
    LEVEL 1 (oc): process exit code: 0
      aa0001  set=aaa   status=executed
      aa0002  set=aaa   status=queued      <- un-run, still `queued`, NOT laundered
      aa0003  set=aaa   status=queued

    LEVEL 2 (oc): process exit code: 0
      ba0001  set=saa   status=executed
      ba0002  set=saa   status=executed
      bb0001  set=sbb   status=queued      <- un-run, still `queued`
      bb0002  set=sbb   status=queued
    ```

    The test additionally asserts positively that the un-run item's status is NOT in `SUCCESS_STATES`, so laundering cannot satisfy it:

    ```
    tests/test_runner_stop_levels12.py::ExitContractTests::test_exit_zero_while_the_queue_still_shows_queued_items PASSED [100%]
    ```

    (b) A STOP WHOSE RUN ITEM FAILED EXITS NONZERO. Real run, level-2 stop with item 2 made to fail genuinely (no outcome JSON, no plan move, child exit 3):

    ```
    LEVEL 2 (oc): FAILED run item -> nonzero
    process exit code: 1
    --- state.json queue (id6/setid/status) ---
      fa0001  set=sfa   status=executed
      fa0002  set=sfa   status=failed-safely     <- not recorded executed
      fa0003  set=sfa   status=executed
    ```

    ```
    tests/test_runner_stop_levels12.py::ExitContractTests::test_a_stop_whose_run_item_failed_exits_nonzero PASSED [ 95%]
    ```

    (c) THE NORMAL CONTRACT IS UNCHANGED, which matters because a loosened predicate would be a silent regression far beyond this plan: with NO stop observed, `queued` still counts as failure exactly as before.

    ```
    stopped [executed,queued,queued]        -> 0
    stopped [executed,failed-safely,queued] -> 1
    NOT stopped [executed,queued]           -> 1
    NOT stopped [executed,executed]         -> 0
    ```

    ```
    tests/test_runner_stop_levels12.py::ExitContractTests::test_a_run_with_no_stop_keeps_the_original_exit_contract PASSED [ 90%]
    tests/test_runner_stop_levels12.py::SharedBoundaryTests::test_exit_code_ignores_queued_only_for_a_real_stop PASSED [ 10%]
    ```

    No item's status is rewritten anywhere in the implementation to manufacture the 0; the exit path ignores `queued` items only when a stop was actually observed, and only because they never ran.
  - Result: pass
