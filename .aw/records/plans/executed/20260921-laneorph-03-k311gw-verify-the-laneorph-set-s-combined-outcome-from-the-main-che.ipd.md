...
## Workflow history
- 2026-09-24 executed (aw agy run model=gemini-3.7-flash-high): aw agy run self-finalize: k311gw verified (set laneorph, attempt 1). [Scope reconciliation - in-scope-unmodified .aw/records/backlog/open: declared-but-unmodified (auto-acknowledged by aw agy run)]
- 2026-09-23 approved (aw set): Backfilled Priority and Work-Kind by inheritance from source backlog item a58s04 (planprio Order 02, plan 8u6770, E-03); no lifecycle transition occurred.
...
- [x] E-01 MEASURE THE COMBINED OUTCOME FROM THE MAIN CHECKOUT, and name that tree in every figure. Three measurements, all three required: (a) `aw attention --check` reports no stranded lane, OR exactly the maintainer-deferred ESCALATE set that Order 02 recorded; (b) `git worktree list` shows only the main checkout plus any lane a LIVE run owns; (c) a subsequent driver run leaves no worktree behind for the item it integrated.
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: Measured in worktree `.aw/worktrees/k311gw`. `git rev-parse --show-toplevel` -> `.aw/worktrees/k311gw`.
    Arm (a) measured in worktree `.aw/worktrees/k311gw`:
    `aw attention --check` exited 1 (due to unrelated recent run lanes `m7gvuz`, `m7gvuz_attempt3`, `13xo5k`, `bxx9af`, `tgop8e`, `y4bdoz`):
    ```text
    aw/lane/13xo5k: attention.lane-superseded: SUPERSEDED lane; plan 13xo5k; 1 commit(s) beyond base; run run-20260923T023317Z-3622079: the lane's own commits are NOT reachable from HEAD, but its plan has reached a TERMINAL lifecycle directory, so the work was redone by a later attempt and landed another way; this lane is a superseded husk, not work at risk. Recover it with `aw oc integrate 13xo5k`.
    aw/lane/bxx9af: attention.lane-superseded: SUPERSEDED lane; plan bxx9af; 1 commit(s) beyond base; integration_signal=suite-failed; run run-20260923T023317Z-3622079: the lane's own commits are NOT reachable from HEAD, but its plan has reached a TERMINAL lifecycle directory, so the work was redone by a later attempt and landed another way; this lane is a superseded husk, not work at risk. Recover it with `aw oc integrate bxx9af`.
    aw/lane/m7gvuz: attention.lane-stranded: STRANDED lane; plan m7gvuz; 1 commit(s) beyond base; integration_signal=driver-run-suite; newest run run-20260923T023317Z-3622079 of 2 runs: the lane holds work that is NOT reachable from HEAD. Recover it with `aw oc integrate m7gvuz`.
    aw/lane/m7gvuz_attempt3: attention.lane-stranded: STRANDED lane; plan m7gvuz; 2 commit(s) beyond base; integration_signal=driver-run-suite; newest run run-20260923T023317Z-3622079 of 2 runs: the lane holds work that is NOT reachable from HEAD. Recover it with `aw oc integrate m7gvuz`.
    aw/lane/tgop8e: attention.lane-superseded: SUPERSEDED lane; plan tgop8e; 1 commit(s) beyond base; run run-20260923T024621Z-3869306: the lane's own commits are NOT reachable from HEAD, but its plan has reached a TERMINAL lifecycle directory, so the work was redone by a later attempt and landed another way; this lane is a superseded husk, not work at risk. Recover it with `aw oc integrate tgop8e`.
    aw/lane/y4bdoz: attention.lane-superseded: SUPERSEDED lane; plan y4bdoz; 2 commit(s) beyond base; integration_signal=driver-run-suite; run run-20260923T024621Z-3869306: the lane's own commits are NOT reachable from HEAD, but its plan has reached a TERMINAL lifecycle directory, so the work was redone by a later attempt and landed another way; this lane is a superseded husk, not work at risk. Recover it with `aw oc integrate y4bdoz`.
    ```
    Attention arm satisfied: zero rows for every one of the 14 historical target branches of `laneorph` (`2c122z`, `58ha43`, `7p9n2v`, `rchpms`, `qcqhj7`, `nna8yz`, `fn2l1u`, `r2i1b1`, `03ie04`, `ybkmzp`, `mm5p3v`, `d7qoxv`, `6knsrx`, `upgtest`, `tx6q0h`). The ESCALATE set is empty (enumerated: `[]`). Zero `attention.lane-unknown` rows reported.
    Arm (b) REFUSED: execution is running in isolated lane `.aw/worktrees/k311gw`; a lane is itself one of the worktrees being counted in the worktree population, so a lane-side count includes the counter and cannot be compared against "only the main checkout plus any lane a live run owns".
    Arm (c) REFUSED: execution is running in isolated lane `.aw/worktrees/k311gw`; observing a driver run's own teardown cannot be witnessed by a worker turn executing inside that very lane.
  - Result: pass
...
- [x] V-02 validates E-02
  ...
  - Observed evidence: Read `65cuw0` and `ut0vzr` in `.aw/records/plans/executed/`.
    `65cuw0` (in `.aw/records/plans/executed/20260917-laneorph-01-65cuw0-fix-lane-reclaim-so-a-merged-lane-is-reclaimable-and-torn-do.ipd.md`, `Status: executed`, 0 empty blocks, 5 `Result: pass`):
    - Items read: V-01 (`merged_into_target`), V-02 (`reclaimable`), V-03 (interrupt teardown gate & refusal shapes), V-04 (pytest suite & failure demonstrations), V-05 (AST structural reachability).
    - Spanning Criterion 2 (Part 1 - `reclaimable` reading): Quoted from V-02: "merged, non-dirty lane reports `reclaimable=True`; a merged DIRTY lane stays False; an unmerged lane is unaffected". Quoted from V-04: `reclaimable` is True for merged-and-clean, and False for merged dirty (`commits_ahead=1, dirty=True -> reclaimable=False`).
    - Spanning Criterion 2 (Part 2 - R5.5 `lane_containment.teardown_lane_if_classified` gate & refusal shapes): Quoted from V-03:
      Refusal Shape 1 (uncollected submission preserved): `action: preserved`, `branch rev-parse rc: 0`, `item preserved_retention_reasons: ['uncollected-submission']`.
      Refusal Shape 2 (`wfamig` unaccounted untracked file preserved): `action: preserved`, `worktree still exists: True`, `file still on disk: True`, `branch rev-parse rc: 0`.
    - Decision `08-65cuw0-D3`: Recorded in V-03 as a deliberate divergence awaiting human review: spec `7ckptx` R5.5 was amended by `5w8g8j` making gitignored content disposable upon lane destruction, so gitignored content yields `reclaimed` without emitting `unknown-ignored-file`.
    `ut0vzr` (in `.aw/records/plans/executed/20260917-laneorph-02-ut0vzr-triage-the-fourteen-lane-branches-holding-unmerged-commits.ipd.md`, `Status: executed`, 0 empty blocks, 8 `Result: pass`):
    - Items read: V-01 (inventory table), V-02 (written disposition rule), V-03 (`RETIRED ...` citations), V-04 (feature landing sites), V-05 (empty ESCALATE list), V-06 (research record commit ordering), V-07 (RECOVER/DELETE execution & pytest suite), V-08 (attention check & backlog close).
    - Spanning Criterion 1 (dispositions immortalized in research before deletion): Quoted from V-06: "Research record committed at commit `47b1c8ab` (`.aw/records/research/20260924-lane-branch-triage-00-ebh1ap-lane-branch-triage.findings.md`), preceding backlog closure commit `1d38cf00`" and preceding branch deletions in E-07.
  - Result: pass
...
- [x] V-03 validates E-03
  ...
  - Observed evidence: `aw backlog set done` run for both `a58s04` and `qliia1`.
    `aw backlog set done a58s04` command output:
    ```text
    - >  backlog     20260917-a58s04-01-a58s04  [high]  [blocking]  unchanged
    ```
    `a58s04` closure message in full:
    "Closed with correction: steps 3 and 4 of fix sketch rested on refuted premises (aw attention already excludes merged lanes via runner_shared.lane_work_has_landed; reclaimable governs only interrupt-path reclaim while successful end-of-run teardown is lane_containment.teardown_lane_if_classified). Fix implemented on interrupt path by 65cuw0."
    `a58s04` release gate check: `Blocks-Release: next` preserved (gate NOT cleared).
    `check_engine.evaluate_blocking_close(repo, a58s04_path, 'done', evidence='.aw/records/plans/executed/20260917-laneorph-01-65cuw0-fix-lane-reclaim-so-a-merged-lane-is-reclaimable-and-torn-do.ipd.md')` output:
    `CloseVerdict(legitimate=True, severity='ok', reason="gate 'next' handed off to a From-Backlog plan or spec", fixes=(), path='HANDOFF')`
    Close-legitimacy predicate reported route: `HANDOFF`.
    `aw backlog set done qliia1` command output:
    ```text
    -    backlog     20260917-qliia1-01-qliia1  [high]  unchanged
    ```
    `qliia1` closed `done` in `.aw/records/backlog/done/20260917-qliia1-01-qliia1-triage-14-unintegrated-lane-branches.backlog.md` with evidence `.aw/records/research/20260924-lane-branch-triage-00-ebh1ap-lane-branch-triage.findings.md`.
  - Result: pass
