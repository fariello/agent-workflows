...
- 2026-08-22 executed (opencode (its_direct/pt3-claude-opus-4.8-1m-us)): E-01..E-05 executed via agy/Gemini (committed d750f58+8174d85: verify_roles.py + tests, scope-clean, Turn-2 audit ran, left in pending for orchestrator). opencode independently verified (6 roles + forbidden-action matrix, clean packet excludes executor prose, per-requirement results, corrective routing w/ immutable original + rerun), 28 module tests + full suite 1520 passed 1 skipped (pytest rc=0), filled V-01..V-05. Terminal transition to executed/.
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: verify_roles.py defines coordinator/executor/read-only investigator/independent verifier/corrector/human-approver roles with explicit inputs/outputs/permissions/state-authority/forbidden-actions. tests.test_verify_roles_packet.TestVerifierRolesAndPermissions asserts every forbidden role action is rejected (negative matrix). PASS.
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: Clean verifier packet builder contains frozen requirements, base/head identity, actual diff + untracked inventory, raw-evidence manifest, declared scope, test-change diff, prior-attempt metadata, rubric - and EXCLUDES the executor's conclusion prose. tests.test_verify_roles_packet.TestCleanVerifierPacketBuilder golden test: all primary artifacts present, no executor prose, base/head bound, missing/mismatched input rejected. PASS.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: Verifier procedures produce explicit satisfied|partial|failed|not_verifiable per requirement with evidence ids (requirement-by-requirement, scope audit, symbol-wiring, negative cases, falsifiability, targeted+full, artifact presence, residual search, evidence validation). tests.test_verify_roles_packet.TestVerificationProcedures: each gap class blocks completion. PASS.
  - Result: pass
- [x] V-04 validates E-04
  ...
  - Observed evidence: Corrective routing yields a bounded in-scope correction OR an explicit pending corrective-IPD artifact (never prose); the original failure is preserved immutably and invalidated checks rerun after correction. tests.test_verify_roles_packet.TestCorrectiveRouting passes. PASS.
  - Result: pass
- [x] V-05 validates E-05
  ...
  - Observed evidence: `tests/test_verify_roles_packet.py` exists and passes (28 tests). Full suite green: make test / pytest -n auto -> 1520 passed, 1 skipped, rc=0.
  - Result: pass
