...
- 2026-08-18 executed (opencode Opus 4.8): E-01..E-05 performed, V-01..V-05 pass; code committed dfa9efc (--rename-to-grammar flag + config key + interactive ask + class-correct facet transform riding the atomic move, comms/research excepted); full serial suite 1030 passed 1 skipped; sanitize clean; conforms at aw ipd lint --phase pre-transition. Closes backlog u9cicx.
...
- [x] E-02 Add the interactive ASK: when the migration runs interactively (a TTY, mirroring the existing confirm/leftovers prompts) and neither the flag nor the config key was given, present a self-contained P12 question - "Also rename migrated records to the uniform `.type.md` grammar? (default: no, leave existing names)" - and use the answer. When non-interactive and unset, default OFF (never rename silently).
  - Depends on: E-01
...
- [x] E-03 Implement the rename in the migration apply path, at the destination computed at layout_migration.py:737 (`dst_p = self._resolve_destination_path(mapping["destination_relpath"], target_backend)`): when rename-on-migrate is ON, append the correct `.<type>` facet to `dst_p`'s name. Class resolution is TWO-LEVEL: the mapping's `destination_root_class` gives the high-level class (only `records` items are eligible; `system`/`config`/`doc`/`host-adapter-in-place` are never faceted), and for a `records` item the SUB-TYPE is parsed from the `records/<subtype>/...` segment of `destination_relpath` (plans->`.ipd`, specs->`.spec`, walkthroughs->`.walkthrough`, roadmaps->`.roadmap`, backlog->`.backlog`, prompts + prompt-library->`.prompt`). Reuse `plans_refs.ARTIFACT_TYPE_FACETS`. CRITICAL ORDERING: compute the faceted `dst_p` BEFORE the `dest_seen`/dedup-twin check (line 764) and BEFORE journaling (`entry["destination"]`, line 773), so twins still dedup on the final name and rollback (which reverses via the journaled `destination`, line 1095/1108) restores correctly. The rename thus rides the SAME atomic `_perform_move` (no second pass, no history loss). Skip an already-faceted name (idempotent).
  ...
  - Execution state: performed
- [x] E-04 Guard the exceptions + sub-type resolution: only `records`-class items are eligible; comms (routing-named) and research (`.<model>.<kind>.md`) sub-types are NEVER renamed even when the flag is on; a `records` sub-type with no durable facet, a non-`records` class, or a README/INDEX/STATUS basename is left bare. The comms/research exclusion is by SUB-TYPE (parsed from `destination_relpath`), mirroring awnaming Order 02.
  - Depends on: E-03
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: `aw migrate-layout --help` lists `--rename-to-grammar` (ConfigKeyParsingTests::test_flag_in_help passed); a `--config` with `"rename_to_grammar": true` parses without a config-parse error (test_config_key_parses passed); the CLI resolves the flag (True when present) over the config key over an interactive ask over OFF (cli.py precedence block).
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: RenameOnMigrateIntegrationTests::test_rename_off_leaves_bare_names passed - a non-interactive migration with no flag/key (`selected_rename` resolves via `sys.stdin.isatty()` False -> OFF) leaves `plans/pending/20260101-0001-01-a.md` bare (no `.ipd.md`). The interactive ask path is the `elif sys.stdin.isatty(): _confirm(...)` branch (cli.py); non-interactive-unset never renames silently.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: test_rename_on_facets_plans_and_specs_not_comms_research passed - with `rename_to_grammar=True`, `records/plans/pending/20260101-0001-01-a.ipd.md` + a `specs/*.spec.md` exist; test_rename_off_leaves_bare_names shows the bare name with OFF. test_rename_rides_atomic_tracked_move passed - `git ls-files` shows the faceted destination is tracked (rode the move, not a copy) and the move journal records the faceted `destination` (so rollback via `entry["destination"]` reverses correctly).
  - Result: pass
- [x] V-04 validates E-04
  ...
  - Observed evidence: test_rename_on_facets_plans_and_specs_not_comms_research passed - plan + spec faceted; research files keep `.analysis.md` (no `.ipd.md`) and comms have no `.comms.md` (both documented exceptions). FacetResolutionTests passed: comms/research/non-records/doc classes return None; README/INDEX untouched; `_apply_grammar_facet` idempotent on an already-`.ipd.md` name.
  - Result: pass
- [x] V-05 validates E-05
  ...
  - Observed evidence: `1030 passed, 1 skipped in 195.64s` (up from 1021; +9 = `tests/test_awmigrename.py`, no regressions); `aw sanitize --agent` exit 0.
  - Result: pass
