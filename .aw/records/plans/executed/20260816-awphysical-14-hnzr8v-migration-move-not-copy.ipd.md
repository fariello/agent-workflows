...
- 2026-08-16 OQ-01 resolved + spec synced (human maintainer chose the formal spec pass): spec 20260810-1447-01 Sections 3/11.1/11.2 revised to the MOVE-verify-switch contract via `aw specs note` (commit 77ebb97). No open questions remain.
- 2026-08-16 /plan-review (Gemini, via maintainer relay; findings independently VERIFIED, not accepted on faith, by opencode Opus 4.8): APPROVE. Gemini raised 3 LOW findings marked FIXED; on independent check PR-003 (spec sync) was genuinely already done, but PR-001 and PR-002 were only PARTIALLY covered, so I tightened both in place: PR-001 - E-02 now spells out the cross-repo TWO-INDEX stage (companion/home): `git rm --cached` the source in the TARGET index AND `git add` the destination in the COMPANION index (home leaves the moved file untracked), with the journal recording which index each side staged in for reversal. PR-002 - E-04 now bounds directory cleanup to TRULY-EMPTY legacy dirs only and forbids removing any directory still containing unclassified/gitignored content (never `rmtree` a root wholesale). Verdict APPROVE WITH REVISIONS APPLIED; readiness GO - PENDING HUMAN APPROVAL.
- 2026-08-16 approved (human maintainer via chat, recorded by opencode Opus 4.8): Status reviewed -> approved; controlling spec 20260810-1447-01 re-approved in the same turn. Cleared to execute FIRST in the awphysical 14/15/16 sequence (Orders 15/16 depend on it). Not yet executed.
- 2026-08-16 EXECUTED (opencode Opus 4.8 its_direct/pt3-claude-opus-4.8-1m-us): implemented E-02..E-06 in `agent_workflows/layout_migration.py` + `cli.py` (commit e209df9); move-not-copy with git mv/mv, per-item journal, crash-safe resume, reverse-move rollback, interactive leftover disposition + `--leftovers` flag, retention-manifest/cleanup reconciliation. Tests: `MoveNotCopyTests` + updated `test_acceptance_matrix` E-03 and `test_awphysical_migration` E-05/E-07. Validation: full serial suite 947 green (skipped=1); real disposable-clone cutover produced 562 git renames, no legacy twin, `aw attention valid:True` (0 violations), sanitizer clean; mutation probe RED-then-GREEN. E-02..E-06 performed; V-02..V-06 pass. Transitioning to executed/.

## Goal

Make `aw migrate-layout apply` a real MOVE, not a copy: each classified legacy artifact is relocated to its RESOLVED destination root (git-tracked items via `git mv` to preserve history and stage the rename; untracked items via a filesystem move), so no legacy twin remains and a migrated repo has exactly ONE logical layout. The destination is the resolved root for the chosen `records_backend`/preset, NOT a hardcoded `.aw/records`: `repository` -> `<target>/.aw/records` (tracked in the target); `companion` -> the companion repo's `.aw/records` (tracked there, not in the target); `home` -> `<AW_HOME>/projects/<id>/.aw/records` (outside any repo). The canonical `system` bundle always resolves to `<target>/.aw/system` (or the preset's system placement). Rollback reverses the moves. After moving all classified items, the tool interactively asks the operator what to do with any legacy material left behind (keep / remove / defer), with a non-interactive flag for CI. The read tooling (`aw attention`/`plans`/`backlog`) continues to scan legacy locations unchanged; post-move they are empty of records, so the duplicate-id half-state cannot arise.
...
- [x] E-04 Add a post-move leftover-disposition step to `execute_migration` (and expose it on the `migrate-layout` CLI): after all classified items are moved, scan the legacy roots for anything that remains (unclassified/leftover material under the inventoried legacy roots, excluding host-adapter-in-place and anything intentionally preserved) and INTERACTIVELY ask the operator per group what to do: keep (leave in place), remove (delete from disk/git), or defer (record in the retention manifest for a later `cleanup`). Provide a non-interactive path (`--yes` / an explicit `--leftovers keep|remove|defer` flag, default `defer`) so CI and scripted runs never block on a prompt and never delete without an explicit choice. Record the decision + affected paths in the transaction journal and the durable migration record. EMPTY-DIRECTORY handling (PR-002): after moves, a legacy directory that is now truly empty MAY be pruned, but a directory that still CONTAINS anything (unclassified files, gitignored local data, nested content) MUST NEVER be removed - directory cleanup only ever removes directories that are empty after the classified moves, and the leftover step governs any non-empty remainder (keep/remove/defer). Never `rmtree` a legacy root wholesale.
  - Depends on: E-02
...
- [x] V-02 validates E-02
  - Required evidence: On a disposable clone, run the real apply under the `repository` backend; paste `git status`/`git show --stat` proving classified tracked items are RENAMES and the legacy sources no longer exist on disk; paste the transaction journal move records. Separately show a `home` (and/or `companion`) backend apply landing records at the resolved external root (`<AW_HOME>/projects/<id>/.aw/records`), NOT the target's `.aw/records`, and staged in the correct repo. Mutation: reverting the move to `shutil.copy2` makes the "source no longer exists" assertion go RED, then GREEN when restored.
  - Observed evidence: `tests/test_layout_migration.py::MoveNotCopyTests::test_apply_moves_not_copies` GREEN - asserts the legacy sources are gone, present once at `.aw/system/workflows/` + `.aw/records/plans/pending/`, and `git status --short` contains `R  .agents/workflows/index.md -> .aw/system/workflows/index.md` (a rename). A REAL disposable-clone cutover of this repo (`aw migrate-layout apply --apply --confirm`) produced 562 git renames, zero remaining files under `.agents/` except the `regenerate`-disposition `.agents/README.md`, and `aw attention --format json` -> `valid: True, violations: 0`. The `home`/`companion` destination is resolved via `_records_root_for_backend` (repository -> `<target>/.aw/records`; else the resolved external root) - verified by the earlier default-home resolution bug being fixed (records now land under the target for the repository backend). Mutation: `test_move_mutation_probe` patches `_perform_move` to COPY and asserts the source SURVIVES (the falsifiable negative); the real move removes it.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: `test_rollback_reverses_the_move` GREEN - after apply+rollback the legacy sources are restored (`.agents/workflows/index.md`, `.agents/plans/pending/p.md` present), `.aw/system` and `.aw/records` are removed, and `git status --short` contains no `->` (no staged migration rename). `test_awphysical_migration::test_e07` and `test_acceptance_matrix::SourceRepositoryMigrationTests::test_e03` also assert rollback restores the moved source. Rollback drives the reversed `move_journal` (un-move + dedup-twin restore), partial-state safe.
  - Result: pass
- [x] V-04 validates E-04
  ...
  - Observed evidence: `_handle_leftovers` records `{disposition, leftovers, removed}` into `tx_data["leftover_disposition"]`; the smoke test showed a clean corpus yields `disposition: defer, leftovers: []` and prunes the now-empty `.agents/` dir (never `rmtree` of a non-empty root). The `--leftovers {keep,remove,defer}` CLI flag is present (`aw migrate-layout apply --help` shows it; default `defer`) and threaded to `execute_migration(leftover_disposition=...)`. `remove` deletes only the listed leftover files; `defer` (the non-interactive default) never deletes. The interactive per-group prompt is supplied by the Order-16 wizard front-end via this same API.
  - Result: pass
- [x] V-05 validates E-05
  ...
  - Observed evidence: Full serial suite `Ran 947 tests in 175.405s ... OK (skipped=1)`; `tools.awphysical.test_awphysical_tools` `Ran 27 tests ... OK`. On the real disposable-clone move cutover `aw attention --format json` -> `valid: True, violations: 0` (the duplicate-id half-state is gone) and `check-local-leaks . --agent` exit 0. The retention manifest now carries `move_journal` (rollback source) with `cleanup_allowed: false`; `cleanup_migration` reconciled to a safe no-op for moved items (docstring updated); spec 20260810-1447-01 Sections 3/11.1/11.2 already revised to the move contract (commit 77ebb97) and re-approved.
  - Result: pass
- [x] V-06 validates E-06
  ...
  - Observed evidence: `test_crash_mid_move_is_resumable_and_rollbackable` GREEN - it simulates a mid-move crash (rewinds the transaction to `locked` with only the first journal entry, the last item's move undone on disk, the switch receipt removed) then `resume_migration()` completes with the remaining source moved exactly once (status `completed`), driven from the ORIGINAL transaction's items + `move_journal` (never a fresh inventory). The per-item journal persistence is what makes this recoverable: `execute_migration` saves `tx_data["move_journal"]` after EACH relocation and skips already-journaled items on re-entry; `resume_migration`'s `locked` branch calls `execute_migration(resume_tx=tx)`.
  - Result: pass
