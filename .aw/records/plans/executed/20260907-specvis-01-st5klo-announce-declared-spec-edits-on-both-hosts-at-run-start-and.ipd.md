...
## Workflow history
- 2026-09-14 executed (opencode/recovery): Recovered stranded lane and finalized: E-01..E-05 performed and V-01..V-05 verified with pasted evidence in run-20260914T020813Z-2543555; integration was refused only by the binary suite gate on an unrelated red test. The lane also OVERTURNED its own plan's central premise (decision 07-st5klo-D1): spec_impacts_for_queue read item['path']/['plan_path'] while a real queue entry carries only 'configured_file', so the start-of-run announcement was inert on BOTH hosts from the day it shipped. Attribution via run-record-exact (gys47u). [Scope reconciliation - in-scope-unmodified tests/test_spec_visibility.py: declared-but-deliberately-unmodified-the-review-withdrew-F1-F2-so-E-02-became-a-regression-guard-in-the-new-file-instead]
