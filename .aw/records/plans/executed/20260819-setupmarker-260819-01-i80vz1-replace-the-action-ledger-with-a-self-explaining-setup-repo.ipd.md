...
- 2026-08-19 executed (opencode Opus 4.8): E-01..E-08 performed, V pass; action ledger deleted, write-on-read killed, setup reminder = self-explaining .aw/setup-repo-needed.md marker; 26 stamped repos cleaned (sticky); full serial suite 1180 passed, 1 skipped.
...
- [x] E-03 Remove the action-ledger attention source (the write-on-read culprit): delete the `ActionManager` scan block in `attention.py` (~171-215, the "External AW operational actions scan") and its `actions` tree handling; remove the `actions` entry from `attention_contract.py` (`_ACTIONS_MAP` + the `CLASS_MAPS["actions"]` + any tracked-tree/`class_of("actions",...)`). Attention no longer reads or creates `.aw/state/actions/`.
  - Depends on: none
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: engine.write_setup_marker creates a self-explaining .aw/setup-repo-needed.md ('setup not yet run', run aw setup / delete to dismiss, gitignored, safe) + a .aw/.gitignore 'setup-repo-needed.md' line; remove_setup_marker clears it; both idempotent (test_setup_marker_lifecycle).
  - Result: pass
...
- [x] V-02 validates E-02
  ...
  - Observed evidence: install writes the marker via engine.write_setup_marker + records history via install_history.record_install_history (state/history/installs.jsonl); test_install_history_atomic_append + redaction pass.
  - Result: pass
...
- [x] V-03 validates E-03
  ...
  - Observed evidence: attention.scan on a fresh dir creates NO .aw/ (test_attention_scan_does_not_create_aw + test_scan_does_not_stamp_aw); no 'actions' key in attention_contract.CLASS_MAPS.
  - Result: pass
...
- [x] V-04 validates E-04
  ...
  - Observed evidence: setup_needed returns True iff the marker exists; aw attention notice + aw doctor derive from it; no consumer references the deleted ledger (test_setup_notice_present seeds the marker).
  - Result: pass
...
- [x] V-05 validates E-05
  ...
  - Observed evidence: aw complete/dismiss/reopen/history are invalid choices (exit 2); no state/actions scaffolded on install; install_history.record_install_history importable + works; aw record-history intact; python3 -c 'import agent_workflows.cli' OK.
  - Result: pass
...
- [x] V-06 validates E-06
  ...
  - Observed evidence: tests/test_actions.py (5) + updated attention/acceptance tests pass; write-on-read regression test present + passing; full serial suite 1180 passed, 1 skipped.
  - Result: pass
...
- [x] V-07 validates E-07
  ...
  - Observed evidence: removed empty scaffolding .aw/ from 26 configured repos (agent-workflows 803-file real .aw/ skipped); after `aw status` TWICE, the cleaned repos stay clean (no re-stamp) and show .agents layout - sticky.
  - Result: pass
...
- [x] V-08 validates E-08
  ...
  - Observed evidence: `rg -n 'complete setup-repo' .aw/system/` empty; setup-repo.md terminal step now removes .aw/setup-repo-needed.md (rm -f) instead of 'aw complete setup-repo'.
  - Result: pass
