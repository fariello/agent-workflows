...
- 2026-08-23 executed (opencode (its_direct/pt3-claude-opus-4.8-1m-us)): awcliux Set complete. All 5 children (Orders 01-05) executed + independently verified; Orders 01/02/03/04 via agy/Gemini (code + code/self-docs; 01/03 also authored the shared docs/cli-output-contract.md), Order 05 (conformance harness + docs + rollout) direct. E-01 (contract frozen in Order 01), E-02 (01-05 executed), E-03 (conformance report + migration guide delivered) performed. Residual audit: full suite 2016 passed 1 skipped (pytest rc=0), aw sanitize --agent exit 0, git tags unchanged, branch NOT pushed. V-01..V-03 filled. NO tag/publish/push. Terminal transition to executed/ - final artifact of awcliux.
...
- [x] E-01 Freeze Order 01's precedence, schema, streams, exits, compatibility, and token-budget decisions (record them as resolved in Order 01, including the `Drift`-reconciliation decision above and the resolution of the blocking non-TTY migration question). Human contract sign-off is a separate approval step, not this action.
  - Depends on: none
...
- [x] E-02 Execute Orders 01-05; Orders 02 and 03 may run in parallel only after Order 01.
  - Depends on: E-01
...
- [x] E-03 Require the whole-surface conformance report and migration guide before release.
  - Depends on: E-02
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: Order 01 (hd3kln, executed) froze the output contract: precedence (--json/--format > --agent > non-TTY=>agent > TTY=>human), aw.agent/v1 schema, stdout/stderr streams, 0/1/2 exits, hard-cutover non-TTY migration (OQ-01), token budget, and the Drift-reconciliation decision (aw.agent/v1 supersedes Drift/drift_exit_code) - recorded in docs/cli-output-contract.md. PASS.
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: Orders 01-05 all in executed/ (5 children), post-transition conforming; 02/03 built on 01. Full command surface migrated through the shared boundary (Order 04). Full suite 2016 passed 1 skipped (pytest rc=0). PASS.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: Whole-surface conformance report (Order 05: generated per-leaf matrix over 78 leaves, 40 conformance tests + 12 goldens pinned Py3.9-3.14, CI output-conformance job) + migration guide (docs/cli-migration.md, loud hard-cutover byte break) delivered before release; spec 1525-01 G6 superseded. aw sanitize --agent exit 0; no unauthorized tag/publish/push. PASS.
  - Result: pass
