...
- [x] V-01 validates E-01
  ...
  - Observed evidence: `agent_workflows/ipd_lifecycle.py`: `finalize_precheck` now COMPUTES both delta directions into `evidence.scope_audit` (`out_of_scope_paths`, `in_scope_unmodified`) reusing Order 04's exact `_paths_changed_by_this_execution` + `_scope_match`; `_reconcile_scope` requires a non-empty reason per out-of-scope path (empty/missing -> refuse) and an ack per declared-but-unmodified path, from `--scope-reason`/`--scope-ack` flags OR a single batched TTY `prompt`; a non-empty delta with missing answers returns `ok=False` with `needs_input_command` naming the exact re-invocation; `finalize` folds the verbatim answers into the attributed history via `_reconciliation_history_note`. `tests/test_ipd_lifecycle_cli.py::ReconciliationTests` (9 tests) proves: headless out-of-scope missing-reason fail-closed naming `--scope-reason` + path; headless with-reason records verbatim + proceeds; empty reason does NOT finalize; in-scope-unmodified missing-ack fail-closed + ack records + proceeds; both-directions-at-once; clean delta = no reconciliation note (frictionless); TTY ONE batched prompt covers both directions (asserted single call); TTY empty-reason refuses; flag parsing. Manual e2e: `aw ipd finalize ... --agent` returned `outcome:findings,exit:1` on missing answers; the human-mode refusal printed the exact `aw ipd finalize abc123 --actor 't' --message 'm' --apply --scope-reason tests/extra.py=<why...> --scope-ack tests/declared.py[=not-needed]` command; supplying them recorded `executed (opencode/test): did the work [Scope reconciliation - out-of-scope tests/extra.py: needed a new test helper discovered mid-stream; in-scope-unmodified tests/declared.py: not-needed]`.
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: `pytest tests/test_ipd_lifecycle_cli.py::ReconciliationTests -v` -> `9 passed in 0.64s`; full `pytest tests/test_ipd_lifecycle_cli.py` -> `34 passed` (15 begin + 10 finalize + 9 reconciliation). Full suite `pytest -n auto` -> `2268 passed, 1 skipped in 122.67s (0:02:02)`. The new `--scope-reason`/`--scope-ack` finalize flags are declared in command_surface.py (DECISION 11-qmt3yk-D1) so the CLI conformance harness is green (test_command_surface_declarations + test_cli_conformance_matrix pass). `aw ipd lint --all` exits 0; `aw specs check` conforms; `check-local-leaks` clean. Durable attribution verified in the moved plan's `## Workflow history` (the verbatim `[Scope reconciliation - ...]` note).
  - Result: pass
