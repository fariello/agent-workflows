...
| --- | --- | --- | --- | --- |
| F-1 | HIGH | `render_stream.py:2152-2178` (re-measured at round 2; was `:2124-2150`) | The summary's diagnostics block keys on a hardcoded status allowlist (`dependency-blocked`, `failed-safely`, `integration-blocked`, `merge-conflict`, `interrupted`). A new refusal reason produces NO diagnostic line. RE-VERIFIED UNCHANGED IN SUBSTANCE at round 2: the allowlist is still exactly those five and still has no default branch. (NOTE 2026-09-21, `l2mzxn`: these statuses were RENAMED - `integration-blocked` -> `merge-needs-human`, `merge-conflict` -> `merge-refused` - and the allowlists now carry BOTH spellings so durable run directories still render; expect to see four entries where this text names two.) | source re-read at round 2; the block sits inside `render_run_summary_table` (now `:1656-2180`, was `:1628-2152`) |
