...
## Workflow history
- 2026-08-28 executed (opencode its_direct/pt3-claude-opus-4.8-1m-us): Orchestrator rollup: rstodo children p3o9je+lpqy64 executed; intake->todo rename + migration verified (0 docs still intake). Post-hoc; children committed declared paths pre-receipt. [Scope reconciliation - in-scope-unmodified .aw/records/research/: children migrated docs pre-receipt; in-scope-unmodified agent_workflows/attention.py: children pre-receipt; in-scope-unmodified agent_workflows/attention_contract.py: children pre-receipt; in-scope-unmodified agent_workflows/cli.py: children pre-receipt; in-scope-unmodified agent_workflows/research_archive.py: children pre-receipt; in-scope-unmodified agent_workflows/research_cmd.py: children pre-receipt; in-scope-unmodified agent_workflows/research_contract.py: children pre-receipt; in-scope-unmodified agent_workflows/research_index.py: children pre-receipt; in-scope-unmodified agent_workflows/term.py: children pre-receipt; in-scope-unmodified tests/: children pre-receipt]
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: Post-hoc rollup verification 2026-08-28 (children p3o9je rename + lpqy64 migration executed overnight run-20260828T035444Z, both in executed/). (2) `grep -rl '^status: intake' .aw/records/research/` -> empty (0 docs still carry the legacy token; all migrated to `todo`). `todo` is in `research_contract.STATUSES` with `STATUS_NORMALIZATIONS = {"intake": "todo"}` for backward-compat reads. (3) `aw research index --check` clean for research docs (the one reported line is a PRE-EXISTING unrelated `adopted-without-consumer` on an archived 202607 doc `nbbl7d`, not this Set's concern). (4) `aw attention` renders the migrated research docs as READY `todo`. Both children are in `.aw/records/plans/executed/`.
  - Result: pass
