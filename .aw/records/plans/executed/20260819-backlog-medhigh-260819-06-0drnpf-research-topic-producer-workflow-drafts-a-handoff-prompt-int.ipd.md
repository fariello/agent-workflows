...
- 2026-08-19 executed (agy_run.py Gemini 3.7 + opencode independent-validate): E performed; agy added the research-prompt producer workflow (body+README), registered it as the `research` VERB in the manifest (invoked /aw research via the Order 05 dispatcher, NO standalone shim), + test + closed 6wlo04. opencode INDEPENDENTLY verified: workflow encodes the 3 prompt-purity rules + writes to prompts/pending, manifest routes /aw research, no standalone research-prompt shim, test_research_prompt_workflow 6 pass, set-boundary full serial suite 1227 passed 1 skipped, 6wlo04 done. (agy status ERROR = write_to_file false-ERROR uhbdt1; committed 7e73254.)
