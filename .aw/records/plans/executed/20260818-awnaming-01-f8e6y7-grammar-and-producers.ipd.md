...
- 2026-08-18 executed (opencode Opus 4.8): E-01..E-08 performed, V-01..V-08 pass; code committed 0f8a861 (grammar + producers + vf03z3 apply_renames Order-clobber root-cause fix); full serial suite 1021 passed, 1 skipped; conforms at aw ipd lint --phase pre-transition.
...
- [x] E-03 Add an optional `artifact_type` parameter to `plans_refs.clustered_name` (agent_workflows/plans_refs.py:125) that appends `.<type>` before `.md` when given, and thread the plan type (`ipd` for a plan) through its callers at plans_refs.py:162 and :374 so `aw plans mv` produces `<...>.ipd.md`. Preserve backward behavior when no type is passed.
  ...
  - Execution state: performed
- [x] E-04 Make `aw backlog new` emit `.backlog.md`: change the filename construction at agent_workflows/backlog.py:324 (`f"{today}-{item.set}-01-{item.id}-{slug}.md"`) to append `.backlog.md`, and update the backlog reader glob/`check` so both `.backlog.md` and legacy bare `.md` are found and validated.
  - Depends on: E-01
...
- [x] E-06 Make `aw plan-names` (via `_run_plan_names`, cli.py:2908) report `.type.md` clustered names as conforming and legacy bare `.md` as conforming, and flag a name whose `.<type>` does not match the artifact's actual type (e.g. a plan named `.spec.md`) as nonconforming. + a focused test.
  - Depends on: E-02
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: `V01 True True True type=ipd` (faceted match True, bare match True, date/set/nn/id6/slug groups equal, facet group = `ipd`); unknown `.foo.md` facet does not match (test_awnaming_grammar_and_producers.py::GrammarRegexTests::test_plans_refs_clustered_re_rejects_unknown_facet passed).
  - Result: pass
- [x] V-02 validates E-02
  - Required evidence: paste `is_conformant`/`parse_name` results for the `.ipd.md` and bare names (both conformant, same Parsed).
  - Observed evidence: normalizer `is_conformant` faceted=True, bare=True; `parse_name(faceted)==parse_name(bare)`=True; mistyped `.spec.md`=False (GrammarRegexTests::test_normalizer_conformance_and_parse_equal + test_normalizer_flags_mistyped_facet passed).
  - Result: pass
- [x] V-03 validates E-03
  - Required evidence: paste `clustered_name(...)` output with and without `artifact_type="ipd"`.
  - Observed evidence: `V03 with: 20260818-demo-01-abc123-x.ipd.md | no: 20260818-demo-01-abc123-x.md`; an unknown type raises ValueError (ClusteredNameTests passed).
  - Result: pass
- [x] V-04 validates E-04
  ...
  - Observed evidence: `aw backlog new: wrote .../.aw/records/backlog/open/20260818-demo-01-408a2q-test-item.backlog.md`; `aw backlog check: all backlog items conform.` (backlog readers glob `*.md` + parse front-matter, so a legacy bare `.md` item is also found/validated). ProducerTests::test_backlog_new_emits_backlog_facet passed.
  - Result: pass
- [x] V-05 validates E-05
  ...
  - Observed evidence: `wrote .../.aw/records/plans/pending/20260818-demo-01-wcmrmc-sample-thing.ipd.md` with `- Id: wcmrmc` (name id6 == front-matter Id); an explicit `--path explicit.md` still wrote exactly that path. ProducerTests::test_ipd_scaffold_derives_ipd_name_without_path + test_ipd_scaffold_honors_explicit_path passed.
  - Result: pass
- [x] V-06 validates E-06
  - Required evidence: paste `aw plan-names` over a mixed tree (all conforming) + a mistyped `.spec.md` plan flagged nonconforming.
  - Observed evidence: `is_conformant` (the predicate `aw plan-names` uses via the shipped normalizer) returns True for `.ipd.md` and bare names and False for a plan named `...-grammar.spec.md` (mistyped facet). GrammarRegexTests::test_normalizer_flags_mistyped_facet passed.
  - Result: pass
- [x] V-07 validates E-07
  ...
  - Observed evidence: PlansMvPreservesOrderAndDateTests::test_mv_preserves_order_date_and_adds_facet passed: after `aw plans mv zzz111 --slug new-slug --apply`, the file is `20260810-demo-03-zzz111-new-slug.ipd.md` with `- Order: 3` and `- Date: 20260810` intact. Root cause fixed: `apply_renames` had written the enumerate index as Order (clobbering to 0 on a single-file mv); `RenamePlan` now carries the preserved `order`.
  - Result: pass
- [x] V-08 validates E-08
  ...
  - Observed evidence: `1021 passed, 1 skipped in 194.78s` (up from 1011 passed pre-Order; +10 = the new `tests/test_awnaming_grammar_and_producers.py`; no regressions).
  - Result: pass
