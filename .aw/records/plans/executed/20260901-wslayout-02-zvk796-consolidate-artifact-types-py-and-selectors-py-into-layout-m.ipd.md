...
## Workflow history
- 2026-09-05 executed (aw oc run): aw oc run self-finalize: zvk796 verified (set wslayout, attempt 1). [Scope reconciliation - out-of-scope tests/test_awcmdsurf_vocab_and_parsers.py: changed by the plan's approved execution (auto-reconciled by aw oc run); out-of-scope tests/test_layout.py: changed by the plan's approved execution (auto-reconciled by aw oc run)]
