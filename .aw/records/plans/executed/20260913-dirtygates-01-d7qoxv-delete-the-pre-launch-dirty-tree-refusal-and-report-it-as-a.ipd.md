...
- Resolution or deferral rationale: NOT resolvable from repository evidence, and NOT the same question OQ-02 already settled. OQ-02 split the SPEC and the BEHAVIOR by path, which dissolved the behavioral collision cleanly. It said nothing about the TEST FILE, and there the two plans give contradictory instructions about one file, with a human approval behind one of them.
  RESOLVED 2026-09-13 by the maintainer: OPTION (c), SEQUENCE DELIBERATELY. Approved plan `3i0aaz` runs FIRST and collects its empty-diff proof; THIS plan runs second and is explicitly permitted to break that property, because the proof is meaningful only at `3i0aaz`'s execution and is spent once collected. Neither plan loses anything and no approved validation is edited.
  CONSEQUENCE THE EXECUTOR MUST APPLY: this plan's `- Item-Dependencies:` becomes `executed:3i0aaz` (it currently declares `none`), which is a real ordering constraint the Set did not previously carry. E-05 may then retarget the four assertions in `tests/test_lane_clean_base.py`, and the file stays declared in this plan's `Scope-Paths`.
  THE MAINTAINER ALSO SPECIFIED A LARGER DESIGN THAT NEITHER PLAN CURRENTLY COVERS, and it is recorded here so it is not lost, but it is deliberately NOT folded into this plan. Their stated target behavior: the start-of-run check should refuse only when a path the plan DECLARES IT WILL CHANGE is dirty, ignoring unrelated dirty files entirely, with a flag to override that specific refusal; the shared-tree case should leave the agent to deal with foreign changes to those declared paths; and for an isolated run, a merge blocked at integration time should be RETRIED on a configurable interval up to a configurable maximum before failing that one artifact and moving to the next, with an open question about whether a consecutive-failure limit should abort the whole run. THIS IS A SEPARATE PLAN. It changes the check's SHAPE from whole-tree to path-scoped, which this plan deliberately does not do, and it adds merge-retry machinery that exists nowhere. MEASURED WHILE DISCUSSING IT, and it corrects the maintainer's stated premise that a stranded artifact should merge cleanly once main is tidied: that holds only if the operator DISCARDS the conflicting change. If they COMMIT it, the retry meets `CONFLICT (content)` instead, which no amount of waiting resolves, so a retry design must tell the two apart and stop retrying the second.
