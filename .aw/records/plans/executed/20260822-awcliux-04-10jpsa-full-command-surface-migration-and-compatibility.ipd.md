...
- 2026-08-23 executed (opencode (its_direct/pt3-claude-opus-4.8-1m-us)): E-items executed via agy/Gemini (committed ef55ead code + 938be64/0a0899e E-V: migrated the root cli.py dispatcher + all command families through the shared output boundary + aw.agent/v1 receipts + legacy compatibility; adapted existing tests to the new envelope, NOT weakened - assertions now check schema=aw.agent/v1). Pure code, no .md. opencode independently verified: test adaptations strengthen (schema tag asserted) not gut; full suite 1976 passed 1 skipped (pytest rc=0); no domain behavior/exit-code change; V-items pass. Terminal transition to executed/.
