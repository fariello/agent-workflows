...
- 2026-08-22 executed (opencode (its_direct/pt3-claude-opus-4.8-1m-us)): E-01..E-03 executed via agy/Gemini (committed 0870c8c: term.py components + 256 palette extension, cli.py check recipe + help standardization, renderers.py; tests). Code + self-docs only, no .md authored. Run errored on spurious brain-path AFTER commit, before checkbox marking; opencode independently verified (Term components + ASCII fallback, aw check exit contract 0/1/2 preserved, help standardization), 28 module tests + full suite 1914 passed 1 skipped (pytest rc=0), filled E/V. Terminal transition to executed/.
...
- [x] E-01 Extend `Term` with title, outcome, section, table, badge, path, diagnostic, preview, evidence, fix, and next-action components using a documented xterm-256 palette. The palette EXTENDS the existing `STATUS_COLOR_256` map and `severity_label` codes (`agent_workflows/term.py:92-112`, `:159-177`); do not define a second, parallel palette. Reconcile the new roles with the existing entries (e.g. reuse `deferred:208`; add only genuinely new roles such as `paths:33`). Every component MUST emit an ASCII fallback for its glyphs (e.g. `✓`/`!`/`→` degrade to `OK`/`!`/`->`) so meaning survives non-Unicode/Windows terminals and `NO_COLOR`.
  - Depends on: none
...
- [x] E-03 Standardize root/family/leaf help and usage errors around purpose, syntax, defaults, safety, exits, agent behavior, and two realistic examples; adapt to width.
  - Depends on: E-01
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: term.py extends Term with title/outcome/section/table/badge/path/diagnostic/preview/evidence/fix/next-action components on the xterm-256 palette EXTENDING existing STATUS_COLOR_256/severity_label (no parallel palette; reuses deferred:208, adds paths:33). Every component emits an ASCII fallback (glyphs degrade to OK/!/-> under non-Unicode/NO_COLOR). tests.test_term_components (28 total across suite) pass. PASS.
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: aw check migrated to inspect/list + check/diagnose + preview/apply recipes extracted from doctor; exit contract preserved EXACTLY (0 clean / 1 findings / 2 cannot-run via Drift/drift_exit_code, spec 20260818-1525-01 G6). Facts + exit codes unchanged; layout only. tests.CheckRecipeUnitTests + CheckRecipeGoldenAndWidthTests pass. PASS.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: Root/family/leaf help + usage errors standardized around purpose, syntax, defaults, safety, exits, agent behavior, two realistic examples; width-adaptive. tests.test_cli_help_and_errors pass. PASS.
  - Result: pass
