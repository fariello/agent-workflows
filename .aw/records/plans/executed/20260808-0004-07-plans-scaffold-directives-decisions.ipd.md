...
- 2026-08-08 executed (opencode its_direct/pt3-claude-opus-4.8-1m-us): added the "Browsing and regrouping plans" AGENTS.md pointer section (via the generator; regenerated by installer, empty-diff + AGENT-PLANS byte-identical), documented the manifest + weekly-shard layout + `aw plans` verbs in the plans README, added DECISIONS D124 (cites D123 + D122), and updated the TODO note (plans done, prompts next). CORRECTION at execution (E-02): no new installer scaffold dir was needed (plan terminal dirs already scaffold; shards are on-demand), so the created-count assertion is UNCHANGED (the review's 24->27 expectation was itself corrected). Product commit 8411ab2 (+ installer sync 1bb2447); full suite green (Ran 673 tests OK, skipped=1); leak-clean; no em/en dashes. All E-01..E-06 performed and V-01..V-06 pass.
...
- [x] E-02 document the plan weekly-shard layout in `.agents/plans/README.md`. CORRECTION (found at execution): unlike research (whose `reference/`/`archive/` shard PARENTS were NEW dirs the installer had to create), the plan terminal disposition dirs (`executed/`/`superseded/`/`not-executed/`) ALREADY exist via `PLAN_LIFECYCLE_SUBDIRS` in `create_setup_artifacts`, and the weekly `YYYYMM-Www/` shards inside them are created on demand by `aw plans archive`. So there are NO new scaffold dirs to add and the created-count assertion does NOT change (adding empty `.gitkeep`s would be gold-plating). Instead, document the shard layout + the `aw plans archive` convention in the plans README so it is discoverable.
  ...
  - Execution state: performed
- [x] E-03 add the thin AGENTS.md pointer: a SHORT new `###` section in `agents_pointer_prose` naming the `aw plans` grouping/manifest verbs ("browse/regroup plans by Set with `aw plans`; do not hand-name or hand-maintain the plans index"); regenerate AGENTS.md; the AGENT-PLANS sibling untouched (byte-identical).
  - Depends on: E-01
...
- [x] E-04 add a DECISIONS pointer entry (pin the number at execution; next is D124) referencing the plans-adopter spec `20260808-plansadopt-01-qkc93l-shared-artifact-core`; note it applies the D123 model to plans via the shared core + the stable `Id` + Set-clustering, and cites D122 (the ipd_schema it extends).
  - Depends on: E-01
  - Expected outcome: the entry is present, points to the spec, and cites D123 (the model) and D122 (ipd_schema).
  - Execution state: performed
- [x] E-05 update the TODO future-work note: mark `plans/` DONE (this Set) and name `prompts/` the subsequent adopter (weakest case; low volume; research-prompt lineage already handled), with `comms/`/`walkthroughs/` after.
  - Depends on: E-01
  - Expected outcome: the TODO note reflects plans done + prompts next.
  - Execution state: performed
- [x] E-06 add scaffold tests (terminal-dir shard parents created; dry-run; idempotent; created-count updated; real/dry-run parity) in the existing scaffold harness; run them plus the full suite and paste output; confirm the AGENTS.md regen diff is limited to the new pointer section.
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: Orders 01/03/05 are executed (`20260808-plansadopt-01/03/05-*` in executed/); `artifact_core`, `aw plans index`, and `aw plans archive` are importable/invokable (used throughout Order 06). E-01 gates on symbols, not populated shard dirs.
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: `.agents/plans/README.md` now has an "Identity, sets, and the clustering filename grammar" + "The plans manifest and weekly shards" section documenting `Id`, `Set: <terse> (<descriptive>)`, the clustering grammar, `aw plans index/find/set-assign/mv/archive`, and the terminal-dir weekly-shard layout (pending/reusable stay flat). Per the correction, `create_setup_artifacts` was NOT changed (no `PLAN_SHARD_SUBDIRS` added) and `test_setup_artifacts` still asserts the SAME created-count (24) - the full suite is green with that assertion unchanged.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: the generator gained a "### Browsing and regrouping plans" section; AGENTS.md was regenerated by the installer (commit `1bb2447`), and the empty-diff invariant holds (`agents_managed_sections()[0].body == the AGENTS.md pointer body`, verified True); `AGENT-PLANS:BEGIN` present and byte-identical (the sectioned refresh touches only the `aw:pointer` section).
  - Result: pass
- [x] V-04 validates E-04
  - Required evidence: confirm the DECISIONS entry (D124) exists, points to the plans-adopter spec, and cites D123 + D122.
  - Observed evidence: `DECISIONS.md` D124 "The artifact-organization model is generalized to plans" points to spec `20260808-0004-01` and cites D123 (the model it extends) and D122 (the `ipd_schema` it reuses).
  - Result: pass
- [x] V-05 validates E-05
  - Required evidence: confirm the TODO note marks plans done and names prompts the next adopter.
  - Observed evidence: `TODO.md` now reads "Apply the artifact-organization model to `prompts/` (the next adopter after research and plans)", records research + plans DONE (D123/D124) sharing `artifact_core`, and names prompts the weakest next adopter with comms/walkthroughs after.
  - Result: pass
- [x] V-06 validates E-06
  ...
  - Observed evidence: full suite `python3 -m unittest discover -s tests -t .` -> `Ran 673 tests in 154.534s / OK (skipped=1)`. `aw sanitize --agent` exit 0. No em/en dashes in the changed authored files. AGENTS.md regen limited to the new pointer section (empty-diff invariant holds; AGENT-PLANS byte-identical).
  - Result: pass
