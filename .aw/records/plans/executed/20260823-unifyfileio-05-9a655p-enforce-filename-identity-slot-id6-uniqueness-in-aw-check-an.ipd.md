...
## Workflow history
- 2026-08-24 executed (opencode its_direct/pt3-claude-opus-4.8-1m-us, run-20260824T150827Z-2301181): enforced the D140 filename identity-slot invariant. E-01 extended `check_engine.check_collisions` with a `check.id6-identity-slot` rule (parses the slot via the Order 01 naming authority `_naming.parse_clustered`, guarded by an HHMM-setid exclusion + a real-id6 discriminator so legacy slug-word slots like `assess`/`agents` are not mass-flagged): rule (a) declared `- Id:` must equal the slot id6; rule (b) an undeclared file's slot id6 must be owned by no one else. E-02 mapped the rule into `doctor.build_remediation` with the D140/Target-Id remediation. E-03 (after Order 01 E-06's facet migration) `git mv`'d the one violator `...-p7dqwz-execution.walkthrough.md` to its own minted id6 `hey7r7` (`...-hey7r7-execution.walkthrough.md`) with `- Id: hey7r7` + typed `- Target-Id: p7dqwz`; the three descriptive-prose references to the old name were intentionally left as historical record (decision D1). Doc-sync: updated `check_collisions` docstring and `.aw/records/walkthroughs/README.md` (facet naming + identity-slot/Target-Id convention). Tests: `IdentitySlotTests` (5) + doctor remediation test (1) added; `aw check all` and `aw doctor --agent` report zero identity-slot findings; `pytest -n auto` = 2169 passed, 1 skipped. Status set to executed; moved pending/ -> executed/. See decisions D1-D2 in run-20260824T150827Z-2301181.
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: added `IdentitySlotTests` to `tests/test_check_engine.py` (5 tests, all pass): `test_rule_a_declared_id_differs_from_slot` (declared `fmbbb1` vs slot `slotaa` -> 1 finding naming both); `test_rule_b_reuse_of_another_files_identity` (a walkthrough with NO `- Id:` whose slot `abc123` is a plan's declared identity -> exactly 1 finding whose location is the walkthrough and whose detail names the owning plan - the p7dqwz shape); `test_conformant_declared_matching_slot_is_clean` (declared==slot, plus a walkthrough that declares its own id6 -> 0); `test_legacy_hhmm_slug_word_not_flagged` (legacy `20260101-1357-01-assess-bugs...` where the 4-digit HHMM matches the setid segment and `assess` matches `[0-9a-z]{6}` -> 0, via the HHMM-setid guard + a real-id6 discriminator that requires the slot token be a declared id or visibly mix digits+letters); `test_all_clean_tree_no_slot_findings`. Existing `CollisionTests` (frontmatter id6 + setid uniqueness) are unchanged and still pass. On the REAL repo BEFORE E-03: `check_collisions` returned exactly ONE `check.id6-identity-slot` finding (the p7dqwz walkthrough), reproducing `tmp/find_id6_dupes.py`, and did NOT mass-flag the legacy `assess`/`agents` slug-word names (a naive first cut flagged 5; the discriminator + HHMM guard reduced it to the 1 true case). `python3 -m pytest tests/test_check_engine.py -q` green.
  - Result: pass
- [x] V-02 validates E-02
  - Required evidence: a test asserts `aw doctor` (human and `--agent`) renders the new rule with the D140 remediation text.
  - Observed evidence: added `test_id6_identity_slot_remediation` to `tests/test_doctor_remediations.py` (passes): `doctor.build_remediation(Drift(..., "check.id6-identity-slot", ...))` returns a Remediation whose title contains "identity-slot", whose `detailed_fix` contains "D140" and "Target-Id", and whose `summary_fix` tells the operator to give the file its OWN id6 - an actionable remediation, not a bare failure. `build_remediation` is the single synthesis point rendered by both the human and `--agent` doctor paths (doctor.py:872,928,1230), so both surfaces render it. `python3 -m pytest tests/test_doctor_remediations.py -q` green.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: the violator `20260823-artifactenginefix-01-p7dqwz-execution.walkthrough.md` was `git mv`'d to `20260823-artifactenginefix-01-hey7r7-execution.walkthrough.md` (own newly-minted, collision-checked id6 `hey7r7`) and its frontmatter now carries `- Id: hey7r7` and `- Target-Id: p7dqwz` (typed source reference to the plan it documents; the prose `- Target IPD:` line retained). `aw check all --agent`: total 28 findings, `check.id6-identity-slot` = 0 (down from 1); no new dangling-citation finding (the walkthrough is not cited by id6/PLAN- handle, and `p7dqwz` still resolves to the existing plan). `aw doctor --agent`: `check.id6-identity-slot` = 0. The three inbound references to the OLD name (DECISIONS.md D140 context, the executed o6b8l3 OQ-05 note, this plan's own Concern/E-03) are DESCRIPTIVE historical prose of the violation and were intentionally NOT rewritten (decision D1: rewriting would falsify the historical record and touch a terminal executed/ plan; they are not navigational citations). `pytest -n auto`: `2169 passed, 1 skipped in 134.73s`.
  - Result: pass
