...
- Owner: maintainer
- Finding: PR-001
- Resolution: RESOLVED 2026-09-19 BY ASSIGNING EACH ORPHANED CRITERION TO THE CHILD THAT OWNS ITS SURFACE, with the maintainer's authorization to edit the child plans. The edits landed in the CHILDREN, never on this parent, exactly as the question required. Re-measured after the change: A1, A4, A6, A19 and A21 are each now claimed by one child file, where before each was claimed by none.
  THE ASSIGNMENT AND WHY EACH LANDED WHERE IT DID. **A1, A4, A6, A19 -> `udgilu`** (V-01), because that child creates the one canonical semantic module all four criteria describe: A1 IS the "one canonical module" claim, A4 and A6 are glyph-rendering assertions over its table, and A19 (work-kind has no effect) is a property of its resolver. Each now demands specific pasted evidence rather than a mention: a repo-wide grep for a competing table, the four text-form glyphs, the plan progression `○`/`◔`/`◑`/`◕`/`✓`, and the same status resolved twice under two work-kinds proving byte-identical output. **A21 -> `qdd5jq`** (V-05), because A21 is a whole-tree gate (full suite plus `git diff --check`) and is only meaningful once every conversion has landed; `qdd5jq` is the last code child. Note A12 needed no assignment: the review's finding listed it, but re-measurement showed it was already claimed by two child files.
  THE REVIEW WAS RIGHT THAT AN ITEM ON THIS PARENT WOULD HAVE BEEN THE WRONG FIX, and that reasoning is preserved here because it is the part worth keeping: a runner retires an Order-0 orchestrator once every child is `executed` and deliberately SKIPS the pre-transition E/V checkpoint, so an `E-09` here reading "confirm the six criteria were assigned" would be marked complete having never been performed. The orchestrator coverage gate exists to catch precisely that, and it fired on a different Set (`s0gnha`) the same day for the same reason. The fix therefore had to be child edits, and was.
  WHAT REMAINS TRUE AND UNFIXED BY TOOLING: no tool cross-checks a spec's acceptance criteria against a Set's coverage, which is why this defect reached review at all. The parent's Cross-IPD validation section states the rule ("if any of A1 to A21 has no child claiming it at review time, that is a missing child") but nothing enforces it. That gap is worth a backlog item on its own merit and is NOT claimed to be fixed by this resolution.
