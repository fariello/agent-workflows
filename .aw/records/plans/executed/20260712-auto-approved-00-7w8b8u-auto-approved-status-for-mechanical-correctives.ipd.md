...
- Scope: extend the D52 readiness vocabulary with `auto-approved`; update the shared parser
  ...
  DECISIONS, and wire `/verify-execution` (IPD 20260712-verify-execution-00-c5685e-verify-execution-workflow) to emit it under strict criteria.
- Status: executed
...
- Id: 7w8b8u
- Set: auto-approved (auto approved status for mechanical correctives)
- Order: 0
...
- The validate-before-executed rule (IPD 20260712-falsely-marked-00-y6kuar-falsely-marked-executed-tests-still-red, in flight) still binds: `auto-approved`
...
4. **Wire `/verify-execution` (IPD 20260712-verify-execution-00-c5685e-verify-execution-workflow) to emit `auto-approved` under criteria judged by
...
`git mv` to executed/. Dependency: IPD 20260712-verify-execution-00-c5685e-verify-execution-workflow (/verify-execution) consumes this token, so
