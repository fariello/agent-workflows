...
Spec `25kzda` (`.aw/records/specs/approved/20260826-25kzda-01-25kzda-aw-run-deterministic-run-and-verify.spec.md`, `- Status: approved`) GOVERNS THIS DIRECTLY. Its §4.3 `IPD-DEP-CASCADE` row and the propagation text beneath it describe cascade semantics in terms of a node ending "failed, needs_input, or a terminal/gated skip that does not satisfy its outgoing requirement", and state that "On a later explicit resume, the engine re-evaluates every dependency-not-met item. A now-satisfied chain may return to `planned`".
