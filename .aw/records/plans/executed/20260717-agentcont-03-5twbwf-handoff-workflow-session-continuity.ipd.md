...
- 2026-07-20 /plan-review re-review (security/privacy lens, human-prompted): PR-009 (HIGH). The PR-005 reframe made the RAW SESSION CONVERSATION the primary input written to a durable, pushable file, but privacy handling was one buried passive clause. Added a MANDATORY "Sensitivity and privacy contract": actively classify sensitive/personal/embarrassing/third-party/secret/identity-leak content while drafting; omit-if-not-needed / reframe-to-minimum-and-confirm-wording-via-the-question-tool-if-needed / never write raw secrets; run `aw check-local-leaks` on the output; and NEVER auto-stage/commit/push - the human promotes the file to durable/tracked (output posture changed accordingly). Validation + the runbook exit-gate updated to enforce it. Still `reviewed`.
...
`/handoff [optional focus note]` produces ONE markdown handoff document: a resume DOCUMENT (essentially a resume prompt with context + first-move instructions for the next session), NOT a plan/IPD. It is READ-ONLY with respect to all product code and the durable record; it only creates the handoff file. OUTPUT POSTURE (PR-009): after writing, it runs the sensitivity gate + `aw check-local-leaks` on the file, then STOPS - it does NOT `git add`, stage, commit, or push the handoff. Because the file can hold raw session context, making it durable/tracked is the HUMAN's explicit decision (the workflow tells the human the path and that it is un-staged); `/handoff` never auto-commits it.
...
- Out-of-repo / sensitive context: governed by the dedicated "Sensitivity and privacy contract" section below (do NOT just dump session candor into a durable, pushable file). In brief: omit-if-not-needed, reframe-and-confirm-if-needed, never raw secrets, scan before done, human promotes.
...
### Sensitivity and privacy contract (MANDATORY - added at /plan-review, PR-009)

A handoff's primary input is the RAW SESSION CONVERSATION, which is the most sensitive, least-filtered content in the whole workflow, and the output is a durable, committable, publicly-pushable file. Capturing session context therefore RAISES privacy risk, so the workflow MUST apply this gate while drafting (this repo already leaked a maintainer identifier to PyPI once; see D92/D93):

1. Actively CLASSIFY as you draft. Watch for, and treat as sensitive: secrets/credentials/tokens/keys; personal, career, health, legal, financial, or family matters; other people's names or private info (third parties named in the conversation); candid or venting remarks, criticism of people/orgs, or anything embarrassing or reputationally risky if public; internal/confidential business context; and machine/identity leaks (home paths, usernames, private repo names, hostnames, session ids - the D92/D93 class).
2. For each sensitive item, choose in this order:
   a. NOT needed for continuity -> OMIT it. Leave at most a neutral pointer: "there is out-of-repo context the human holds; ask them if it becomes relevant." Never restate the sensitive content in the pointer.
   b. Needed for context -> REFRAME to the minimum non-sensitive form that preserves the continuity value, then use the interactive `question` tool to get the human's approval of the exact wording, or their choice to omit it. Do not write the reframed version durably until the human approves.
   c. Raw secrets/credentials -> NEVER write them, reframed or not. Reference by location only (as `assess-secrets` requires).
3. SCAN the finished document with `aw check-local-leaks <path> ` (or `python -m agent_workflows check-local-leaks`) before considering it done; resolve any hit (omit/reframe/allowlist) exactly as above.
4. The human is the gate. See the output posture in "What /handoff does": the file is written and scanned but is NEVER auto-staged or auto-committed by `/handoff`; the human reviews and decides whether it becomes durable/tracked. When in doubt about an item, ASK rather than write it.

## Proposed changes (ordered, validatable)

1. Create `.agents/workflows/handoff/handoff.md` - the portable prose runbook that instructs an agent to (a) harvest the LIVE SESSION CONTEXT as the primary body, then draw a THIN supporting frame from the record (per the reframed Goal/sources), (b) emit the required sections (session-context + nuance leading) including the nuance contract AND the Sensitivity and privacy contract (classify -> omit/reframe-and-confirm/never-raw-secrets), (c) write to `.agents/prompts/pending/YYYYMMDD-HHMM-NN-session-handoff-<slug>.md` with `Kind: session-handoff`, `Status: draft`, (d) remain read-only w.r.t. product code, (e) run `aw check-local-leaks` on the output and resolve any hit, (f) NEVER auto-stage/commit/push - tell the human the path and that promotion to durable/tracked is their call, and (g) satisfy a terminal exit-gate checklist that INCLUDES the sensitivity gate + the scan. Written per P9 (instructions for the model that runs them): explicit output contract + a terminal tick-list the model must satisfy (the compaction-recovery doc's lesson: models honor an explicit end-of-section checklist far better than scattered prose).
...
1. Sets reconciliation: RESOLVED (done 2026-07-20 via IPD `20260717-agentcont-01-6a3myl-whatnext-surveyor-workflow`). TWO distinct Sets: `agent-continuity-workflows` (whatnext=1, research=2, handoff=3) and `research-prompt-pipeline` (the prompts pipeline). No action here.
...
- The workflow is prose (irreducibly - it needs the LLM's session context; see Deferred), so validation is: (a) run `/handoff` in a real, context-rich session and confirm the SESSION-CONTEXT and NUANCE sections are substantive and dominate (not a filesystem re-summary) - this is the primary acceptance test; (b) confirm all required sections are present (or explicitly N/A); (c) confirm the output is written to `.agents/prompts/pending/` with `Kind: session-handoff`, `Status: draft`, NOT into `.agents/plans/`; (d) PRIVACY (PR-009): confirm the runbook applies the sensitivity gate (a planted sensitive item is omitted or reframed-with-human-approval, never dumped raw), runs `aw check-local-leaks` on the output, and does NOT auto-stage/commit/push (verify `git status` shows the handoff untracked/unstaged after a run); (e) confirm it made NO product-code changes; (f) confirm `/list-workflows` / the manifest lists `/handoff`; (g) `tests/test_dir_readmes.py` passes (the new `handoff/README.md` is required by its Category-2 check); (h) diff the generated doc against the golden example (`1950-01`) for STRUCTURAL parity only (shape, not stale facts). Full suite `python -m pytest -q` stays green (current baseline 293 passed, 1 skipped). No unit tests for the prose itself.
