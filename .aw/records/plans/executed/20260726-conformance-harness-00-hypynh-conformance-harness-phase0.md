...
- Id: hypynh
- Set: conformance-harness (conformance harness phase0)
- Order: 0

## Workflow history

- 2026-07-26 created (opencode its_direct/pt3-claude-opus-4.8-1m-us): Phase 0 of the clean-delta/skills workstream defined by the D109 spec (`.agents/docs/specs/20260726-1239-01-clean-delta-and-tracking-modes.spec.md`, "Build decomposition") and the host-probe reconciliation's 9-point required-release fixture (`.agents/docs/research/20260726-0054-aw-delivery-and-clean-delta-research/20260726-hostprobe-04-36rfym-external-delivery-host-probe.reconciliation.reconciliation-report.md`). Maintainer decision at authoring: build the deterministic harness (A) now as one IPD; the actual host runs (B) are the operator-run protocol it drives, not CI-automated.
...
|------|--------|--------|-------|------------------|------------|
| 1 | H2 | Fixture scaffolder: a function/CLI that creates an isolated probe fixture UNDER a caller-supplied base dir - a clean temp `$HOME`, an empty temp git repo (the "target"), and an external-content dir OUTSIDE the repo - places the tier-specific fixture (a `SKILL.md` for T2; an out-of-repo pointer for T1; a global-location file for T3) carrying a unique nonce and the "create `PROBE-OK-<host>-<version>-<nonce>.txt`" instruction. ISOLATION CONTRACT (PR-001, safety-critical): the base dir is an explicit REQUIRED argument (no default to `~`); the scaffolder resolves it and ASSERTS it is not `Path.home()` and the fixture home is not equal to nor a parent of the real home, refusing (raising) otherwise; every path it creates or writes MUST be under the provided base (a containment check), and the operator runs the host with `HOME`/`XDG_CONFIG_HOME` pointed INSIDE the fixture, so the real `~/.config`, `~/.claude`, `~/.gemini`, etc. are never read or written. Pure + parameterized (base path injected). | new harness module (under a workflow `tools/` or `agent_workflows/`, per OQ1) | Medium | scaffolder builds the fixture tree deterministically under the injected base; nonce unique per run; RAISES if the base is the real home / a parent of it / would write outside the base; renders the `HOME`/`XDG_CONFIG_HOME` env the operator must set; unit-tested with a tmp_path |
...
| 4 | H1,H2,H3,H5 | Unit tests (the deterministic half): scaffolder builds under an injected base + unique-nonce + the isolation guard (RAISES on real-home base / parent-of-real-home / attempted write outside the base - the safety-critical test) + renders the HOME/XDG env; renderer per-host/tier output + precedence/permission variants + unknown-host handling; recorder/validator resolved-vs-followed rules + required-field enforcement. Model on `tests/test_bench_env.py`. NO test launches a host. | `tests/test_conformance_harness.py` (new) | Medium | full suite green; the deterministic core AND the isolation guard are covered; paste ACTUAL output |
...
- Scaffolder: builds the fixture tree under an injected `tmp_path` base (clean temp home + temp repo + external content + tier fixture + unique nonce); RAISES when the base is the real home, a parent of it, or when a write would land outside the base; renders the `HOME`/`XDG_CONFIG_HOME` env the operator must set; two runs produce distinct nonces.
...
- OQ1 (tool home + surface): RESOLVED from the repo precedent (plan-review). A `tools/`-style Python module with a pure/testable core + a thin CLI (mirroring `.agents/workflows/benchmark/tools/bench_env.py`), exposed as an `aw` verb only if genuinely useful to an operator (decided at execution). Does not change the deterministic core; no human decision blocked.
- OQ2 (report/protocol home): RESOLVED from convention (plan-review). The results report is probe EVIDENCE, so it lives under `.agents/docs/research/`; the operator protocol is a short runbook near the tool. Execution may refine exact filenames (per the `YYYYMMDD-HHMM-NN-<slug>` convention).
- OQ3 (host matrix format): RESOLVED from P8/configurable (plan-review). A small data file (JSON/TOML) so adding a host/version is DATA, not code; the renderer consumes it.
