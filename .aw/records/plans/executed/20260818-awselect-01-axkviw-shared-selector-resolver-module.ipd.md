...
- 2026-08-18 executed (opencode Opus 4.8): E-01..E-06 performed, V-01..V-06 pass; code committed; full serial suite 1036 passed 1 skipped; record_dirs hardened for bare repos.
...
- [x] E-01 Create the new file `agent_workflows/selectors.py` with this exact header and imports, and nothing else yet:
  ```python
...
- [x] E-03 Add three small readers that extract front-matter fields from a record file's text, matching the existing bullet-metadata format used across the repo (e.g. `- Id: abc123`). Use these EXACT regexes (they mirror plans_index/backlog parsers):
  ```python
...
- [x] E-04 Add `resolve_one(repo_root: Path, record_type: str, token: str) -> List[Path]` that resolves ONE token against one type's record dirs. Rules, checked IN THIS ORDER, returning ALL files that match the FIRST rule that produces any match:
...
- [x] E-05 Add the public `resolve_selectors(repo_root: Path, record_type: str, tokens: List[str]) -> List[Path]` that calls `resolve_one` for EACH token and returns the OR-union of all matches, de-duplicated, sorted by path. An empty `tokens` list returns `[]`. This is the function Order 02 + awcmdsurf import.
  ```python
...
- [x] E-06 Create `tests/test_selectors.py` with a `unittest.TestCase` subclass `SelectorResolverTests` that builds a tmp repo fixture and asserts each resolution mode. Write EXACTLY these test methods (build the fixture in `setUp` using `tempfile.TemporaryDirectory`; create `.aw/records/plans/pending/` and write two plan files: `A` with `- Id: aaa111`/`- Status: approved`/`- Set: demo` and slug `alpha`, `B` with `- Id: bbb222`/`- Status: draft`/`- Set: other` and slug `beta`):
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: `python3 -c "import agent_workflows.selectors; print('ok')"` -> ok (verified).
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: record_dirs(root,'plans') -> the plans dir; record_dirs(root,'bogus') -> [] (verified; hardened to also use the direct literal layout so a bare repo resolves).
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: _read_id/_read_status/_read_setid return abc123/approved/demo for the sample lines (verified).
  - Result: pass
- [x] V-04 validates E-04
  ...
  - Observed evidence: resolve_one id6/status/setid/filename all return the fixture plan (covered by test_selectors).
  - Result: pass
- [x] V-05 validates E-05
  ...
  - Observed evidence: resolve_selectors union deduped+sorted; empty -> [] (test_multiple_tokens_union + test_empty_and_unknown pass).
  - Result: pass
- [x] V-06 validates E-06
  ...
  - Observed evidence: pytest tests/test_selectors.py -> 6 passed; full serial suite 1036 passed, 1 skipped (was 1030; +6).
  - Result: pass
