...
- [x] V-01 validates E-01
  ...
  - Observed evidence: `agent_workflows/ipd_lifecycle.py` adds `acquire_finalize_lock`/`release_finalize_lock` (lock at `.aw/state/runtime/locks/ipd_finalize_writer.lock`, PID-based, EPERM=alive/ESRCH=stale-reclaim) and a per-plan journal at `.aw/state/runtime/transactions/ipd_finalize_<id>.json` written atomically (temp + os.replace) with phase enum PREPARED/MUTATING/READY_TO_COMMIT/COMMITTED_INCOMPLETE/UNKNOWN_OUTCOME/COMPLETE and the full ownership snapshot (plan id/digest, original path+bytes, pre_head, owned_paths, INDEX.{json,md} bytes, exact `git ls-files --stage` entries for owned paths, receipt id). `RollbackFailureSemanticsTests`: `test_journal_records_ownership_and_is_atomic_before_mutation` (fault at `before_mutation` leaves plan/HEAD untouched, journal cleared after rollback), `test_second_finalizer_is_locked_out_with_retry_diagnostic` (a live foreign PID raises TransactionLockError), `test_stale_lock_is_reclaimed_via_dead_pid` (dead PID reclaimed, not blindly deleted). Reuses the layout_migration.MigrationManager pattern.
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: `_rollback_precommit` restores the plan's original bytes at its original path (atomic), removes the moved destination only when it still matches THIS transaction's `moved_bytes` (else unknown-outcome, no destructive restore), restores the exact prior Git-index entries for owned paths via `git restore --staged` (never touching disjoint work), and regenerates the plans index from the current corpus fail-loud. `RollbackFailureSemanticsTests`: `test_fault_after_move_rolls_back_plan_and_index` + `test_fault_after_index_rolls_back` (plan restored to pending, no executed file, HEAD unchanged), `test_crash_restart_before_commit_recovers_on_reinvocation` (a fresh finalize after a rolled-back fault succeeds), `test_rollback_preserves_disjoint_dirty_and_staged_work` (unrelated dirty + staged files survive untouched, staged entry preserved), `test_rollback_failure_retains_journal_and_reports_not_restored` (mocked rollback failure -> journal retained in UNKNOWN_OUTCOME, message says NOT reported restored). Receipt is left unconsumed until COMPLETE.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: `_lifecycle_commit_exists` classifies the boundary by OBSERVED state (HEAD advanced past pre_head AND the tip subject carries the deterministic `lifecycle(<id>):` marker); a failed commit with unchanged HEAD -> pre-commit rollback; HEAD moved without our marker -> UNKNOWN_OUTCOME (fail closed, journal retained); our commit present -> COMMITTED_INCOMPLETE. `_complete_after_commit` runs post-transition on the moved plan: pass -> COMPLETE (journal cleared, receipt consumed); fail -> report COMMITTED-INCOMPLETE naming the same-command `aw ipd finalize <plan>` resume with NO amend/reset/second commit. Early recovery in `finalize` resolves the committed-incomplete journal by plan id (plan now in executed/) BEFORE precheck and calls `_resume_post_commit` (verifies the recorded commit + dest still exist, reruns only post-transition). `RollbackFailureSemanticsTests`: `test_committed_incomplete_then_same_command_resume` (post fail -> exit 1 + journal committed-incomplete + receipt kept; resume -> exit 0, SAME HEAD i.e. no second commit, journal cleared, receipt consumed), `test_persistent_post_transition_failure_stays_incomplete`, `test_corrupt_journal_at_partial_state_is_unknown_outcome` (planted unknown-outcome journal -> fail closed). Manual e2e confirmed identical HEAD across the committed-incomplete -> resume boundary.
  - Result: pass
- [x] V-04 validates E-04
  ...
  - Observed evidence: `pytest tests/test_ipd_lifecycle_cli.py::RollbackFailureSemanticsTests -v` -> `12 passed in 0.86s` covering: journal ownership/atomicity, live-lock lockout + stale-lock reclaim, fault-after-move + fault-after-index rollback, crash/restart-before-commit recovery, disjoint dirty+staged preservation, rollback-failure -> unknown-outcome, committed-incomplete + same-command resume (no 2nd commit), persistent post-transition failure stays incomplete, corrupt/unknown-outcome journal fail-closed, and the clean-complete baseline (journal cleared + receipt consumed + no lock left). Full lifecycle file `pytest tests/test_ipd_lifecycle_cli.py` -> `46 passed` (15 begin + 10 finalize + 9 reconciliation + 12 rollback). Full suite `pytest -n auto` -> `2280 passed, 1 skipped in 133.32s (0:02:13)`. `aw ipd lint --all` exits 0; `aw specs check` conforms; `check-local-leaks` clean. In-Scope-Paths intervening-commit refusal remains inherited from Order 04/05 (computed + surfaced).
  - Result: pass
