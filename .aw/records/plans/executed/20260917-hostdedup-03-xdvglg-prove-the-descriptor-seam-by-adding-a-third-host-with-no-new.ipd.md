...
## Workflow history
- 2026-09-24 executed (opencode/its_direct-pt3-claude-opus-5-1m-us): Landing the verified lane work from run run-20260924T050407Z-3108751. That run finalized nothing for two reasons, both now resolved: the begin receipt went stale when the plan was edited mid-turn, and one declared scope path was removed with no recorded reason. The removal is a STALE-CITATION fix (the path was deleted from main by 7ebc2964 one day BEFORE the review that declared it, and its pins moved to tests/test_rununify_initialize_run.py which this plan does declare and does edit), now explained in the plan. All 6 E-items performed and 6 V-items verified on the lane. Third host 'scripted' reaches DEFAULT_HOSTS, the descriptor seam is exercised in the direction it will actually be used, and the gap analysis is immortalized as research le9q02. Suite on the merged result: 8850 passed, 5 skipped, 2 xfailed (main was 8840). [Scope reconciliation - in-scope-unmodified .aw/records/research: modified by this plan's lane commit 2e3268f2 and present in this tree (verified by git diff --numstat against main); the begin receipt was necessarily re-issued AFTER the lane merge because the original went stale mid-run, so the edit predates the receipt base and finalize cannot see it as a change; in-scope-unmodified agent_workflows/agy_runipd.py: modified by this plan's lane commit 2e3268f2 and present in this tree (verified by git diff --numstat against main); the begin receipt was necessarily re-issued AFTER the lane merge because the original went stale mid-run, so the edit predates the receipt base and finalize cannot see it as a change; in-scope-unmodified agent_workflows/host_cmd.py: modified by this plan's lane commit 2e3268f2 and present in this tree (verified by git diff --numstat against main); the begin receipt was necessarily re-issued AFTER the lane merge because the original went stale mid-run, so the edit predates the receipt base and finalize cannot see it as a change; in-scope-unmodified agent_workflows/oc_runipd.py: modified by this plan's lane commit 2e3268f2 and present in this tree (verified by git diff --numstat against main); the begin receipt was necessarily re-issued AFTER the lane merge because the original went stale mid-run, so the edit predates the receipt base and finalize cannot see it as a change; in-scope-unmodified agent_workflows/run_analytics_sources.py: modified by this plan's lane commit 2e3268f2 and present in this tree (verified by git diff --numstat against main); the begin receipt was necessarily re-issued AFTER the lane merge because the original went stale mid-run, so the edit predates the receipt base and finalize cannot see it as a change; in-scope-unmodified agent_workflows/run_viewer.py: modified by this plan's lane commit 2e3268f2 and present in this tree (verified by git diff --numstat against main); the begin receipt was necessarily re-issued AFTER the lane merge because the original went stale mid-run, so the edit predates the receipt base and finalize cannot see it as a change; in-scope-unmodified agent_workflows/runner_shared.py: modified by this plan's lane commit 2e3268f2 and present in this tree (verified by git diff --numstat against main); the begin receipt was necessarily re-issued AFTER the lane merge because the original went stale mid-run, so the edit predates the receipt base and finalize cannot see it as a change; in-scope-unmodified tests/test_hostdedup_third_host.py: modified by this plan's lane commit 2e3268f2 and present in this tree (verified by git diff --numstat against main); the begin receipt was necessarily re-issued AFTER the lane merge because the original went stale mid-run, so the edit predates the receipt base and finalize cannot see it as a change; in-scope-unmodified tests/test_rununify_initialize_run.py: modified by this plan's lane commit 2e3268f2 and present in this tree (verified by git diff --numstat against main); the begin receipt was necessarily re-issued AFTER the lane merge because the original went stale mid-run, so the edit predates the receipt base and finalize cannot see it as a change]
...
- [x] E-02 Implement the maintainer's OQ-02 ruling: add an explicit host-id field to `HostLabels`, have a run record that identity, and re-point `run_analytics_sources.driver_generation` and `run_viewer` at the id. MUST include the fallback for PRE-CUTOVER records, which hold a module path and cannot be rewritten, so historical runs keep attributing correctly.
  **SCOPE-PATHS AMENDED 2026-09-24: `tests/test_rununify_initialize_run_characterization.py` REMOVED,
  AND THIS IS A STALE-CITATION FIX, NOT A CONTRACT REDUCTION.** Round 2 of `/plan-review` declared that
  file as a ninth affected path on 2026-09-18, citing its `driver.path == __file__` / `driver.sha256`
  pins. THAT FILE NO LONGER EXISTS AND DID NOT EXIST WHEN THIS PLAN EXECUTED: commit `7ebc2964`
  ("tests: consolidate characterization suites and eliminate brittle source pins", 2026-09-17, on
  `main`) DELETED it along with three sibling characterization suites, one day BEFORE the review that
  declared it. The pins it carried live in `tests/test_rununify_initialize_run.py`, which IS declared
  and IS edited. So the declaration was stale on arrival, and `aw ipd finalize` correctly refused the
  removal as an unexplained contract reduction until this note existed; the reduction is legitimate
  because the obligation moved rather than being dropped. VERIFIED: `git log --all --` on that path
  shows `7ebc2964` as its last commit and no file at that path in the working tree.
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: Derived from source analysis across `runner_shared.py`, `oc_runipd.py`, `agy_runipd.py`, `host_sandbox_profile.py`, and immortalized in `.aw/records/research/20260924-hostdedup-00-le9q02-third-host-descriptor-contract-and-gap-analysis.findings.md`:
    1. `HostLabels` fields: 9 fields (`id`, `command`, `review_command`, `argv_tokens`, `argv_subcommands`, `product`, `report_title`, `shell_tool`, `emits_launch_identity`, `full_auto_actor`) with no defaults. All modelled in `HostLabels`. Consumed by `runner_shared.py`, `run_analytics_sources.py`, `run_viewer.py`.
    2. Host-specific options: 13 keys (7 oc-only: `agent`, `auto`, `launch_profile`, `no_audit`, `opencode`, `validate`, `variant`; 6 agy-only: `agy_executable`, `dangerously_skip_permissions`, `effort`, `new_session`, `no_verify`, `timeout`; plus conditional `verify_*` keys on oc). NOT modelled in `HostLabels`. Consumed by runner `initialize_run` wrappers and downstream spawners.
    3. Driver identity: `state["driver"]["id"]`, `state["driver"]["path"]`, `state["driver"]["sha256"]`. Modelled in `HostLabels.id` and frozen by `initialize_run_core`. Consumed by `run_analytics_sources.driver_generation` and `run_viewer.load_run_summary`.
    4. Turn launch argv construction: `oc_runipd.py:5648` vs `agy_runipd.py:2730`, and verifier argv `runner_shared.py:15192`. NOT modelled in `HostLabels`.
    5. Turn spawn function: `run_opencode` (13 params) vs `run_agy_turn` (12 params). NOT modelled in `HostLabels`.
    6. Permissions & sandbox posture: `host_sandbox_profile.detect_host_capabilities`. NOT modelled in `HostLabels`.
    7. Label binding sites: 9 binding sites per runner inside runner modules. NOT accessible outside runner modules.
    8. Shared code host branches: `_render_turn_stream_fallback` (15149), `_build_verifier_turn_argv` (15192), `enforce_orchestrator_coverage` (15560), `enforce_mixed_type_gate` (23355). NOT modelled in `HostLabels`.
  - Result: pass
...
- [x] V-02 validates E-02
  ...
  - Observed evidence: Dual-direction attribution verified on tracked fixtures and new driver ID field:
    ...
    4. Updated `run_viewer.load_run_summary` to inspect `driver.id` first, falling back to substring/stem matching on `driver.path`.
    5. A runner-less host records `driver: {"id": "scripted", "path": None, "sha256": None}`, honestly reflecting no module on disk. No product consumer reads `sha256`.
    6. Re-based `tests/test_rununify_initialize_run.py` characterization test `EachHostRecordsItsOwnDriverIdentity`: asserted `driver.id` in `state["driver"]` matches `DRIVER_IDENTITY[name]["id"]` and that driver IDs differ across hosts, while preserving path basename, digest equality, generation inference, and viewer label assertions. `56 passed` in `tests/test_rununify_initialize_run.py`.
    7. Dual-direction attribution verified on tracked fixtures (`tests/test_run_analytics_sources.py:141-155, 449` covering `oc_runipd.py`, `runipd.py`, `ipdrunner.py`, `agy_runipd.py`).
  - Result: pass
...
- [x] V-03 validates E-03
  ...
  - Observed evidence: Third host initialized and boundary walls documented:
    ...
    4. Hit all three predicted walls when attempting turn execution without a runner module:
...
- [x] V-04 validates E-04
  ...
  - Observed evidence: Classified all identified gaps across all 4 categories in `.aw/records/research/20260924-hostdedup-00-le9q02-third-host-descriptor-contract-and-gap-analysis.findings.md` section 4:
    - (a) Missing `HostLabels` field: Driver identifier (`id`) missing from descriptor (resolved by adding `id: str` to `HostLabels`).
    - (b) Genuine host capability: Process tree kill, landlock sandbox, tool events streaming, session resume (owned by `host_sandbox_profile.HostSandboxCapabilities`).
    - (c) Structural limits: `HostLabels` is passive metadata and lacks an execution launcher interface; label binding sites live inside runner modules rather than shared dispatch.
    - (d) Blocked on remaining fork: `execute_item`, `run_queue`, `build_parser`, and `main` remain forked per runner; turn execution is blocked on finishing unification of these functions.
  - Result: pass

- [x] V-05 validates E-05
  - Required evidence: the committed research record path, containing the contract and the gap analysis.
  - Observed evidence: Created research record at `.aw/records/research/20260924-hostdedup-00-le9q02-third-host-descriptor-contract-and-gap-analysis.findings.md` with contract, third-host results, 4-way gap classification, and architectural recommendations; indexed cleanly via `aw research index` (120 docs).
  - Result: pass
...
- [x] V-06 validates E-06
  ...
  - Observed evidence: Permanent guard and regression tests verified:
    ...
    4. Full test suite baseline: `env -u AW_EXECUTION_ROLE python3 -m pytest` measured `1 failed, 8790 passed, 3 skipped, 2 xfailed, 6 warnings in 228.77s` (the single failure is pre-existing `CitationAnchorAdvisoryTests.test_the_detector_discriminates_on_the_real_corpus_with_the_gate_disabled`). Post-change suite run verified no new failures.
  - Result: pass
