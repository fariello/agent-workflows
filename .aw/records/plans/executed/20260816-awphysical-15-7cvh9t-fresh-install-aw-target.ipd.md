...
- 2026-08-16 /plan-review (opencode Opus 4.8 its_direct/pt3-claude-opus-4.8-1m-us; independent single-file pass, complementing the earlier Gemini review): APPROVE WITH REVISIONS APPLIED; readiness GO - PENDING HUMAN APPROVAL. Structural lint conforming (author + review-finalize). Verified every cited engine anchor exists (install_into_repo:4190, install_all:1707, collect_source_members:600, collect_target_framework_files:1767, prune_stale:1794, generate_shim_members:766, update_agents_pointer:1940). Findings FIXED in place: PR-A01 (MEDIUM, UNDER-SCOPE) - E-02 conflated the engine bundle-write (in scope) with the full `.aw/` records/config/state hierarchy; verified the CLI install path is TWO mechanisms (engine.install_into_repo for bundle+shims+pointer; install_wizard.persist_project_policy for .aw/config) and that project_layout.install_system_tree is NOT wired into CLI install; added a scope note bounding E-02 to the bundle-write + generated-content and calling out any extra .aw scaffolding as its own sub-item. PR-A02 (LOW) - disambiguated the Order-14 `aw migrate-layout` tool (E-03 delegates to it) from the pre-existing engine.migrate_legacy_layout pre-D17/D19 migrator (engine.py:2076) that already runs in install. No open questions remain; NO-GO only in the sense of pending human approval + Order 14 terminal.
...
- 2026-08-16 EXECUTED (implemented by Antigravity/Gemini 3.7 Flash High under a frozen-interface brief; reviewed, verified, and committed by opencode Opus 4.8 orchestrator): E-02..E-04 in `agent_workflows/engine.py` + `cli.py` + tests (commit 60147fe). Fresh install now targets `.aw/system/workflows/` (generated shims + AGENTS pointer follow the resolved layout); legacy `.agents/`-only repos are auto-detected and offered migration (delegating to the Order-14 MigrationManager), keep-legacy + deprecation notice otherwise. Orchestrator verification caught + fixed one out-of-brief-scope regression (`tests/test_dir_readmes.py`). Validation: full serial suite 957 green (skipped=1); real fresh `aw install` -> `.aw/system/workflows/` only, no `.agents/workflows/`. E-02..E-04 performed; V-02..V-04 pass. Transitioning to executed/.
...
- [x] E-02 Introduce a resolved TARGET install-root concept so the installer no longer hardcodes `.agents/workflows`. Add a function (e.g. `resolve_target_layout(repo_root)`) that returns the layout the install/update should write for THIS target with a DETERMINISTIC rule: if `.aw/system` is present the layout is `aw` and it is AUTHORITATIVE (no migration prompt) even if a legacy `.agents/workflows` also exists (e.g. from an interrupted manual action; PR-003); a fresh repo (neither present) is `aw`; a repo with ONLY the legacy tree that has not opted to migrate is `legacy`. Thread the resolved bundle-root + records-root through `install_into_repo`/`install_all`/`collect_source_members` mapping/`collect_target_framework_files`/`prune_stale`, replacing bare `WORKFLOWS_DIR` uses with the resolved root. Keep `WORKFLOWS_DIR` as the LEGACY constant only. The prune/collect sets MUST be computed against the SAME resolved root so install and prune cannot diverge. CRUCIALLY, the GENERATED CONTENT must also reflect the resolved layout (PR-001): `shim_body` emits `Read and execute @{workflow.body}` where the manifest body paths are `.agents/workflows/...`, and `agents_pointer_prose` hardcodes `.agents/workflows/` + `.agents/docs/research/` etc.; under an `aw` target these must be rewritten to the resolved `.aw/system/workflows/` (and `.aw/records/...`) bundle paths, or a generated shim/pointer will reference a non-existent legacy path. Parameterize the shim `@`-body path and the pointer prose on the resolved layout (map the manifest body prefix to the resolved bundle root).
  - Depends on: none
  - Expected outcome: a fresh install writes `.aw/system/workflows/index.md` and NO `.agents/workflows/`; an existing `.aw/` repo updates `.aw/` (and is treated as authoritative even if a stray legacy tree exists); an existing legacy-only repo (pre-migration) still updates `.agents/workflows/` when the user declines migration; generated shims (`Read and execute @...`) and the AGENTS.md pointer reference the RESOLVED bundle path, not a hardcoded legacy path.
  - Execution state: performed
  - Scope note (composition of the two install mechanisms): the CLI install path is TWO mechanisms - `engine.install_into_repo` (via `_install_one`) writes the workflow BUNDLE + shims + pointer + READMEs, and `install_wizard.persist_project_policy` writes `.aw/config/project.json`/`local.json` per the chosen preset/`records_backend`; `project_layout.install_system_tree` (the full `.aw/system` materializer) is NOT currently wired into the CLI install path. THIS E-item owns ONLY converting the engine bundle-write target to the resolved `.aw/system/workflows/` (and the generated-content paths). The records/config placement per preset is already produced by the existing wizard/policy path and is NOT re-implemented here; if a fresh `.aw` install must also create `.aw/records`/`.aw/state` scaffolding beyond `config/`, that wiring is called out as its own sub-item rather than folded silently into the bundle write. Do not conflate this with `engine.migrate_legacy_layout` (see E-03).

### Task group 2: Legacy auto-detect and offered migration

- [x] E-03 On `aw install`/`aw update`/`aw setup`, detect a legacy `.agents/`-only layout (legacy bundle present AND no `.aw/system`) and, interactively, OFFER to migrate to `.aw/` by delegating to the Order-14 `aw migrate-layout` TOOL (the `MigrationManager`/CLI; never a bespoke path). NOTE the naming collision: this is DISTINCT from the pre-existing `engine.migrate_legacy_layout` function (engine.py:2076), which handles the pre-D17/D19 in-place layout moves (`release-review/` -> `.agents/workflows/`, `repository-review/` -> `workflow-artifacts/`) and already runs inside every install; do not conflate or route through it. The `.agents/` -> `.aw/` relocation is ONLY the Order-14 tool. If accepted, run the migration (move) then complete the install against `.aw/`. If declined (or non-interactive without an opt-in flag), continue updating the legacy `.agents/` layout in place and emit a one-time DEPRECATION notice naming the compatibility-window removal release. Never create a second divergent layout (both `.agents/workflows/` and `.aw/system/workflows/` freshly written in the same run). Provide non-interactive flags (`--to-aw` / `--keep-legacy`) so CI is deterministic.
  - Depends on: E-02
...
- [x] V-02 validates E-02
  ...
  - Observed evidence: Real fresh `python3 -m agent_workflows install <clean-repo> --yes` produced `.aw/system/workflows/index.md` present and `.agents/workflows/` ABSENT; the generated shim `.opencode/commands/assess.md` contains `@.aw/system/workflows/assess/assess.md` and `AGENTS.md` contains `.aw/system/workflows/`. `engine.resolve_target_layout` implements the deterministic rule (`.aw/system` present -> aw authoritative; legacy-only -> legacy; fresh -> aw). Falsifiability: `tests/test_installer.py::InstallerEndToEndTests::test_fresh_install` asserts `.aw/system/workflows/index.md` + `.aw/system/VERSION` + NO `.agents/workflows` + the shim `@`-path + the AGENTS pointer text - reverting the resolved root to the hardcoded `WORKFLOWS_DIR` constant fails the "no `.agents/workflows/`" and shim-path assertions.
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: `cli._handle_legacy_migration` detects legacy `.agents/`-only (legacy bundle present AND no `.aw/system`) and, on `--to-aw`, delegates to the Order-14 `MigrationManager.execute_migration(target_backend="repository", leftover_disposition="defer")` (never `engine.migrate_legacy_layout`); `--keep-legacy` and the non-interactive default (OQ-01) keep legacy in place and emit the deprecation notice; interactive offers migration. Tests `tests/test_cli.py::<Order-15 class>` cover `test_install_legacy_repo_keep_legacy_flag` (asserts "legacy .agents/ layout is deprecated"), `test_install_legacy_repo_to_aw_flag`, `test_install_legacy_repo_unattended_defaults_to_keep_legacy`, and `test_install_legacy_repo_interactive_accept_migrates_to_aw`. All green in the full suite.
  - Result: pass
- [x] V-04 validates E-04
  ...
  - Observed evidence: Full serial suite `Ran 957 tests in 183.153s ... OK (skipped=1)` (a first full-suite pass caught one out-of-brief-scope regression - `tests/test_dir_readmes.py` still asserted the legacy fresh-install path - which the orchestrator fixed to `.aw/system/workflows/`, demonstrating the verify-not-trust discipline; re-run green). Gemini's targeted run reported `tests.test_installer`+`tests.test_cli` `Ran 165 tests ... OK`. The fresh-install mutation is falsifiable via `test_fresh_install` as noted in V-02.
  - Result: pass
