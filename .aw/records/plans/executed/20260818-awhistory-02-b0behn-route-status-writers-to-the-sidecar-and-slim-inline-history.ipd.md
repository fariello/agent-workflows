...
- 2026-08-18 /plan-review (opencode Opus 4.8, RIGOROUS): APPROVE WITH REVISIONS APPLIED. Verified anchors (specs _append_history:224/run_set:330/run_note:550, backlog _reattach_history:413) and that attention `last_history_at` returns the LAST record date, so slimming to latest-one is safe for attention. PR-002 (MEDIUM): added an EXPLICIT guard that this Order touches ONLY specs.py+backlog.py and MUST NOT slim plans/IPD history (IPD-S405 needs the inline executed entry). Conforms at review-finalize. GO - PENDING HUMAN APPROVAL.
- 2026-08-18 executed (opencode Opus 4.8): E performed, V pass; backlog+specs writers route to sidecar + slim inline to latest-one; plans/IPD untouched; full serial suite 1076 passed 1 skipped.
...
- [x] E-04 Create `tests/test_history_routing.py` with a `unittest.TestCase` subclass `HistoryRoutingTests` that builds tmp-repo fixtures (in `setUp` via `tempfile.TemporaryDirectory`, `self.root = Path(...)`) and proves BOTH the routing and the attention-preservation invariant. Write EXACTLY these test methods:
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: Verified: backlog set appends sidecar + slims inline to 1 record; attention last_history_at preserved; specs slim inline (no id6 -> no sidecar yet); suite 1076p/1s.
  - Result: pass
