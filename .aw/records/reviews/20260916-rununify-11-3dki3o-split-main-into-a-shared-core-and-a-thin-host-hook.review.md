...
| PR-003 | BLOCKER | IN-SCOPE | round 1 finding, discharged by directive | this plan's resolved `OQ-03`; the maintainer's 2026-09-16 directive | Hardcoding either host's empty-selection class in a shared core returns the wrong exit code, which the spec pins. | C:Low; U:Low; S:Low; F:Low; Overall:Low | FIXED | Not discharged by fiat: the maintainer's ruling requires the split, and this finding names a concrete correctness requirement the split must satisfy. It is carried as a binding constraint on the work rather than an open blocker: the shared entry point MUST preserve each host's spec-pinned exit codes (0 ok, 1 findings, 2 cannot-run), and this plan's E-03 already pins that contract as its own item. Verified as the correct treatment because the finding is satisfiable by construction, unlike a scope question. |
