...
- `SR-404` - `OPEN`:
  - Reason: the accessibility lens forbids assuming 256-color and carries only a narrow `aw attention`
    exception (D133), while this spec extends xterm-256 to every renderer with no 16-color fallback. The
    lens is a rubric this spec must SATISFY, not a competing spec, so the maintainer's override ruling does
    not reach it.
  - Remediation Risk: Medium-High
  - Axis: usability and functionality
  - Required decision or evidence: a maintainer ruling either widening the D133 exception to every renderer
    named here, or requiring a 16-color fallback column plus the color-depth detection it needs (measured:
    `should_color` is boolean and the package has zero depth detection, so this is a new mechanism).
  - Consequence if unresolved: the implementing plan cannot satisfy both this spec and the lens that spec
    `20260706-0000-01` Goal 9 delegates to, so a reviewer holding it to the lens would refuse it.
  - Raised in the spec as blocking OQ-01 naming this finding.
