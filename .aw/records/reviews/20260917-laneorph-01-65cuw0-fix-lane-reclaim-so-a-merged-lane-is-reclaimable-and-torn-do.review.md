...
| PR-103 | BLOCKER | IN-SCOPE | A. correctness; B. data-safety (an unsatisfiable validation requirement) | `lane_containment.py:3329`; `runner_shared.py:1004-1012`; `worktree_lease.py:701`; measured `torn_down True`, `rev-parse --verify` **rc=128**, reflog "unknown revision" | **ROUND 1's V-03 WAS UNSATISFIABLE:** it demanded the lane branch survive a reclaim, but the shared gate's default remover force-deletes it and empties its reflog. Routing through the gate buys safety, not ref preservation, so OQ-02's "never delete the ref" answer had no mechanism. | C:Medium; U:Low; S:Medium; F:Medium; Overall:Medium | OPEN | ESCALATED as OQ-04 (`Blocking: yes`, `Finding: PR-101` names the shared question; this finding is cited in OQ-04's body). Four options recorded with their costs: inject a non-deleting remover at this call site, add a `keep_branch` flag to the shared primitive, accept deletion for a provably-merged lane, or drop the reclaim half. Not decided by me: option (b) edits a destructive primitive four call sites depend on, and option (c) contradicts this plan's own Goal. V-03 now requires the branch outcome to be STATED rather than assumed, and OQ-02 is annotated that its answer stands while its implementation is not free. |
