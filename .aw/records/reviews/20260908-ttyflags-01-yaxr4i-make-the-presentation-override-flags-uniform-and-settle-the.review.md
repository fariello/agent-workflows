...
| ID | Severity | Scope | Area | Evidence | Finding | Remediation Risk | Decision | Resolution |
|----|----------|-------|------|----------|---------|------------------|----------|------------|
| PR-001 | HIGH | IN-SCOPE | B. Contracts (published promise never implemented) | plan OQ-01 (`- Status: resolved`, `- Finding: PR-001`) | Carried forward from round 1 and now CLOSED BY THE MAINTAINER. Round 1 found the output contract promises that non-TTY stdout adopts `aw.agent/v1` JSONL "immediately upon release with no deprecation window" while piped output has always been human prose, so either the code or the document had to yield. Ruling of 2026-09-10: CORRECT THE DOCUMENT; piped output stays human-readable and `--agent` remains the explicit opt-in. | C:Low; U:Low; S:Low; F:Low; Overall:Low | FIXED | Closed on the maintainer's decision. Re-verified that nothing can depend on the documented behavior (it never shipped) while an unknown number of external consumers depend on the actual one, and that capability is not lost since `--agent` emits the JSONL and CI already uses it. Obligation recorded: the retraction must be EXPLICIT in the document with date and reason, not a quiet deletion, or the next reader re-files it as an unimplemented-contract defect. |
