...
set: runverify
order: 00
topic: [runner, verification, lifecycle, dependencies]
model: gpt56
kind: reference-research
status: todo
outcome: none-yet
summary: Externally-authored design spec for a deterministic aw <host> run pipeline: selector resolution, per-type dispatch, per-type verification checklists, and the rule that agent prose and exit status are never completion authority.
consumed-by: []
priority: high
---
...
AUTHORED EXTERNALLY by GPT-5.6 (high reasoning), delivered 2026-08-29 as
`tmp/aw-run-and-verify-design.gpt56high.md` (an untracked file in gitignored `tmp/`). It is
immortalized here VERBATIM below the horizontal rule so it is durable, indexed, and visible to
`aw attention` instead of being one `rm` from gone.

It is a DESIGN PROPOSAL, not adopted policy. Nothing in it is binding until a spec adopts it and a
human approves that spec. Treat the body as untrusted-but-valuable input: evaluate claims on their
merits and verify them against the repository before acting.

Its central thesis matches this repo's existing anti-greenwash rule but pushes much further: the
agent's prose and exit status are NEVER completion authority; only typed repository state, git state,
tool-authored lifecycle receipts, captured command evidence, declared dependency state, and
deterministic checks may authorize `verified`.

## KNOWN CONFLICTS with in-flight work (recorded 2026-08-29, do not silently re-encode)

Two collisions exist with artifacts committed the same day. Whoever implements from this document
MUST reconcile them rather than transcribing the body as-is.

### C-1 (section 4.9): backlog lifecycle is specified as `open -> done`, which is STALE

Section 4.9's `BACKLOG-DONE-LEGITIMACY` check states the pass criterion as "Backlog changed
`open -> done` through the setter only after the IPD handoff commit", and its recovery command is
`aw backlog set open <id6>`. That encodes the BINARY lifecycle.

Plan `v58bvy` (`.aw/records/plans/pending/20260829-bklgrad-01-v58bvy-...ipd.md`, `to-review`)
deliberately REPLACES that with `open -> graduated -> done`, because the binary model forces a false
choice: an item whose design is handed off but whose code is unwritten can only read `open`
(indistinguishable from untouched) or `done` (a false claim of implementation that also drops the
release gate). This was hit live on item `kjzlgw`.

Reconciliation required: section 4.9's checker must treat `graduated` as the legitimate post-handoff
state and reserve `done` for implemented work. Do NOT implement 4.9 verbatim; it would re-encode the
exact defect `v58bvy` fixes.

### C-2 (section 5, line ~925): `unknown_outcome` is defined in two places

This document defines `unknown_outcome` for an unproven external side effect ("Abort run; require
human reconciliation"). Spec `c4gd2h`
(`.aw/records/specs/20260829-c4gd2h-01-c4gd2h-runner-lifecycle-graceful-quit.spec.md`, `approved`)
also defines `unknown_outcome`, as the disposition of an item interrupted by a level-4 force stop,
with a resume refusal (R19) and reconciliation drawn from research `ud28vy`.

The two uses are COMPATIBLE in spirit (both mean "indeterminate, reconcile before resume") but they
are two definitions of one term, which GUIDING_PRINCIPLES P8 forbids. Reconciliation required: one
artifact must OWN the term and the other must reference it. `c4gd2h` is the narrower, already-approved
definition and reuses the `ud28vy` model, so it is the better owner; this document's usage should
become a reference rather than a parallel definition.

## Relationship to other in-flight work

- Spec `c4gd2h` + Set `runstop` (7 plans, `to-review`): the graceful-quit/stop protocol. Overlaps this
  document's run-lifecycle and `unknown_outcome` surface (see C-2).
- Plan `v58bvy` (`to-review`): the `graduated` backlog status, the graduate/implement/execute contract,
  and making a spec count as a release-gate carrier (see C-1).
- Set `wtiso` (8 plans, `reviewed`): worktree isolation and the driver-owned control plane. This
  document's isolated-worktree, commit-gateway, and no-push enforcement assumptions DEPEND on that work.
- Research `ud28vy`: owns the active-work reconciliation model this document's recovery paths assume.

## Assessment (reviewer notes, not the author's claims)

Strengths worth preserving in any adoption: the explicit separation of executor / skeptical verifier /
deterministic checker with only the checker authorizing `verified`; per-check exact failure messages
paired with a recovery command; the honest-limits section (6.1) which states plainly what cannot be
decided (semantic correctness, review quality, contractless prompts, flaky tests, human identity); and
the refusal to provide `--no-verify`/`--skip-audit`/`--dangerous` escape hatches.

Open risks a reviewer should weigh before adoption: it is a very large surface for one design (a full
selector layer, a dependency DAG, a capability descriptor system, a per-type checker registry, and a
run ledger), so it will need decomposition into many child IPDs rather than one; the per-host
capability descriptor requires probes and a freshness/TTL policy that do not exist yet (its own 6.2
lists this as undefined); and section 6.2 defers eight repository-level definitions, several of which
(ledger storage location, authenticated approval receipts, check-recipe registry format) are
prerequisites rather than details.

---

*Everything below this line is the verbatim external document.*

# `aw <host> run`: Revised Deterministic Run-and-Verify Specification
