...
id: j2000q
created: 20260714
set: same-box-agent-wakeup-mechanisms
order: 00
topic: []
model:
...
summary: Migrated from 20260714-same-box-agent-wakeup-mechanisms-00-j2000q-same-box-agent-wakeup-mechanisms.research-report.md.
consumed-by: []
---
...
5. Acknowledgements are a CLOSED-ENUM state machine, no free text. An ack is metadata (one token from a fixed set), never content. It stays inside the payload-blind invariant BECAUSE it is closed: a token cannot smuggle instructions and cannot be an injection vector, and the full set is human-inspectable. Any free-form content (a question, an explanation, "see reply") is NOT an ack; it is a normal inbox reply message the target sends back through the ordinary channel. "see-reply" is therefore explicitly excluded (the target can always send a message), as is anything needing prose.
   - Authorized writer per token (each state has exactly one legitimate author):
     - BROKER writes ONLY content-free DELIVERY observations, which only it knows and which carry no payload: `scheduled` (Not-Before in the future, not yet fired), `queued` (eligible, about to fire), `delivered` (nudge landed: headless message accepted or attended toast shown), `agent-not-running`, `agent-not-responding`, `expired` (never deliverable / cancelled).
     - TARGET AGENT writes ONLY its own read/work states, which only it can truthfully assert: `read`, `in-progress`, `done`, `not-done`, `executed`, `not-executed`.
     - The broker MUST NEVER assert a target state (e.g. forge `read` or `executed`); doing so is both incorrect and an invariant breach.
   - `unread` is modeled as the ABSENCE of a `read` ack after `delivered`, not a written token, so there is a single source of truth.
   - Ack storage keeps the broker payload-blind: acks are their own tiny files, e.g. `.agents/comms/local/acks/<msg-id>.<from-agent>.<state>.json` = `{ re, state, by, at }`. `state` is validated against the closed enum on write (anything outside the set is rejected, so free text can never enter). The broker/registry aggregates ack files into a status view WITHOUT ever opening a payload.
   - Trust: a target-asserted ack (e.g. `executed`) is a CLAIM by that agent, exactly as trustworthy as the agent, i.e. NOT automatically trusted under our stance. No automation may treat `executed` as proof of execution.
   - Scope: the FULL lifecycle is implemented in v1 (maintainer decision). The complete machine: `scheduled -> queued -> delivered -> read -> in-progress -> done` with terminal branches `executed` / `not-executed` / `not-done`, broker failure states `agent-not-running` / `agent-not-responding`, and `expired`. Building the whole set is acceptable here (unlike Not-Before conditions) because acks are closed metadata and add no injection surface.
...
- Related artifacts: `research/20260712-filesystem-inter-project-agent-comms-concept-00-dw4wrj-filesystem-inter-project-agent-comms-concept.concept.md`, the canonical comms spec `docs/specs/20260715-1722-01-agent-comms-convention.md` (the earlier `-2133-02-` draft has been removed), and `research/opencode/` (OpenCode inter-instance comms + runtime-artifacts references).
