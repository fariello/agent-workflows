...
- `Set` and `Order` are REQUIRED together when the IPD belongs to an ordered Set and MUST both be absent otherwise; one present without the other is an error.
  THE `Set` VALUE'S SETID IS LENGTH-BOUNDED (added 2026-09-23, plan `x75obw`; catalog invariant I-17, spec `2lcqno` N8 normative). The setid is the leading token before any parenthetical. A setid of more than 24 characters is a metadata ERROR reported as `IPD-M109`, which is a DEDICATED code rather than the generic `IPD-M104` metadata-field bucket, so the refusal is routable by a consumer and assertable by a test. A setid of 15 to 24 characters is a non-blocking ADVISORY carried in `LintResult.advisories` (also `IPD-M109`), leaving the disposition `conforming`: 14 or fewer is strongly preferred, and nothing about the warn band blocks a plan. The comparison is strictly `> 24`, because the longest setid in the corpus is EXACTLY 24 characters and an off-by-one would refuse a live record.
  THE TWO TIERS LIVE IN DIFFERENT PLACES, AND THAT IS A CORRECTNESS CONSTRAINT RATHER THAN A STYLE CHOICE. The ERROR is emitted by `ipd_schema.validate_metadata`, which is PURE, so it applies the shipped default maximum and needs no repository. The ADVISORY needs the per-repository cutover date (`cutovers.setid_length`) to suppress a pre-cutover plan, and reading it is I/O, so it is merged in `ipd_lint.lint_file` exactly as the review-escalation and durable-carrier rules are. A text-only `lint_text` call therefore cannot report the advisory, by design. A pre-cutover plan is SUPPRESSED rather than downgraded, for the reason recorded at `CITATION_ANCHOR_CUTOVER_DATE`: a downgrade is a no-op for a rule already advisory in both tiers, and mass advisories on plans nobody is editing are ones readers learn to skip. A terminal plan never reaches the check at all (`lint_text` returns the `legacy` disposition first).
...
- `Scope-Paths`, `Priority`, and `Work-Kind` are REQUIRED at the `pre-execution` checkpoint and for any plan whose `Status` is at the ready-to-execute tier (`approved`/`auto-approved`); they are OPTIONAL at every earlier drafting/review phase and on terminal (grandfathered) records. For `Scope-Paths`, see Section 4.5 for its grammar and Section 9.2 for the checkpoint rule. For `Priority` and `Work-Kind`, the shared vocabularies are `low|medium|high` (Priority) and `bug|feature|chore|security|followup` (Work-Kind); `grandfathered` is the recognized audit exemption sentinel.
- WHERE `Priority` AND `Work-Kind` ARE DECIDED (amended 2026-09-24, planprio `lkexaw`, maintainer ruling). Both are decided where the work is FIRST RECORDED, so that the ready-to-execute requirement above is a backstop that should never fire on a healthy path: (1) for backlog-originated work, on the backlog item, where `aw backlog new` REFUSES without `--priority` and `--work-kind` (there is no default, because a defaulted value is one nobody chose); (2) `aw ipd scaffold --from-backlog <id6>` INHERITS both from that item, together with its `Blocks-Release`, and records `From-Backlog`; an explicit `--priority`/`--work-kind` overrides the inherited value; (3) without `--from-backlog`, the scaffold IS the first recording, so it REFUSES unless both are passed. The `unresolved` sentinel remains the value `build_skeleton` writes when called with no values (the byte-pinned templates and hand authoring), where the gate refuses it. `aw ipd set` to a ready-to-execute status REFUSES a plan whose `Priority` or `Work-Kind` is missing or undecided, counting a value passed in the same call; `grandfathered` passes there as it passes the gate.
...
| `pre-execution` | `review-finalize` passes; no declared blocking question remains unresolved; the persisted lifecycle state authorizes execution; no action has an illegal pre-execution state; a `Scope-Paths` value is present (Section 4.5) - a plan with NO `Scope-Paths` field is a blocking error, a `Scope-Paths: grandfathered` marker is advisory-satisfied (non-blocking), and a real allowlist is validated against the Section 4.5 grammar (malformed is a blocking error); `Priority` and `Work-Kind` values are present - a plan missing either field or carrying an unresolved scaffold placeholder is a blocking error, while `grandfathered` is advisory-satisfied (non-blocking) and valid vocabulary values pass. This same `Scope-Paths`, `Priority`, and `Work-Kind` requirement also applies to any plan whose persisted `Status` is at the ready-to-execute tier (`approved`/`auto-approved`), so an approved plan cannot slip through without them. |
