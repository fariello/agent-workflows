...
| Skip the DETERMINISTIC verification (the V-evidence checklist) | Dropped | A run may not report success while any `V-*` item is unchecked or lacks concrete observed evidence. Enforced TODAY by `ipd_lint`'s `--phase pre-transition` checkpoint (`ipd_lint.py:694-724`: every `E-*` leaf `Execution state == performed`, every `V-*` leaf `Result == pass`, every `V-*` leaf non-empty `Observed evidence`) together with the `ipd_lifecycle` finalize gates, both of which read the repository's own artifacts and are therefore unreachable by any CLI flag. Section 4.2 SPECIFIES this same guarantee as `IPD-EXEC-V-EVIDENCE`, `IPD-EXEC-E-COMPLETE`, `IPD-EXEC-EV-BIJECTION`, and `IPD-EXEC-PRE-TRANSITION`, but those four codes are NAMES ONLY: verified 2026-09-05, each greps to ZERO files under `agent_workflows/`, as does every other non-`RUN-*` finding code in Section 4.2. Cite the shipped enforcers, not the codes, until the codes are bound. Amended 2026-09-05: the pre-amendment reason read "A command called `run` cannot claim success without verification", which is an appeal to the verb's name rather than a stated harm, and it was read as banning flags (see Section 2.1) rather than as requiring V-evidence. |
| Skip the INDEPENDENT VERIFIER TURN (a second model reviewing the work) | Retained, host-configurable | Distinct from the row above and NOT a bypass of it. The verifier turn is an additional model opinion; the deterministic V-evidence check runs either way, and when no verifier turn runs the DRIVER runs the suite itself and that observed result is the trust signal (`novalnomerge-01` / `evgi9n`). Whether a second opinion is worth its cost is a per-model judgement: some executors need no material correction, others benefit from a stronger checker. Until per-role model routing exists (`runprofile` Order 6 `kgpptv` plus `f2mrsw` E-03 make it configuration), a host-level control is the only available mechanism. |
| Audit only | The host-neutral checker, `aw runs verify-ledger <run-id>`, or `aw <host> run resume <run-id>` | Verification is a run-state operation, not another execution mode. |
...
aw <host> run resume <run-id>
aw runs show <run-id>
aw runs evidence <run-id>
aw runs verify-ledger <run-id>
...
- `--full-auto` implies `--unattended`. It does not imply `--allow-mixed`, `--allow-unverifiable`, `--unverifiable-ok`, `--with-dependencies`, permission bypass, network access, hook bypass, or human approval.
...
- `--allow-concurrent-driver` permits this run to integrate a verified lane to `main` WITHOUT holding the repository-scoped integration lock, and it TAKES A JUSTIFICATION STRING rather than being a bare boolean, for the same reason `--allow-uncovered-orchestrator-work` does: the risk it accepts is that a peer driver advances `main` between this run's completed validation and its publish, so the record must say WHY that was accepted and not merely that somebody accepted it. The justification is recorded in the run ledger beside the decision. WHAT THE LOCK DOES AND DOES NOT SERIALIZE, stated here because the distinction is the whole design: a repository holds at most one INTEGRATING driver, never at most one driver, so two runs may EXECUTE in parallel and this flag is irrelevant to that; only the publish to `main` is serialized. WITHOUT the flag, an integration whose peer holds the lock WAITS (bounded, naming the holder, reporting progress) and on expiry DEFERS with the lane preserved and re-attempted, never fails the lane, because a lane failed on a lock timeout would discard a completed validation, which is the exact cost the serialization exists to avoid. An UNPROBEABLE lock platform never refuses and never wedges: the acquire either succeeds or reports busy, and a crashed holder strands only the lock FILE, which the next acquirer rewrites, because the OS drops an `flock` when its holder dies. It acknowledges only that one risk, and it waives no other gate. Added 2026-09-22 by plan `vddpml`, after two unattended drivers in one checkout were measured invalidating each other's validations twice in one session.
- `--ack-spec-edits` is the unattended half of the spec-edit acknowledgement gate defined in Section 2.5c, and it TAKES A JUSTIFICATION STRING rather than being a bare boolean, for the same reason `--allow-uncovered-orchestrator-work` and `--allow-concurrent-driver` do: the risk it accepts is that an unattended run rewrites a contract every other plan is reviewed against, so the record must say WHY that was accepted and not merely that somebody accepted it. The justification is recorded in the run ledger beside the decision. It acknowledges only that one risk, and it waives no other gate.
...
Amended 2026-09-23 (maintainer decision of 2026-09-22, recorded in plan `kl18sz` OQ-01). This section is a NARROW CARVE-OUT from one sentence of Section 2.1 and it changes nothing else in this spec.

WHAT SECTION 2.1 SAYS AND WHY THAT REMAINS TRUE. That section states the `--on-integration-blocked` ladder "never applies to a genuine merge conflict, a stale base, a non-passing combined revalidation, or a scope violation, none of which repetition fixes", and that no rung "reclassifies a failure as a deferral". Every word of that stands. The prohibition's own stated REASON is that REPETITION does not fix those classes, and that reasoning is correct: re-running a merge that conflicted produces the same conflict.

THE DISTINCTION THE CARVE-OUT TURNS ON IS RECOMPUTED VERSUS RETRIED. Re-derivation is not repetition. It does not re-attempt the merge, does not consult the ladder, does not spend a retry budget, and does not defer anything. It RECOMPUTES the incoming side's intended edit against the settled tree and writes that instead of merging stale bytes. That is a different operation with a different failure mode, so the premise behind Section 2.1's prohibition does not reach it. A carve-out phrased as "a conflict may be automatically handled when ..." would soften the prohibition and misstate this reasoning; the rule below is phrased on the recomputation.

THE RULE. A positively classified RECORDS-ONLY ORTHOGONAL-FRONT-MATTER conflict is RE-DERIVED against the settled tree rather than merged, and EVERY OTHER CONFLICT CLASS REMAINS TERMINAL ON ITS FIRST ATTEMPT. In particular a stale base, a scope violation, and a non-passing combined revalidation are untouched by this section and stay terminal exactly as Section 2.1 requires, as does any genuine merge conflict that does not positively classify.

THE CLASSIFICATION IS A POSITIVE PROOF AND THE GATE IS ALL-OR-NOTHING. Re-derivation is reachable only when EVERY conflicting path in the set is PROVED to be this shape. The classifier is three-valued and reports UNKNOWN for anything it cannot prove, a partially classified set refuses ENTIRELY rather than being half-applied, and a refusal leaves the main checkout exactly as it was found. The failure mode of the mechanism is therefore a refusal, never a wrong write, and that bounded hazard is the basis on which the amendment was accepted.

THE SHAPE, stated as the conjunction it is. The path is under the records tree and carries a MEASURED record type; the INCOMING side added only front-matter keys on an explicit ALLOW-LIST, removed none, and changed none; its non-bullet metadata lines are unchanged; its body change is history-bullet insertion only; the TARGET side really did transition (its status changed or the file moved lifecycle directory); and no allow-listed key it adds already exists on the target with a different value.

THE ALLOW-LIST IS AN ALLOW-LIST, NEVER A DENY-LIST, and this is the requirement that keeps the carve-out safe. A rule of the form "any key except status" silently admits every field carrying lifecycle, gate, identity or attestation meaning: a readiness attestation only a review may write and the auto-approve predicate reads first, a human approval record, a release gate, a dependency declaration that changes queue ordering, and the identity fields. An integration able to write any of those automatically would be strictly worse than the conflict it resolves. The keys eligible for re-derivation MUST therefore be enumerated, and widening the enumeration is a deliberate, visible change.

STATUS AND THE LIFECYCLE DIRECTORY ARE NEVER WRITTEN BY THIS PATH. Re-derivation writes allow-listed keys onto the target's CURRENT content; it moves no file and writes no status line. A record that sits in a terminal lifecycle directory on the target can never acquire a non-terminal status through this mechanism, which is the anti-revert guarantee the measured incident requires.

IT IS REPORTED AS ITS OWN OUTCOME, NEITHER A PLAIN SUCCESS NOR THE EXISTING REFUSAL. A re-derived integration is a SUCCESS (the incoming intent is on the target) whose content was written by the runner rather than carried by the merge, so it carries a distinct reported outcome and the run record states that content was RECOMPUTED rather than merged. Reporting it as an ordinary integration would hide the one event an auditor most needs to see; reporting it as the measured-and-refused kind would assert a refusal that did not happen.
...
### 2.5c Spec-edit acknowledgement gate

A run whose queue declares edits to `.spec.md` files must pause for explicit acknowledgement before any agent turn or host session is launched. A spec is the contract against which other plans and implementations are reviewed and verified, so unattended modification of a spec without consent risks silently altering shared contracts.

- **Trigger**: any queued item whose `- Scope-Paths:` names a `.spec.md` file (computed by `runner_shared.spec_impacts_for_queue`). Queues with no declared spec edits are completely unaffected.
- **Interactive (TTY) behavior**: prompts the operator with a `y/N` question listing each declaring plan `id6` and the declared `.spec.md` paths. Only `y` or `yes` (case-insensitive) proceeds; an unanswered prompt or any other response refuses the run.
- **Unattended behavior**: in unattended mode (`--unattended`, `--full-auto`, or when no TTY is available), the run is refused unless `--ack-spec-edits <justification>` was supplied on the command line.
- **Durable refusal record**: a refusal records a durable refusal on every declaring plan item using refusal code `spec-edit-unacknowledged`, persists state and an event to `events.jsonl`, and prints a clear remedy stating how to acknowledge the spec edits interactively or via flag.
- **Deterministic and operates under `--prepare-only`**: because declared scope paths are known from plan metadata without running host turns or spending model calls, this gate executes deterministically at run initialization and applies under `--prepare-only` as well, preventing unacknowledged queues from being prepared and later resumed without gating.
...
- Newly generated IPDs are ALWAYS reported as generated next actions and never join the frozen run (the frozen queue is what resume reads, and no producer exists; see plan `hzdq8y`).
...
- No override may create human approval, ignore a gate, broaden scope after execution starts, skip the DETERMINISTIC V-evidence verification of Section 4.2, include uncommitted pre-existing edits in a run commit, push, or bypass git hooks. Clarified 2026-09-05: "skip verification" here means the V-evidence checklist, not the optional independent verifier turn; see the two Section 1.3 rows and Section 2.1.
...
Satisfaction depends on the CONSUMING ACTION. An `execute` turn and a `review` turn consume a
prerequisite differently, so `executed:` has two rows below rather than one. The distinction is
normative: an execute turn imports, calls, or builds on the prerequisite's code, so that code must
genuinely exist and be verified; a review turn reads and critiques plan prose, writing no code and
importing nothing, so it needs only that the prerequisite's DESIGN is settled.

| Edge | Consuming action | Satisfied when |
| --- | --- | --- |
| `executed:<id6>` | `execute` | The target resolves uniquely to an IPD, is in `executed/` with status `executed`, passes terminal lint, and has valid deterministic execution/finalization evidence. If it is in this run, its current outcome is `verified`. |
| `executed:<id6>` | `review` | The target resolves uniquely to an IPD whose state is `executed`, `reviewed`, or `approved`. Terminal execution evidence is NOT required, because a review turn writes no code and therefore cannot be invalidated by an unexecuted prerequisite. |
| `exists:<type>:<id6>` | any | Exactly one target of the named type exists and passes that type's structural checker. |
| `state:<type>:<status>:<id6>` | any | Exactly one target of the named type exists, passes its structural checker, and has exactly the required status. |

THE REVIEW-ACTION ROW IS DELIBERATE AND WAS MEASURED, NOT INFERRED. Without it a Set cannot be
reviewed as a Set: in run `run-20260904T042705Z-1025943` a six-item all-`review` run of the
`wslayout` Set reviewed Orders 00 and 01, and the instant Order 01 reached `reviewed` the cascade
marked Orders 02 through 05 `fail-depend` (legacy `dependency-blocked`) with "prerequisite reached a non-success terminal
state", making a review-mode Set run impossible to complete. Requiring terminal execution evidence
for a review turn would permanently serialize plan review behind plan execution, which is backwards:
review is what QUALIFIES a plan to execute.

THE RELAXATION APPLIES TO THE REVIEW ACTION ONLY, AND WIDENING IT FURTHER IS PROHIBITED. An
`execute` action MUST keep requiring a genuinely `executed/` prerequisite with its evidence; a
`reviewed` or `approved` target NEVER satisfies an edge consumed by an execute turn. The two rows
must stay distinguishable by the consuming action and by nothing else: not by queue membership, not
by which host is running, and not by whether the target happens to be in the current run. An
implementation that lets the same edge be satisfied or refused depending on queue membership is the
evadability defect this section exists to prevent.
...
- the item fails; or
- a generated child is recorded as a generated next action and excluded from the run.
...
| `graduated` | Yellow skip; validate that its handoff artifacts exist and still carry `From-Backlog` plus any inherited `Blocks-Release`. | Same. | Re-graduating it (producing duplicate IPDs for an item whose design is already handed off). |
...
The terminal status for a graduated item is `graduated`, never `done`. `graduated` means the design is handed off and the artifacts exist; `done` means the code is written and validated. Closing an item `done` because its plans exist is a false claim of implementation, and it also drops the release gate, which is why `aw backlog set done` fails closed on a release-blocking item. A run may produce more than one IPD, and a spec when the work needs one, because a single item's design does not always decompose into exactly one plan.
...
PROVE  = aw runs verify-ledger <run-id>
```
...
No other FINDING may abort the whole queue. Clarified 2026-09-05: this constrains findings, not
normal queue exhaustion. A run legitimately ENDS, without any finding and without aborting, when no
queued item is dispatchable and nothing further can change that (the drivers mark the remainder
`fail-depend` (legacy `dependency-blocked`) and leave the loop). That is the correct behavior and not a seventh abort class:
there is no work left to do and no wait that would help. What the drivers get WRONG there is only the
status they write - `fail-depend` (legacy `dependency-blocked`) is terminal, so an item is never re-examined even if its
prerequisite later succeeds within the same run (tracked in backlog `nueip1`), and a deferred
orchestrator can never finalize (backlog `kxkc04`). A deliberate operator stop is likewise not an
abort; it exits 0 per `c4gd2h` A1/A4.
...
`RUN-COMMIT-GATEWAY`'s `git commit --no-verify` was qualified with `git` on 2026-09-05 so the row
cannot be read as governing a RUNNER flag that merely selects whether an independent verifier turn
runs. The two senses of `--no-verify` are unrelated: this row is about the git flag that bypasses
pre-commit hooks, which stays prohibited. See Section 2.1 and the two Section 1.3 rows.
...
| Check | What is inspected | Pass criterion | Exact failure message and recovery command | Action |
| --- | --- | --- | --- | --- |
| `SPEC-PLAN-COUNT` | Baseline/current IPD inventory and `From-Spec` links | At least one new IPD links to the spec for this authoring action, every one carries `From-Spec`, and no duplicate active plan already existed for the same phase | `[SPEC-PLAN-COUNT] Spec <id6> produced <count> new linked IPDs; expected at least one, each carrying From-Spec. Quarantine the authoring action, reconcile duplicates, run aw check all, then: aw <host> run <id6>` | FAIL ITEM after containment; ABORT RUN only for duplicate stable identity ambiguity |
...
For `blocked`, `parked`, `graduated`, and `done` items, the checker validates the current record and proves that the run started no host session and made no action-owned repository change. A malformed blocked gate is a preflight failure, not a skip.
...
| `NONRUN-SKIP-REASON` | Per-item outcome | Outcome is `skipped` with stable reason `type_not_runnable` and the type-specific guidance | `[NONRUN-SKIP-REASON] <path> lacks a valid non-runnable disposition. Rebuild the report with: aw runs verify-ledger <run-id>` | FAIL ITEM |
...
further items cascaded to `fail-depend` (legacy `dependency-blocked`), and a run spent 2h 10m and $55.02 producing no integrated
...
A run MAY additionally write per-invocation resource telemetry. Its contract is deliberately the OPPOSITE of the ledger's above, and every clause below exists to keep the two from being confused:

- **It is DERIVED and BEST EFFORT, never authoritative.** The hash-chained ledger is the run's record and replaying it is authoritative; telemetry is an observation ABOUT the run. Telemetry is not chained, not required, and not replayed. No consumer may treat it as evidence of what a run did.
- **It is NEVER A GATE, and no failure of it may change a run's outcome.** A telemetry fault (an unwritable tree, a full disk, a probe that raises or times out, a collector that fails to construct or to close) MUST leave the run's exit status, item statuses, state transitions, merge decisions, and cleanup report byte-identical to an uninstrumented run. It may not turn successful work into failure and it may not mask an existing failure. This is the strongest requirement in this subsection and it is what makes the feature admissible at all: it wraps the launch of the process that does the actual work.
- **One stream per INVOCATION, and identity is recorded rather than inferred.** An item may be attempted repeatedly, may be verified by an independent turn, and may be resumed in the same run directory, so a stream records its own run id, position, item id, Set, attempt, PHASE, and host as FIELDS. Phase is explicit; it is not inferred from a filename. Two invocations of the same item, attempt and phase separated by a resume MUST NOT share an identity, which means the identity carries a per-invocation component (the persisted attempt counter alone does not reset across a resume and therefore cannot distinguish them).
- **Location is RESOLVED, not composed.** Telemetry lives inside the resolved run directory, under a `telemetry/` subdirectory, so a relocated records root (a `companion` or `home` `records_backend`) places it correctly. It is NOT inside the reserved `analytics/` namespace, which holds the DISPOSABLE derived cache: a run's own observation of itself must not sit in a tree whose documented contract is that deleting it loses nothing.
- **DEFAULT ON for the two lifecycle events, DEFAULT OFF for periodic sampling.** Bracketing an invocation with a start and an end event is what makes its duration interpretable at all and costs no measurable time. Periodic sampling is unbounded in count and is therefore opt-in.
- **DISABLED MEANS NOTHING ON DISK.** With telemetry disabled no directory and no stream file is created, so "disabled" is observable rather than inferred from an empty file.
- **PRIVACY: codes, counts and numbers only.** Telemetry carries numeric observations, closed-vocabulary labels, and structured failure CODES. It carries no prompt text, no child stdout, no session transcript, no absolute path, no network address, no environment value, and no raw hostname. The node identifier is a salted PSEUDONYM, correlatable only within one machine and one salt generation, and it must never be described as anonymous.
...
5a. Any divergence between the requested order and the executed order MUST be announced at queue build, before the first host session, naming both orders and, per moved item, whether a declared dependency or a lower-ranked tiebreaker caused the move. The executed order is announced whether or not it diverged, and the comparison is recorded in durable run state, so the order is auditable from the run record rather than reconstructible from event timestamps.
...
8. For an edge consumed by an `execute` turn, `executed` status text alone does not satisfy an `executed:` edge. Terminal structure and deterministic execution/finalization evidence must pass. For an edge consumed by a `review` turn, section 2.9's review-action row applies instead: `executed`, `reviewed`, or `approved` satisfies it and no execution evidence is required. The two readings are distinguished by the consuming action and by nothing else.
9. A `superseded`, `not-executed`, `blocked`, `parked`, or `deferred` target satisfies only an explicit `exists:` edge or a matching `state:` edge. It never satisfies `executed:`, under either consuming action. The review-action relaxation in section 2.9 widens the satisfying set to `reviewed` and `approved` only; it does not admit any state in this rule.
10. Newly generated IPDs are recorded in the result as generated next actions and never join the frozen run. A generated IPD must still resolve `Item-Dependencies` before review-readiness.
...
Human output uses color only on a TTY and never makes color the sole carrier of meaning. Lifecycle color, semantic glyph, and ASCII fallback are defined by spec `uonrjg` (`.aw/records/specs/approved/20260913-uonrjg-01-uonrjg-cross-artifact-lifecycle-symbols-and-ansi-status-styling.spec.md`, Section 5), which supersedes earlier per-surface palettes for presentation while leaving this section's outcome vocabulary, exit codes, and reporting columns unchanged.
...
| 130 | An UNCOOPERATIVE interruption only (the process was cut without completing a graceful wind-down); durable state is resumable unless reconciliation reports unknown outcome. Narrowed 2026-09-05: a DELIBERATE stop exits **0**, not 130. Spec `c4gd2h` A1 and A4 both require exit 0 for a graceful stop (a single SIGINT is a level-1 graceful REQUEST, not an interruption, per `c4gd2h` R12), and the shipped code implements that: `runner_stop.deliberate_stop_exit_code` (`runner_stop.py:762-788`, whose docstring cites A1/A4) is called at `oc_runipd.py:7142` and `agy_runipd.py:4233`, while 130/143 survive only on the uncooperative path (`oc_runipd.py:7965`). The unnarrowed row said "user interruption" flatly and would have made an implementer return 130 for a graceful Ctrl-C, failing `c4gd2h` A1. |
