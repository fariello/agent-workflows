...
marked Orders 02 through 05 `dependency-blocked` with "prerequisite reached a non-success terminal
...
| --- | --- | --- | --- |
| `open` | Author the artifacts the item needs: any spec it requires (created and approved under the authority of this request, not deferred to a separate round trip) plus one or more conformant IPDs, each carrying `From-Backlog` and inheriting `Blocks-Release`; set the backlog item to `graduated` using the handoff receipt; verify. | Same. Do not run the generated IPDs unless `--follow-generated` is present. | Setting the item `graduated` before a conformant handoff exists, or setting it `done` (which would claim implementation that has not happened). |
...
`dependency-blocked` and leave the loop). That is the correct behavior and not a seventh abort class:
there is no work left to do and no wait that would help. What the drivers get WRONG there is only the
status they write - `dependency-blocked` is terminal, so an item is never re-examined even if its
