...
- AMENDED 2026-09-08 (`awmetastore` ingest): a `<model>` token identifies a model TOGETHER WITH ITS REASONING EFFORT, and the effort suffix is available to EVERY family rather than to `gpt56` alone. As first written this vocabulary encoded effort only for `gpt56` (`gpt56medium`, `gpt56high`), so a genuine high-effort Sonnet or Gemini report COULD NOT BE NAMED: `parse_name` rejects an unknown facet and `validate_frontmatter` rejects the `model:` key, so the contract blocked ingesting real artifacts. Added: `gpt56solhigh`, `gemini31prohigh`, `gemini38flash`, `gemini38flashhigh`, `sonnet5high`, each a configuration that produced a report in the `awmetastore` set. WHY EFFORT BELONGS IN THE IDENTITY: effort materially changes the output, and a comparison set exists precisely to hold two such outputs side by side, so two efforts of one model must not collide on a single name. A NEW MODEL IS NOT A SPELLING VARIANT: `gemini38flash` is added as its own token and deliberately NOT normalized onto `gemini36flash`, because the validator's closest-match hint is string proximity and not a claim about model identity; collapsing them would file a report under a model that did not write it. The canonical set now lives in `agent_workflows/research_contract.py` `MODELS`, with drift handled in `MODEL_NORMALIZATIONS`; extend it there (a token plus a one-line justification in review) and amend THIS section in the same change, since E3 makes the vocabulary an enumerated `[Must]`.
- AMENDED 2026-09-20 (`hostskill` ingest): added `gemini31prodeepthink`, Google's "Deep Think" variant of Gemini 3.1 Pro. It is a DISTINCT reasoning configuration and NOT a spelling of `gemini31prohigh`, so it is its own token per the rule above; the `hostskill` set contains reports from both configurations, which is exactly the collision that rule prevents. Occasioned by research report `i5gj61`, whose true author could not be recorded at all: `aw research mv --model gemini31prodeepthink` refused with "unknown model ... did you mean 'gemini31pro'?", so the artifact was filed with an EMPTY `model:` facet. THE CLOSED-LIST DESIGN IS ITSELF A FILED DEFECT (backlog `6b9zd9`, bug, gating next release): `normalize_model` is a bare membership test with no escape hatch, so every future model or reasoning tier (Gemini 4, GPT-5.5, DeepSeek, a new variant) is refused until someone edits Python AND amends this section. This is the SECOND occurrence, the first being the 2026-09-08 amendment above, which records the same symptom ("blocked ingesting real artifacts"). The accepted fix is to WARN rather than REFUSE, move the known list out of code into an editable data file (the `.aw/config/local-leaks-allowlist.toml` precedent), and ship a CLI verb that adds a model, at which point requirement E3's "enumerated" obligation must change too. This amendment is therefore an INTERIM unblock under the existing rule, not an endorsement of it.
...
- AMENDED 2026-09-23 (`promptid6` `ubac5n`): STAGED PROMPTS ARE NOW AN ACTUAL ADOPTER of the clustered `<id6>` grammar, not a named future one. Section 7's implementation order still listed `prompts/` among "subsequent adopters" (item 3), and this amendment records that the adoption HAPPENED so a reviewer checks a prompt name against a spec that no longer exempts it. `aw prompts new` mints an `<id6>` through `artifact_core.mint_id6` and emits `YYYYMMDD-<setid>-NN-<id6>-<slug>.prompt.md` via `artifact_naming.build_clustered_name`; `aw check prompts` enforces that grammar for a prompt dated at/after the repository's `cutovers.prompt_id6` boundary (resolved by `config.resolve_cutover_date`, module fallback `check_engine.PROMPT_ID6_CUTOVER_DATE`), GRANDFATHERING every pre-cutover legacy `YYYYMMDD-HHMM-NN-<slug>.prompt.md` name (all 17 measured on 2026-09-20 remain valid), which are converted on demand with `aw rename prompts <legacy> --to-id6`.
  THIS REVERSES A RECORDED DECISION, and the reversal is recorded here rather than presented as a bug fix: IPD `jxqdcw` OQ-02 deliberately chose the legacy id6-less form for prompts and corrected this repository's prompts README to match it, on the evidence that the whole on-disk corpus used that shape. THE AUTHORITY FOR THE REVERSAL IS THE MAINTAINER'S EXPLICIT INSTRUCTION of 2026-09-20, verbatim: "I want it to look like all the other files. YYYYMMDD-<setid>-<num>-<id6>-<slug>.<type>.md. plainlang = <setid> for THIS artifact. prompt = <type>." THE REASON IT MATTERS is not cosmetic uniformity: every `aw` verb resolves an artifact BY `<id6>`, so an id6-less prompt is invisible to `aw find`, cannot appear in a `consumed-by` or `From-*` field, and cannot be paired with the research report it produced except in prose, which is precisely the pairing Section 5.3's comparison scaffold puts at `NN=00`.
  WHERE A PROMPT'S id6 LIVES DIFFERS FROM EVERY OTHER TYPE, and that difference is contractual. A spec or plan declares `- Id:` as a front-matter bullet; a prompt declares `Id:` INSIDE its single leading `<!-- aw-prompt: ... -->` comment, because approved spec `20260808-1958-01-prompt-purity-lint` (R1/P4) forbids any other content before the prompt body and a bullet would render as visible text in a pasted prompt. `NN` in a prompt name is therefore ORDER WITHIN THE SET, as in every other clustered name, and NOT the per-minute sequence the pre-`ubac5n` prompt names carried.
...
- 2026-09-23 note (aw specs): IPD ubac5n (promptid6): AMENDED section 5.4 and corrected section 7's adopter list: STAGED PROMPTS ARE NOW AN ACTUAL ADOPTER of the clustered id6 grammar, not a named future one. aw prompts new mints an id6 via artifact_core.mint_id6 and emits YYYYMMDD-<setid>-NN-<id6>-<slug>.prompt.md via artifact_naming.build_clustered_name; aw check prompts enforces it for a prompt dated at/after cutovers.prompt_id6 (resolved by config.resolve_cutover_date, module fallback check_engine.PROMPT_ID6_CUTOVER_DATE=20260921) and GRANDFATHERS all 17 pre-cutover legacy YYYYMMDD-HHMM-NN names, which convert on demand with aw rename prompts <legacy> --to-id6. WHY THE AMENDMENT IS REQUIRED RATHER THAN OPTIONAL: requirement E3 makes the type vocabularies an enumerated [Must] and section 7 listed prompts among subsequent adopters, so without this edit the next reviewer would check a prompt name against a spec that still exempts it. THIS REVERSES A RECORDED DECISION: jxqdcw OQ-02 deliberately chose the legacy id6-less form and corrected the prompts README to match it; the authority for the reversal is the maintainer's explicit instruction of 2026-09-20 (verbatim: 'I want it to look like all the other files. YYYYMMDD-<setid>-<num>-<id6>-<slug>.<type>.md'). The reason it is not cosmetic: every aw verb resolves an artifact BY id6, so an id6-less prompt is invisible to aw find and cannot be cited by the research report it produced. A PROMPT'S id6 LIVES IN ITS SINGLE <!-- aw-prompt: ... --> COMMENT, not in a - Id: bullet, because approved spec 20260808-1958-01-prompt-purity-lint R1/P4 forbids visible non-prompt content above the body. NN in a prompt name is now ORDER WITHIN THE SET. NOTE the spec's - Status: implemented was NOT changed: this amends the BODY only and claims no implementation of the spec itself.
- 2026-09-08 note (aw specs): AMENDED section 5.4: the <model> vocabulary now generalizes the REASONING-EFFORT suffix to every model family, and gains gemini38flash as a distinct model. Amended while ingesting the awmetastore comparison reports from .aw/inbox/ on maintainer instruction (2026-09-08); the spec edit ships in the SAME change as the code, per E3 making this an enumerated [Must]. WHAT WAS BROKEN: effort was encoded for the gpt56 family ONLY (gpt56medium/gpt56high) with no equivalent for Sonnet or Gemini, and the facet is validated in TWO places (parse_name research_contract.py:328-332 rejects the filename; validate_frontmatter :454-460 rejects the model: key), so four real reports could not be named or indexed at all. Measured before the change: normalize_model rejected gpt56solhigh, sonnet5high, gemini31prohigh and gemini38flashhigh, suggesting gpt56/sonnet5/gemini31pro respectively, and suggesting gemini31pro for gemini38flashhigh, which is string proximity and not model identity. ADDED to MODELS: gpt56solhigh, gemini31prohigh, gemini38flash, gemini38flashhigh, sonnet5high, plus MODEL_NORMALIZATIONS entries for the hyphenated spellings. RATIONALE: effort materially changes the output and a comparison set exists to hold two such outputs side by side, so two efforts of one model must not collide on one name. DELIBERATELY NOT DONE: gemini38flash is NOT normalized onto gemini36flash, because they are different models and collapsing them would file a report under a model that did not write it; the maintainer was offered that cheaper option and rejected it in favor of extending the vocabulary properly. Existing tokens and their normalizations are unchanged, so no already-named artifact is invalidated, and an unknown model (llama99) is still rejected. Tests added to tests/test_research_contract.py covering the four tokens, the drift spellings, and the 3.8-is-not-3.6 distinction.
