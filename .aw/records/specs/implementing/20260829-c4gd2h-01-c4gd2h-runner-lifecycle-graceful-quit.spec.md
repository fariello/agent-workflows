...
- R12. Ctrl-C has TWO paths, chosen by whether a human can actually answer a question, and both must reach level 1 on a first press. AMENDED 2026-09-09 on the maintainer's decision; the pre-amendment text read "First SIGINT (Ctrl-C) requests level 1. Repeated SIGINT escalates 1 -> 3 -> 4, with a printed hint that pressing again stops harder", which described only the second path and was contradicted by shipped behavior on a terminal.
  - R12.1 INTERACTIVE (a real terminal on BOTH the input and the output stream, and no `AW_NONINTERACTIVE`/`CI` signal): the driver prints a four-choice menu and waits for a choice: resume, finish the current item then clean up and exit (level 1), clean up and exit (level 4), or exit leaving a mess (level 4). A repeated Ctrl-C during the prompt stops asking and cleans up, so an operator pressing harder is never met with another question.
  - R12.2 NON-INTERACTIVE (anything else, including piped output, an unattended run, and CI): the escalation LADDER governs. The Nth SIGINT requests `SIGINT_LADDER[N-1]`, i.e. 1 -> 3 -> 4, with a printed hint that pressing again stops harder, and further presses hold at the terminal rung.
  - WHY THE MENU IS THE PREFERRED INTERACTIVE PATH, since this reverses a requirement rather than merely relaxing it. The ladder satisfies R12's letter but defeats R16's purpose on a terminal: its levels are UNDISCOVERABLE, so an operator who wants "finish this item and stop" cannot learn that this is level 1, must guess, and cannot change their mind because escalation is monotonic (R9). The menu states the options, reaches level 1 EXPLICITLY, and adds a resume choice the ladder cannot express. It therefore delivers R12's INTENT (a first press can stop gently) through a better mechanism, and it is why backlog `1m3nul` filed the levels as undiscoverable in the first place.
  - THE PATH CHOICE IS A SAFETY PREDICATE, NOT A PREFERENCE, and it MUST require the OUTPUT stream to be a terminal as well as the input stream. Requiring only the input stream is a measured hazard: a parent that spawns the driver with its output PIPED but its input INHERITED lets the driver see a terminal, print the menu where nobody reads it, and then block forever inside a signal handler while holding the run lock, which blocks the operator's own escape path. The identical predicate error elsewhere in this repository wedged a finalize for 1h49m. An explicit `AW_NONINTERACTIVE` or `CI` setting therefore forces R12.2 regardless of the streams, and it overrides any force-interactive override.
  - FALLING BACK IS FAIL-SAFE: when the menu is not safe to show, R12.2 runs, so a first press still requests level 1 and a stop is always requested. Refusing the menu must never mean refusing to stop.
...
- Note (2026-09-21, added by plan `2iye0e`; the resolution above STANDS and is not reopened): the two plans this resolution cites as a PRECONDITION are retired, so that sequencing clause is unsatisfiable as written. `7p9n2v` (Phase 3) and `58ha43` (Phase 4) both sit in `.aw/records/plans/superseded/`, retired 2026-09-02, and `58ha43` was retired UNLANDED with no successor for its main deliverable. The accessors it would have created, `platform_state.state_home` and `platform_state.checkout_state_root`, DO NOT EXIST on `main`: there is no `agent_workflows/platform_state.py`, so a reader should not go looking for them. NOTHING IS BROKEN BY THIS. The resolved ANSWER is the part that matters and it is fully satisfied today: the flag is per-machine control state, it lives inside the driver's run directory beside `driver.lock`, and it is resolved through the drivers' own single `state_root` accessor (`runner_stop.resolve_stop_request_path` takes that accessor as a parameter) rather than through any path constructed locally or worktree-relative. The `dh0uno` fork this resolution guards against is independently closed by `ipd_lifecycle.checkout_control_root`, which collapses every worktree of a checkout onto one control root via `git rev-parse --git-common-dir`. What remains outstanding is only the out-of-repo RELOCATION itself, which is now tracked as backlog item `e820ka`; because the flag already resolves through the one accessor, that relocation would move the flag with it and require no change here.
