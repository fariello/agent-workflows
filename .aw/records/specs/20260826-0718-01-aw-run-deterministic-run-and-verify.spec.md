...
- 2026-09-07 note (aw specs): AMENDED 2.9 (and rules 8/9 of 5.4) ON MAINTAINER RULING 2026-09-07: an executed: edge consumed by a REVIEW turn is satisfied by a target in executed, reviewed, or approved, with no execution evidence required; an EXECUTE turn is unchanged and still demands a genuinely executed/ prerequisite with its evidence. WHY THE SPEC MOVED RATHER THAN THE CODE: plan 03ie04 (OQ-04, PR-001) found the three-state acceptance already present in oc_runipd.edge_satisfied (:3346, 'allowed = ("executed",) if is_exec else ("executed","reviewed","approved")') and in SUCCESS_STATES (:327) while 2.9 sanctioned only the single executed state, so shipped code and an approved spec disagreed. The maintainer ruled the relaxation IS the intended contract and that the spec is amended FIRST, before 03ie04 makes the external-target branch read the status field, so behavior never contradicts an approved spec. EVIDENCE THE RELAXATION IS CORRECT, not merely convenient: run run-20260904T042705Z-1025943 lost a six-item all-review wslayout run when Order 01 reached reviewed and the cascade marked Orders 02-05 dependency-blocked, i.e. requiring execution evidence for a review turn permanently serializes plan review behind plan execution, which is backwards. tests/test_runner_item_dependencies.py:891 already pins the in-run half of this behavior and passes today, so the amendment sanctions shipped, tested behavior for the in-run case and authorizes 03ie04 to extend the same reading to the external-target case. NOT WIDENED: rule 9's superseded/not-executed/blocked/parked/deferred set still never satisfies executed: under either action, and the two readings are distinguished by the consuming action and by nothing else (not queue membership, not host), because a queue-membership-dependent answer is the evadability defect revgate 7nkcgp closed. Section 2.1 was deliberately NOT amended in this change: the two ladder flags plan 51vw4y needs cannot be declared before they are registered, since tests/test_run_flag_surface.py binds spec and code bidirectionally, so that amendment lands inside 51vw4y's own execution as one atomic change (maintainer ruling, same date).
