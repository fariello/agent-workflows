...
`run resume <run-id>`, not a flag; `aw runs verify-ledger <run-id>` is not a leaf (the leaf is
`verify-ledger`, and `verify` is silently absorbed as a target token, so that spelling EXITS 0 having
verified nothing); `aw <host> prompt` does not exist (the nearest shipped surface is
...
- 2026-09-06 note (aw specs): AUDIT + SIX CORRECTIONS (2026-09-05), on the maintainer's instruction to align the spec with its research source and raise anything open to interpretation. METHOD: diffed this spec against the external source it was promoted from; 1076 of 1187 lines are BYTE-IDENTICAL to .aw/records/research/20260829-runverify-00-ig9bai-...gpt56.reference-research.md, 136 of those carry normative language, there is no review record for this spec, and approval commit aa0a6a26 has an empty body. So those lines were never independently reasoned about here, and the --no-verify line corrected earlier today was a SAMPLE, not an outlier. THE STRUCTURAL FINDING, near-perfectly correlated: every section a maintainer later amended (2.4a, 2.5a, 5.4, 1.3, 2.1, 2.6, the header) is honest about its gaps and largely ships; the sections still identical to the external output either do not ship or contradict something that does. CORRECTED HERE. (1) MY OWN DANGLING CITATION, the most important: this morning's amendment rested its safety argument on IPD-EXEC-V-EVIDENCE and three siblings, and all four grep to ZERO files under agent_workflows/. The reasoning was sound but the citation was vapor, which is the very defect this audit exists to catch. Both places now cite the SHIPPED enforcer (ipd_lint.py:694-724 pre-transition checks plus ipd_lifecycle finalize gates) and say plainly that the codes are names not yet bound. (2) RUN-COMMIT-GATEWAY at 4.2 still said bare --no-verify, the surviving residue of the conflation fixed at 2.1/2.6/5.8 earlier today; now qualified 'git commit --no-verify'. (3) unknown_outcome: spec c4gd2h Section 0.0 OWNS the term, names THIS spec's source research explicitly, and requires adoption to REFERENCE rather than restate it because two definitions of one token violate GUIDING_PRINCIPLES P8; this spec restated it, so the row now cross-references c4gd2h and records that shipped code follows c4gd2h (runner_stop.py:1038, :1277-1281). (4) Exit 130 narrowed to UNCOOPERATIVE interruption only: c4gd2h A1/A4 require exit 0 for a graceful stop and runner_stop.deliberate_stop_exit_code implements that, so the flat 'user interruption' row would have made an implementer fail A1 on a plain Ctrl-C. (5) Exit 4 marked as an UNRECONCILED CONFLICT with the shipped run_cli.py:46-62 table (which uses 4=invalid evidence, 5=ledger corruption, 6=ownership conflict), recorded rather than silently resolved because no shipped surface emits this row today. (6) Header now warns that Section 4.2's finding codes are SPECIFICATION not shipped behavior (only RUN-* exists; ~49 other codes grep to zero) and that several Section 2.1 command spellings do not exist (--json unregistered; --resume ships as a positional subcommand; 'aw runs verify' is not a leaf and EXITS 0 having verified nothing; 'aw <host> prompt' absent; run unverifiable/--allow-unverifiable/--unverifiable-ok all zero), noting that six shipped recovery strings in run_evidence.py already transcribe the non-existent --resume spelling. NOT CHANGED, raised for the maintainer: --allow-unverifiable at 3.4 promises unattended contractless execution that the capability gate can never grant (measured: supports_commit_gateway and supports_deny_push are permanently False by design, host_sandbox_profile.py:527-555); --action plan's entire legal domain is unreachable because no selector reaches specs or backlog; the retry-budget precedence text asserts a repository-policy tier that does not exist and the whole budget is dormant (backlog trjfyy, dh3us4); SELECTOR-TYPE-CONFLICT greps to zero; 5.4's type rank is deliberately unimplemented per oc_runipd.py:3690-3693; and 4.1's 'exhaustive' ABORT RUN set does not cover the drivers' queue-wide dependency-blocked BREAK. Spec stays approved: every edit removes a contradiction or replaces an unbacked citation with a measured one.
