...
| Skip the DETERMINISTIC verification (the V-evidence checklist) | Dropped | A run may not report success while any `V-*` item is unchecked or lacks concrete observed evidence. This is enforced by `IPD-EXEC-V-EVIDENCE`, `IPD-EXEC-E-COMPLETE`, `IPD-EXEC-EV-BIJECTION`, and `IPD-EXEC-PRE-TRANSITION` in Section 4.2, which read the repository's own artifacts and are therefore unreachable by any CLI flag. Amended 2026-09-05: the pre-amendment reason read "A command called `run` cannot claim success without verification", which is an appeal to the verb's name rather than a stated harm, and it was read as banning flags (see Section 2.1) rather than as requiring V-evidence. |
...
- 2026-09-06 note (aw specs): AMENDED Section 1.3, 2.1, 2.6 and the 5.8 parity table on the maintainer's 2026-09-05 ruling, on TWO grounds the maintainer raised and this edit verified in-repo. GROUND 1, the rationale was not a reason: the dropped row read 'A command called run cannot claim success without verification', which the maintainer identified as equivalent to 'because I said so' - an appeal to the verb's NAME rather than a stated harm. GROUND 2, the maintainer's own reading was the correct one and was not what the text enforced: he understood it to mean every V-## item must be checked off, and the spec ALREADY enforces exactly that in Section 4.2 via IPD-EXEC-V-EVIDENCE ('every V item has a passing result, nonempty concrete observed evidence'), IPD-EXEC-E-COMPLETE, IPD-EXEC-EV-BIJECTION and IPD-EXEC-PRE-TRANSITION, all of which read repository artifacts and are therefore unreachable by any CLI flag. So the deleted sentence protected nothing the checker was not already protecting, while costing two real capabilities. THREE VERIFIED DEFECTS IN THE DELETED LINE. (1) PROVENANCE: it entered at the FIRST draft (3d6668af, whose own message calls it a 'Two-pass frontier-model design') VERBATIM from external model output, .aw/records/research/20260829-runverify-00-ig9bai-deterministic-run-and-verify-design.gpt56.reference-research.md:146; no review record for this spec exists anywhere in .aw/records/reviews/; and the approval commit aa0a6a26 has an EMPTY body. It reached 'approved' without ever being independently reasoned about, which is precisely the untrusted-external-input case AGENTS.md requires be evaluated on its merits. (2) IT CONTRADICTED AN APPROVED SPEC: 7ckptx R4.1c forbids any work from flipping Antigravity's --dangerously-skip-permissions default, because an unattended turn cannot answer an interactive prompt and the measured outcome is repeated failure or deadlock; it is dated, evidence-backed, pinned by tests/test_lane_permission_posture.py:315, and demands explicit supersession to change. Two approved specs therefore required opposite things. --dangerous is REMOVED from the prohibition and 7ckptx R4.1c is named as the controlling authority on host permission posture. (3) CONFLATED SENSES OF --no-verify: the GIT flag bypassing pre-commit hooks (correctly prohibited, and PRESERVED here in Section 2.1, RUN-COMMIT-GATEWAY, and 5.8 row 3, now spelled 'git commit --no-verify' so the two cannot be confused again) versus a RUNNER flag selecting whether a SECOND MODEL reviews the work. The latter is not a bypass: novalnomerge-01 (evgi9n) established that when no verifier turn runs the DRIVER runs the suite itself and that observed result is the trust signal. Banning it removed the only per-model verification control that exists, and the maintainer's operational need is real and measured (some executors need no material correction, others benefit from a stronger checker) while per-role model routing greps to ZERO today; kgpptv and f2mrsw E-03 will make it configuration. Section 1.3 now carries TWO rows that were previously one conflated row: skipping the deterministic V-evidence check is DROPPED, skipping the optional independent verifier turn is RETAINED and host-configurable. Spec stays approved: this is the DECISION an unexamined external-model line needed, and it removes a spec-versus-spec contradiction rather than introducing policy.
