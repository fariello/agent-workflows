...
- `--check` (and `--check --agent`): validate all trees AND evaluate the unintegrated-lane condition (F3a) from the run records, emit `Drift` records, exit `drift_exit_code` (0 clean / 1 any violation or stranded lane; 2 could-not-run). Fail closed.
...
- F3 `aw attention --check` fails closed (exit 1) on ANY of: missing required status; unknown native status; native status with no mapping; missing/unknown/malformed/contradictory gate fields; missing/malformed required history; duplicate ids; duplicate normalized paths; disposition-vs-terminal-status disagreement; invalid/unstable repo-relative path; unsupported encoding / malformed front matter; unreadable included file; included tree without a contract/mapping; an unclassified new tree; AND a STRANDED LANE (see F3a).
- F3a `aw attention --check` also fails closed on an UNINTEGRATED LANE: a driver-allocated lane that holds work (commits beyond its own base, or an uncommitted tree) which has NOT reached the integration target, and on a lane holding work whose landing cannot be determined. This is an ADDITION to F3's set, not a change to any existing condition: nothing that previously failed stops failing, and Section 8.6's "local and CI observe the same validity result" is preserved because both sides evaluate the same predicate over the same records.
  THE VERDICT COMES FROM THE RUN RECORD, NEVER FROM A FILESYSTEM WALK. The facts (the preserved lane branch, base, lane id and integration signal) are read from the durable per-run state a driver already writes; `SCAN_ROOTS` is NOT extended to `.aw/worktrees` or `.aw/records/runs`, and this clause must not be read as licensing that. A verdict derived from a live filesystem audit reports a RECOVERED run clean and so rewrites history, which is why the record is authoritative here.
  TWO EXCLUSIONS ARE NORMATIVE, because a check that fails on correct behavior is a check operators bypass. A lane owned by a LIVE driver process (`driver.lock` holder liveness, or a live lane owner record) MUST NOT fail the gate; and a lane whose work HAS reached the integration target MUST NOT fail it either, which requires a reachability test against the target and cannot be decided from "the lane holds commits beyond its own base" alone.
  ...
  THREE CONSTRAINTS ON THE CONTENT READING ARE NORMATIVE, because each of them was measured to be a hazard rather than a nicety. (a) A DIRTY LANE MUST NOT BE SILENCED BY IT: patch ids describe COMMITS only, so a lane whose commits all landed while its worktree still holds uncommitted changes MUST stay reported, and an implementation MUST gate the content conclusion on the lane being clean. (b) AN UNANSWERABLE READING MUST NOT RESCUE A LANE: if ancestry says the work is absent and the content reading cannot answer, the lane stays reported, since a false LANDED hides real loss while a false STRANDED costs only a row. (c) AN EMPTY COMPARISON IS NOT LANDED: "no commit is absent" MUST NOT be satisfied vacuously by there being no commit to compare. The composition is therefore strictly NARROWING and fail-closed: the content reading may only ever turn a reported lane silent, never the reverse.
  ONE LANE IS AT MOST ONE ROW. The reported set MUST be de-duplicated per lane BRANCH, not per (run, lane) pair: a lane touched by N runs is ONE thing needing ONE human act, and emitting N rows inflates the violation count and misrepresents the size of the problem. The retained row MUST carry the run evidence forward (how many runs named the lane, and which is newest) so collapsing loses no fact.
  A WORKTREE THAT IS NOT ON DISK MUST NOT BE RENDERED. A row naming a reclaimed directory sends the reader to inspect a tree that does not exist, so the worktree segment is OMITTED when the path is absent and rendered repository-relative when it is present (F8a continues to forbid an absolute path in every case). This existence check is a RENDERING decision only and is NOT an exception to the run-record-not-filesystem rule above: it decides whether one already-derived display field is printed, and never any part of the lane's VERDICT.
  This clause adds NO write of any kind. `aw attention` remains READ-ONLY (G3, Section 8.1): it reports the lane and names a recovery route, and performs no merge, deletion, or recovery.
...
- F8 The JSON output is versioned (`schema_version`, `mapping_version`) and sets `valid: false` with all violations when any included artifact is invalid OR any lane is stranded (F3a). The stranded-lane set is carried in a `stranded_lanes` key DERIVED from the same violation set that decides `valid`, so the payload's validity flag and its lane list cannot disagree. Adding the key is a `schema_version` bump.
- F8a No output surface (human board, `--agent`, `--json`) may contain an ABSOLUTE filesystem path for a lane. A lane is identified by its git BRANCH; a worktree is rendered repository-relative or OMITTED. The durable record's worktree field is an absolute path under the operator's home directory, and these surfaces are pasted into shared contexts, so printing it verbatim is forbidden.
...
- OQ10 The exact human-approval token mechanism for `reviewed -> approved` and the evidence-citation format for `implementing -> implemented` (Section 7 transition table). Candidates: an interactive confirmation, a `--approved-by <human>` flag recorded in history, or a signed marker. Phase 0 deliverable. OBSERVED (2026-08-09): the shipped v1 mechanism is a live-TTY interactive confirmation; in practice this forced a maintainer who had already approved in-session to re-run the approve command in their own terminal (the orchestrating agent has no TTY and cannot capture the chat approval). The floor's intent (an agent can never self-approve) held, but the capture ergonomics need refinement - a human-issued out-of-band token/marker that an agent harness cannot forge would keep the guarantee without the terminal round-trip. Tracked in TODO.md ("Consider and possibly implement").
...
- 2026-09-17 note (aw specs): AMENDED by plan pr5b0t (lanestrand-01): F3 gains F3a, the UNINTEGRATED-LANE condition aw attention --check now fails closed on, with the run-record-not-filesystem rule and the two normative exclusions (a LIVE run's lane, and a lane whose work reached the target); 8.1's --check line and the 8.3 payload gain stranded_lanes; F8 covers the new key and its schema_version bump; new F8a forbids an absolute lane path in any surface. ADDITIVE ONLY: no previously-failing condition stops failing, local-equals-CI is preserved, and the READ-ONLY guarantee is untouched.
