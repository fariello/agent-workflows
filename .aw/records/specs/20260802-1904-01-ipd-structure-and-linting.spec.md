...
- 2026-08-24 note (aw specs): Documented the LOCAL pre-commit executed-transition gate (Order dulzpy): a repo:local hook (python3 -m agent_workflows ipd-executed-gate, installed by aw install/setup-repo) that refuses a raw (non-finalize) plan->executed commit (hand-edited status or git mv into executed/) by requiring finalize evidence (the finalize transaction journal in .aw/state/) proving aw ipd finalize performed the transition. LOCAL best-effort only (--no-verify bypasses; proclint via aw check/doctor is the backstop); no CI. Realizes OQ-01 option B against the journal finalize actually leaves at commit time (DECISION 14-dulzpy-D1).
