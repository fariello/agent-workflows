...
- 2026-08-24 note (aw specs): Documented removal of the raw plan-terminal bypass (Order wezhxg): aw set/ipd set executed <plan> (and the done alias) now transparently DELEGATE into the gated aw ipd finalize (keyed on record_type==plans + target executed), requiring an attributed --actor; retirement (superseded/not-executed) + all non-plan + nonterminal transitions preserved. Post-transition attribution lint (IPD-S406) rejects a generic (aw set)/empty actor or empty summary on the newest terminal entry, forward-only, grandfathering the existing executed tree via Order oorry1's Scope-Paths cutoff.
