...
- 2026-08-24 note (aw specs): Documented the COMMIT-SCOPED untooled-status detector (proclint Order 79li67), the intermediate-transition sibling of Order dulzpy: a check.status-untooled rule (staged-vs-HEAD, only commit-changed plan files; executed/ and history-less types excluded; no whole-tree scan, no grandfathering) that flags a plan whose - Status: changed in the commit with no matching tool-authored ## Workflow history line for the new status. Surfaced via aw check/aw doctor (over changed files) AND a LOCAL repo:local pre-commit hook (python3 -m agent_workflows ipd-status-untooled-gate, installed by aw install/setup-repo) refusing the commit naming aw set <status> <id6>. Predicate A (textual, careless-omission only; evadable by a hand-edit that also adds a plausible line; --no-verify bypasses); LOCAL best-effort/backstop only, no CI. Covers the INTERMEDIATE transitions the ipdgates terminal gate does not.
