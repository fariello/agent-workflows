...
Install-target contract (fresh install and legacy compatibility):

- A FRESH install into a repository with no prior AW layout MUST create the physical `.aw/` hierarchy only (canonical system at `.aw/system/`, records/config/state per the resolved preset). It MUST NOT create a legacy `.agents/workflows/` tree in a fresh target.
- When `aw install`/`aw update` runs in a repository that still has a legacy `.agents/`-only layout, the tool MUST detect it and OFFER to migrate to `.aw/` (via the migration tool). If the operator declines, the tool MAY continue to update the legacy `.agents/` layout in place for the documented compatibility window, emitting the deprecation notice; it MUST NOT silently create a second, divergent layout. Readers resolve either layout during the window.
- Migration between layouts MOVES material (it does not copy-and-retain a twin; see Section 11.2). The three retained-material destinations (repository-tracked, private companion, or home `~/`) are selected by preset/`records_backend` and honored by both install and migration.
...
- A fresh install into a clean repository creates ONLY the `.aw/` hierarchy (no legacy `.agents/workflows/` tree); a legacy `.agents/`-only repository running install/update is detected and offered migration, and declining keeps the legacy layout updated in place with the deprecation notice (never a second divergent layout).
- Migration is a MOVE with no retained legacy twin; a completed migration leaves the repo with one logical layout (`aw attention` reports valid, no duplicate-id); the per-item move journal makes resume and rollback recover any interruption to fully-migrated or fully-legacy.
- The migration tool (`aw migrate-layout`) runs as a guided WIZARD by default (backend/destination, leftover disposition) and accepts a config file and/or command-line flags to answer those questions non-interactively for scripted/CI use, with no prompt that blocks a non-interactive run and no deletion without an explicit choice.
- The three retained-material destinations (repository-tracked, private companion repo, home `~/` outside any repo) are each end-to-end proven for both install and migration.
...
- 2026-08-16 note (aw specs): Recorded the maintainer's 5-point physical-.aw end-state as an explicit acceptance contract (Sections 11.3 + 13): (1) fresh install creates ONLY .aw/ (no legacy .agents/workflows in a fresh target); legacy .agents/-only repos are auto-detected on install/update and OFFERED migration, declining keeps legacy updated in place for the compat window with a deprecation notice (never a second divergent layout); (2/3) migration is a MOVE with no retained twin, one logical layout, journal-based resume/rollback; (4) migrate-layout runs as a WIZARD by default with config/flag overrides for non-interactive use; (5) the three retained-material destinations (repository-tracked / private companion / home ~/) are each end-to-end proven for install and migration. New IPDs (fresh-install .aw target + legacy auto-detect; migrate-layout wizard) and the tightened hnzr8v trace to these criteria. Awaits maintainer re-confirmation with the Section 11.2 move-contract revision.
- 2026-08-16 note (aw specs): Human maintainer RE-APPROVED the spec after reviewing the MOVE-verify-switch contract revision (Sections 3/11.1/11.2) and the 5-point acceptance contract (Sections 11.3/13): 'looks good, re-approved' (chat). The spec was never un-approved (approved->to-review is not a legal transition; the revision was recorded via aw specs note while it stayed approved), so this confirms the revised migration contract is human-approved and IPDs 14/15/16 may implement it. Recorded by opencode Opus 4.8.
- 2026-08-17 implementing (aw specs): awphysical Set executing/complete; advancing from approved to implementing ahead of the implemented transition.
- 2026-08-17 implemented (aw specs): Physical .aw/ hierarchy + move-based migration fully implemented and self-migrated onto this repo. All 16 awphysical child Orders + orchestrator executed/ with cited E/V; full serial suite 970 green with .aw/ present; wheel ships the nested .aw/system bundle; aw attention valid; sanitizer clean.
