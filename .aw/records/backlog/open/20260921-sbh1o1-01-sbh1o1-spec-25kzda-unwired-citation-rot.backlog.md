...
WHAT IS WRONG. Multiple artifacts cite spec `25kzda` at `:29` as reading 'the ledger is built but UNWIRED', including plan `i1hlgx` (its Concern, Step 0, F-2 and E-01) and backlog `zrzfkw`. That string is GONE: `grep -n -i 'unwired|built but'` over `.aw/records/specs/20260826-0718-01-aw-run-deterministic-run-and-verify.spec.md` returns nothing. The spec's preamble was rewritten on 2026-09-20 by plan `wenmg4`.
