...
SUGGESTED FIX. Reword row 2 to state the falsey set explicitly and name the predicate, for example: "NO_COLOR (any value, including empty) disables, UNLESS FORCE_COLOR is set to a FORCING value; FORCE_COLOR forces color on unless its value is one of empty, 0, false, no, off (case-insensitive, stripped), in which case it neither forces nor suppresses and detection proceeds normally." Add the five cells above to the worked-cases table beneath it, which currently enumerates only truthy FORCE_COLOR and so does not catch this.

NOT FIXED IN THIS TURN because plan n4xq3l Scope-Paths declares the uonrjg spec only, and editing a published contract doc is outside it; spec uonrjg A13 now records the discrepancy and instructs an implementer to follow the code.
