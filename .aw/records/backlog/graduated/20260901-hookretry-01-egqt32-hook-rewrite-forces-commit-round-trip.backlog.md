...
## Workflow history
- 2026-09-08 graduated (aw set): Graduated to plan lqly9m (Set hookretry, .aw/records/plans/pending/20260908-hookretry-01-lqly9m-...ipd.md), which carries From-Backlog: egqt32 and inherits this item's Blocks-Release: next. Status graduated (design handed off), NOT done: no code is written yet. PARTIALLY OBSOLETE, AND THE OBSOLETE PART IS THIS ITEM'S ENTIRE PRESCRIBED FIX LOCATION, so the plan is a deliberate narrowing rather than a transcription of Fix Layer 1. THIS ITEM'S RECIPE CANNOT WORK AS WRITTEN. It says to add the retry in offer_commit 'before the commit at :245' and to detect a rewrite by re-hashing files on disk. That commit call NO LONGER EXISTS: three commits on 2026-09-06 moved the actual git commit out of offer_commit entirely (3d239cfa serialize every self-committing aw verb on one shared writer lock; 798c5cb5 commit in an isolated worktree so pre-commit cannot clobber a peer's edit; efc7a1a1 the same for finalize). offer_commit now delegates to commit_lock.commit_isolated (git_commit_helper.py:488), which snapshots HEAD into a throwaway detached worktree, copies our paths in, and runs the real hooked commit THERE (commit_lock.py:252). MEASURED, not inferred: in a scratch repo with a hook that strips trailing whitespace and exits 1, commit_isolated returns hook-rejected while the SHARED-tree file still reads 'trailing space here   \n' UNCHANGED, so this item's disk-re-hash detector would never fire; and calling commit_isolated three times in a row returns hook-rejected all three times with the log never advancing past init. The retry must therefore live INSIDE commit_isolated, where the rewrite is observable, which is what the plan does (E-01/E-02). All of this item's offer_commit line citations are stale and were re-located by symbol: :245/:246/:247 -> :488/:499/:510; CommitOutcome :39-51 -> :250-262; the our_staged intersection :233-234 -> :472-473. WHAT SURVIVES AND IS GRADUATED: the defect itself (one attempt, no retry, no rewrite detection) is REAL and live; Fix Layer 2 (normalize on write) survives intact and is E-05/E-06; the test gap survives exactly as described and is E-07. VERIFIED LIVE rather than assumed: the four mutating hooks DO fire on the agent-write paths (running pre-commit run trailing-whitespace against a scratch file under .aw/records/backlog/open/ printed 'Fixing ...' and exited 1); and the renderers' whole-file rstrip genuinely does NOT catch per-line trailing whitespace (rendering a body containing a trailing-space line through backlog._render_item leaves exactly one such line intact). ONE MEASUREMENT THAT REFRAMES LAYER 2 AS PREVENTIVE: the corpus is already CLEAN, 0 of 962 tracked .aw/records markdown files carry a trailing-whitespace line and 0 carry a two-space markdown hard break, so there is no migration to do and the hard-break tradeoff costs nothing today. FIX LAYER 3 IS NOT GRADUATED AND MUST NOT BE: this item already assigns the contract wording to wjl471 (commitguard, still open), and the HARD ORDERING CONSTRAINT runs the other way, since the reworded paragraph DESCRIBES the retry and must not ship before the retry lands. Plan lqly9m is therefore a prerequisite of wjl471's answer.
