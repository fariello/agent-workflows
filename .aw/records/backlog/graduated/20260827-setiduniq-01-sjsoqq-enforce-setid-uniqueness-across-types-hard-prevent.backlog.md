...
## Workflow history
- 2026-09-10 corrected (hand-edited; see the note below on why no verb was used): THIS ITEM'S CENTRAL PREMISE WAS REVERSED BY THE MAINTAINER ON 2026-09-10, so four of its six deliverables must NOT be built. A setid is a SHARED, CROSS-TYPE TOPIC LABEL, not a unique identity: research, specs, prompts, backlog items and plans concerning one issue may all carry the SAME setid, and that is CORRECT because it is what makes a topic visible from filenames alone. DEAD DELIVERABLES: #1 (promote check.setid-collision to hard/fail-closed; it must be DOWNGRADED instead), #2 (prevent a cross-type duplicate at creation/move), #5 (graduation mints a FRESH child setid), #6 (the migration sweep, which would rename 21 correctly-named topics). SURVIVING: #3 (setter UX, resolve a setid WITHIN the requested type), which was always the real fix, and #4 (Graduated-To plus its dangling check), which is useful independently of uniqueness. MEASURED BASIS: 117 of 433 filename setids span types by design (agentadhere alone covers 7 plans + 1 backlog item + 5 research reports); review_findings.build_review_name DELIBERATELY builds a review filename from its subject's setid and id6 as a join key, so 47 of those 117 are automatic; the motivating failure (aw ipd set approved agentadhere) was a LOOKUP defect, since the setter held both the setid and the target type and still refused; and check.setid-collision reports 38 findings by default and 86 with --all, of which only 5 (within-type conflicting descriptives) are genuine. GOVERNING SPEC CHANGED: 4w7d6s is now superseded by 2lcqno (.aw/records/specs/20260910-2lcqno-01-2lcqno-setid-shared-topic-label-and-type-scoped-resolution.spec.md). PLAN FATES: yku4ga, drzbs9 and dw7i3m are all retired to superseded/; bwgyum survives and stands alone. THIS ITEM STAYS graduated, because its design WAS handed off; a reader must simply not resurrect #1/#2/#5/#6 from the list below, which is preserved unedited as the record. WHY HAND-EDITED: aw backlog set on a same-status item DESTROYS the whole workflow history (it rebuilds from a fixed template, replacing every prior line with one). Reproduced here on this very file (1 insertion, 2 deletions) and reverted before committing; filed as backlog hg2oop, priority high. There is also no aw backlog note verb. So appending this record by tool was not possible without losing the two lines below.
