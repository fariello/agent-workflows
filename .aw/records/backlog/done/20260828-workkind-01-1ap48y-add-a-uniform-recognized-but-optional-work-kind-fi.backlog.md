...
## Workflow history
- 2026-09-01 done (aw set): Design shipped: plans 9trlc3 and ng2blv are executed (both From-Backlog: 1ap48y), delivering the recognized-but-optional Work-Kind field. Verified on main: ipd_schema carries Work-Kind (4 hits). CAVEAT recorded rather than hidden: orchestrator c2tvmm is still approved in pending/, but its single E-item is a whole-Set VERIFICATION that one field name landed on one vocabulary, not product work, and it retains its own Blocks-Release: next, so the release gate is still held. Sibling a6cej0 was superseded, which is why the executed pair is the authority here.
