...
## Workflow history
- 2026-09-01 done (aw set): FIXED by 33dc8264. Diagnosed as a FIXTURE bug, not a product defect: no product file was touched. Two independent faults, each silently yielding a dependency-FREE item so the driver correctly treated it as runnable: (1) _PLAN_TEMPLATE placed the metadata bullets BEFORE the H1 title, and the IPD spec defines the block as coming AFTER it, so the shared structural reader saw no fields (verified: template shape -> ([], None); canonical order -> the declared edge); (2) the test injected a LEGACY '- Dependencies:' field whose own comment claimed oc_runipd._DEPS_RE reads it, but 8guhs0 deliberately deleted _DEPS_RE, making '- Item-Dependencies:' canonical. The corrected fixture creates a REAL 'open' backlog item so the edge resolves (preflight passes) yet stays unsatisfied, keeping cb0001 at depth 0 as the genuine set-B competitor. IMPORTANT: my first fix passed VACUOUSLY (an executed:cb0001 edge made cb0001 a prerequisite, so sabotaging the gate outright still left it green); the final fixture is sabotage-verified twice, failing when the dependency gate always passes AND when the level-2 boundary decline is disabled. make test-all: 5 failed -> 4 failed, 4394 passed. The 4 remaining are pre-existing CLI-surface declaration checks (65 undeclared parser leaves), reproduced at 5e5da9a0 and owned by 0soncw's Scope-Paths (command_surface.py).
