...
## Workflow history
- 2026-09-26 done (aw set): Retired DONE (verified 2026-09-26 at 2f77995d): 5d6013e5 made add_output_mode_flags' verbose_help a required per-host parameter; oc's help (-vv: diff hunks and diagnostics) and agy's (-vv: raw tool parameters) each match their own renderer when driven (oc render_stream vs agy render_agy_event are different renderers), so 'at most one is true' no longer holds.
