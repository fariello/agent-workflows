...
## Workflow history
- 2026-09-20 set (aw backlog): closed by aw oc run: IPD h3bjue executed (every IPD carrier is executed and this run executed .aw/records/plans/executed/20260908-gatepin-01-h3bjue-pin-the-approval-gate-corpus-test-to-named-id6s-so-a-legitim.ipd.md); evidence .aw/records/plans/executed/20260908-gatepin-01-h3bjue-pin-the-approval-gate-corpus-test-to-named-id6s-so-a-legitim.ipd.md
- 2026-09-08 graduated (aw set): Graduated to plan h3bjue (Set gatepin, .aw/records/plans/pending/20260908-gatepin-01-h3bjue-...ipd.md), which carries From-Backlog: yw6759 and inherits this item's Blocks-Release: next. Status graduated (design handed off), NOT done. NOTHING IS OBSOLETE: the false-by-construction assertion is byte-for-byte intact at tests/test_plan_readiness.py:789-798, still globbing the live PENDING_DIR (:30), and no plan in any disposition fixes it (searched all five for test_plan_readiness, ApprovalGateRealCorpusTests, newest_verdict, corpuspin, yw6759; corpuspin and yw6759 have ZERO hits anywhere). Twelve pending plans record it as a BASELINE FAILURE TO IGNORE (9xycbh:111, 6eq3oq:169, 5f2h8i:152, lhccjf:158, m7gvuz:114 and :138 among them) and two explicitly DISCLAIM fixing it (daexj1:127 'AUTO-FIXING THE CURRENT RED TEST ... belongs to whoever owns plan 32ij2j's REPLAN verdict'; superseded xtklpd:254 'Do not fix either here'), which is itself evidence of the cost this item describes. BUT THREE OF THIS ITEM'S CLAIMS ARE NOW WRONG AND ARE CORRECTED HERE RATHER THAN CARRIED INTO THE PLAN. FIRST, THE STATED REPRO NO LONGER REPRODUCES: python3 -m pytest tests/test_plan_readiness.py -o addopts='' at HEAD fac69fbd reports 67 passed, and the bare suite reports 5859 passed, 3 skipped, 2 xfailed. The cause is that 32ij2j MOVED from pending/ to superseded/ in commit 32e4b74f (its REJECT record survives one entry deeper, so its verdict is still negative; the tree it sits in changed), and the test globs pending/ only. So the bug is LATENT, not active, and the plan's E-01 must MANUFACTURE the condition on a fixture instead of quoting this repro. That also means this item's core cost argument ('trains every agent to treat a red suite as normal') is not currently being paid, which a reviewer should weigh against the inherited Blocks-Release: next. SECOND, THIS ITEM IS WRONG THAT THE SIBLING INSTANCE SELF-HEALED. It says test_orchestrator_retirement's RealRepositorySets case 'self-healed by corpus drift rather than being fixed'. It was DELIBERATELY FIXED: renamed and rewritten to test_runprofile_is_not_refused_for_unauthored_rows (tests/test_orchestrator_retirement.py:890) in commit 31169afd, whose new docstring (:891-901) reasons that asserting the refusal 'pinned a transient repository state rather than the behavior this test is named for'. That is the SAME insight this item asks for, already applied once in-repo, so it is a second local PRECEDENT that STRENGTHENS the item rather than obsoleting it. The old node id no longer exists (pytest on it collects 0 items), and that module now reports 112 passed. THIRD, the graduation briefing's baseline of '1 failed, 5648 passed' naming test_orchestrator_retirement does not hold in this worktree at all. WHAT SURVIVED UNCHANGED: the fix shape (pin named id6s through the existing _find helper at :772-779, which already resolves across five dispositions and skips honestly), the sibling models (:781-787 and :800-818, the latter carrying a checked-counter so it 'says so rather than lying'), the prohibition on editing 32ij2j or any review record (which would forge review evidence), and suggestion 2's defensible corpus property (no plan whose newest record contains an APPROVING token is refused). Suggestion 3 (a guard against the whole class) became the plan's E-04 as a RECORDED DECISION rather than a build, for two reasons: some live-corpus coupling in this very class is DELIBERATE (its docstring at :770 says a fixture-only suite can pass while the gate misjudges reality), so a blunt detector would forbid the good case; and utwr6y E-03 already establishes that the obvious implementation, a grep over test source, is unsound because 'it would pass while an implicit dir=. still reached the live tree', while its E-04 sweep is scoped explicitly to REPORT, NOT FIX. ORDERING: 8v5pwa (rdattest-02) declares tests/test_plan_readiness.py in Scope-Paths but its E-04 targets the auto-approve predicate, a different class, so that is merge ordering not a dependency; utwr6y is not a dependency at all (different file, different fixture axis, and this fix needs no new fixture). NOTICED OUTSIDE THIS ITEM and reported rather than acted on: utwr6y's E-01/E-02 appear largely satisfied already by commit e167c9b3 (which added an in-file fixture and a hazard note at tests/test_run_viewer.py:22-27) while its E-03 file tests/test_run_viewer_isolation.py does not exist, and it carries Blocks-Release: next.
