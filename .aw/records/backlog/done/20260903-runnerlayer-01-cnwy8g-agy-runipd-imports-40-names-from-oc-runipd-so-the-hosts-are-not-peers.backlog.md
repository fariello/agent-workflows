...
## Workflow history
- 2026-09-23 set (aw backlog): Closed by runnerlayer Order 02 (1f7xno), which re-homed the host-neutral names into runner_shared. MEASURED: the oc-to-agy import surface went from 56 names to 4 and the reverse direction stayed at 0, both by AST walk; E-06's two set differences print EMPTY, so the residual is EXACTLY what the plan deferred with nothing left behind and nothing moved unsanctioned. THE FOUR THAT REMAIN ARE A DIFFERENT DEFECT, not this item's: each already has a runner_shared definition whose body DIVERGES from the host's, so re-homing one is a reconciliation rather than a pure move. Filed as zt2b16 (the recovery-routing trio, where the shared classify_recovery_disposition is DEAD ON ARRIVAL: it reads st.path/st.base_commit off a LaneState whose real fields are worktree_path/base_sha) and tm5vnx (the spec-edit recorder, whose two copies write different keys in different shapes, so the end-of-run spec report reads a key no run writes). THIS ITEM'S OWN CONSEQUENCES ARE DISCHARGED: consequence 1 (a fix reaching one driver only) is closed for 52 of 56 names; consequence 3 (a pairwise sharing check being blind to an imported symbol) is closed by the freeze guard child 01 added plus the 14 REFORK_TABLE rows E-05 promoted out of _SHARED_NAMES, whose AST half forbids a re-definition that object identity cannot see (proven by mutation). One further live defect was found and fixed in passing: ToolIdentityError was two distinct classes, so a run-fatal tool-identity mismatch degraded to an item-local failure (232wcg). No Blocks-Release gate was added, faithfully to this item.
