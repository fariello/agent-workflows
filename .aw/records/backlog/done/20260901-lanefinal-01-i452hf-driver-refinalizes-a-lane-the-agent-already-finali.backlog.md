...
## Workflow history
- 2026-09-01 done (aw set): FIXED on main by cdef9c90, which cherry-picks rchpms's two owning commits WITHOUT its Phase-1 prerequisite qcqhj7 (maintainer decision: take the targeted fix, defer Phase 1, because qcqhj7's prompt-isolation design COLLIDES with the design main built independently while the lane sat unmerged). Reproduced BEFORE and AFTER in throwaway clones with the same script: on main both the agent's in-lane finalize and the driver's self-finalize refused ('the begin receipt is STALE'), plan stayed in pending/, work stranded; after the fix the agent is cleanly refused with AW-LIFECYCLE-ROLE-001 and the DRIVER's finalize succeeds ('finalized bl9q3d -> executed'), plan reaches executed/. MEASURED that BOTH halves are required: the frozen-region digest alone (1c751814, cherry-picked clean, 0 conflicts) fixes the STALE refusal but the driver's own second finalize then still refuses ('no begin receipt' / 'pre-transition gate did NOT conform'); the worker-role refusal is what stops the double-finalize. cfe446d8 did NOT apply cleanly (it drags in Phase-1 machinery), so only its authority rule was taken, wired minimally and symmetrically into both drivers via the SHARED pinned_child_env. Suite in the PRIMARY checkout, bare: 3996 passed, 3 skipped, 4 xfailed (baseline 3983; +12 new tests +1 retargeted, zero regressions). NOT yet proven: that a real unattended aw oc run now integrates end to end - that needs an actual run, and is the remaining step toward the maintainer's stated goal. Siblings still open: dh0uno (lane-relative state) and the deferred Phase-1 prompt-isolation design decision.
