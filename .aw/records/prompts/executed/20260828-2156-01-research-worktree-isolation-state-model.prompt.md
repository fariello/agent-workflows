<!-- aw-prompt: Kind: research | Status: executed | Created: 2026-08-28 | Author: opencode (its_direct/pt3-claude-opus-4.8-1m-us) | Targets: GPT-5.6 | Concerns: worktree-isolation state/artifact model for a multi-agent IPD runner | Results-go-to: FILED under .aw/records/research/worktree-isolation-state-model/ once completed. This HTML comment is pipeline metadata only; it is invisible when pasted into a chat and is not part of the prompt. -->
