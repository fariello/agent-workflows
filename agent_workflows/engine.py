...
# wslayout Order 04 (hauwqh), spec kw5y2s Section 6.1: the machine-readable layout document and its
# JSON Schema, emitted into a target workspace at install time so a NON-PYTHON tool can resolve the
# workspace hierarchy without reimplementing it. Both are GENERATED and GITIGNORED (see
# `_AW_GITIGNORE_TEMPLATE` / `_ensure_aw_gitignore`): committing generated output is exactly the git
# drift install-time emission exists to avoid.
AW_LAYOUT_JSON_PATH = f"{AW_SYSTEM_DIR}/layout.json"
AW_LAYOUT_SCHEMA_PATH = f"{AW_SYSTEM_DIR}/layout.schema.json"
...
# wfartifacts Order 04 (l1c1iz) E-02: the ONE fallback used when the shipped
# `templates/workflow-artifacts-README.md` cannot be read (an OSError path only). It is deliberately
# SHORT AND POINTER-SHAPED rather than a second copy of that README, so the template stays the single
# source of the prose and the two cannot drift.
#
# THE DEFECT THIS REPLACES: both fallback sites (`show_install_diffs` and
# `ensure_workflow_artifacts_readme`) each inlined a FULL copy of the README instructing the reader
# NOT to gitignore the folder and saying its contents were "intended to be committed to git", the
# exact OPPOSITE of Order 07's ruling and of D92. Correcting only the template file would therefore
# have left two live copies of the bad prose shippable on any OSError. Keep this pointer-shaped: if it
# ever grows into a full README again, the drift it exists to prevent is back.
#
# It states the RULE and names the ENFORCING FILE (a "do not commit" line that names nothing invites a
# reader to delete the ignore rule), and it points at the template for the full text.
_ARTIFACTS_README_FALLBACK = (
    ...
    "anchored pattern `/workflow-artifacts/`; confirm with `git check-ignore -v "
    f"{ARTIFACTS_DIR}`.\n"
...
def agents_pointer_prose(target_layout: str = "legacy") -> str:
    ...
    return (
        ...
        "filename clusters by Set (`YYYYMMDD-<set-id>-<NN>-<id6>-<slug>.ipd.md`, the uniform "
        "artifact-naming grammar `YYYYMMDD-<setid>-NN-<id6>-<slug>.<type>.md` with the plan type "
        "facet `.ipd.md`). KEEP A SETID SHORT: <= 14 characters is strongly preferred, over 14 "
        "WARNS, and over 24 is REFUSED outright by the authoring verbs and by `aw check` "
        "(`check.setid-length-warn`/`check.setid-length-error`). Artifacts predating the "
        "repository's stamped cutover are grandfathered PER ARTIFACT, so an existing long setid "
        "keeps working while a NEW artifact may not adopt one. To browse plans by "
        "topic, regroup them, or shelve aged ones, use the noun-verb grammar (`aw index plans "
        "[--check]`/`aw find plans` for the manifest, `aw group plans`/`aw rename plans` to regroup, "
        "`aw archive plans` to weekly-shard terminal plans, `aw ipd board` for the board); do "
        f"not hand-name plans or hand-maintain the plans index. See `aw ipd --help` and "
        ...
        "spec carries a bare-enum `- Status:` (`draft`->`to-review`->`reviewed`->`approved`->"
        "`implementing`->`implemented`, plus `deferred`/`parked`/`superseded`) and a `## Workflow "
        "history`; an agent records human approval with `aw spec set approved <id6> --by-human "
        "--message ...` (or `aw specs set <path> --status approved --by-human`), an explicit attested "
        "speed bump; no TTY, no 'I am human' claim; and may "
        "NOT set `implemented` (needs cited evidence). A `deferred` spec MUST carry a typed gate "
        "(`- Gate-Kind:`/`- Gate-Ref:`). Specs now carry the stable `<id6>` in the filename GOING "
        ...
        "any spec the work needs and record the maintainer's instruction as the approval attestation "
        "(`aw specs set <path> --status approved --by-human --message ...`) rather than stopping for "
        "a separate approve round trip; (2) write REVIEW-READY plans every time, meaning `to-review` "
        "and never `draft`, with no `TODO` placeholders, real citations, an `E-*`/`V-*` bijection, and "
        "each `V-*` demanding concrete pasted evidence (`aw ipd lint` must report conforming); "
        "(3) carry the provenance and the gate, so every plan or spec you produce carries "
        ...
        "inferring runtime behavior from plan prose. The recurring failure this paragraph exists to "
        'stop: an agent reads author-time warnings inside IPDs ("these are the highest-contention '
        'files in the repo", "three other pending plans declare them", "must not run '
        'concurrently"), treats them as LIVE operational hazards, and tells the maintainer a queue is '
        "unsafe to run. Those notes are guidance for a HUMAN OR AGENT EDITING FILES BY HAND. They are "
        "not the runner's behavior, and repeating them as runtime risk wastes the maintainer's time on "
        "a question the runner settled.\n"
        "WHAT THE RUNNER DOES FOR YOU, each verifiable BY SYMBOL in `oc_runipd.py` or, for the "
        "queue-ordering and dependency names, in `runner_shared.py`, where `cnwy8g` re-homed "
        "them so both hosts reach one definition: the queue is "
        "sorted with DEPENDENCY DEPTH as the FIRST sort key (`queue_sort_key`, `dependency_depth`), so "
        "a dependent never precedes its dependency; dependencies are RE-CHECKED AT DISPATCH, not only "
        "at queue build, and an unmet edge marks that one item `dependency-blocked` and continues "
        "instead of failing the run; an ORCHESTRATOR is not agent-executed, and once every child of "
        "its Set is `executed` ON DISK the runner retires it to `executed` in that same run, on BOTH "
        "hosts, spending no agent turn, so an Order-0 parent in the queue is correct and needs no "
        "human step; and each execute item gets its own "
        "ISOLATED WORKTREE by default (`isolate_worktree`, opt out with `--no-isolate-worktree`) whose "
        "changes return through the merge-and-revalidate gate, which is why two plans naming the same "
        "file is not a hazard even in PARALLEL, let alone serially.\n"
        "WHAT RETIREMENT DELIBERATELY REFUSES, stated because the opposite claim was once made here "
        "and was false. Retirement is gated on EVERY child being `executed` and on nothing else "
        "qualifying, so the runner REFUSES to retire an orchestrator whose child table declares an "
        "unauthored row, whose Set has no children, whose children this run cannot finish, or whose "
        "own transition refuses; and it names WHICH of those it hit. A refusal leaves the plan in "
        "`pending/` and is not a failure of the run. Those refusals ARE worth reporting to a human, "
        "because each needs a human act (author the missing child, run the remaining children, or "
        "resolve whatever the transition refused on).\n"
        "THE ORCHESTRATOR COVERAGE GATE, which is what stops that retirement being a lie. Retirement "
        "SKIPS the pre-transition E/V checkpoint on the premise that a parent's own items are "
        ...
        "causes the very lost work the gate exists to prevent. The verdict is CACHED on the parent's "
        "item text plus its child table, so an unmodified orchestrator is never re-probed while the "
        "real fix re-probes automatically and a ticked checkbox does not. If the model host cannot be "
        ...
        "and say `aw oc run all` (or a setid). Legitimate things to raise instead: a plan whose SCOPE "
        "the human may not want executed yet, an `- Item-Dependencies:` edge pointing at something "
        "that will never execute, a genuinely OPEN `- Blocking: yes` question, a capability "
        "`aw host capabilities` reports as REFUSED for the action class, a REFUSED orchestrator "
        "retirement with the reason the run recorded, or a run the ORCHESTRATOR COVERAGE GATE refused "
        "(or warned past) with the plan it named. That last one is worth raising precisely because it "
        "needs a human act too: the remedy is to author the missing child, and the honest report is "
        "that the parent carries work no child covers, never that the gate is broken. Do NOT raise "
        "file overlap, queue order, or an "
        "Order-0 parent's PLACEMENT in the queue: those are solved, and asserting otherwise is a "
        "claim about code you have not read.\n\n"
        ...
        "uncommitted changes and untracked files you did not create are NOT yours; never revert, "
        "stage, commit, discard, reformat, or 'clean up' another party's work, even when it looks "
        "broken or unfinished; and if a file you must edit is being changed under you and the two "
        "sets of changes cannot be safely combined, STOP and report rather than overwriting.\n\n"
        ...
        "whatever is ALREADY STAGED for those paths, including edits a co-worker made to the same "
        "file. Scope the command AND verify the staged set. If you sweep someone else's work into "
        "your commit, their provenance is lost and their own tooling can no longer see its changes "
        "as pending; say so plainly in your report if it happens.\n\n"
        "RE-VERIFY AFTER A FAILED RAW COMMIT. When you commit with raw `git commit`, `pre-commit` "
        "stashes unstaged changes and restores them when a hook rejects the commit, and that restore "
        "can leave paths you never staged sitting in the index. So a hook failure INVALIDATES the "
        "check you did before it: re-run `git diff --cached --name-only` after EVERY failed raw "
        "commit attempt, before retrying. A MUTATING hook (whitespace or eof fixer, formatter) also "
        "REWRITES your file and then rejects, so your retry must re-stage the rewritten path. Never "
        "'fix' a polluted index with a bare `git reset` or `git stash` (that discards or unstages a "
        "co-worker's work); unstage precisely, path by path, with `git restore --staged <path>`.\n\n"
        "### Before you publish to main by hand, hold the integration lock\n"
        "A driver in this checkout serializes its own publishes to `main` behind a repository-scoped "
        "integration lock, so two drivers never merge at the same time. You hold nothing, so a hand "
        "merge can still race one. That is not hypothetical: it was measured on 2026-09-22, when a hand "
        "integration and a live driver each advanced `main` under the other, discarding two completed "
        "full-suite validations and refusing a prepared fast-forward publish.\n\n"
        ...
        "```\n\n"
        "Wrap the whole publish, not just the merge, and re-read `main`'s tip INSIDE the lock rather "
        "than before acquiring it: a tip you read while waiting is exactly the stale read this exists "
        "to prevent. A live holder makes the command WAIT and name the holder, and it gives up (exit 1, "
        "nothing merged) rather than waiting forever.\n\n"
        "THIS IS A PROTOCOL, NOT AN ENFORCEMENT BOUNDARY, and the distinction decides what you may "
        "trust. The lock is real and it binds every party that USES it; it cannot stop a raw `git merge` "
        "typed by someone who does not. A merge-blocking git hook is DELIBERATELY not part of this: it "
        "would be local-only and skippable, and it would also miss the driver's own happy path, because "
        "a fast-forward creates no commit for such a hook to run on. Publishing with `--ff-only` is a "
        "useful BACKSTOP, since a diverged tip then refuses instead of merging, but it is NOT a "
        "substitute for holding the lock: it is blind to a peer that advanced `main` in a way your "
        ...
        "When you execute a task or plan here you MUST: commit ONLY files you changed, limited to the "
        "paths you name, through `aw commit <plan> -- <paths>` (or `aw commit --no-plan -m <msg> -- "
        "<paths>` when no plan governs the change), never `git add -A`/bare/`-a`, and never push; when you "
        "report tests passed, paste the ACTUAL runner output (never claim success you did not run); "
        "write no em or en dashes in USER-FACING prose you author (READMEs, CHANGELOG, docs meant for "
        "end users) - this keeps user-facing text from reading as machine-written; it does NOT apply "
        "to internal or AI-facing artifacts (IPDs/plans, research findings, prompts, specs, "
        ...
        "time.\n\n"
        "YOUR SHELL IS PERSISTENT: DO NOT RE-`cd` EVERY COMMAND. The shell session keeps its working "
        "directory between calls, and it already starts at the repo root, so prefixing each command "
        "with `cd /path/to/repo && ...` is pure waste: it re-sends the absolute path on every call, "
        "inflates what the human has to read, and (because the path is a machine-local absolute path) "
        "is exactly the kind of string the leak-sanitizer exists to keep out of shared output. Run the "
        "bare command. If a tool call genuinely needs a DIFFERENT directory, use your tool's own "
        "working-directory parameter (OpenCode's bash tool has `workdir`, and its own instructions say "
        "to prefer it over `cd`) rather than chaining `cd`. The one legitimate use of `cd` is a "
        "subshell for a single throwaway command, e.g. `(cd /tmp/x && ...)`, which cannot leak the "
        "change back into the session.\n\n"
        ...
        "When you author or execute an Implementation Plan Document (IPD), do NOT hand-number ids or "
        "hand-place checklists: use the tools and follow the canonical spec. `aw ipd scaffold` writes "
        ...
        "did not do."
        "\n\nAN ORCHESTRATOR HOLDS ORCHESTRATION, NOT WORK OF ITS OWN, AND THE DISTINCTION IS NOT "
        "'has no checklist'. An Order-0 orchestrator SHOULD carry a checklist of its children: that "
        "checklist is what makes a Set execute completely and in order when a human simply tells an "
        "agent 'execute <setid>' with no runner involved, which is how most Sets are run. DELETING it "
        "causes exactly the lost or partial work it exists to prevent. What must NOT be there is work "
        "that exists ONLY on the parent and is covered by no child: producing a deliverable, "
        "establishing a baseline before any child runs, reconciling records afterwards. The reason is "
        "mechanical: the runner RETIRES an orchestrator once every child is `executed` and deliberately "
        "SKIPS the pre-transition E/V checkpoint, on the premise that the parent's items are performed "
        "by nobody. Work parked on the parent is therefore marked complete having never been performed "
        "OR verified. So if you are authoring a Set and find yourself writing a STEP on the Order-0 "
        "plan that no child covers, do NOT delete it: ADD A CHILD for it. EVERY plan carrying "
        "`- Kind: orchestrator` carries checklist items, without exception, and most of those items "
        "are legitimate orchestration, which is why the distinction matters and why a blunt 'no items "
        "on a parent' rule is wrong. That is stated as a PROPERTY rather than as a fraction because "
        "the fraction rotted three times: it read '46 of 130', whose denominator was the filename "
        "`-00-` population, most of which are pre-`Kind` legacy plans carrying ZERO E-items and so "
        "diluting a ratio they are not members of; measured on the plans that actually declare the "
        "kind it has been 100 percent at every single measurement. AND A RUN NOW CHECKS THIS RATHER "
        "THAN TRUSTING YOU TO: see the ORCHESTRATOR COVERAGE GATE described under 'The runners own "
        "ordering, isolation, and orchestrators' above, which refuses before spending anything and "
        "tells you to add a child."
        "\n\nA PLAN MAY AMEND A SPEC, AND MUST DECLARE IT. Specs are living contracts, not immutable "
        "history: a plan that changes behavior a spec describes SHOULD carry the spec amendment in the "
        "same change, so the two never drift apart. The alternative was measured and is a dead end "
        "(plan `51vw4y` had to extend a spec-governed flag table with no spec-edit authority, leaving no "
        "route that did not either edit an approved spec or weaken a shipped contract test). Two "
...
# wfartifacts Order 05 (y4pptx): the RETIRED repo-root run-scratch directory. Order 07 (spec
# `20260817-2124-01`) moved run scratch to `.aw/workflow-artifacts/` (`ARTIFACTS_DIR`); Order 01
# stopped the installer CREATING this one; this constant names what an ALREADY-INSTALLED repo is
# still carrying, so the migration below can find it. Measured 2026-09-12: 29 repos on one machine
# have it, and two of them TRACK real run records under it (11 files and 3 files respectively).
# It is deliberately a separate constant from LEGACY_ARTIFACTS_DIR (the pre-D19 `repository-review/`
# name): that is an older migration with its own destination rule, and merging the two would make
# each one's behavior unreadable.
RETIRED_ROOT_ARTIFACTS_DIR = "workflow-artifacts/"


def _run_record_relpaths(root: Path) -> list[str]:
    """Every FILE under `root`, as `/`-joined paths relative to `root`, sorted.

    The unit the migration works in. Used both to enumerate what to move and (in the tests) to
    prove the before/after path sets are equal modulo the prefix.
    """
...
def _is_readme_only(rel_paths: list[str]) -> bool:
    """Whether the retired directory holds NOTHING but the installer's own stray README.

    This is case (c), and it is the COMMON case: measured 2026-09-12, most of the 29 affected repos
    track only the README the old `ensure_workflow_artifacts_readme` wrote at the repo root. Its
    content was the exact OPPOSITE of Order 07's ruling ("DO NOT gitignore this folder... intended
    to be committed"), so a relocated COPY of it is precisely what must not survive.
    """

    return rel_paths == ["README.md"]


def migrate_root_workflow_artifacts(
    repo_root: Path,
    *,
    use_git: bool,
    dry_run: bool = False,
) -> list[str]:
    """Relocate an EXISTING install's run records from the retired repo-root
    `workflow-artifacts/` into `.aw/workflow-artifacts/` (`ARTIFACTS_DIR`).

    wfartifacts Order 05 (y4pptx). Orders 01-04 fix what a FRESH install produces and change
    nothing for the 29 repos already carrying the retired layout, two of which TRACK real run
    records there. This is the ONLY child that touches a user's committed history, which is why its
    invariants are stated here rather than left to the call site.
    ...
    THREE CASES, distinguished because git can only preserve history in the first:

    (a) TRACKED content -> `git mv`, so the rename is recorded and `git log --follow` still reaches
        the pre-migration commits, THEN `git rm --cached` the destination so the relocated bytes end
        up genuinely UNTRACKED and the `.aw/.gitignore` rule governs them. Both steps are required
        and neither is sufficient: measured 2026-09-18, `git mv` into an ignored destination
        succeeds but leaves the path in the INDEX, where ignore rules do not apply, so
        `git check-ignore` reports it as not ignored and the run scratch stays committed at a new
        path - defeating the whole Set. Untracking does NOT lose the history: `--follow` traverses
        the rename in the commit graph after the untrack commit (measured in the same probe). This
        is the same rule `ensure_workflow_artifacts_readme` adopted in Order 01 (decision D-01) for
        the same tree, and for the reason it records: git's ignore rules never untrack an
        already-tracked path.
    (b) UNTRACKED content (already ignored, or never committed) -> a plain filesystem move. Git has
        nothing to preserve, and invoking it would only risk an error on an ignored path.
    (c) The stray README ALONE and nothing else -> REMOVED rather than relocated, because Order 04
        replaced its content and a copy of the retired "do not gitignore this folder" prose is
        exactly what should not survive the migration. This is the only removal in the function.

    THE DESTINATION IS OFTEN ALREADY POPULATED, SO THIS MERGES AND DOES NOT MOVE (F-7). Measured in
    the framework repository at authoring time, both trees held content and three workflow names
    (`assess-bugs`, `assess-documentation`, `release-review`) existed in BOTH. An implementation
    that assumes an absent destination either fails on the existing directory or replaces it, so
    the merge is per FILE: each source file is placed at its own relative path under the
    destination, creating parent directories as needed, which preserves the `<workflow>/<RUN_ID>/`
    shape Order 07 specified (`workflow-artifacts/assess-bugs/20260726-115243/report.md` ->
    `.aw/workflow-artifacts/assess-bugs/20260726-115243/report.md`).

    REFUSAL, not a guess, on a conflict: when the destination already holds a file at the same
    relative path with DIFFERENT bytes, both files are left exactly where they are and the conflict
    is reported. A silent overwrite of a run record is unrecoverable for the user. Identical bytes
    are not a conflict; the source copy is redundant and is left in place too (still never deleted),
    so nothing is lost and a re-run remains safe.

    THE RETIRED DIRECTORY IS REMOVED ONLY WHEN GENUINELY EMPTY of files (orchestrator OQ-01):
    keeping an empty shell would preserve the double-home confusion Order 07 exists to end, but a
    directory still holding content the migration REFUSED to move must stay, because the
    alternative is data loss.

    Silent when there is nothing to do (OQ-01): a repo without the retired path returns `[]`, on the
    same convention every other back-fill in this module follows, because an installer that narrates
    work it did not do trains users to ignore its output.

    Args:
        repo_root: the target repo.
        use_git: whether git is usable here (`git_available`). False forces case (b) throughout.
        dry_run: report what WOULD happen and touch nothing.

    Returns:
        Human-readable action lines (empty when there was nothing to do), suitable for the
        installer summary. A conflict or a mis-placed lane appears here as a reported line.
    """

    repo = Path(repo_root)
    retired = repo / RETIRED_ROOT_ARTIFACTS_DIR
    actions: list[str] = []

    # Report a lane an agent already mis-relocated, and NEVER move it (E-04, decision D-02). This
    # runs even when the retired directory is absent, because the two situations are independent.
    actions.extend(_report_misplaced_run_records(repo))

    if not retired.is_dir():
        return actions

    rel_paths = _run_record_relpaths(retired)

    # Case (c): the installer's own stray README and nothing else - the COMMON case across the 29
    # affected repos. Remove it (framework-written, superseded by Order 04) rather than carrying a
    # copy of the retired prose to the new home.
    if _is_readme_only(rel_paths):
        rel = f"{RETIRED_ROOT_ARTIFACTS_DIR}README.md"
        if dry_run:
            actions.append(
                f"{rel} [would remove: superseded stray README, no run records here, dry-run]"
            )
            return actions
        if use_git and git_is_tracked(repo, rel):
            git_run(repo, ["rm", "-q", "--", rel])
        else:
            (retired / "README.md").unlink()
        _remove_retired_dir_if_empty(retired, actions)
        actions.append(
            f"{rel} [removed: superseded stray README (Order 04 replaced its content); "
            "no run records were present]"
        )
        return actions

    if not rel_paths:
        # An empty shell: nothing to move, and removing it ends the double home (OQ-01).
        if dry_run:
            actions.append(
                f"{RETIRED_ROOT_ARTIFACTS_DIR} [would remove: empty retired run-scratch dir, dry-run]"
            )
            return actions
        _remove_retired_dir_if_empty(retired, actions)
        return actions
    ...
    conflicts: list[str] = []
    redundant: list[str] = []
    # (source, destination) for each TRACKED file whose rename is staged and awaits the batch commit.
    relocated_tracked: list[tuple[str, str]] = []
    ...
    for rel in rel_paths:
        ...
        if dst.exists():
            # MERGE, and refuse rather than guess. Same path, same bytes: the source is redundant,
            # so there is nothing to move and nothing to lose; different bytes: REFUSE, leaving
            # BOTH in place, because overwriting a run record cannot be undone.
            same = False
            try:
                same = dst.is_file() and src.is_file() and _same_bytes(src, dst)
            except OSError:
                same = False
            if same:
                redundant.append(rel_src)
                if not dry_run:
                    actions.append(
                        f"{rel_src} [left in place: destination already holds identical bytes]"
                    )
                else:
                    actions.append(
                        f"{rel_src} [would leave in place: destination identical, dry-run]"
                    )
            else:
        ...
        if dry_run:
            how = (
                "git mv + untrack"
                if (use_git and git_is_tracked(repo, rel_src))
                else "move"
            )
            actions.append(f"{rel_src} -> {rel_dst} [would {how}, dry-run]")
            moved.append(rel_src)
            continue

        tracked = use_git and git_is_tracked(repo, rel_src)
        dst.parent.mkdir(parents=True, exist_ok=True)
        if tracked:
            # Case (a): `git mv` records the rename. The COMMITS happen once for the whole batch
            # below, not per file, so a repo with 11 committed run records gets two commits rather
            # than 22.
            _git_mv(repo, rel_src, rel_dst)
            relocated_tracked.append((rel_src, rel_dst))
            actions.append(
                f"{rel_src} -> {rel_dst} [migrated with git mv (history preserved, committed), "
                "then untracked so the .aw/.gitignore rule governs it; file kept on disk]"
            )
        else:
            # Case (b): git has nothing to preserve.
            shutil.move(str(src), str(dst))
            actions.append(f"{rel_src} -> {rel_dst} [migrated (was untracked)]")
        moved.append(rel_src)

    if relocated_tracked:
        _commit_relocation(repo, relocated_tracked, actions)

    if not dry_run:
        # Only when genuinely empty: a refused conflict or a redundant copy legitimately keeps the
        # directory alive, and nothing is deleted to make it empty (orchestrator OQ-01).
        _remove_retired_dir_if_empty(retired, actions)

    if conflicts or redundant:
        actions.append(
            f"{RETIRED_ROOT_ARTIFACTS_DIR} [kept: {len(conflicts)} refused conflict(s), "
            f"{len(redundant)} redundant file(s) still here; nothing was deleted]"
        )

    return actions


def _commit_paths(repo_root: Path, rel_paths: list[str], message: str) -> bool:
    """Commit ONLY `rel_paths`, tolerating "nothing to commit". Returns True if a commit was made.

    wfartifacts Order 05 (y4pptx). The migration needs its own commits because a staged-only
    relocation never records the rename, and `git log --follow` can then not reach the pre-migration
    history (measured 2026-09-18) - which is the single property this plan exists to preserve. The
    plan's Scope check authorizes exactly this: "it does not rewrite git history (no filter-branch);
    it relocates files in a new commit".

    ALWAYS PATH-SCOPED, never `git add -A` and never a bare `git commit`, so a co-worker's or the
    user's unrelated STAGED work in this shared checkout is not swept in (the repository's execution
    contract, verified by measurement).

    A "nothing to commit" result is NOT an error: it means the caller's step had nothing to record.
    Any other git failure is raised, matching `git_run`'s contract.
    """

    result = subprocess.run(
        ["git", "-C", str(repo_root), "commit", "-q", "-m", message, "--", *rel_paths],
        capture_output=True,
        text=True,
    )
    if result.returncode == 0:
    ...
    if "nothing to commit" in blob or "nothing added to commit" in blob:
        return False
    raise SystemExit(
        f"git commit -- {' '.join(rel_paths)} failed:\n"
        f"{(result.stderr or result.stdout or '').strip()}"
    )


def _commit_relocation(
    repo_root: Path, pairs: list[tuple[str, str]], actions: list[str]
) -> None:
    """Commit a batch of staged run-scratch renames, then untrack the destinations.

    wfartifacts Order 05 (y4pptx). TWO COMMITS, and the split is load-bearing rather than fussy.
    Measured 2026-09-18: a `git mv` and a `git rm --cached` applied to the same index state leave the
    rename recorded in NO commit, so `git log --follow -- <dest>` returns NOTHING and the committed
    history this plan exists to preserve is unreachable. So:

    1. COMMIT THE RENAME. This records it in the commit graph, which is what `--follow` traverses.
    2. COMMIT THE UNTRACK. This takes the destination out of the index so the `.aw/.gitignore` rule
       finally governs it (D92: run scratch carries absolute home paths and session detail). Verified
       measurement: after this commit `--follow` STILL reaches the pre-migration commits, so
       untracking costs no history.

    THE UNTRACK NEEDS A WORKTREE DODGE, and the reason is worth stating because the obvious code is
    wrong. `git commit -- <paths>` is path-scoped by re-reading those paths from the WORKING TREE, so
    with the file still present it sees "no change" and refuses ("nothing added to commit"), leaving
    the destination TRACKED - the exact defect this function exists to avoid, and the failure this
    branch was first written with. A bare `git commit` would record the index-only removal, but it
    would also sweep in a co-worker's staged work, which this shared checkout forbids. So each
    destination is moved aside to a temporary sibling for the duration of the commit and restored
    immediately after, in a `finally`, so the bytes are never at risk even if git fails.

    Batched: one pair of commits for the whole migration, not per file, so a repo with 11 committed
    run records gets two commits rather than 22.
    """

    repo = Path(repo_root)
    sources = [src for src, _ in pairs]
    destinations = [dst for _, dst in pairs]
    scope = sorted(set(sources) | set(destinations))
    count = len(pairs)
    ...
    _commit_paths(
        repo,
        scope,
        f"agent-workflows: relocate {count} run-scratch {noun} to {ARTIFACTS_DIR}\n\n"
        "Moved with `git mv` off the retired repo-root `workflow-artifacts/` so the committed\n"
        "history follows the rename (`git log --follow` still reaches the original commits).\n"
        "The next commit untracks these paths; the files stay on disk.",
    )

    # Step 2, with the worktree dodge described above.
    ...
    try:
        for dst in destinations:
            ...
            if not target.is_file():
                continue
            aside = target.with_name(target.name + ".aw-untrack-tmp")
            try:
                target.rename(aside)
            except OSError:
                continue
            stashed.append((aside, target))
        _commit_paths(
            repo,
            destinations,
            f"agent-workflows: untrack {count} relocated run-scratch {noun} (D92)\n\n"
            f"Run scratch is never committed: it carries local context, absolute home paths and\n"
            f"session detail, and git history is permanent. The framework-owned `.aw/.gitignore`\n"
            f"ignores `{ARTIFACTS_DIR}`, but git's ignore rules do not untrack an already-tracked\n"
            "path, so these are removed from the index here. THE FILES REMAIN ON DISK.",
        )
    finally:
        for aside, target in stashed:
            try:
                aside.rename(target)
            except OSError:
                actions.append(
                    f"{target.relative_to(repo).as_posix()} [WARNING: left at "
                    f"{aside.name}; restore it by hand. No content was deleted.]"
                )


def _same_bytes(a: Path, b: Path) -> bool:
    """Whether two files have identical content. Used only to tell a redundant copy from a
    genuine conflict, so a re-run of the migration is safe and a real difference always refuses."""

    return a.read_bytes() == b.read_bytes()


def _remove_retired_dir_if_empty(retired: Path, actions: list[str]) -> None:
    """Remove the retired repo-root run-scratch directory IFF it holds no files at all.

    Orchestrator OQ-01: an ignored empty shell would preserve exactly the double-home confusion
    Order 07 exists to end (the maintainer's report is that the directory APPEARS), but a directory
    still holding content the migration refused to move MUST stay. Nothing is ever deleted to make
    it empty. Empty leftover SUBDIRECTORIES are pruned, since they hold no user bytes.
    """

    if not retired.is_dir():
        return
    if _run_record_relpaths(retired):
        return
    try:
        ...
        return
    actions.append(
        f"{RETIRED_ROOT_ARTIFACTS_DIR} [removed: now empty; run scratch lives at {ARTIFACTS_DIR}]"
    )


def _report_misplaced_run_records(repo_root: Path) -> list[str]:
    """REPORT (never move) run records an agent previously mis-relocated under a records lane.

    wfartifacts Order 05 (y4pptx) E-04, decision D-02. THE OBSERVED CASE: told about the retired
    repo-root directory, a repo agent moved run records into `.aw/records/reviews/untracked/`. That
    is a TRACKED typed tree's path with an `untracked/` lane bolted on, not the run-scratch home,
    and `records/reviews/` holds typed `.review.md` artifacts written by `/plan-review`.
    ...
    and loses nothing; guessing wrong is unrecoverable. The same invariants would apply if this were
    ever automated (preserve structure, never delete, refuse on conflict).

    Verified 2026-09-18 in this repository: no such path exists (`ls -d .aw/records/*/untracked` ->
    exit 2; `git log --all -- .aw/records/reviews/untracked` -> empty), so this is a guard for OTHER
    repos rather than a fix for this one.

    Detection is deliberately NARROW: a lane is only reported when it holds a directory whose name
    looks like a workflow-run id (`<RUN_ID>`, a timestamp such as `20260726-115243` or
    `20260918T045900Z`), which is the shape run scratch actually writes. A lane holding ordinary WIP
    files is silent, so the common legitimate case never nags the user.
    """

    repo = Path(repo_root)
    records = repo / ".aw" / "records"
    if not records.is_dir():
        return []
    run_id = re.compile(r"^\d{8}([-T]\d{6}Z?)?$")
    found: list[str] = []
    for lane in sorted(records.glob("*/untracked")):
        if not lane.is_dir():
            continue
        for candidate in sorted(lane.rglob("*")):
            if candidate.is_dir() and run_id.match(candidate.name):
                found.append(candidate.relative_to(repo).as_posix())
    if not found:
        return []
    shown = ", ".join(found[:3]) + (" ..." if len(found) > 3 else "")
    report = (
        f".aw/records/*/untracked/ [REPORT ONLY, nothing moved: {len(found)} run-record-shaped "
        f"directory(ies) found in a records quarantine lane ({shown}). Run scratch belongs at "
        f"{ARTIFACTS_DIR}<workflow>/<RUN_ID>/. The installer will NOT move these: it cannot tell a "
        "mis-placed run record from your in-progress typed records there. Move any you recognize by "
        "hand.]"
    )
    return [report]
...
/inbox/
# The install-time-emitted machine-readable layout document and its JSON Schema (wslayout Order 04,
# spec kw5y2s Section 6.1): GENERATED from the installed framework version by
# `engine.emit_layout_artifacts`, regenerated by every install, and therefore never committed -
...
# bklggrad f1dhht: the OPT-IN backlog-blocking-close pre-commit hook block. NOT installed by the
# default setup path; wired only on an explicit opt-in via create_backlog_close_gate_hook. It refuses
# committing a release-blocking backlog item closed to `done` without a preserved-or-satisfied gate,
# delegating to the shared check_engine.evaluate_blocking_close predicate (same authority as the setter
# and `aw check`). Standalone-config variant (written when the repo has no pre-commit config yet).
_BACKLOG_CLOSE_GATE_HOOK_ID = "backlog-blocking-close-gate"
_BACKLOG_CLOSE_GATE_PRECOMMIT_TEMPLATE = """\
# Pre-commit hooks (created by agent-workflows). If you already use pre-commit, MERGE the
...
def create_backlog_close_gate_hook(
    repo_root: Path,
    use_git: bool,
    *,
    install: bool,
    ...
    """OPTIONALLY (opt-in) wire the backlog-blocking-close pre-commit hook into a TARGET repo
    (bklggrad f1dhht E-03).

    NOT called by the default setup path: the hook is installed ONLY on an explicit ``install=True``
    request (an operator opt-in / flag), mirroring ``create_local_leaks_backstop``. Idempotent
    (re-running does not duplicate the hook), no-clobber (never edits a user's existing config beyond
    appending our own block when absent), dry-run aware, and opt-out honored (``install=False`` writes
    nothing). Returns ``{"created": [...], "skipped": [...], "notes": [...]}``.
    """
    created: list[str] = []
    skipped: list[str] = []
    notes: list[str] = []
    ...
    if not pc_path.exists():
        if dry_run:
            notes.append(
                f"would create {PRE_COMMIT_CONFIG} with the {_BACKLOG_CLOSE_GATE_HOOK_ID} hook"
            )
        else:
            _create_if_absent(
                repo_root,
    ...
    if _BACKLOG_CLOSE_GATE_HOOK_ID in existing:
        skipped.append(
            f"{PRE_COMMIT_CONFIG} [{_BACKLOG_CLOSE_GATE_HOOK_ID} already wired]"
        )
        return {"created": created, "skipped": skipped, "notes": notes}
    if dry_run:
        notes.append(
            f"would append the {_BACKLOG_CLOSE_GATE_HOOK_ID} hook block to {PRE_COMMIT_CONFIG}"
        )
        return {"created": created, "skipped": skipped, "notes": notes}
...
def _ensure_aw_gitignore(repo_root: Path) -> bool:
    ...
    # installed BEFORE emission existed. The template above covers a FRESH install only, so without
    # this the generated `.aw/system/layout.json` would show up as an untracked file in every
    # already-installed repo - the exact pre-existing gap the `records/history.jsonl` note above
    # records having been hit once already.
...
def emit_layout_artifacts(repo_root: Path, *, dry_run: bool = False) -> list[str]:
    ...
    Called from `install_into_repo` ITSELF, not from a caller, because that function is the SHARED
    CHOKEPOINT every install path reaches: `aw install` via `engine.run()`, `aw setup` (the
    machine-wide first-run wizard) via `cli._run_setup` -> `cli._install_one`, and library callers.
    Wiring emission into `run()` instead would leave `aw setup` silently emitting nothing.

    DETERMINISTIC AND SKIP-UNCHANGED. `layout.to_json`/`to_schema_json` are byte-stable for a given
    model + version (fixed key order, no set iteration), so an identical re-install rewrites nothing:
    a path whose content already matches is left alone, keeping its mtime and keeping a repeated
    install a true no-op. Files are written at mode 0o644.

    LEGACY LAYOUT IS SKIPPED. The emitted paths are `.aw/system/` paths by definition; a `legacy`
    (`.agents/workflows`) target has no `.aw/system` tree, and manufacturing one here would half-migrate
    a repo the installer deliberately left on the legacy layout.

    Failure is NON-FATAL and warns: these artifacts serve non-Python consumers, and Python callers
    build the model in-process from `layout.build_default_layout()`, so an unwritable file must not
    fail an otherwise good install. It is the same posture the ownership manifest takes above.
    """

    from agent_workflows import layout as _layout

    root = Path(repo_root)
    if resolve_target_layout(root) != "aw":
        return []

    model = _layout.build_default_layout()
    framework_version = read_installed_version(root) or ""
    payloads = (
        (AW_LAYOUT_JSON_PATH, model.to_json(framework_version)),
        (AW_LAYOUT_SCHEMA_PATH, model.to_schema_json()),
    )
    ...
    for rel, content in payloads:
        ...
        try:
            if target.is_file() and target.read_text(encoding="utf-8") == content:
                continue  # byte-identical: a re-install at the same version is a no-op
        except OSError:
        ...
        try:
            ...
            print(
                f"Warning: could not write {rel} ({exc}); non-Python consumers will not "
                "see the layout document until the next successful install.",
                file=sys.stderr,
            )
            continue
        written.append(rel)
...
def install_into_repo(
    ...
    *,
    ...
    # wiring into `run()` would leave `aw setup` silently doing nothing - the measured reason
    # `emit_layout_artifacts` records for the same placement.
    # Runs AFTER `migrate_legacy_layout` (which has its own, older pre-D19 source) and BEFORE the
    # README ensurer, so a repo whose only run-scratch content was the retired stray README ends the
    # install with the new tree's correct README rather than a relocated copy of the retired prose.
    # It reports ONLY when it actually did something (OQ-01) and is a silent no-op otherwise.
    # NOTE the ordering dependency this plan declares on Order 02: the relocated files must land in
    # a tree that is already ignored, which `_ensure_aw_gitignore` guarantees. Both Orders 01 and 02
    # are `executed`, and the migration additionally untracks what it moves, so the destination is
    # ignored IN EFFECT rather than by assumption (asserted with `git check-ignore` in the tests).
    migrated.extend(
    ...
    # create_setup_artifacts so `.aw/.gitignore` (which ignores both emitted paths) already exists on
    # a fresh install, hence git never sees them even transiently. Pre-existence is sampled first so
    # only a genuinely NEW file enters the `--undo` record below (a rolled-back repo must not be left
    # holding generated output whose ignore rule the same rollback just removed).
    ...
    # wslayout Order 04 (hauwqh): record a FRESHLY emitted layout artifact for `--undo` too, on the
    # same "remove what this run created" rule the setup artifacts follow. A path that already existed
    # is NOT recorded, so rollback never deletes a previous install's copy.
    newly_created.extend(
        rel for rel in layout_artifacts if not _layout_pre_existing.get(rel)
    )
    ...
    return {
        ...
        "migrated": migrated,
        "artifacts": artifacts,
        # wslayout Order 04 (hauwqh): the layout paths this run actually WROTE (empty on a no-op
        # re-install at the same version, and on a `legacy`-layout target, which has no .aw/system).
