...
def agents_pointer_prose(target_layout: str = "legacy") -> str:
    ...
    return (
        ...
        "instead of failing the run; an ORCHESTRATOR is not agent-executed but self-finalizes once "
        "every child of its Set reached `executed` (`action_for` returns `orchestrate`, and the "
        "dispatcher calls `_set_children_all_executed` then `finalize_orchestrator`), so an Order-0 "
        "parent in the queue is correct and needs no human step; and each execute item gets its own "
        ...
        "that will never execute, a genuinely OPEN `- Blocking: yes` question, or a capability "
        "`aw host capabilities` reports as REFUSED for the action class. Do NOT raise file overlap, "
        "queue order, or orchestrator finalization: those are solved, and asserting otherwise is a "
