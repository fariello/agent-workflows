...
def agents_pointer_prose(target_layout: str = "legacy") -> str:
    ...
    return (
        ...
        "RE-VERIFY AFTER A FAILED HOOK. `pre-commit` stashes unstaged changes and restores them when "
        "a hook rejects the commit, and that restore can leave paths you never staged sitting in the "
        "index. So a hook failure INVALIDATES the check you did before it: re-run "
        "`git diff --cached --name-only` after EVERY failed commit attempt, before retrying. Never "
