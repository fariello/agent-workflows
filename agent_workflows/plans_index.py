"""Plans tiered manifest + browse-by-Set + drift check (Set plans-adopter, Order 03).

``aw plans index [--check]`` regenerates ``.agents/plans/INDEX.json`` (every plan, all fields) and a
browse-by-``Set:`` human view (``INDEX.md``: each Set lists its members in ``Order``, Sets ordered by
most-recent activity, bounded to the 40 most-recent Sets, the rest available in the JSON). ``aw plans
find`` queries the manifest. ``--check`` fails on the four drift classes (missing/invalid ``Id``,
name-vs-metadata mismatch on a clustered name, stale generated view, dangling plan citation).

Consumes the shared core (Order 01: id6, Drift shape, dangling detector) and the required plan
``Id`` (Order 02). The scan is RECURSIVE so plans that Order 05 moves into
``<disposition>/YYYYMM/`` shards remain visible; disposition is the TOP-LEVEL directory even
when a plan sits in a shard subdir. Complements (does not replace) the disposition-grouped
``STATUS.md`` from ``plans.py``.
"""

from __future__ import annotations

import argparse
import json
import re
from pathlib import Path
from typing import Dict, List, NamedTuple, Optional, Tuple

from agent_workflows import artifact_core as _core
from agent_workflows import artifact_naming as _naming

DEFAULT_INDEX_LIMIT = 40  # OQ5: browse-by-Set view bounded to the N most-recent Sets.
# (The former module-level `PLANS_DIR = ".agents/plans"` was dead - resolution is via `_dirs()` /
# `resolve_record_path` - and was removed in Order 05, S5-DC01.)
INDEX_JSON = "INDEX.json"
INDEX_MD = "INDEX.md"
_MD_HEADER = "<!-- generated by `aw index plans`; do not edit by hand -->"

_META_RE = {
    "Date": re.compile(r"(?m)^- Date:\s*(.+?)\s*$"),
    "Kind": re.compile(r"(?m)^- Kind:\s*(.+?)\s*$"),
    # PARITY CONSTRAINT (IPD e32j35 E-03): this `Status` pattern captures the WHOLE line, while the
    # SELECTOR's `selectors._STATUS_RE` (selectors.py:89) uses `(\S+)` and therefore yields no match
    # for a multi-word value. Measured 2026-09-01: they diverge on 24 of 469 plans. The index is the
    # DISPLAY/manifest reader and the selector is the MATCHING reader, so the difference is currently
    # harmless - but any code that resolves a selector FROM this column must re-apply the selector's
    # single-token rule, or `aw find plans EXECUTED` silently widens. See the note at that line.
    "Status": re.compile(r"(?m)^- Status:\s*(.+?)\s*$"),
    "Id": re.compile(r"(?m)^- Id:\s*([0-9a-z]{6})\s*$"),
    "Set": re.compile(r"(?m)^- Set:\s*(.+?)\s*$"),
    "Order": re.compile(r"(?m)^- Order:\s*(\d+)\s*$"),
}

_EXCLUDE_NAMES = {"INDEX.json", "INDEX.md", "STATUS.md", "README.md"}


class PlanEntry(NamedTuple):
    """One indexed plan (derived from its disposition dir + metadata)."""

    plan_id: Optional[str]
    path: str  # posix, relative to the plans dir
    disposition: (
        str  # top-level dir (pending/executed/superseded/not-executed/reusable)
    )
    date: str
    set_id: Optional[str]
    order: Optional[int]
    status: Optional[str]
    kind: Optional[str]


def _meta(text: str, key: str) -> Optional[str]:
    m = _META_RE[key].search(text)
    return m.group(1) if m else None


def set_terse_id(raw: Optional[str]) -> Optional[str]:
    """Extract the canonical terse set-id from a `Set:` value of the form ``<terse> (<descriptive>)``.

    The leading whitespace-delimited token before any ``(`` is the set-id; the parenthetical is a
    human-readable display name (plans-adopter Order 06). A bare terse id (no parenthetical) also
    parses. Returns None for None.
    """

    if raw is None:
        return None
    return raw.split("(", 1)[0].strip() or None


def scan_plans(plans_dir: Path) -> Tuple[List[PlanEntry], List[_core.Drift]]:
    """Recursively scan every plan under ``plans_dir``; return (entries, drift).

    Disposition is the first path component under ``plans_dir`` (so a plan inside a
    ``<disposition>/YYYYMM/`` shard keeps its top-level disposition). Drift here covers the
    metadata classes (missing/invalid Id); name-vs-metadata and stale-view/dangling are added by
    ``check_drift``.
    """

    entries: List[PlanEntry] = []
    drift: List[_core.Drift] = []
    if not plans_dir.is_dir():
        return entries, drift
    ignored_dirs = _core.get_ignored_dirs(plans_dir)
    for p in sorted(plans_dir.rglob("*.md")):
        if p.name in _EXCLUDE_NAMES or _core.is_ignored_path(
            p, plans_dir, ignored_dirs
        ):
            continue
        rel = p.relative_to(plans_dir).as_posix()
        disposition = rel.split("/", 1)[0]
        text = p.read_text(encoding="utf-8")
        plan_id = _meta(text, "Id")
        order_raw = _meta(text, "Order")
        entries.append(
            PlanEntry(
                plan_id=plan_id,
                path=rel,
                disposition=disposition,
                date=str(_meta(text, "Date") or ""),
                set_id=set_terse_id(_meta(text, "Set")),
                order=int(order_raw) if order_raw else None,
                status=_meta(text, "Status"),
                kind=_meta(text, "Kind"),
            )
        )
        # Drift class (a): missing/invalid Id. Terminal grandfathered plans are still expected to
        # carry an Id after migration; report a missing/invalid one regardless of disposition.
        if plan_id is None:
            drift.append(_core.Drift(rel, "id-missing", "no `- Id:` metadata line"))
        elif not _core.is_valid_id6(plan_id):
            drift.append(
                _core.Drift(
                    rel, "id-invalid", f"Id '{plan_id}' is not a 6-char base36 token"
                )
            )
    return entries, drift


def build_index_json(entries: List[PlanEntry]) -> str:
    """Render the complete plans manifest as deterministic JSON (sorted by path)."""

    payload = [e._asdict() for e in sorted(entries, key=lambda e: e.path)]
    return json.dumps(payload, indent=2, sort_keys=True) + "\n"


def _set_last_touched(entries: List[PlanEntry]) -> Dict[str, str]:
    lt: Dict[str, str] = {}
    for e in entries:
        key = e.set_id or f"(singleton) {e.path}"
        if key not in lt or e.date > lt[key]:
            lt[key] = e.date
    return lt


def build_index_md(entries: List[PlanEntry], limit: int = DEFAULT_INDEX_LIMIT) -> str:
    """Render the browse-by-Set view: group by Set, members in Order, Sets by recent activity,
    bounded to the N most-recent Sets; Set-less plans in a singleton band."""

    last_touched = _set_last_touched(entries)
    # Group by Set (Set-less -> the singleton band, keyed by None).
    by_set: Dict[Optional[str], List[PlanEntry]] = {}
    for e in entries:
        by_set.setdefault(e.set_id, []).append(e)

    real_sets = [s for s in by_set if s is not None]

    def _inv_date(d: str) -> str:
        return "".join(str(9 - int(c)) if c.isdigit() else c for c in d) if d else ""

    # Sets ordered by most-recent activity (desc), tie-broken by set-id asc.
    ordered_sets = sorted(
        real_sets, key=lambda s: (_inv_date(last_touched.get(s, "")), s)
    )
    shown = ordered_sets[:limit]

    lines = [_MD_HEADER, "", "# Plans by topic (Set)", ""]
    lines.append(
        f"Showing the {len(shown)} most-recent of {len(real_sets)} Sets; the full manifest is INDEX.json."
    )
    lines.append("")
    for s in shown:
        members = sorted(
            by_set[s], key=lambda e: (e.order if e.order is not None else 0, e.path)
        )
        lines.append(f"## {s}")
        lines.append("")
        for e in members:
            oid = f"{e.order:02d}" if e.order is not None else "--"
            pid = e.plan_id or "??????"
            lines.append(f"- `{pid}` [{e.disposition}] {oid} {e.path}")
        lines.append("")
    singletons = by_set.get(None, [])
    if singletons:
        lines.append("## (singletons: no Set)")
        lines.append("")
        for e in sorted(singletons, key=lambda e: e.path):
            pid = e.plan_id or "??????"
            lines.append(f"- `{pid}` [{e.disposition}] {e.path}")
        lines.append("")
    return "\n".join(lines)


# --------------------------------------------------------------------------------------
# find
# --------------------------------------------------------------------------------------


def query(
    entries: List[PlanEntry],
    *,
    plan_id: Optional[str] = None,
    set_id: Optional[str] = None,
    status: Optional[str] = None,
    disposition: Optional[str] = None,
) -> List[PlanEntry]:
    out = list(entries)
    if plan_id:
        out = [e for e in out if e.plan_id == plan_id]
    if set_id:
        out = [e for e in out if e.set_id == set_id]
    if status:
        out = [e for e in out if e.status == status]
    if disposition:
        out = [e for e in out if e.disposition == disposition]
    return sorted(out, key=lambda e: (e.set_id or "", e.order or 0, e.path))


# --------------------------------------------------------------------------------------
# citation matcher + current-id resolver for the dangling detector (plan area)
# --------------------------------------------------------------------------------------

# A plan CITATION: the clustered/full plan filename OR a bare stem that resolves to a PLAN via the
# manifest. To avoid the specs-namespace collision, the dangling detector resolves ids from the
# plan manifest only; a bare stem is matched by the caller-supplied resolver, not blindly.
_PLAN_ID_CITE_RE = re.compile(r"\bPLAN-([0-9a-z]{6})\b")


def _plan_cite_matcher(line: str) -> List[str]:
    """Extract plan-id citations: the explicit ``PLAN-<id6>`` handle (bare stems/filenames resolve
    via the manifest path, not by id, so they are handled by the rename tool in Order 04)."""

    return _PLAN_ID_CITE_RE.findall(line)


# --------------------------------------------------------------------------------------
# check
# --------------------------------------------------------------------------------------


def check_drift(
    repo_root: Path, plans_dir: Path, limit: int = DEFAULT_INDEX_LIMIT
) -> List[_core.Drift]:
    entries, drift = scan_plans(plans_dir)
    # Drift class (b): name-vs-metadata mismatch on a CLUSTERED name only. Old-style
    # YYYYMMDD-HHMM-NN-slug.md names can coincidentally fit the clustering regex (HHMM as a set-id,
    # a slug's first letters as an id6), so distinguish by the second segment: the clustering grammar
    # never has a 4-digit HHMM there (a set-id is kebab, not exactly 4 digits). A name is treated as
    # clustered only when the regex matches AND the second segment is not a bare 4-digit HHMM.
    for e in entries:
        base = Path(e.path).name
        # The clustered grammar is parsed by the single naming authority (IPD o6b8l3).
        m = _naming.parse_clustered(base)
        if not m or re.fullmatch(r"\d{4}", m.group("set")):
            continue  # not clustered (or an old-style HHMM name)
        if e.plan_id and m.group("id6") != e.plan_id:
            drift.append(
                _core.Drift(
                    e.path,
                    "name-metadata-mismatch",
                    f"name id {m.group('id6')} != metadata Id {e.plan_id}",
                )
            )
        if e.set_id and m.group("set") != e.set_id:
            drift.append(
                _core.Drift(
                    e.path,
                    "name-metadata-mismatch",
                    f"name set {m.group('set')} != metadata Set {e.set_id}",
                )
            )
    # Drift class (c): stale generated view (byte-compare in memory vs disk).
    want_json = build_index_json(entries)
    want_md = build_index_md(entries, limit=limit)
    jp = plans_dir / INDEX_JSON
    mp = plans_dir / INDEX_MD
    # idxuntrack 02 (yvvf98) E-02: MISSING and STALE are reported as DISTINCT rules, because once
    # the manifest is a gitignored generated view they are materially different conditions. An absent
    # manifest is the NORMAL state of a fresh clone or a fresh worktree (nothing has generated it
    # yet), so it is informational and must not fail the gate; a manifest that is present but not
    # byte-equal to a rebuild is real drift. Severities live in `check_engine.RULE_REGISTRY`, never
    # here: `artifact_core.drift_exit_code` fails on anything that is not `info`.
    #
    # The severity is STAMPED HERE (via `check_engine.enrich_drift`) rather than left for a caller to
    # apply, and that is load-bearing rather than tidy. MEASURED: `run_index` computes its exit status
    # with `artifact_core.drift_exit_code` on whatever this function returns, and `check_engine.
    # check_content` forwards these findings WITHOUT enriching them. An un-enriched `Drift` carries
    # `severity=""`, and `drift_exit_code` fails the gate for anything that is not `info`, so merely
    # REGISTERING the rules left a missing manifest still exiting 1 on both surfaces. Enriching at the
    # emitter is what makes the registry the single source of severity for every consumer.
    from agent_workflows import check_engine as _ce

    for path_const, exists, matches, label in (
        (
            INDEX_JSON,
            jp.is_file(),
            jp.is_file() and jp.read_text(encoding="utf-8") == want_json,
            "INDEX.json",
        ),
        (
            INDEX_MD,
            mp.is_file(),
            mp.is_file() and mp.read_text(encoding="utf-8") == want_md,
            "INDEX.md",
        ),
    ):
        if not exists:
            drift.append(
                _ce.enrich_drift(
                    _core.Drift(
                        path_const,
                        "check.stale-index-missing",
                        f"{label} has not been generated; run 'aw index plans'",
                    ),
                    recovery="aw index plans",
                )
            )
        elif not matches:
            drift.append(
                _ce.enrich_drift(
                    _core.Drift(
                        path_const,
                        "check.stale-index-stale",
                        f"{label} is out of date; run 'aw index plans'",
                    ),
                    recovery="aw index plans",
                )
            )
    # Drift class (d): dangling plan citation via the shared-core detector, using the ONE unified
    # id6-handle matcher (IPD 3cmnfc E-04): the PLAN-<id6> handle recognized uniformly with the
    # research RSCH-<id6> handle. The dead-bare-filename flag (OQ-01 option B) is delivered as a
    # tested library primitive (artifact_refs.dead_filename_citations) but is NOT wired as an
    # always-on drift rule here: on real prose it flags legitimate historical/example filenames
    # (false positives), which would violate the Set's "no new aw check findings" criterion and
    # OQ-01's own low-false-positive mandate; enabling it awaits a durable "known real names"
    # source (run decision 05-3cmnfc-D3). setid citations are NOT checked (option C deferred).
    from agent_workflows import artifact_refs as _refs

    current_ids = {e.plan_id for e in entries if e.plan_id}
    for d in _core.find_dangling_citations(
        repo_root,
        current_ids=current_ids,
        cite_matcher=_refs.make_cite_matcher("PLAN"),
        exclude_root=plans_dir,
    ):
        drift.append(
            _core.Drift(f"{d.file}:{d.line}", "dangling-citation", f"PLAN-{d.id6}")
        )
    return drift


# --------------------------------------------------------------------------------------
# CLI
# --------------------------------------------------------------------------------------


def _dirs(args: argparse.Namespace) -> Tuple[Path, Path]:
    from agent_workflows.project_context import resolve_verb_repo_root

    repo_root = resolve_verb_repo_root(getattr(args, "dir", None))
    from agent_workflows.record_producers import resolve_record_path

    try:
        plans_dir = resolve_record_path("plans", target_repo=str(repo_root))
    except Exception:
        plans_dir = repo_root / ".aw" / "records" / "plans"
    if not plans_dir.is_dir() and (repo_root / ".agents" / "plans").is_dir():
        plans_dir = repo_root / ".agents" / "plans"
    elif not plans_dir.is_dir() and (repo_root / ".aw" / "records" / "plans").is_dir():
        plans_dir = repo_root / ".aw" / "records" / "plans"
    return repo_root, plans_dir


def run_index(args: argparse.Namespace) -> int:
    repo_root, plans_dir = _dirs(args)
    limit = getattr(args, "limit", None) or DEFAULT_INDEX_LIMIT
    if getattr(args, "check", False):
        drift = check_drift(repo_root, plans_dir, limit=limit)
        if getattr(args, "agent", False):
            print(_core.render_agent_drift(drift), end="")
            return _core.drift_exit_code(drift)
        if not drift:
            print("plans index --check: clean")
            return 0
        for d in drift:
            print(f"{d.location}: {d.rule}: {d.detail}")
        # idxuntrack 02 (yvvf98) E-02: defer to the SHARED severity convention instead of a hardcoded
        # `return 1`, so this human-readable branch and the `--agent` branch above cannot disagree
        # about one repository state. MEASURED: with the literal 1, an `info` finding (a manifest that
        # has merely never been generated, the normal state of a fresh clone or fresh worktree) still
        # failed the gate here while `--agent` correctly returned 0 on the same tree.
        return _core.drift_exit_code(drift)
    entries, drift = scan_plans(plans_dir)
    json_path = plans_dir / INDEX_JSON
    md_path = plans_dir / INDEX_MD
    new_json = build_index_json(entries)
    new_md = build_index_md(entries, limit=limit)

    json_changed = (not json_path.exists()) or (
        json_path.read_text(encoding="utf-8") != new_json
    )
    md_changed = (not md_path.exists()) or (
        md_path.read_text(encoding="utf-8") != new_md
    )

    if json_changed:
        json_path.write_text(new_json, encoding="utf-8")
    if md_changed:
        md_path.write_text(new_md, encoding="utf-8")

    # Regeneration always writes the CURRENT state (a snapshot); metadata drift (e.g. a plan still
    # lacking an Id) is reported but does NOT block the write. Use `--check` as the gate.
    if drift and not getattr(args, "quiet", False):
        for d in drift:
            print(f"{d.location}: {d.rule}: {d.detail}")
    if not getattr(args, "quiet", False):
        # THE OUTCOME WORDS BELOW ARE NOT LIFECYCLE STATUSES, and this module is deliberately NOT
        # converted to the shared lifecycle resolver (plan `9zvl2w` E-01, spec `uonrjg` R10.3).
        # Measured 2026-09-20: all four `status_256` calls in this function render `up to date`,
        # `wrote` or `updated`, each of which greps to ZERO in spec `uonrjg`. They are exactly the
        # "command-level OK, WARN, and FAIL" class R10.3 keeps valid and OUTSIDE that spec, with its
        # explicit warning not to "mechanically replace every checkmark". Routing them through
        # `resolve_lifecycle` would send each to criterion A20's UNKNOWN path and print `?` where a
        # user reads `up to date`: a regression delivered as compliance.
        #
        # AND THIS MODULE HAS NO LIFECYCLE TERMINAL VIEW TO CONVERT. A plan's status does appear in
        # `INDEX.md`, but that is a COMMITTED markdown file (`generated by aw index plans; do not
        # edit by hand`), not human terminal output, and Section 9.5 plus criterion A14 keep
        # committed and machine artifacts ANSI-free. R10.3's "plan index" obligation is satisfied
        # for this module by the plan statuses that reach a human through `aw find`, `aw ipd lint`,
        # `aw set` and `aw attention`, every one of which IS converted.
        #
        # WHAT WAS ACTUALLY WRONG HERE WAS THE COLOR DECISION, not the palette (criteria A14/A11).
        # `Term(color=not no_color)` FORCED color on for every caller that did not pass
        # `--no-color`, bypassing `should_color` entirely, so this line emitted ANSI under `--agent`,
        # under `--json`, under `NO_COLOR=1`, under `TERM=dumb` and into a pipe (all five measured
        # 2026-09-20; `aw find --agent` and `aw ipd lint --agent` emitted zero by contrast). The fix
        # is `color=False if no_color else None`, the package-standard idiom, which defers to the ONE
        # precedence chain in `term.should_color` (flag beats env beats detection), plus the explicit
        # machine-mode suppression below. The WORDS and their palette indices are unchanged.
        from agent_workflows.result_types import select_output
        from agent_workflows.term import Term

        # MACHINE MODE FORCES COLOR OFF, read through the shared output contract rather than by
        # re-testing the flags here, so this command cannot disagree with every other command about
        # what `--agent` means. `select_output` already resolves `color=False` for both AGENT and
        # JSON modes and applies `should_color` for human output.
        try:
            _ctx = select_output(args)
            _color = _ctx.color
        except Exception:
            # A conflicting-flag pair is the caller's error to report, not this print path's; fall
            # back to the ordinary human decision rather than crashing a successful regeneration.
            _color = None if not getattr(args, "no_color", False) else False
        term = Term(color=_color)
        try:
            rel_dir = plans_dir.relative_to(repo_root).as_posix()
        except ValueError:
            rel_dir = plans_dir.as_posix()
        dir_prefix = f"{rel_dir}/" if rel_dir and not rel_dir.endswith("/") else rel_dir
        dir_styled = term.color256(dir_prefix, 33)

        if not json_changed and not md_changed:
            status_lbl = term.status_256("up to date", width=12)
            files_str = f"{dir_styled}{INDEX_JSON}, {INDEX_MD}"
            term.line(f"{status_lbl} {files_str} ({len(entries)} plans)")
        elif json_changed and md_changed:
            status_lbl = term.status_256("wrote", width=12)
            files_str = f"{dir_styled}{INDEX_JSON}, {INDEX_MD}"
            term.line(f"{status_lbl} {files_str} ({len(entries)} plans)")
        elif md_changed:
            status_lbl = term.status_256("updated", width=12)
            files_str = f"{dir_styled}{INDEX_MD}"
            term.line(
                f"{status_lbl} {files_str} ({INDEX_JSON} up to date; {len(entries)} plans)"
            )
        else:
            status_lbl = term.status_256("updated", width=12)
            files_str = f"{dir_styled}{INDEX_JSON}"
            term.line(
                f"{status_lbl} {files_str} ({INDEX_MD} up to date; {len(entries)} plans)"
            )
    return 0


def run_find(args: argparse.Namespace) -> int:
    repo_root, plans_dir = _dirs(args)
    entries, _drift = scan_plans(plans_dir)
    results = query(
        entries,
        plan_id=getattr(args, "id", None),
        set_id=getattr(args, "set", None),
        status=getattr(args, "status", None),
        disposition=getattr(args, "disposition", None),
    )
    if not results:
        from agent_workflows.result_types import NextAction
        from agent_workflows.term import Term

        filters_dict = {}
        for k in ("id", "set", "status", "disposition"):
            v = getattr(args, k, None)
            if v:
                filters_dict[k] = v
        Term().empty_result(
            summary="no matching plans",
            filters=filters_dict if filters_dict else None,
            next_action=NextAction(
                command="aw find plans", description="find across all plans"
            )
            if filters_dict
            else NextAction(
                command="aw ipd scaffold", description="scaffold a new plan"
            ),
        )
        return 0
    for e in results:
        print(f"{e.plan_id or '??????'}\t{e.disposition}\t{e.set_id or '-'}\t{e.path}")
    return 0
