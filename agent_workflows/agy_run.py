...
This is the packaged core (runnernorm follow-up puot79e04): graduated from
``tools/agy_run.py`` and exposed as the non-colliding ``aw agy exec`` subcommand.
``tools/agy_run.py`` remains as a thin compat shim that re-exports this module.
(``aw agy run``/``runagy``/``runipd`` stay aliased to the separate multi-IPD queue
driver ``agy_runipd`` - this single-target multi-mode runner is genuinely distinct.)
...
# root, so resolve the prompt dir as <repo-root>/tools/awphysical rather than the (nonexistent)
# agent_workflows/awphysical a bare parent/"awphysical" would yield.
_PROMPT_DIR = Path(__file__).resolve().parent.parent / "tools" / "awphysical"
