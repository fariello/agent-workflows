...
### D126. Adopt four logical AW project roots with explicit ownership boundaries

- **Context:** the installed layout spreads installer-owned framework files, user configuration, workflow-created project records, and tool-operational metadata across `.agents/`, `workflow-artifacts/`, host-specific dot directories, and installer scratch paths. The proposed three-root design (`system`, `config`, `records`) still forced per-project AW reminders and install history into either user configuration or project records, neither of which has the right ownership or lifecycle.
- **Decision:** define four logical roots for every AW-enabled project: `system` (AW-installed and updated content, written only by the CLI), `config` (human-controlled policy), `state` (AW operational facts, actions, histories, and transactions), and `records` (project artifacts created by workflows and humans). The tree is logical: a selected backend may resolve a root outside the target and a literal target `.aw/` need not exist. Files outside the roots are permitted only as thin, manifest-owned adapters at host-required discovery paths. `state` is deliberately separate from `records` because installing, updating, or configuring AW is not project work.
- **Applied:** design only in `.agents/docs/specs/20260809-2211-01-aw-project-layout-storage-wizard-and-state.spec.md`; implementation deferred to the `awlayout (AW project layout)` IPD Set. Current product paths and behavior are unchanged.

### D127. Keep records outside the target by recommended default; separate delivery mode from records backend

- **Context:** local-only `workflow-artifacts/` reduces accidental publication but is not durable by itself, while tracking plans, prompts, research, assessments, and candid conversations in a public target can disclose material that should remain private. D109 correctly defined tracked versus clean-delta DELIVERY, but coupled clean-delta records to one sibling-companion design and did not give tracked installs an independent private-records choice.
- **Decision:** preserve exactly two delivery modes (`tracked`, `clean-delta`) and add an independent records-backend choice (`home`, `companion`, `repository`). Recommend `home` on first interactive install, resolving records under the project's `AW_HOME` tree and creating no target `.aw/records/`. Keep `repository` as an explicit collaboration choice and `companion` as the project-isolated private choice. Report durability separately as observable state (`unversioned`, `local-git`, `durable-private`, `repository-managed`, or `unknown`); external placement is not called durable until a private remote or another acknowledged backup exists. AW never creates or pushes a remote without explicit authorization and never promises secrecy it cannot verify.
- **Applied:** design only in the canonical 2026-08-09 specification; implementation deferred to Orders 01 through 03, 08 through 10 of the `awlayout (AW project layout)` Set. This extends D109 and supersedes only its coupling of clean-delta to a sibling companion.

### D128. `aw install` has an accessible color wizard on first install and a policy checkpoint on update

- **Context:** storage and delivery choices have privacy, collaboration, portability, host-support, migration, and durability consequences. Silent defaults are unsafe, but replaying a long first-install interview on every update is burdensome. The maintainer wants detailed pros and cons and values useful color, while the existing terminal contract requires color to remain optional and redundant.
- **Decision:** on a TTY, first install runs a self-contained repository-specific wizard before writes unless a complete explicit policy was supplied. It explains delivery, records backend, durability, paths, host integrations, tracking, migration, and uninstall consequences; labels the external home records choice as recommended; and ends with an exact change review. Every interactive update shows a concise policy checkpoint with installed/proposed versions, current routes and durability, warnings, and `keep current policy and update` as the default, plus a visible path into the full detailed choices. Use the existing accessible 16-color `Term` rules, including `NO_COLOR`, `FORCE_COLOR`, `--no-color`, TTY detection, `TERM=dumb`, word labels, and linear screen-reader output. Noninteractive first install requires explicit policy or a complete saved profile and otherwise fails before writes; configured updates may reuse policy under `--yes`.
- **Applied:** design only in the canonical 2026-08-09 specification; implementation deferred to Order 04 and end-to-end verification in Order 11. Current installer behavior is unchanged.

### D129. Per-project AW actions are a native state source for the existing attention projection

- **Context:** post-install guidance such as running `/setup-repo` must persist until completed or dismissed and must appear in `/whatnext`, but it is AW operational state rather than a general project task or record. D125 now provides the canonical cross-tree `aw attention` projection and `/whatnext` consumes it fail-closed. A second status aggregator would conflict with that owner model. A task file per update would also create noise, while status only inside front matter would make cross-item state expensive to survey. Prefixing every file and command with `aw-` repeats scope already supplied by the resolved `state` root.
- **Decision:** store AW actions under the resolved `state/actions/{open,completed,dismissed,superseded}/` directories, with location as authoritative native status. Use concise logical IDs such as `setup-repo`, integer generations, and stored filenames such as `setup-repo-v2.md`; omit an `aw-` filename prefix. Humans use `aw todo`, `aw show setup-repo`, `aw dismiss setup-repo`, `aw complete setup-repo`, `aw reopen setup-repo`, and `aw history setup-repo`; generation is implicit for the current open action and optionally addressable as `setup-repo@1`. A fresh install creates the current `setup-repo` generation. Updates append to `state/history/installs.jsonl` but create an action only when attention persists. Completed or dismissed generations are not resurrected; materially new obligations create a new generation and may supersede unresolved older ones. Extend D125's pure attention contract with a native action-source mapping (`open` -> `ready`, `completed` -> `done`, `dismissed` or `superseded` -> `parked`). `aw todo` owns action writes and direct queries; `aw attention` remains the only cross-tree projection; `/whatnext` continues to consume `aw attention --format json` rather than scanning action files or calling a competing aggregator.
- **Applied:** design only in the canonical 2026-08-09 specification; implementation deferred to Orders 06 and 07 of the `awlayout (AW project layout)` Set. Current product behavior is unchanged. EXTENDS D125.

### D130. Replace the logical-only layout with a physical `.aw/` namespace and dogfood it in the source repository

- **Context:** D126 through D129 and the implemented 2026-08-09 specification introduced four logical roots, but deliberately allowed tracked system content and records to remain under `.agents/` and allowed no literal target `.aw/` directory. That did not implement the maintainer's intended physical segregation. It also left portable and machine-local configuration together, mixed durable facts with transaction scratch, and made the agent-workflows source checkout a special legacy layout rather than a proof of the installed model.
- **Decision:** supersede `.agents/docs/specs/20260809-2211-01-aw-project-layout-storage-wizard-and-state.spec.md` with `.agents/docs/specs/20260810-1447-01-physical-aw-hierarchy-placement-and-migration.spec.md`. Every selected storage container uses the canonical physical tail `.aw/{system,config,state,records}`; config is split into `config/project.json` and never-tracked `config/local.json`; state is split into policy-trackable `state/durable/` and never-tracked `state/runtime/`. Presets decide whether each class is target-tracked, target-ignored, home-untracked, companion-tracked, companion-untracked, or protected source-checkout. Clean-target may contain no target `.aw/`, but its external container still uses the same physical namespace. The agent-workflows repository must migrate its canonical workflow source to `.aw/system/` and its own records to the resolved `.aw/records/`, using a verified source-checkout role and the same inventory, transaction, rollback, and independent-postcheck contracts as other projects. Migration is copy-verify-switch-retain and never deletes legacy material during cutover.
- **Applied:** design and review package only. The superseding spec is `to-review`; the `awphysical` Set remains NO-GO until human approval, then Orders 01 through 12 implement and dogfood the decision. This entry revises D126's logical-only allowance and D127's backend abstraction where they conflict, while preserving D128's wizard/accessibility contract and D129's AW-action semantics.

### D131. Keep `## Workflow history` free-form prose; decline machine-readable history for now

- **Context:** a 2026-08-07 plan (`20260807-ipd-history-01-wrt0wq`) proposed a machine-readable workflow-history grammar (`- <date> [Status: <from> -> <to>] (<actor>): <event> - <summary>`), an `aw ipd log-event` CLI, and a lint rule. Reviewing it 2026-08-13 found two of its four items already shipped (plan-review appends a `reviewed` line; the lifecycle requires `approved`/`executed` lines before `git mv`), and the remaining structured-history proposal conflicts with D52's deliberately free-form prose provenance (every existing plan's history, across the whole `executed/` corpus and downstream repos with legacy free-text lines, is prose). The maintainer asked whether the current format is script-interpretable: `- Status:` IS a fully machine-parseable bare enum; workflow-history lines are SEMI-structured (a script can extract date + actor + event token, but the status transition and outcome are prose).
- **Decision:** keep `## Workflow history` as appended free-form prose (D52 unchanged). DECLINE the machine-readable history grammar and the `aw ipd log-event` CLI. Rationale, in the maintainer's terms: it is not obvious WHEN a machine-readable history would actually be needed, and adopting a rigid grammar would NOT be backward compatible (it would flag or require migrating the existing prose corpus and downstream repos' legacy lines). The cost is concrete and immediate; the benefit is speculative. Revisit only if new information shows a real consumer that needs the transition/outcome parsed from history rather than from the already-machine-readable `Status:` field. SALVAGE the one still-useful, convention-compatible item as a scoped follow-up: a lint guard (`IPD-S405`) that a terminal `Status: executed` plan MUST carry an `executed` workflow-history line (a PRESENCE check keyed on the free-form event token, imposing no grammar).
- **Applied:** retired `20260807-ipd-history-01-wrt0wq` to `superseded/` (E-03/E-04 already shipped; E-01/E-02 declined per this entry). The salvaged `IPD-S405` guard is planned as the singleton IPD `20260813-ipdexechist-01-...` (id 69xrut, `to-review`). This entry does not change D52; it records the deliberate decision NOT to structure history and the condition to reopen it.

### D132. Honest human-approval attestation (--by-human replaces --yes-i-am-human)

- **Context:** D125 established owner-written spec status and an anti-self-approval floor requiring an interactive TTY confirmation and `--yes-i-am-human` to set `approved`. In practice, this forced a dishonest ritual: an executing agent on a human's behalf cannot satisfy `sys.stdin.isatty()`, and `--yes-i-am-human` forces the agent to assert "I am human" (a falsehood). A local CLI gate cannot stop a truly malicious agent that can rewrite code; attempting to do so is security theater. The defensible intent of the floor is a conscious speed bump ensuring an honest agent does not silently auto-advance human-owned transitions without explicit confirmation.
- **Decision:** replace the TTY-enforced `--yes-i-am-human` gate on `aw specs set` with an honest, non-TTY `--by-human` attestation flag. Setting `reviewed -> approved` succeeds when `--by-human` is explicitly passed and is refused byte-identical otherwise. The required `--message` argument records attributed provenance (who approved and how) in the workflow history line as `- <date> approved (aw specs, --by-human): <message>`. The `implemented` evidence citation gate, `deferred` typed gates, atomic writes, and git-side-effect-free properties remain unchanged. This deliberately narrows D125's floor from "TTY-enforced, agent-unsatisfiable" to "explicit --by-human attestation, agent-recordable".
- **Applied:** `agent_workflows/specs.py`, `agent_workflows/cli.py`, `agent_workflows/attention_contract.py`, `agent_workflows/engine.py`, `.agents/docs/specs/README.md`, `AGENTS.md`. Implements approved specification `20260815-0151-01` per IPD `20260815-humanapproval-01-0g0rid-by-human-attestation.md`. REVISES D125.

### D133. `aw attention` human view: 256-color, tree-tagged, gate-in-header (non-TTY output unchanged)

- **Context:** the attention board printed one uniform plain line per item, `- [tree] path (status){gate}`. The maintainer wanted a scannable human view: drop the leading `[tree]` bracket, color the native status and the tree name, and fold a blocked item's shared gate artifact into the section header (`## blocked (2) in TODO.md`). `term.py` was previously restricted to the 16 named ANSI colors under the accessibility lens (which says to "not assume 256-color... prefer the 16 named colors").
- **Decision:** add an xterm-256 helper (`Term.color256`, SGR `38;5;N`) and render the attention board in two modes keyed on the stream's color decision (`should_color`, which still honors `NO_COLOR`/`FORCE_COLOR`/`TERM=dumb`/`isatty`): (a) COLOR ON (real TTY): no `[tree]` bracket, native status in bold + a status-specific 256-color, tree name as a bold-blue trailing tag, blocked gate folded into the header; (b) COLOR OFF (piped/agent/`--no-color`/`NO_COLOR`): the UNCHANGED machine-readable `- [tree] path (status){gate}` form, so agents and grep keep a stable parseable shape. `--check` and `--format json` are untouched. This is a maintainer-directed DELIBERATE loosening of the 16-color-only preference to 256-color for this view; the accessibility invariant that MATTERS is preserved (the status WORD and the readiness-class section name are always printed, so meaning survives monochrome / piping / a screen reader; `NO_COLOR`/non-TTY/`TERM=dumb` still degrade to plain).
- **Applied:** `agent_workflows/term.py` (`color256`), `agent_workflows/attention.py` (`render_board` two-mode render + `run` threads a `Term`), `tests/test_term.py`, `tests/test_attention.py`, and a note in `.agents/workflows/assess/lenses/accessibility.md`. Maintainer-directed 2026-08-15.

### D134. Canonical `.aw/system/` is a NESTED bundle with SIBLING VERSION/manifest

- **Context:** the physical `.aw/` migration (Set awphysical) had to settle where the workflow bundle and its VERSION/manifest live under `.aw/system/`. Two executed Orders had shipped contradictory assumptions: Order 04 (resolver/packaging) assumed FLAT (bundle directly at `.aw/system/`), while Order 09 (clean-delta pointers) and spec S4.1 assumed NESTED (`.aw/system/workflows/`). The first real self-migration forced the choice and the mismatch broke source resolution.
- **Decision:** the canonical shape is NESTED with SIBLING metadata: the invokable workflow bundle (index.md + bodies) lives at `.aw/system/workflows/`, while `VERSION`, `managed-sections.json`/`manifest.json`, and `templates/` are SIBLINGS at the `.aw/system/` root. `resolve_source_root` descends into `workflows/`; readers resolve VERSION at the system-root sibling. Rationale worked out with the maintainer: agents reach workflows by EXPLICIT PATH (host shims + `index.md` manifest), never by scanning `system/`, so nesting depth is discovery- and token-neutral; nested wins only on reasoning clarity (a clean invokable namespace separated from system metadata). Matches spec S4.1.
- **Applied:** `agent_workflows/engine.py` (`resolve_source_root` descent, `_nested_bundle_version_path`), `agent_workflows/_compat.py`, `agent_workflows/project_context.py`, `tools/awphysical/aw_layout_inventory.py` (classifier VERSION-sibling split), `pyproject.toml` (force-include `.aw/system`). Corrective IPD `20260816-awphysical-13-xzuxet` (executed).

### D135. Layout migration MOVES, it does not copy-and-retain

- **Context:** the shipped `aw migrate-layout apply` engine COPIED each classified legacy artifact into `.aw/` and left the source in place ("copy-verify-switch-retain"). Because it only switched WRITE authority while the read tooling (`artifact_core.SCAN_ROOTS`, `aw attention`/`plans`/`backlog`) scans BOTH `.agents/` and `.aw/records/` on the filesystem, every migrated repo ended with two live tracked copies of every record and `aw attention valid:false` (a duplicate id per pair). That is a duplicate installation, not a migration.
- **Decision:** the migration MOVES each classified item to its resolved destination (git-tracked items via `git mv`, preserving history and staging the rename; untracked items via a filesystem move), so no legacy twin remains and a migrated repo has exactly ONE logical layout. Reversibility comes from a per-item move journal (rollback un-moves), not from a permanently retained legacy copy. The move phase is journaled per item so a crash resolves to fully-migrated (resume, continuing from the transaction item list) or fully-legacy (rollback), never a torn state. After moving all classified items, an INTERACTIVE leftover disposition (keep / remove / defer, non-interactive default defer) governs anything not moved and never deletes without an explicit choice; only truly-empty legacy directories are pruned. `aw attention` keeps scanning legacy locations (near-zero cost; empty post-move). REVISES the spec 20260810-1447-01 copy-verify-switch-retain language to move-verify-switch.
- **Applied:** `agent_workflows/layout_migration.py` (`MigrationManager.execute_migration` move phase, `_perform_move`/`_handle_leftovers`, journal-driven `resume_migration`/`rollback_migration`), `agent_workflows/cli.py` (`--leftovers`), spec `20260810-1447-01` Sections 3/11.1/11.2. IPD `20260816-awphysical-14-hnzr8v` (executed).

### D136. Fresh installs target `.aw/`; legacy `.agents/` repos are auto-detected and offered migration

- **Context:** the installer hardcoded `WORKFLOWS_DIR = ".agents/workflows"`, so a fresh `aw install` produced the legacy layout even after `.aw/` became canonical. Existing `.agents/`-only repos must not break on upgrade.
- **Decision:** a FRESH install into a repo with no prior AW layout creates ONLY the `.aw/` hierarchy (canonical system at `.aw/system/workflows/`, records/config per preset), and generated content (command shims, the AGENTS pointer prose) references the resolved `.aw/system/workflows/` paths. `aw install`/`aw update`/`aw setup` DETECT a legacy `.agents/`-only layout and OFFER to migrate (delegating to the move-based tool); declining keeps updating the legacy layout in place for a documented compatibility window with a one-time deprecation notice, never creating a second divergent layout. The layout is resolved deterministically: `.aw/system` present is authoritative (no prompt) even beside a stray legacy tree; the non-interactive default on a legacy-only repo is keep-legacy + notice (never auto-migrate without consent, never block CI).
- **Applied:** `agent_workflows/engine.py` (`resolve_target_layout`/`resolve_workflows_dir`, layout-aware shim + pointer generation), `agent_workflows/cli.py` (`_handle_legacy_migration`, `--to-aw`/`--keep-legacy`). IPD `20260816-awphysical-15-7cvh9t` (executed).
...
### D139. Canonical workflow source is a typed YAML package compiled to generated projections (build-time only)

- **Context:** the `awoptimize` architecture Set (research `y1eb0q`; orchestrator `p070c8`; Order 01 `nmwy3m`) replaces prose-only workflow definitions with ONE typed canonical source per workflow, compiled into portable projections (prompt bundle, just-in-time step packets, machine-readable manifest, evidence requirements, human catalog row, adapter-neutral command descriptor) and thin host adapters. Order 01's gate requires this serialization/architecture decision (an ADR) to be approved BEFORE the loader is written; Order 01 OQ-01 resolved the format question. This entry is that ADR.
- **Decision:** the canonical workflow source is authored in YAML and parsed ONLY at DEV/BUILD/AUTHORING time. The compiler runs at build/authoring time and emits stdlib-parseable projections (JSON manifests/packets/evidence + Markdown) that are what ship and what the installed CLI consumes; the installed `agent_workflows/*` runtime NEVER imports a YAML parser. The YAML parser is therefore a BUILD-TIME-ONLY dependency (same category as `hatchling`), consistent with dependency minimization as a principle, not a prohibition (D138). YAML is chosen over JSON for the SOURCE because a human authors and reviews it and YAML supports comments and friendlier merges; over TOML because workflows are deeply nested and stdlib `tomllib` is read-only (no writer). Determinism (byte-stable generated output) is achieved by sorting keys and pinning writer options at emit time, independent of source format. The single-source-of-truth guarantee comes from the compiler plus a `check-generated` drift test that fails on any hand-edited generated artifact (Order 01 E-06/E-07), NOT from the format choice.
- **Boundaries / non-goals:** this ADR covers only the schema/source-format/compiler-output contract. Runtime state, the evidence ledger, host adapters, benchmarking, and workflow-family migration are separate approved Orders (02-08). No existing workflow behavior changes when the schema/compiler land; migration of the workflow families is Order 07, gated separately. The build-time YAML parser MUST NOT leak into a runtime import path (enforced by an Order 01 test).
- **Status:** APPROVED by the human maintainer (Gabriele Fariello, 2026-08-21) as the precondition ADR for executing Order 01 `nmwy3m`.
- **Applied:** referenced by awoptimize Order 01 (`nmwy3m`) E-01/E-02 (schema + canonical source layout) and E-05 (docs); to be cited by the schema reference/authoring guide that Order 01 produces. Evidence base: research `y1eb0q`.

### D140. The id6 in the filename identity slot is the unique identity of exactly one file; walkthroughs get their own id6 and reference their source plan, never by reusing it

- **Context:** `aw find p7dqwz` returned two files sharing the id6 `p7dqwz`: the plan `20260823-artifactenginefix-01-p7dqwz-...ipd.md` and its execution walkthrough `20260823-artifactenginefix-01-p7dqwz-execution.walkthrough.md`. The walkthrough had reused the PLAN's id6 in its own `YYYYMMDD-<setid>-NN-<id6>` filename identity slot (to make the plan<->walkthrough pairing greppable) and declared no `- Id:` frontmatter of its own. A repo-wide scan (`tmp/find_id6_dupes.py`) confirmed this is the ONLY real case: frontmatter `- Id:` values are globally unique (0 duplicates across 422 declaring files), and exactly one REAL id6 (`p7dqwz`) appears in two files' identity slots. The other apparent duplicates were legacy `YYYYMMDD-HHMM-NN-<slug>` names (no id6 slot; slug words like `agents`/`assess` matched greedily). The reuse was a one-off habit of one agent (Gemini in the Antigravity IDE, which also over-produced walkthroughs), not a systemic pattern. Walkthroughs are also OPTIONAL, not per-executed-plan (14 walkthroughs for 270 executed plans; the README obligation was corrected, see below), so encoding a plan<->walkthrough pairing by overloading the identity slot is doubly unjustified.
- **Decision:** the `<id6>` in the `YYYYMMDD-<setid>-NN-<id6>-<slug>` filename identity slot is the UNIQUE IDENTITY of that ONE file, and must equal that file's own declared `- Id:`. Identity and reference are distinct roles: an artifact MUST NOT place another artifact's id6 in its own identity slot. A walkthrough (or any artifact that records/derives from another) carries its OWN id6 in the slot and, when it needs to point at a source plan, expresses that link as a TYPED frontmatter field (e.g. `Target-Id:`/`References: <id6>`), never by reusing the source's id6 as its own identity. id6/setid citations elsewhere remain stable references and are never rewritten (unchanged by this decision).
- **Enforcement gap identified:** `check_engine.check_collisions` today reads only the frontmatter `- Id:` line (`_ID_LINE_RE`, `check_engine.py:199,237`); it NEVER inspects the filename identity slot, so a foreign id6 sitting in a filename slot (with no `- Id:` declared) is invisible to `aw check`/`aw doctor`. That is why `p7dqwz` passed. Closing this (make `aw check`/`aw doctor` also detect a filename-slot id6 that is not the file's own declared identity, i.e. the check `tmp/find_id6_dupes.py` performs) is a code change and is tracked as a new IPD in the `unifyfileio` Set.
- **Applied:** `.aw/records/walkthroughs/README.md` + the installer template `.aw/system/workflows/templates/agents-docs-walkthroughs-README.md` corrected (walkthroughs are OPTIONAL, not per-executed-plan). Fed into `unifyfileio` Order 01 (`o6b8l3`, naming authority + walkthrough grammar migration) as a blocking OQ, and a new `unifyfileio` child IPD adds the `aw check`/`aw doctor` identity-slot enforcement and renames the one offending `p7dqwz` walkthrough to its own id6 with a typed plan reference. Evidence: `tmp/find_id6_dupes.py`.
- **Applied (2026-09-20, IPD `sk7ggr`):** the "Enforcement gap identified" paragraph above is now PARTLY HISTORICAL and partly SUPERSEDED, and is deliberately left intact rather than rewritten. The identity-slot rule it asked for SHIPPED (`_check_identity_slots`, the (a)/(b) rule). Three further things were then measured and addressed, and one was NOT anticipated by this decision. (1) PREVENTION: id6 minting was per-TREE at all eleven call sites, so a cross-type collision could be CREATED even with the slot rule in place; every site now mints against a repository-wide, terminal-inclusive set (`artifact_core.global_id6s` / `mint_id6`). (2) DETECTION CORPUS: the collision scan inherited the engine's default liveness filter, so a collision whose other side sat in `executed/` was invisible to `aw check all` while `aw doctor` reported it; the id6 pass now ignores that filter (its setid and identity-slot neighbours deliberately do NOT, to avoid mass-flagging the legitimate shared-setid and walkthrough-slot conventions). Measured effect on this repository: `check.id6-collision` 1 -> 10, of which 8 are one pre-existing defect (backlog items tracked in both `graduated/` and `done/`, filed as `fwq5nu`). (3) NOT ANTICIPATED HERE: the remaining `uyeko5` findings are a FRONT-MATTER PARSER reading a `- Id:` out of QUOTED PROSE (a research document citing a plan's metadata block as an example), which is a reader bug and not an identity violation; the fix is in the reader and is owned by IPD `76w6mq` (backlog `cqytxf`), with a third unbounded reader outside that plan's scope filed as `q1ov25`. A quoted example is legitimate content, so no record was edited.

### D141. `aw ipd finalize` reconciles scope in two directions as a double-check gate; the frozen Scope-Paths stays; recovery captures truth, never fabricates

- **Context:** the `ipdgates` Set builds a gated terminal transition (`aw ipd begin` freezes base HEAD + `Scope-Paths`; `aw ipd finalize` compares changed paths against that allowlist) to prevent the p7dqwz failure (a plan marked executed while it edited a file outside its scope, with an unattributed `executed (aw set)` history). During /plan-review of Order 06 (remove-bypass) the maintainer raised three concerns: (1) a `Scope-Paths` allowlist frozen at start is unrealistic - in complex work, discovering mid-stream that you must touch an unforeseen file is common, often the rule, so a hard refusal on every unforeseen edit trains agents to treat the gate as noise (which then misses the real violation); (2) a refusal-only check catches only EXTRA work, never MISSING work (a file declared in scope but silently never touched - the "went off the rails / didn't do what it said" failure); (3) agents are NOT trained to update `Scope-Paths` en route or even to reliably run `aw ipd finalize` (they forget to write IPDs, forget to finalize, forget to move to executed), so any mechanism that DEPENDS on the agent remembering a mid-stream step will be skipped.
- **Decision:** `aw ipd finalize` is a DOUBLE-CHECK gate ("did you do what you said, and only that?"), NOT a correctness oracle and NOT a rewrite-history machine. (a) The begin-time frozen `Scope-Paths` is KEPT UNCHANGED (do not re-derive scope from the final diff - that would be tautological and lose the said-vs-did signal that catches p7dqwz). (b) finalize reconciles the scope delta in TWO directions in ONE batched, low-friction prompt at the unskippable step (Order 07 makes even `aw set executed` delegate into finalize, so it cannot be bypassed): an out-of-scope CHANGED path requires a short RECORDED REASON (given -> record + proceed; empty -> do not finalize); an in-scope-but-UNMODIFIED path requires a one-word ACKNOWLEDGMENT (acknowledge-and-proceed, NOT a hard blocker, because a declared-but-unneeded file is normal). Both answers are written verbatim into the terminal history/evidence. A clean delta is a silent no-op; a headless run with a non-empty delta and no answers fails closed. (c) The mechanism's power is SURFACING + ATTRIBUTION, not judgment: a low-barrier reason field is trivially greenwashed ("n/a"), so it does not adjudicate legitimacy - it makes the deviation impossible to HIDE, cheap to legitimately explain, and permanently on the record for a human/verifier to weigh. (d) finalize (the unskippable chokepoint), NOT an en-route verb, is the load-bearing mechanism, since agents will not reliably use an en-route verb; an OPTIONAL `aw ipd scope add <plan> <path> --reason` may pre-record an unforeseen need (pre-filling the finalize answer) but is never required and never edits the frozen receipt. (e) RECOVERY from a stuck IPD captures the truth of the misstep and lets the agent move on, NEVER fabricating or re-enacting a step that did not happen: forgetting `begin` does NOT get a retroactively back-dated receipt (a lie) - finalize records an honest "executed without a pre-execution receipt; scope checked retroactively, no frozen baseline" advisory and proceeds; a legitimate out-of-scope edit is light-touch acknowledge-and-own (widen `Scope-Paths` with a note, or record an accepted deviation), reverted only if genuinely wrong; a grandfathered/no-`Scope-Paths` plan gets an advisory-only scope check, never a lockout. "No escape hatch" (39fz2x OQ-02) means finalize never FAKES success, not that an IPD can get permanently stuck.
...
### D149. The concise-reporting contract has ONE source, reaches shims by POINTER, and never touches the review command line

- **Context:** agents invoked directly or through the runners spent tokens on preambles, routine narration, praise, recaps, and closing offers. No repository-wide contract separated concise user-facing REPORTING from complete engineering execution, and host-specific settings could not cover OpenCode, Codex CLI, Claude Code, and Antigravity consistently (different instruction surfaces, inconsistent provider verbosity fields).
- **Decision:** one host-neutral contract in `agent_workflows/reporting_contract.py` is the single source, delivered three ways: a separately owned `aw:reporting` managed section (so a repo can decline it or edit it without clobbering), a one-line POINTER in every generated command shim, and the FULL text in both IPD drivers' worker and verifier prompts (a fresh non-interactive worker must not depend on ambient host instructions). Three sub-decisions are load-bearing. (1) POINTER, not copy, in shims: the prose is ~1.7KB and the corpus is 48 files / ~42.7KB, so duplication measured +192% of input re-read on EVERY invocation, defeating the 2.x lower-token-cost goal; a size-budget test prevents regression, and the accepted residual risk is that a host which never resolves the pointer gets a weaker signal (mitigated by naming the key exception inline). (2) The review turn's prompt keeps returning EXACTLY `/plan-review <path>`: it is passed as one argv element, so appended prose would be absorbed as `$ARGUMENTS` path arguments; the review session inherits the contract from the shim pointer plus `AGENTS.md`. (3) Precedence is explicit and bidirectional: a workflow-mandated report is produced IN FULL with the word cap suspended, and brevity never licenses skipping required pasted runner output. Rejected: editing global user configuration (`~/.config/opencode`, `~/.claude`), provider-only fields (`textVerbosity`, temperature), and any output-token cap or truncation (can sever an error, an evidence paste, or a JSON outcome).
- **Status:** APPROVED by the human maintainer as part of the standing approval of the reviewed IPDs (2026-08-30).
- **Applied:** `agent_workflows/reporting_contract.py` (new), `agent_workflows/engine.py` (`AW_REPORTING_SLUG`, the sibling section, the shim pointer, the customization allowlist), `agent_workflows/oc_runipd.py` + `agent_workflows/agy_runipd.py` (worker/verifier prompts; review prompt documented as deliberately prose-free), regenerated `AGENTS.md` + `.opencode/commands/**` + `.claude/commands/**`, `docs/reporting-contract.md`, `tests/test_reporting_contract.py`. Executed per IPD `20260829-terseout-01-ntf6sx`. Honest limit recorded: the tests prove DELIVERY deterministically; model obedience is probabilistic and is not claimed.

### D150. The default correction budget is 2, per the spec, aligned while the value was still dormant

- **Context:** approved spec `25kzda` 2.1 states "The default correction budget is 2; the valid frozen range is 0 through 10", but `run_recovery.DEFAULT_RETRY_LIMIT` shipped as `3`. The discrepancy was found while scoping the `runcodes` successor to the retired `detrun` Set and was first recorded as a deliberately non-blocking deferred question (that plan's OQ-03) rather than silently "fixed", because changing a shipped default is a behavior change.
- **Decision:** the SPEC is authoritative; the default is 2. Two supporting sub-decisions are load-bearing. (1) TIMING: apply it NOW rather than when the runner consumes this layer, because `plan_retry`, `retry_budget_remaining` and `correction_required` were verified to have ZERO production callers (only tests exercise them), so the change cost two test edits and no behavior change; once the runner wires the layer up, the identical edit becomes a real change to how many paid model turns every failed step buys. (2) TESTS DERIVE, NEVER HARD-CODE: the two tests that pinned `3` as a literal now compute their expectations from `DEFAULT_RETRY_LIMIT`, so the default and its tests cannot drift apart again. Rationale on the merits, not merely deference to the spec: a retry here is a CORRECTION attempt rather than a network-flake retry, and `plan_retry`'s own contract is that "a retry cannot turn failure into success by mere repetition"; a corrector still failing after two passes is usually facing a plan defect, so a third attempt mostly buys another paid turn and delays escalation. Explicitly NOT decided here: the 0..10 range validation, which is independent and remains scoped to the `runcodes` plan.
- **Status:** APPROVED verbally by the human maintainer (Gabriele Fariello, 2026-08-31): "Default 2 I think is better."
- **Applied:** `agent_workflows/run_recovery.py` (`DEFAULT_RETRY_LIMIT: int = 2`, with the reasoning in a comment beside the constant), `tests/test_run_recovery_cli.py` (`test_retry_limit_escalates_not_loops` and `test_retry_budget_remaining` now derive from the constant). `runcodes-01` (`wlxkoz`) OQ-03 closed as resolved and its E-04 retargeted to the range check only. Verified: `python3 -m pytest tests/test_run_recovery_cli.py` -> 55 passed.

### D151. The open-question asking protocol gets an invocable command and a gate, because the principle alone was measured insufficient

- **Context:** `GUIDING_PRINCIPLES.md` principle 12 ("Ask self-contained questions") and the `AGENTS.md` managed pointer already required interactive, self-contained questions with full context and a recommendation. Both are in every agent's loaded context. Agents still finished work with the human's questions unasked. Measured 2026-09-08 in one session: an agent authored five plans carrying eleven open questions, asked the maintainer none of them, and reported completion; in the same session it addressed the maintainer using internal symbol names and bare `id6` handles as nouns, and a follow-up prompt referred to "the automatic block" from a previous answer instead of restating it, drawing the reply "What automatic check? I have zero context here." The maintainer's framing was that repeating the instruction across sessions is itself the waste. Separately, the pre-existing enforcement had a hole: `ipd_lint.check_open_questions` runs unconditionally while `check_checkpoint` does not, so a `Blocking: yes` / `Status: open` question was refused only at `pre-execution`, and a DEFAULT `aw ipd lint` reported such a plan `conforming` (verified against pending plan `xipfy1`).
- **Decision:** add `/askme` (also `/aw askme`) as the invocable entry point, and extend the existing blocking-question refusal to EVERY checkpoint. The workflow body REFERENCES P12 rather than restating it (P8), and adds only what P12 does not cover: sorting questions into resolve-from-evidence / ask-the-human / defer-with-an-owner, where the durable answer goes, and what to do when there is no human. Its memory kernel encodes the measured failures as rules: one question per prompt, never refer to a prior answer instead of restating it, no `id6`/symbol/finding-code as a subject, an explicit `(Strong Recommend)`/`(Recommend)`/`(Weak Recommend)` label, measure before asking when the answer depends on a number, and ask before reporting rather than after. THE GATE IS SCOPED TO `Blocking: yes` ONLY, and this is the load-bearing sub-decision: the alternative (refusing every open question) was measured first and would have failed 66 of 103 pending plans, most of them other agents' live work in a shared checkout, because a non-blocking open question with a stated lean is the NORMAL state of a plan in progress. Only 14 plans carried a blocking one. The narrow gate flags exactly those 14 and nothing else, and no pre-commit hook runs `ipd lint`, so no co-worker's commit breaks. Rejected explicitly, on the maintainer's ruling: any attempt to detect or prevent a DISHONEST tick. A checkbox and an evidence block are text an agent can write, no prompt wording changes that, and an earlier draft that promised to "never let the agent tick a box without doing the work" was security theater with no implementable mechanism behind it. The honest limit is recorded instead: this catches forgetting, not evasion; the run already holds independent evidence (diff, commits, session logs) against which a false claim is checkable, and detecting that contradiction belongs to the verifier path (backlog `wyw936`), not here.
- **Status:** APPROVED interactively by the human maintainer (Gabriele Fariello, 2026-09-08), through five recorded prompts using the very mechanism under discussion: the name `askme`, "Command, plus an automatic block", answers recorded "In the plan or spec that raised the question", self-resolvable questions to be "answered myself, recorded, and shown afterward", and after being shown the 66-versus-14 measurement, "Hard gate, but only on questions marked blocking". The maintainer initially chose a hard gate at every stage; the scope narrowed only after the measurement was put in front of them.
- **Applied:** `.aw/system/workflows/askme/askme.md` + `README.md` (new), one manifest row in `.aw/system/workflows/index.md` plus its narrative paragraph, generated shims `.opencode/commands/askme.md` and `.claude/commands/askme.md` (produced deterministically rather than by a full install, since this is a shared checkout; zero drift verified across all 54 shim members), a new family and disposition in `agent_workflows/migration_inventory.py` (whose fail-closed inventory caught the omission, as designed), the gate in `agent_workflows/ipd_lint.py` `check_open_questions`, `tests/test_askme_blocking_gate.py` (new, 12 tests across firing conditions, every-checkpoint coverage, no-double-report, and the real corpus), and one narrowed assertion in `tests/test_review_findings_gate.py` whose fixture is legitimately a blocking-open question, so it now asserts its actual claim (the `Finding:` subfield contributes no diagnostic of its own) plus a stronger resolved-question case. Doc sync: `README.md` core-workflows row, `ARCHITECTURE.md` capability tree, `CHANGELOG.md` (2.0.0 pending), and spec `20260802-1904-01-ipd-structure-and-linting` Section 7 plus its Section 9.2 `author`/`review-finalize` rows and Section 17 test-obligation line, whose "permitted during authoring and review but rejected at `pre-execution`" text this change made factually wrong. Verified: `python3 -m pytest` -> 5885 passed, 3 skipped, 2 xfailed; the single failure is the known environmental `test_reporting_contract` case (backlog `8kttqq`, a gitignored local dump), proven unrelated by moving the directory aside (53 passed) and restoring all 1746 files.
- **Partly superseded by D152 (2026-09-09):** this entry's memory-kernel description remains accurate as history, but two of its rules were measured insufficient and have changed. The anti-jargon rule ("no `id6`/symbol/finding-code as a subject") stated only the prohibition and was therefore satisfiable by DELETING NOUNS, which produced a prompt the maintainer could not read; it now carries a positive obligation and names the generic-placeholder anti-pattern a violation. The mandated P12 re-read described here has been REMOVED (it was performed in the failing run and prevented nothing), with the citation kept and P12's pre-flight self-check list relocated into the kernel. The nine-item exit gate has become ten, the tenth being deliberately non-mechanical. See D152 for the measurement and the reasoning; do not restore either rule's earlier form from this entry.

### D152. The askme composition rules gain a positive half, a symptom requirement, and one non-mechanical gate item, because a wholly mechanical gate passed a prompt the maintainer could not read

- **Context:** D151 shipped `/askme` with a nine-item exit gate. On 2026-09-08 the maintainer ran it on plan `zzcrlo`'s open questions and could not read the result: "I wanted to pick write your own and say I did not understand a single word of your explanation." The prompt had satisfied EVERY gate item and P12 had been re-read first, as the body then mandated. THE STRUCTURAL FINDING, which is the whole reason this entry exists: all nine items are MECHANICALLY DECIDABLE (did you use an id6 handle? did you label a recommendation?), and comprehensibility is not, so the gate was fully satisfiable by a prompt that failed its only purpose. P11's corollary: a gate made only of deterministic checks cannot gate a semantic property. The maintainer filed backlog `t156g1` with four diagnosed causes. (1) The anti-jargon rule had NO POSITIVE HALF, so complying by substitution produced "a shipped pair of helpers", strictly worse than the banned name because a named thing can be looked up while an anonymous one cannot be pictured. (2) The prompt described the FIX and never the PROBLEM: the actual malfunction (a run reports success while its work never landed) appeared nowhere, and P12's "the general reason a decision is needed" was satisfied by describing the remedy. (3) THE QUESTION WAS NOT THE HUMAN'S: the plan's own open question already stated the decision rule and the deciding measurement was already in hand. (4) The re-ask rule framed "I have zero context here" as the HUMAN lacking context rather than the PROMPT failing to supply it, and the maintainer used the tool's free-text option to report incomprehension instead. The same failure recurred twice more while diagnosing it: a follow-up prompt said "the change I am about to plan" without naming it ("I have 12 opencode sessions, I don't remember what this one is about"), and another claimed part of the fix could be "automated".
- **Decision:** fix all four causes in the workflow BODY, add exactly ONE non-mechanical exit-gate item, and DROP the mandated P12 re-read while keeping the citation and RELOCATING P12's pre-flight self-check list into the memory kernel where composition happens. Four sub-decisions are load-bearing. (1) THE SORT IS THE ROOT CAUSE AND IS FIXED FIRST: Step 1 now tests whether the owning artifact already states a decision rule ("do X if measurement M, else Y and record it"), which is YOURS once M is measured; the comprehensibility defects are real but downstream, since a question never asked is never composed badly. The test is confined to rules DECIDABLE ONCE MEASURED so a mere lean is not mistaken for a decision, and the when-in-doubt-ask tiebreak is preserved and cited so this cannot become a licence to skip asking. (2) THE ANTI-JARGON RULE GETS ITS POSITIVE HALF plus a named anti-pattern (a generic container word such as `a module` or `a mechanism` is a VIOLATION, not compliance) plus an honest escape (name it and explain it in one clause), because without the escape the rule sometimes has no compliant option, which is how the void was produced. (3) THE MANDATED RE-READ GOES, on the maintainer's own recommendation: it was PERFORMED in the failing run and prevented nothing, because the needed guidance was not in P12, and it manufactures false confidence of exactly the kind D151 was created to correct. The pre-flight list RELOCATES rather than being copied: measured against all seven P12 consumers (`plan-review`, `plan-review-long`, `advise`, `spec`, `getting-started`, the local-leaks lens, and the condensed copy `engine.py` writes into every managed `AGENTS.md`), NOT ONE references the list, so removal weakens no caller and P8 is satisfied with exactly one home. (4) THE TENTH GATE ITEM IS DELIBERATELY NOT MECHANICAL (restate the PROBLEM, the CHOICE, and each option's CONSEQUENCE as a reader who has never opened this repository) AND ITS LIMIT IS STATED IN THE BODY: it is a self-assessment by the composing agent, can be ticked without being performed, and is NOT proof. It is worth adding because it changes what a CONSCIENTIOUS agent does at composition time, which is the population that produced the failure; that prompt was written by an agent following the rules, not evading them.
- **Rejected:** ADDING PROSE TO P12, forbidden by `t156g1` in terms ("a fourth copy of the same guidance is the drift P8 forbids, and three copies already failed"). A DETERMINISTIC READABILITY PROXY (reading grade, banned-word list, length cap), because the lesson of the failure is that mechanical proxies for comprehensibility are what passed the broken prompt; a proxy would add a tenth mechanical check while appearing to solve the semantic problem. A TRIAL-RUN GATE making the fix conditional on the maintainer confirming a freshly composed question was clear: proposed as the only evidence that tests the actual claim, and declined by the maintainer, who also corrected the framing that any part of this could be automated: "neither my request (prompt) nor your response go through any tool that you or I control." That correction is why the tests are scoped as anti-deletion presence guards and why NEITHER the plan NOR this entry claims the fix is proven to produce understandable questions. The evidence for that is the next real reply.
- **Status:** APPROVED interactively by the human maintainer (Gabriele Fariello, 2026-09-09), who chose "One plan covering all four causes plus the readability check" over fixing the sort alone, set the validation posture ("the only thing we can do is try to improve the instructions for the `/aw askme` command and skills"), approved plan `5wtzqv`, and resolved its OQ-01 to RELOCATE.
- **Applied:** `.aw/system/workflows/askme/askme.md` (Step 1's sort test; kernel item 3 the new symptom-before-choice rule; kernel item 4 the anti-jargon positive half; the discard-and-re-ask rule in Step 3; the tenth exit-gate item with its stated limit; the mandated re-read removed with the P12 citation retained; P12's pre-flight list relocated into the kernel), `GUIDING_PRINCIPLES.md` P12 (pre-flight list removed with a pointer to its new home; the askme entry-point paragraph retained), `tests/test_askme_workflow_body.py` (new, 29 tests: presence guards for every added rule, four must-not-regress guards, and a both-directions assertion that the relocated list has exactly ONE home). Verified: the 29 new tests FAIL 25-of-29 against the pre-change body (the 4 that pass are precisely the must-not-regress guards, as designed), the one-home assertion fails in BOTH wrong states (present in both files; absent from both), `tests/test_askme_blocking_gate.py` remains 12-for-12 green, and a bare `python3 -m pytest` passes.

### D153. A setid is a SHARED cross-type TOPIC label, not a unique identity, so the ambiguous lookup is the defect and uniqueness enforcement is abandoned

- **Context:** `aw ipd set approved agentadhere` FAILED with "selector 'agentadhere' resolved to artifact(s) of type ['backlog', 'research'] ... scoped to 'plans'", because that setid names a plan Set AND a closed backlog item AND five research reports. Spec `4w7d6s` (2026-08-27) diagnosed this as a NAMING defect and proposed making a setid unique across every record type: hard fail-closed enforcement (its I1/I2), prevention at every creation and move verb (I3), a fresh setid minted on every graduation (G1), and a migration sweep renaming the existing collisions. It graduated into the four-plan `setidhard` Set, two of whose children reached `go-pending-approval`. THE MEASUREMENTS TAKEN BEFORE ANSWERING ITS LAST OPEN QUESTION INVERTED THE DIAGNOSIS. (1) Cross-type sharing is the DOMINANT pattern, not drift: of 433 distinct filename-slot setids, **117 span more than one record type**, and the widest are genuine topics (`agentadhere` = 7 plans + 1 backlog item + 5 research reports; `lanectn` = 7 plans + 7 reviews + 1 walkthrough). (2) It is partly AUTOMATIC AND DELIBERATE: `review_findings.build_review_name` builds a review's filename from its SUBJECT's setid and id6, documented in that function as "the join key ... not a fresh identifier", so plans-and-reviews sharing a setid is designed behavior and accounts for 47 of the 117. (3) The motivating failure was a LOOKUP defect: the setter held BOTH the setid and the target type and still gave up, when resolution by `(type, setid)` was available and unused. (4) The check was MISLABELLING correct behavior: `check.setid-collision` ships at severity `error` and reports 38 findings by default / 86 with `--all`, of which 78 are the endorsed backlog-plus-plans sharing and only **5** (within-type conflicting descriptives) are genuine. (5) The `- Set:` FIELD is nearly unused outside plans (specs, prompts and reviews carry it ZERO times) while the filename SLOT is near-universal, so a plan-execution-batch reading would gut the token's usefulness.
- **Decision:** a setid is a SHARED, CROSS-TYPE TOPIC LABEL. The same token MAY appear under any number of record types when the artifacts concern one topic, and nothing may prevent, rename, or report-as-drift a setid used that way. Identity remains the id6 (D140), every cross-tree link stays id6-keyed, and every setid-taking verb MUST resolve within the requested type rather than failing on cross-type multiplicity. `check.setid-collision` is RE-SCOPED, not deleted: its within-type conflicting-descriptive half is a real defect and stays. Graduation PRESERVES the source's setid by default rather than minting a fresh one, and the typed links (`From-Backlog`/`From-Spec`, `Graduated-To`) carry the machine-readable relationship because a shared name is a human affordance and not a resolvable reference. The maintainer's own reasoning is the load-bearing part: artifacts concerning one issue ARE naturally one set to a user, and a shared setid makes that relationship obvious while distinct setids OBFUSCATE it.
- **Rejected:** UNIQUENESS AS DRAFTED (`4w7d6s` I1/I2/I3/G1), which would have forbidden 117 existing legitimate groupings, refused shipped review-naming behavior at write time, and required renaming 21 topics; superseded rather than revised, because roughly half its normative content died and 23 artifacts cite it. A DISTINCT-BUT-ECHOING CHILD SETID (source name plus a suffix), which preserves recognizability but still discards the exact-match affordance and still needs a 69-pair migration. THE PLAN-EXECUTION-BATCH READING of a setid (only artifacts that "run together" share one), stated honestly by the maintainer and rejected on their own analysis that it makes the token useless for most artifacts and ALWAYS useless for specs, since no "spec set" exists. FIXING THE LOOKUP ONLY, leaving 86 findings reported at `error`, rejected because the noise trains agents to ignore a real gate.
- **Status:** APPROVED interactively by the human maintainer (Gabriele Fariello, 2026-09-10), who chose "Keep names shared as topics; fix the lookup and downgrade the false error" over proceeding with unique names, over a lookup-only fix, and over parking the question. The maintainer also REFRAMED the question before answering it, observing that research, specs, prompts and IPDs concerning one issue are all naturally one set, and asking what serves humans reading filenames, agents consuming setids, and scripts respectively; that reframing is what exposed the reversed premise, since the original prompt had assumed uniqueness was settled and asked only about the naming convention.
- **Applied:** spec `2lcqno` (new, `.aw/records/specs/20260910-2lcqno-01-2lcqno-setid-shared-topic-label-and-type-scoped-resolution.spec.md`) carrying seven normative items, the five measurements, three accepted costs, and `- From-Spec: 4w7d6s`; spec `4w7d6s` -> `superseded` with a reversal banner at its Section 3 and its design text preserved UNEDITED; spec `pqsx96` gains catalog invariant **I-16** (setid semantics) plus a Section 4 record that `check.setid-collision` is misfiled under I-09 (which is filename-grammar conformance) and that the code must be repointed in the same commit that re-scopes the rule; plans `yku4ga` (orchestrator), `drzbs9` (the sweep) and `dw7i3m` (fresh mint plus prevention) all RETIRED to `superseded/`, with `drzbs9`'s `executed:dw7i3m` edge cleared to `none` FIRST so no plan was left pointing at a never-executing target; plan `bwgyum` KEPT and re-justified (its Goal had asserted the reversed premise) after an audit confirmed its resolution is already plans-scoped and needs no change; backlog `sjsoqq` annotated with which four of its six deliverables are dead. NOT YET DONE and tracked: re-scoping the rule in `check_engine.py` and building type-scoped resolution, which graduate from `2lcqno` as fresh plans. ALSO FOUND while applying this: `aw backlog set` on a same-status item DESTROYS the entire workflow history (measured, two lines became one, exit 0), filed as backlog `hg2oop` at high priority; and `doctor.py:530` hardcodes `include_retired=True` while `check_engine.py:1760` defaults it to `False`, which is the whole 38-versus-86 discrepancy and is recorded in `2lcqno` Section 6 as work not to inherit.

### D154. Every live bug blocks the next release, and inefficiency a user can NOTICE is a bug, so both rulings are written down instead of re-derived per session

- **Context:** The maintainer asked "I think I've said before that every bug is blocking for next. We don't ship known bugs. How do we ensure that approach persists?" The answer was that it does not: the rule was recorded NOWHERE. A grep for it across `AGENTS.md`, `DECISIONS.md`, `GUIDING_PRINCIPLES.md`, `.aw/records/backlog/README.md`, `CONTRIBUTING.md` and `RELEASING.md` returned nothing, so it survived only in conversation, and measurably did not survive: of 187 backlog items 103 were `Work-Kind: bug`, 60 of those were live, and **28 of the 60 carried no `- Blocks-Release:`**. TWO OF THE 28 WERE FILED BY AN AGENT HOURS AFTER THE MAINTAINER STATED THE RULE TO IT IN THE SAME SESSION (`kyb0v5`, `mqmlug`), which is the sharpest available evidence that loaded context is not a durable mechanism. Re-measured a day later the counts had all moved (196/112/68/46/22) while the oral-rule grep still returned nothing. A SECOND, INDEPENDENT LEAK was found while measuring: of the live gateless bugs that were `graduated`, 11 of 11 had a plan carrying their `- From-Backlog:` and **0 of 11** of those plans carried a gate, so the gate was also lost at the graduation handoff and not only at creation. THEN THE UNWRITTEN RULE WAS CLARIFIED TWICE IN ONE DAY WHILE STILL UNWRITTEN: an agent filed the `aw find` display-layer double read as `chore` (`59t9x5`) because the command returns entirely correct answers, the maintainer reclassified it to `bug` and gated it, and the agent then over-corrected into a rule covering ALL knowingly redundant work, which the maintainer narrowed again. Both the misfiling and the over-correction came from one cause: nothing in the repository stated the test, so each agent supplied its own.
- **Decision:** TWO RULINGS, both now written into the hand-maintained "Release gates (Blocks-Release)" section of `AGENTS.md` and pointed at from `.aw/records/backlog/README.md` where a filer chooses `- Work-Kind:`. (1) 2026-09-11: a backlog item, spec or plan whose `- Work-Kind:` is `bug` MUST carry `- Blocks-Release:` while it is LIVE (`open`, `blocked`, `graduated`), because we do not ship known bugs; the gate TRAVELS to whatever plan or spec the item graduates into, which the managed block's inheritance clause already obliges once the gate exists for it to inherit. This is a policy about WHICH artifacts carry the field, not a change to its meaning, so the BLOCKS-RELEASE versus BLOCKED-BY distinction is untouched and a merely `open` bug is still a blocker. `bug` is the only gating work-kind today, and making that set configurable per repository (defaulting to `bug` alone) is designed but not built, tracked as backlog `0htqmm`. (2) 2026-09-12: INEFFICIENCY IS A DEFECT WHEN A USER CAN PERCEIVE IT and is not one when they cannot, so `bug` reaches a correct-but-slow path a human waits on. The discriminator is user-perceptible impact, NOT redundancy: redundancy is evidence, so doubling a 2ms internal call stays a `chore` while a path with no redundancy at all can be a bug. The filer owes a MEASUREMENT of the end-to-end command a user runs, recorded on the item, so a reviewer can dispute the number rather than a vibe; an unmeasured hunch that something feels slow is not a bug. NO NUMERIC CUTOFF is set, deliberately: "noticeable" stays a judgement carrying its number. TWO LIMITS ARE STATED IN THE RULE ITSELF rather than discovered later: the gate keys on an AUTHOR'S CLASSIFICATION, so a defect misfiled as `chore` or `followup` escapes it and the rule is not a completeness claim; and it governs LIVE items only, since gating a bug already `done` would assert a history that did not happen.
- **Rejected:** LEAVING `bug` TO MEAN ONLY A WRONG ANSWER, which is the reading that produced `59t9x5`'s misfiling and which the maintainer explicitly declined by reclassifying it on 2026-09-12. THE OPPOSITE OVER-CORRECTION, a rule covering every knowingly redundant code path, declined in the same exchange because inefficiency users cannot notice is not a defect. A MILLISECOND THRESHOLD, not set, because a cutoff invites satisfying the number instead of measuring the user's experience; `59t9x5` qualifies on ~128ms of a ~530ms interactive command and at 3ms would correctly have stayed a `chore`. GATING `security` TOO, offered and declined in favour of per-repository configuration (`0htqmm`), because the maintainer has measured agent security classifications in THIS repository to be overstated while for most of their other projects every security finding genuinely must block, making it a project-dependent property that a shipped rule cannot encode. PUTTING THE RULE IN `agent_workflows/engine.py`, rejected because that text is INSTALLED INTO EVERY ADOPTER REPO and this is a repo-local policy; the target section is hand-maintained and sits below the `<!-- /aw:block -->` marker, so a direct edit is correct and survives every install. WRITING IT AS AN ABSOLUTE, avoided per the maintainer's standing instruction of 2026-09-10 ("We enforce these things until such a time as we receive evidence that the rules need relaxing or changing").
- **Status:** ACCEPTED. Ruling (1) by the human maintainer (Gabriele Fariello) 2026-09-11; ruling (2) by the same maintainer 2026-09-12 while reclassifying `59t9x5`. Recorded here because the whole point is that neither survived as conversation.
- **Applied:** plan `zqs0px` (Set `nobugship`, Order 01) wrote the rule into the existing hand-maintained `## Release gates (Blocks-Release)` section of `AGENTS.md` as a new "Every live bug gates the next release" subsection carrying both rulings, the perceptibility test with `59t9x5` as its worked example, and both limits; added a POINTER (not a copy) from `.aw/records/backlog/README.md` beside the `- Work-Kind:` definition; and added this entry. NO CODE CHANGED: the managed-block parity assertions `tests/test_shared_checkout_contract.py::NoDriftTests::test_repo_agents_block_equals_generated` and `::test_contract_is_not_duplicated_inside_and_outside_the_block` were RUN and pass unchanged, which is the correct result because the edit is outside the managed block, and driving `engine.merge_aw_block` over the edited file returns `refreshed` with the new rule text intact, proving it survives an install. DELIBERATELY NOT DONE HERE, each owned by a sibling plan: defaulting the gate at creation and preserving it through graduation (`di08i9`), enforcement in `aw check` plus backfilling the violations (`rgaasb`), and the configurable work-kind set (`0htqmm`). Documentation alone is known to be insufficient, which is why the Set has three children: the measured evidence is that an agent told the rule directly still filed two ungated bugs the same day.
...
- **Context:** `engine._apply_section_consent` decided a managed section's fate by comparing the on-disk body only to the manifest's recorded hash. Two defects followed (backlog `krwl3t`, IPD `b4bvas`). First, a freeze defect: when a tracked commit edited a managed section together with the generator to the exact same text, the recorded hash became stale; later installs saw disk != recorded, treated it as user drift, preserved the text, and never re-recorded the hash, freezing future generator updates forever (measured on `AGENTS.md#aw:pointer`). Second, a silent clobber defect: when no recorded hash existed (`manifest.recorded_hash(key) is None`), the drift check was skipped and unrecorded user hand-edits were silently overwritten. Third, preserving user drift produced no warning, so users had no feedback when a section was held back.
- **Decision:** Amend D104's consent clause ("on-disk body differing from OUR recorded hash -> preserved as user drift; else written") into four explicit, ordered cases evaluated per section when a manifest is present and the section exists on disk:
  1. `disk.body == sec.body` (normalized via `manifest.hash_content`): adopt the generator output, write/keep it, and re-record the hash in the manifest (self-healing stale records).
  2. `manifest.matches_recorded(key, disk.body)`: normal refresh, update to the desired generator body, and record the new hash.
  3. Recorded hash exists and matches neither: preserve on-disk user drift, leave the manifest record untouched, and emit a warning naming the key.
  4. No recorded hash exists (unowned section): preserve on-disk user content, write no record, and emit a warning naming the key.
  Warnings include the explicit delete-and-reinstall remedy (`"delete that section (from its <!-- aw:{slug} --> marker to the next marker) and re-run install"`), matching the shim drift warning posture. Warnings are surfaced to callers in `engine.update_agents_pointer` and `engine.ensure_untracked_gitignore` (before its early returns). Absent sections on disk write the desired section and record its hash; `manifest is None` writes desired (back-compat).
- **Applied:** `agent_workflows/engine.py` (`_apply_section_consent` rewritten to 4 branches with keyword `warnings`; `merge_aw_block` threads `warnings`; `update_agents_pointer` and `ensure_untracked_gitignore` colorize and print warnings); `tests/test_section_consent.py` (unit and install-level tests for all 4 cases, warnings, and regressions); `.aw/system/README.md`; `CHANGELOG.md`; `DECISIONS.md`. Reconciles `AGENTS.md#aw:pointer` manifest hash in this repository. Executed per IPD `b4bvas` (Set `sectfreeze`, Order 01, backlog `krwl3t`).
