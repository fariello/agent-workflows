...
- **Early pre-flight gate**: before the audit, take a cursory look at `TODO.md`/backlog items and pending plans/prompts and, when interactive, ask ONCE whether anything should be handled before the release. On "yes" the review ABORTS so you can discuss/address them first (saving a full run); on "no" it forgets the glance and proceeds. When non-interactive it skips the ask and relies on the loud Go/No-Go warning. This is an early safety net in addition to the thorough final-report reconciliation.
