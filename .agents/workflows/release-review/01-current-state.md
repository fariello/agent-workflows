...
After the discovery above and BEFORE starting the audit (and before any parallel audit lanes), apply the **Section 1 pre-flight gate** defined in `00-run-protocol.md`: a cursory look at the discovered TODO/backlog items and pending plans/prompts, a single bounded ask if anything obviously warrants attention first, ABORT-to-discuss on "yes" (recorded in `00-run-metadata.md`), forget-and-proceed on "no", and the loud-WARNING fallback when non-interactive. This is an early safety net in addition to the Section 7 reconciliation and the Section 8 warning.
...
- [ ] Section 1 pre-flight gate applied: cursory TODO + pending-plans/prompts look, and either the user was asked (and answered proceed/abort, recorded in `00-run-metadata.md`) or, when non-interactive, the signal was left to the loud Section 8 warning. On abort, the run stopped here.
