...
status: archive
outcome: rejected
summary: Migrated from 20260712-planrev-01-tsjhfq-prompt-tighten-plan-review.research-prompt.md.
consumed-by: []
---
You previously reviewed and improved an AI-agent instruction file called `plan-review.md` (the controlling runbook an AI coding agent reads and executes to review a proposed implementation plan before any code is written, and to improve that plan in place). Your improved version is included in full at the bottom of this message, between markers.

Your improved version is good but it roughly tripled the file's length (from about 288 lines to about 780). That is too long. This runbook is copied verbatim into many repositories and read by an LLM on every run, so length is a real cost. Produce a TIGHTER version that is about HALF the size of your improved version (target roughly 380 to 420 lines, and no more than 450), WITHOUT losing the substantive correctness fixes.

Preserve these substantive improvements (do NOT drop them; they fix real defects):
- The split between VERDICT (review outcome) and READINESS (GO / NO-GO), and the `REVIEWED - OPEN QUESTIONS` verdict.
- A defined Remediation Risk scale (Low, Medium, Medium-High, High) with "overall risk = highest applicable axis", since the Fix Bar depends on it.
- The review-scope ledger built in Step 0 that the final reviewed/not-reviewed enumeration must use.
- The interactive open-question loop: collect and deduplicate, resolve from authoritative evidence first, ask 1 to 3 related questions at a time with a fixed format (decision needed, context, why it matters, options, trade-offs, recommendation), record answers into the plan, re-check affected areas.
- The tightened non-interactive exception ("a delayed reply is not non-interactive").
- `Status: reviewed` means the review occurred, not approved / GO / executed.
- The two-commit, never-push contract with dirty-tree and commit-failure safety.
- The final report's Evidence reference and a commit-result line, and the reviewed/not-reviewed enumeration as the literal last output.
- The Fix Bar philosophy: fix by default; defer only when the fix ITSELF is Medium-High or higher risk on complexity, usability, security, or functionality; severity never decides; effort/time/cost/tokens never justify deferral.
- Tool-agnostic and project-agnostic design; relative sibling references (`../release-review/fix-decision-policy.md`, `../release-review/00-run-protocol.md`) with graceful degradation when absent; planning-documents-only boundary; the eight personas with security as a cross-cutting lens.

Cut length primarily by:
- Removing verbosity and over-explanation. Prefer one tight line over three. Merge redundant bullets. Assume a capable executor; do not re-explain the obvious.
- Trimming the engineering rubric to the highest-value, broadly-applicable checks. This is a general-purpose, tool-agnostic runbook for projects of ALL sizes, NOT an enterprise/regulated-systems checklist. Keep the core correctness, security, testing, anti-regression, KISS, principles/UX, domain-invariant, and plan-executability checks, but collapse long enterprise-leaning sub-lists (extensive data-lifecycle, elaborate rollout/rollback/recovery, exhaustive abuse-control enumerations) into a few crisp, conditional lines gated by "when relevant". Depth an agent can infer does not need spelling out.
- Consolidating any duplicated instructions to a single place and referencing them.

Do not reintroduce anything the previous pass deliberately fixed, and do not create new internal contradictions. In particular keep the labels consistent: use `NOT REVIEWED` (not `NOT ELIGIBLE`) everywhere, including the report's scope section, so the ledger and the final enumeration match.

House style you MUST follow: plain, precise, concise language; NO em dashes or en dashes (use a regular hyphen or reword); short lines; keep valid structure.

Deliverables (both as downloadable markdown files, actual attachments):
- `plan-review.improved-tight.md`: ONLY the complete tightened file, a drop-in replacement, no commentary inside it, about half the length of the version below.
- `plan-review.tightening-notes.md`: what you cut and compressed and why, what you preserved, the approximate before/after line counts, and confirmation that every "preserve" item above is still present and where.

If you cannot attach files, say so and provide each file's full content in a separate clearly-labeled fenced code block instead.

Here is your improved version to tighten, verbatim between the markers:

===== BEGIN plan-review.improved.md =====
