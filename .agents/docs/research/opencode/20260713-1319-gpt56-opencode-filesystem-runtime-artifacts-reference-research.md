<!--
ARCHIVED IN AGENT-WORKFLOWS AS REFERENCE (framing note added 2026-07-13; body verbatim below).
This report was written FOR the `ocman` project (it names ocman's baseline commit and recommends
ocman code changes). It is kept HERE only as useful reference on OpenCode's on-disk / runtime model,
which informs this framework's own thinking (e.g. the filesystem agent-comms trial). Read every
"ocman should ..." recommendation as directed at ocman, NOT as an agent-workflows task or decision.
The authoritative copy for ocman lives in the ocman project. External GPT-5.6 artifact, kept verbatim
(no em-dash/format normalization applied); runtime claims are source-derived per its own caveat.
-->
