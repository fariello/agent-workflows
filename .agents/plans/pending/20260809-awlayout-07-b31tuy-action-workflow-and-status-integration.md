...
- [ ] E-01 Extend D125's attention contract with an exhaustive native action-source mapping (`open` to `ready`, `completed` to `done`, `dismissed` and `superseded` to `parked`), resolved through the canonical state root with fail-closed validation and output safety.
  - Depends on: none
...
- [ ] V-01 validates E-01
  - Required evidence: contract and scanner tests cover every action directory, unavailable or malformed external state, deterministic ordering, redaction, output safety, and ANSI-free JSON and agent output.
  - Observed evidence:
  - Result: pending
- [ ] V-02 validates E-02
...
STOP if Order 06 is incomplete. Do not execute until this plan and the parent orchestrator are approved.
