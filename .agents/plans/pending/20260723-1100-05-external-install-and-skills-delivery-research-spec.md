...
- This IPD ships DOCUMENTS, not code, so there is no pytest delta. Validation is:
  - The spec doc exists under `.agents/docs/specs/`, contains the tiers (T0-T3) + per-tier criteria, the probe protocol + results-table template, the constraints/ownership section, and an explicit build-deferral.
  - The external-research prompt exists under `.agents/prompts/pending/`, is self-contained and addressed to the target AI (NO user-facing instructions inside), and asks for a downloadable `.md` result (AGENTS.md rules).
