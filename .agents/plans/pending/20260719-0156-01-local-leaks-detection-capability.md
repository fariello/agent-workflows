...
|------|--------------------|--------|-------|------------------|------------|
| 1 | LL1,LL4 | Create the shippable engine `agent_workflows/local_leaks.py`: the identity rule set (fragment-assembled, history-rewrite-safe) merged with a scan_secrets-style text/location core. Add AUTO-DERIVATION of personal tokens at runtime: `$HOME` basename, `git config user.name`/`user.email`, `$USER`, the repo's parent-directory sibling names, and the hostname; each becomes a pattern unless allowlisted. Keep patterns assembled so the module never contains a plain leak literal. | `agent_workflows/local_leaks.py` (new) | Medium | unit tests: auto-derivation produces expected patterns from a synthetic env; fragment self-scan clean |
...
| 6 | LL5 | Author `.agents/workflows/assess/lenses/local-leaks.md` (interactive lens): scans tree + history + optionally the wheel via the shared engine; auto-derives identity tokens; ENUMERATES all distinct emails/usernames and ASKS the human which to keep; writes approved values into the repo-committed allowlist; proposes a scrub IPD for the rest. Cross-reference `secrets.md` (credentials) and `privacy.md` (product PII) to bound scope. Register a manifest row `assess-local-leaks` in `index.md` and regenerate shims (via `scaffold` conventions). | `.agents/workflows/assess/lenses/local-leaks.md` (new), `.agents/workflows/index.md` | Low | lens file present; `test_dir_readmes`/manifest parse pass; a temp `aw install` generates no shim for the catalog row (assess-* rows are catalog, not commands) |
| 7 | LL6 | Add `.github/workflows/local-leaks.yml` (thin backstop mirroring `secret-scan.yml`): push/PR triggers, `fetch-depth: 0`, `permissions: contents: read`, one step `python -m agent_workflows check-local-leaks . --history`. Lower priority; drop if review judges it redundant for a solo repo. | `.github/workflows/local-leaks.yml` (new) | Low | workflow YAML valid; runs the packaged scanner |
...
- New `tests/test_local_leaks.py` (stdlib unittest): auto-derivation from a synthetic env; repo-committed allowlist honored; user-hints file honored (and absent = no-op); the 3 scan modes (tree/history/wheel) each catch a runtime-synthesized planted leak in their respective source and pass clean otherwise; public identifiers (package name, author email, remote URL) NOT flagged; the module's own source is self-clean.
...
- OQ1 (keep or drop the `tools/` shim): after moving logic into `agent_workflows/local_leaks.py`, keep a thin `tools/check_personal_paths.py` wrapper so pre-commit works in a SOURCE checkout before the package is installed, or make the pre-commit hook call `python -m agent_workflows` (requires the package importable, which it is from the repo root)? Lean: pre-commit calls `python -m agent_workflows check-local-leaks` (works from repo root via the local package), delete the `tools/` shim. Confirm.
- OQ2 (repo-committed allowlist location/format): the research flagged NOT to pollute the repo root and NOT to use `config.json`. Proposed `.agents/local-leaks-allowlist.toml` (a small tracked file, sibling to the `.agents/` conventions, clearly scoped). Acceptable, or prefer another path/format (e.g. a section in an existing `.agents/` file)? Confirm.
- OQ3 (auto-derivation aggressiveness): deriving from `$HOME`/`git user`/hostname/sibling dirs is powerful but can false-positive (e.g. a common username). Should auto-derived tokens be HARD failures, or WARNINGS that the interactive lens confirms while only the explicit allowlist/hints drive the non-interactive gate? Lean: auto-derived = warnings in the lens; the non-interactive guard fails only on the curated+hints patterns plus structural patterns (home paths, local-checkout-dir-style, session ids). Confirm the split so CI is not flaky.
- OQ4 (assess-secrets email overlap): `scan_secrets.py` already enumerates emails. Should `/assess local-leaks` call into it for emails, or keep its own email pass? Lean: the lens references assess-secrets for the credential/PII angle and does its own OK-to-keep email enumeration for the identity angle; note the overlap so they do not contradict.
