...
- [ ] E-01 Implement `AW_HOME` precedence and normalization: explicit CLI value, `AW_HOME`, platform default, with `~/.aw` as the documented Unix default.
  - Depends on: none
  - Expected outcome: one resolver returns an absolute AW home and explains the selected source without writing it.
  - Execution state: pending
- [ ] E-02 Implement a versioned registry and stable project-ID generator in `agent_workflows/project_registry.py`, using atomic writes and no credentials, conversations, or generated records in registry entries.
  - Depends on: E-01
...
- [ ] E-03 Match an existing project in this order: explicit project ID, Git common directory identity, canonical target path, normalized origin hints; stop on multiple candidates and never auto-merge entries.
  - Depends on: E-02
...
- [ ] E-04 Add `aw project status`, `aw project attach`, and `aw project move` with dry-run output, explicit confirmation for identity changes, and stable JSON or agent output.
  ...
  - Execution state: pending
- [ ] E-05 Add `tests/test_project_registry.py` for precedence, stable IDs, atomic writes, worktrees, moved paths, origin changes, ambiguity, dry runs, and redaction boundaries.
  - Depends on: E-04
...
- Do not place secret values or record content in examples or fixtures.
...
- [ ] V-01 validates E-01
  - Required evidence: focused tests show every precedence case, absolute normalization, source explanation, and zero writes during resolution.
  - Observed evidence:
  - Result: pending
- [ ] V-02 validates E-02
  - Required evidence: registry round-trip and interrupted-write tests pass; schema inspection confirms only allowed metadata fields.
  - Observed evidence:
  - Result: pending
- [ ] V-03 validates E-03
  - Required evidence: tests cover explicit ID, worktree, path move, origin hint, clone separation, and ambiguous-candidate refusal.
  - Observed evidence:
  - Result: pending
- [ ] V-04 validates E-04
  ...
  - Result: pending
- [ ] V-05 validates E-05
  - Required evidence: focused and full suites pass with temporary homes and repositories only.
  - Observed evidence:
  - Result: pending
