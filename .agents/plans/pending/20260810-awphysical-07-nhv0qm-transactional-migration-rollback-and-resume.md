...
- `python3 -m unittest tests.test_awphysical_migration tests.test_acceptance_matrix` plus new physical-layout migration transaction suites. Do not add awphysical cases to the superseded awlayout-owned `tests.test_layout_migration` module.
...
|---|---|---|---|---|
| E-01 | `python3 -m unittest tests.test_awphysical_migration.TransactionalMigrationTests.test_e01` | `tests/fixtures/awphysical/order07/e01-*` | Apply refuses stale or edited inventory, map, policy, roots, or Git identity; only one transaction/writer can be active; every phase is resumable or rollbackable. | any stale component is accepted, two writers enter, a journal lacks a recovery route, or refusal changes source/destination/index state |
| E-02 | `python3 -m unittest tests.test_awphysical_migration.TransactionalMigrationTests.test_e02` | `tests/fixtures/awphysical/order07/e02-*` | Changes between inventory and apply stop before copy; lock ownership and stale-lock recovery are explicit and tested across interruption. | a planted hash/identity/permission/space change reaches copy, a live lock is stolen, stale-lock recovery is unaudited, or any preflight refusal mutates state |
| E-03 | `python3 -m unittest tests.test_awphysical_migration.TransactionalMigrationTests.test_e03` | `tests/fixtures/awphysical/order07/e03-*` | Every copied item is hash-verified against the frozen inventory and remains non-authoritative until the whole destination set verifies. | a missing, extra, mode-changed, or byte-changed item verifies; staging becomes authoritative early; or a collision overwrites existing content |
| E-04 | `python3 -m unittest tests.test_awphysical_migration.TransactionalMigrationTests.test_e04` | `pre-switch-failure`, `switch-write-failure`, `post-switch-receipt-failure`, and `dual-writer-canary` | Each failure asserts exact journal phase, policy digest, active authority, legacy-writer state, source/destination hashes, and Git indexes; before-switch failure leaves legacy authoritative, and after-switch failure leaves exactly one detectable authority with rollback. | authority is zero or dual, policy/receipt disagree, a writer remains enabled on both layouts, any hash/index differs unexpectedly, or recovery is unavailable |
| E-05 | `python3 -m unittest tests.test_awphysical_migration.TransactionalMigrationTests.test_e05` | `tests/fixtures/awphysical/order07/e05-*` | Users can inspect or roll back after cutover; retained candid material does not become newly tracked or copied into the wrong repository. | a retained hash differs, candid material enters the wrong worktree/index, rollback instructions omit an item, or cleanup becomes available before the retention gate |
| E-06 | `python3 -m unittest tests.test_awphysical_migration.TransactionalMigrationTests.test_e06` | `tests/fixtures/awphysical/order07/e06-*` | Cross-repository moves are represented as independent deltas and recovery instructions; a failure in one Git owner cannot be presented as globally committed. | target and companion deltas are combined, either index contains another owner's path, partial staging is reported as global success, or recovery omits the failed owner |
| E-07 | `python3 -m unittest tests.test_awphysical_migration.TransactionalMigrationTests.test_e07` | `resume-before-switch`, `resume-after-switch`, `rollback-before-switch`, `rollback-after-switch`, `foreign-cleanup-item`, and `changed-retained-item` | Repeated resume and rollback preserve inventory hashes, policy digest, exactly one authority, writer exclusion, and clean unrelated indexes; cleanup remains separate, preview-first, and refuses foreign or changed items. | repeated recovery changes a completed item, creates zero/dual authority, alters an unrelated index, cleanup runs during migration, or a foreign/changed item is deleted |
| E-08 | `python3 -m unittest tests.test_awphysical_migration.TransactionalMigrationTests.test_e08` | named injections: `stale-input`, `concurrent-writer`, `copy-failure`, `verify-mismatch`, `switch-failure`, `kill-after-switch-before-receipt`, `disk-loss`, `permission-loss`, `cross-git-partial-stage`, `resume`, `rollback`, `cleanup-refusal`, `source-protection` | Every injection asserts exact exit, journal phase, authority count, writer state, source/destination hash set, policy/receipt state, target/companion/source index deltas, and available recovery. Resume is idempotent; rollback restores the declared checkpoint; source-protection preserves every canonical source hash and index; switch failures never yield zero or dual authority. | any injection or per-condition assertion is absent, authority is zero/dual, a hash/index/policy/receipt differs from the named expected state, resume duplicates work, rollback misses a checkpoint item, source protection changes canonical content, or recovery is unavailable |
