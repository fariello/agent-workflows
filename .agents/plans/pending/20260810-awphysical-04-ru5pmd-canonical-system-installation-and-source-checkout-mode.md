...
| E-05 | `python3 -m unittest tests.test_installer.PhysicalSystemInstallTests.test_e05` | `source-positive`, `path-equality-only`, `copied-marker-spoof`, `origin-only-spoof`, `ambiguous-source-evidence` | Positive source fixture resolves source role and produces zero canonical-source writes; path equality without package-metadata and Git-common-dir evidence is insufficient; both spoof fixtures remain ordinary targets or refuse; ambiguous evidence fails closed; before/after source hashes and Git index are identical in every case. | any source byte/index changes, path-equality-only detection is retained as sufficient, a spoof gains source role, ambiguity proceeds, or an expected exit code differs |
