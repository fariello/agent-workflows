...
- Id: ey1dex
- Set: version-resolver (version resolver match release tags only)
- Order: 0
...
Execution contract (per `.agents/plans/README.md` and `AGENTS.md`): commit ONLY files changed by this plan, path-scoped (`git commit -m msg -- <path>`), never `git add -A`/`-a`, never push; `git add` new files first. When reporting tests, paste the ACTUAL runner output; never claim a pass not run. No em or en dashes in authored Markdown. STOP and report if execution exceeds this plan's scope. Never create or push a tag / Release / PyPI upload. NOTE: there is uncommitted `/whatnext` work in the tree (IPD 20260717-agentcont-01-6a3myl-whatnext-surveyor-workflow, Steps 1-7 done); this corrective fix is committed FIRST and separately, then the whatnext work is finalized against the now-green suite.
