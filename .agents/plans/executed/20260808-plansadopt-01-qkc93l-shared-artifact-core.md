...
- 2026-08-08 to-review (opencode its_direct/pt3-claude-opus-4.8-1m-us): child of Set `plans-adopter`; the shared foundation every later child builds on. Authored from spec `20260808-plansadopt-01-qkc93l-shared-artifact-core` Section 4.1.
...
Proposal; human review + approval; not auto-executed. Requires the approved spec `20260808-plansadopt-01-qkc93l-shared-artifact-core`. Do NOT claim done or move to `executed/` until every `E-*` is performed+checked AND its matching `V-*` is pass+checked with concrete evidence (including the UNCHANGED research suite); else STOP and report.
