...
| 2 | R2 | Add a Section 9 EXIT-VERIFICATION + a loud "REMAINING MANUAL STEPS" block. Before Section 9 is complete, VERIFY and report the end state: (a) the tag is pushed and on the remote; (b) on rung C, the GitHub Release exists and is NOT a draft (verify with `gh release view <tag> --json isDraft,isPrerelease` -> assert `isDraft=false` and, for a full release, `isPrerelease=false`), and is marked Latest (NOTE, plan-review PR-001: `gh release view --json` has NO `isLatest` field; "Latest" is GitHub-derived. Verify it by having created/edited with `--latest`, and/or check `gh release list` which flags the Latest release in its output); (c) if the toolchain can check, the registry (e.g. PyPI) has the version. ANY dangling step (unpublished draft, un-uploaded registry artifact, a step genuinely handed off because credentials were absent) MUST be surfaced as an explicit "REMAINING MANUAL STEPS" block naming the exact command(s) to run. `gh`-graceful-degradation preserved: if `gh` is unavailable, say so and give the manual verification/command rather than blocking. | `.agents/workflows/release-review/09-release-execution.md` (Step 8 + Exit criteria) | Low | exit criteria include the not-draft (+ not-prerelease for full) GH Release check via a REAL `gh --json` field and the Latest check via `--latest`/`gh release list` (rung C) and the registry check; a run with a leftover draft prints the REMAINING MANUAL STEPS block, not a silent completion |
...
- OQ1 (mechanism): RESOLVED from evidence (plan-review). Both `gh release create <tag> --latest` (no `--draft`) and create-draft-then-`gh release edit <tag> --draft=false --latest` are valid (`--latest`/`--draft=false` verified to exist on `gh release create`/`edit`). Use the SINGLE `gh release create ... --latest` on the confirmed rung-C step (simpler; the confirmation IS the deliberate publish act); fall back to create-draft-then-edit only when notes/assets must be staged first. `09` will present it as the primary command with the create+edit as the documented alternative.
- OQ2 (this-repo timing): RESOLVED (human, 2026-07-21 plan-review): land this fix BEFORE the 1.3.0 release-review, so 1.3.0's own rung C ends in a published, Latest GitHub Release rather than a silent draft. Low-risk prose. (Note: the sanitizer 3-IPD suite is also a 1.3.0 blocker, so 1.3.0 is gated on that suite regardless; this IPD sequences ahead of the 1.3.0 cut.)
