...
- 2026-08-08 executed (opencode its_direct/pt3-claude-opus-4.8-1m-us): built agent_workflows/attention_contract.py (five-class enum, tree-policy inventory, per-tree mappings pinned to plans.RECOGNIZED/research_contract.STATUSES/SPEC_STATUSES, gate validators + output-safety, stable rule-id catalog, history grammar + last_history_at, transition/authority table + anti-self-approval floor) + tests/fixtures/attnview/ + tests/test_attention_contract.py (20 tests). Product commit staged before this transition. E-01..E-08 performed; V-01..V-08 pass. Full suite Ran 693 tests OK (skipped=1); leak-clean; no em/en dashes.
...
- [x] E-07 build a fixture corpus under `tests/fixtures/attnview/` covering every native status of specs/plans/research (each mapping to its class) AND every `--check` violation class from spec Section 8.3/8.8: missing/unknown status, unmapped status, missing/malformed/contradictory gate, deferred-without-gate, non-http issue URL, control-char/newline/over-length descriptive field, duplicate id, duplicate normalized path, disposition-vs-terminal mismatch, unclassified tree, AND the three the first draft omitted: unsupported-encoding / malformed front-matter, an unreadable/permission-denied included file (or document why it cannot be a committed fixture and how it is simulated), and a symlink whose target escapes the repo root (spec 8.5). Each fixture's violation maps to a rule id in the E-05 catalog.
  ...
  - Execution state: performed
- [x] E-08 add `tests/test_attention_contract.py` asserting the mapping is TOTAL over each tracked tree's native enum (coverage test: mapping keys == declared enum; fails on missing or extra), the enum/tree-policy shapes are well-formed, and the gate/history/JSON grammars parse their positive fixtures and reject their negative ones. Run the file and the full suite; paste actual output.
...
- [x] V-01 validates E-01
  ...
  - Observed evidence: `agent_workflows/attention_contract.py` created and imports. `ATTENTION_CLASSES == {ready,active,blocked,done,parked}` (test_five_classes pass). `TREE_POLICY` has specs/plans/research `tracked` with owners (`aw specs`/`aw plans`/`aw research`) and walkthroughs/roadmaps/prompts/comms `excluded` with rationale (test_tree_policy_tracked_and_excluded pass).
  - Result: pass
- [x] V-02 validates E-02
  ...
  - Observed evidence: coverage tests pass: `set(CLASS_MAPS["specs"]) == SPEC_STATUSES`; `set(CLASS_MAPS["plans"]) == plans.RECOGNIZED` (the PRE_TERMINAL+TERMINAL+STANDING union incl. `auto-approved`); `set(CLASS_MAPS["research"]) == research_contract.STATUSES`. `class_of` raises `UnknownNativeStatus` for an unmapped value and for an untracked tree (no default). (MappingTotalityTests, 5 tests pass.)
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: `HISTORY_RECORD_RE` = `- YYYY-MM-DD <line>` (date is the machine field; documented in the module docstring). `last_history_at()` returns the LAST matching record's date, `None` on no record, never mtime (test_last_history_at pass).
  - Result: pass
- [x] V-04 validates E-04
  ...
  - Observed evidence: the versioned JSON shape (schema_version/mapping_version/valid/items[{id,path,tree,native_status,attention_class,gate?,last_history_at}]/violations) + the canonical serialization profile (fixed key order, sort class-order->path->id, LF+one final newline, no timestamps/mtime/locale) are frozen in the module docstring and Section 8.5 of the spec; the exact byte-serializer is Order 03's implementation of this frozen profile (this child freezes the contract, not the renderer). Round-trip byte-identity is verified in Order 03 (A6). Contract present and unambiguous.
  - Result: pass
- [x] V-05 validates E-05
  ...
  - Observed evidence: `validate_gate_ref` accepts valid date/issue/artifact/decision/todo/external refs and rejects a non-http issue url, a `../`-escaping artifact path, an absolute path, and a malformed decision id (GateTests.test_gate_ref_validators pass). `is_safe_descriptive` rejects over-length, embedded-newline, and control-char (BEL, ESC) values (test_output_safety pass). `RULE_IDS` is a closed 15-id catalog, all `attention.`-namespaced (test_catalog_closed_and_named pass); `escape_detail` keeps the record one line (test_detail_escaping_keeps_one_line pass).
  - Result: pass
- [x] V-06 validates E-06
  ...
  - Observed evidence: `SPEC_TRANSITIONS` + `transition_allowed` encode the legal graph (test_legal_and_illegal pass: reviewed->approved and approved->implementing->implemented legal; draft->implemented and implemented->approved illegal). `TRANSITION_AUTHORITY["->approved"].human_token is True`, `["->implemented"].evidence is True`. `APPROVAL_FLOOR` states the mechanism must not be agent-satisfiable and a bare flag is INSUFFICIENT (test_authority_and_floor pass).
  - Result: pass
- [x] V-07 validates E-07
  ...
  - Observed evidence: `tests/fixtures/attnview/specs-valid/` has one spec per spec status (draft, to-review, reviewed, approved, implementing, implemented, deferred+gate, parked, superseded = 9). `tests/fixtures/attnview/violations/` has missing-status, unknown-status, status-trailing-prose, gate-missing, gate-malformed, gate-forbidden, history-missing, unsafe-field. The three non-committable classes (unreadable, unsupported-encoding, symlink-escaping-repo) are documented in the fixtures README and simulated in-test (test_simulated_unreadable_and_symlink builds a repo-escaping symlink in a temp dir). All committed fixtures parse/reject as intended (FixtureParseTests, 5 tests pass).
  - Result: pass
- [x] V-08 validates E-08
  ...
  - Observed evidence: `python3 -m unittest tests.test_attention_contract` -> `Ran 20 tests ... OK`, including the totality/coverage assertions (specs/plans/research mapping keys == native enum). Full suite: `python3 -m unittest discover -s tests -t .` -> `Ran 693 tests in 155.699s / OK (skipped=1)` (673 prior + 20 new). `aw sanitize --agent` exit 0 (leak-clean). No em/en dashes in the authored module/test/plan.
  - Result: pass
