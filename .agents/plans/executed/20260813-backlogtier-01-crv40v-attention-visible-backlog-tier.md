...
- 2026-08-13 approved (human maintainer via chat 'Approved. Go.', recorded by opencode Opus 4.8): cleared to execute. Status reviewed -> approved. Execution remains gated on the controlling spec 20260813-1833-01 reaching Status: approved (spec-gate override recorded above).
- 2026-08-13 execution refinement (opencode Opus 4.8, human-confirmed): the backlog item uses `- Field:` BULLET metadata (`- Id:`/`- Status:`/`- Set:`/`- Priority:`/`- Kind:`/`- Summary:` + `- Gate-Kind:`/`- Gate-Ref:` when blocked), NOT the research-style `---` block the spec/IPD drew literally. Reason: the approved intent (reuse the existing attention gate grammar GATE_KIND_RE/GATE_REF_RE and match the other attention trees) is only achievable with bullet metadata; specs/plans use bullets and the gate/status regexes are bullet-based (attention_contract.py:265/340-341). Surface-form correction only; the design intent is preserved.

- 2026-08-13 executed (opencode Opus 4.8 its_direct/pt3-claude-opus-4.8-1m-us): E-02..E-08 implemented + verified (commits 8c70be6, 4c9792f, 42b457d, 2a1d34b). backlog.py + aw backlog new|set|check; _BACKLOG_MAP + TreePolicy; SCAN_ROOTS + _backlog_record + mandatory _record_for branch (gate for blocked; parked auto-hidden); aw att alias; TODO.md migrated (16 open + 11 parked; 4 DONE dropped) + docs. All V-02..V-08 pass with concrete evidence + PR-001/PR-002 mutation probes RED->GREEN; full parallel suite exit 0; aw attention --check + aw backlog check clean; pre-transition lint conforming. Item format refined to  bullets (recorded above). Status approved -> executed; moved pending/ -> executed/.
...
- [x] E-03 Implement `agent_workflows/backlog.py` with `new`, `set`, `check` and wire them at the two `cli.py` edit points (`_build_parser` adds a `backlog` subparser; `_dispatch` routes), mirroring `aw research`/`aw specs`. `new` creates a conformant item (dry-run by default, `--apply` to write), owning the clustering name + frontmatter. `set` moves the file between `open/blocked/parked/done`, updates the `status`, requires `--gate-kind`/`--gate-ref` (written as the `- Gate-Kind:`/`- Gate-Ref:` bullets) when moving to `blocked`, and appends an append-only `## Workflow history` line (D52 style). `check` validates the tree fail-closed (frontmatter present + valid enums; status-mirrors-directory; `Gate-Kind`/`Gate-Ref` pair present-and-valid iff blocked and absent otherwise, reusing the existing gate validators; id6 present/unique; nonempty summary) with the `Drift`/`--agent`/exit convention. No `index`/`find` in v1 (spec OQ5).
  - Depends on: E-02
...
- [x] E-04 Register the backlog tree in the attention contract: add `_BACKLOG_MAP = {"open": READY, "blocked": BLOCKED, "parked": PARKED, "done": DONE}` to `CLASS_MAPS["backlog"]` in `attention_contract.py` (a PURE, TOTAL mapping; unknown status -> `attention.unknown-status`, never a default), and add `TreePolicy("backlog", ".agents/backlog", tracked=True, owner="aw backlog", reason="...")` to `TREE_POLICY` so `_classify_tree` recognizes the root (its `.aw/records/` -> `.agents/` normalization makes one root cover both layouts). GATE NAMING (review PR-003): the existing gate convention in `attention_contract.py` is capitalized front-matter BULLETS `- Gate-Kind:` / `- Gate-Ref:` with `GATE_KIND_RE`/`GATE_REF_RE` (attention_contract.py:340-341), used by specs. The backlog item (E-02) MUST use that SAME convention (reuse `GATE_KIND_RE`/`GATE_REF_RE` and the per-kind validators), NOT a divergent lowercase `gate-kind:` frontmatter, so there is one gate grammar across trees; update E-02's item format accordingly.
  - Depends on: E-02
  - Expected outcome: `class_of("backlog", s)` is defined for every backlog status and raises on unknown; `_classify_tree` maps a backlog file (either layout) to the backlog policy; backlog gate fields reuse the existing `Gate-Kind`/`Gate-Ref` grammar.
  - Execution state: performed

- [x] E-05 Make `aw attention` discover + record backlog items: add the backlog root(s) (`.agents/backlog` and `.aw/records/backlog`) to `SCAN_ROOTS` in `artifact_core.py`, and add a `backlog` branch in `attention.py` `_record_for` + a `_backlog_record` builder with `scan()` routing backlog files to it. The `_record_for` branch is MANDATORY, not optional (review PR-001): `_record_for` currently `return None, []` for any unmatched tree (attention.py:214), so a backlog file that `_classify_tree` recognizes but `_record_for` does NOT handle is SILENTLY DROPPED from the view with no drift - the exact invisibility this plan exists to prevent. `_backlog_record` reads `status`, `priority`, `summary`, `id`, AND the gate (review PR-002): for a `blocked` item it MUST populate `Item.gate = {"kind": ..., "ref": ...}` (via the `Gate-Kind`/`Gate-Ref` grammar) so the board renders `[gate kind: ref]` (attention.py:360-363) and the JSON carries it (attention.py:323); a `blocked` item recorded with `gate=None` loses the reason it is blocked. Mirror `_plans_record`/`_research_record`/`_spec_record` (the spec-record gate handling at attention.py:225-232 is the template). NOTE (verified during authoring): the hot-glance vs JSON split needs NO new code - `attention.render_board` (attention.py:354) ALREADY hides `DONE`/`PARKED` class groups unless `show_all`, and `render_json` always includes them. So once a backlog item maps to its class (E-04), `open`->`ready` and `blocked`->`blocked` show in the default board while `parked`->`parked` is auto-hidden (shown under `--all`, always in `--format json`). Do NOT build a separate exclusion filter. `priority` is carried into the record and used to order the `ready` group (high first) - confirm whether `render_board` currently sorts within a class; if not, add priority ordering for backlog `ready` items only, minimally.
  - Depends on: E-02
...
- [x] E-07 Migrate `TODO.md` into the backlog tree per spec OQ1, following an EXPLICIT procedure (the migration is human-in-the-loop for judgment calls, NOT a mechanical sweep - the current TODO.md is NOT flat; see the per-item rules). Procedure:
...
- [x] V-02 validates E-02
  ...
  - Observed evidence: `pytest tests/test_backlog.py::BacklogVerbTests::test_new_creates_conformant_item_that_checks_clean ::test_check_fails_closed_on_status_dir_mismatch ::test_check_rejects_gate_on_non_blocked` -> `3 passed`. Real-repo: `aw backlog new --apply` wrote conformant items; the tree `.agents/backlog/{open,blocked,parked,done}/` + `README.md` exist; `aw backlog check` = "all backlog items conform" (exit 0), and returns exit 1 with a `backlog.status-dir-mismatch` / `backlog.gate-unexpected` line on planted malformed items. Impl commit 8c70be6 (agent_workflows/backlog.py validate_item).
  - Result: pass
- [x] V-03 validates E-03
  ...
  - Observed evidence: `pytest tests/test_backlog.py::BacklogVerbTests` -> `9 passed`, covering new (create), set (moves file open->done, preserves the `created` history line + appends the `set` transition record), blocked-requires-gate (rc 2), set-to-blocked-with-gate (writes `- Gate-Kind:`/`- Gate-Ref:`), status-dir-mismatch/duplicate-id/gate-on-non-blocked fail-closed. CLI wired at cli.py (`aw backlog new|set|check`); real-repo `aw backlog new/check` verified. Impl commits 8c70be6 + 4c9792f.
  - Result: pass
- [x] V-04 validates E-04
  ...
  - Observed evidence: `pytest tests/test_backlog.py::BacklogMappingTests` -> `3 passed` (map covers every status; the four class values; `A.class_of("backlog","frobnicated")` raises `UnknownNativeStatus` - falsifiable). Verified `_classify_tree(".agents/backlog/open/x.md")` and `_classify_tree(".aw/records/backlog/open/x.md")` both -> `backlog`. Gate fields reuse `GATE_KIND_RE`/`GATE_REF_RE`. Impl commit 8c70be6 (attention_contract `_BACKLOG_MAP` + TreePolicy).
  - Result: pass
- [x] V-05 validates E-05
  - Required evidence: with fixture backlog items, `aw attention` shows open/blocked in the board and `/whatnext`; `parked` is absent from the default board but present in `--format json`; `aw attention --check` clean. A `blocked` fixture item RENDERS its gate (`[gate <kind>: <ref>]` in the board and `gate` in the JSON), proving `_backlog_record` populates `Item.gate` (PR-002). Mutation-probe A: an item with an invalid status yields `attention.unknown-status` (fail closed), not a silent default. Mutation-probe B (PR-001): with the `_record_for` backlog branch removed, a backlog item is SILENTLY DROPPED from the view (RED - it neither appears nor raises), proving the branch is load-bearing; restored -> item reappears (GREEN).
  - Observed evidence: `pytest tests/test_backlog.py::BacklogAttentionTests` -> `5 passed`: scan includes backlog with correct classes; a blocked item carries `gate={"kind":"artifact","ref":...}` and the board renders `[gate artifact: ...]` (PR-002); the default board hides parked (`## parked (N) [hidden; use --all]`) while `render_json` includes it; and `test_record_for_branch_is_load_bearing` is the PR-001 mutation probe (patched `_record_for` backlog branch to `return None,[]` -> backlog items vanish with NO drift RED; restored -> reappear GREEN). Real-repo `aw attention` shows the 16 open items under `## ready` and hides the parked group. Impl commit 8c70be6 (artifact_core SCAN_ROOTS + attention.py `_backlog_record`/`_record_for`).
  - Result: pass
- [x] V-06 validates E-06
  ...
  - Observed evidence: `diff <(aw att) <(aw attention)` = IDENTICAL (alias equivalent), after fixing the dispatch to match both command names (argparse aliases do not normalize `dest`; the alias initially fell through to help - caught + fixed). `aw att --check` -> "the view is valid." `aw attention --all` shows the `## parked` group that the default board renders as `[hidden; use --all]`. No `aw attention all` subcommand added (the existing `--all` flag covers OQ2). Impl commit 4c9792f.
  - Result: pass
- [x] V-07 validates E-07
  ...
  - Observed evidence: migration created 16 `open/` + 11 `parked/` items (commit 2a1d34b); before->after: Known bugs (2) + Security (2) + committed Planned-next (12) -> open; Consider (11) -> parked; 4 DONE-marked items dropped as already-shipped (D88/D91 + executed plans, recorded in the TODO Notes migration-provenance). `aw backlog check` = conform; `aw attention` `## ready (25)` (16 backlog open + others) and `## parked (25) [hidden]`. `TODO.md` reduced to a pointer + Notes; `.agents/backlog/README.md` added; AGENTS.md "What needs attention" gained the backlog pointer (engine.agents_pointer_prose) and this repo's managed block was regenerated. Split-brain-install item filed `open` (evidence: it is a pure-constant guard, independent of Order 11).
  - Result: pass
- [x] V-08 validates E-08
  ...
  - Observed evidence: `python3 -m pytest tests/ -n 12 -q` -> exit 0 (full suite green, incl. tests/test_backlog.py's 17 tests + the updated tracked-tree-set assertion in test_attention_contract.py). `aw attention --check` -> "the view is valid." `aw backlog check --dir .` -> "all backlog items conform." Test commit 42b457d.
  - Result: pass
