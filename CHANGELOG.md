...
- Added: a portable concise-reporting default for the AGENT's own user-facing prose, so a routine answer leads with the outcome and drops preambles, routine narration, praise, recaps, and closing offers. One host-neutral contract (`agent_workflows/reporting_contract.py`) is delivered four ways: a separately owned `aw:reporting` managed section in `AGENTS.md` (mirrored into an existing `CLAUDE.md`/`GEMINI.md`), a one-line pointer in every generated OpenCode and Claude command shim, and the full text in both IPD drivers' worker and verifier prompts. Concision governs REPORTING only: a workflow that mandates a report (for example `/plan-review`'s findings table or `/release-review`'s final report) still produces it in full, required runner output is still pasted, and structured outcomes, safety warnings, and durable artifacts stay complete. The section is separately owned, so declining it or editing its body is preserved across updates and does not affect the sibling `aw:pointer` section. Command shims carry a POINTER rather than a copy on measured grounds (duplicating the prose into all 48 shims would have tripled the shim corpus that hosts re-read on every invocation), and a size-budget test keeps that from regressing. See `docs/reporting-contract.md`.
...
- Fixed (BEHAVIOR CHANGE): `aw oc run A B` and `aw agy run A B` now run A before B. The order you ask for was recorded and then discarded when the queue was dispatched, so two independent plans in different Sets were run in alphabetical Set order instead, silently. In one measured run the maintainer asked for two plans in a specific order and the runner executed them in the opposite one, and the only way to notice was to compare event timestamps against the stored queue afterwards. The order you request is now authoritative among items that are equally ready. A declared `Item-Dependencies` prerequisite still wins, exactly as before: if a plan you named first must wait for one you named later, it still waits.
- Added: `aw oc integrate <id6>` and `aw agy integrate <id6>` (also spelled `aw oc run integrate <id6>` and `aw agy run integrate <id6>`), which recover a lane that already finished its work, passed its verification and finalized its plan but then failed to merge back to main. Before this, the only routes were a full agent re-run, which pays for work that was already done, or a hand merge outside the tooling, which leaves no record. The verb costs no agent turn. It does re-verify, deliberately: it runs the repository suite in your main checkout and puts the merge through the same gate an in-run integration uses, because a lane verified against yesterday's main is not verified against today's. A failing suite, a real conflict, or an uncommitted local change the merge would overwrite refuses the attempt, leaves main untouched and leaves the lane exactly where it was. The lane is read from the run's own record rather than guessed from the id, so a lane that was allocated under a second-attempt name is found and merged as itself.
- Changed: resuming a run now merges such a lane automatically, before it can consider spending an agent turn on it. This is not behind a flag, because the previous default was worse than doing nothing: a plain resume ignored the item entirely, while `--retry-incomplete` re-ran it from scratch and, because the finished lane still held its commits, allocated a SECOND lane and abandoned the first. That is measured, not hypothetical: one item in this repository accumulated three lanes across resumes and about forty dollars of finished work had to be recovered by hand from the third. A resume now merges the existing lane, reports that it did so with no turn spent, and if the merge cannot be made safely it says which item it held back and why instead of re-running it. `--retry-incomplete` still retries items that genuinely need another turn. Note the new cost: a resume with such a lane runs the suite once per lane before it dispatches anything, which is the right trade against a paid turn but is not free.
- Added: both runners now announce the order they will execute in, before the first agent session starts, and warn loudly when that order differs from the one you requested. The warning names both orders and, for each item that moved, why it moved, distinguishing a real declared prerequisite (expected) from an ordering tiebreak (the case worth a second look). The comparison is also recorded in the run's durable state and event log, so the order is auditable from the run record rather than reconstructed from timestamps. The wording is honest about what it can claim: for a selector that expands to many items (a Set id, `all`, `reviews`, or a file path) the order comes from the manifest, not from your typing, and the message says "requested order" rather than implying you typed it.
- Fixed: the run summary table (including `--prepare-only`, which is how you preview a run) showed the queue's frozen identity numbers under a `#` heading that read as sequence, so a careful preflight could show one order while the run executed another. The table now lists rows in execution order and shows both numbers in separate labelled columns: `Run` is the order the run will execute in, `Pos` is the stable identity that outcome and session filenames key on.
- Changed: the live event stream from `aw oc run` and `aw agy run` now says what the agent actually did, in aligned columns. Each line carries a tool class prefix (`read`, `write`, `edit`, `find`, `todo`, `diag`, `subagent`, `bash`) whose payload starts at one fixed column, so the stream scans as a table instead of a ragged list. An edit reports its real line counts (`+12, -3`), a write reports whether it created or overwrote the file and how many lines it wrote, and a task list reports transitions (which task finished and which started) instead of a bare item count. Paths are shown relative to the repository instead of absolute or stripped to a bare filename. The per-status glyph is kept alongside the new prefix, since outcome and tool class answer different questions. On a terminal that renders the geometric shape glyphs double width, a narrow safe prefix set is available so alignment stays exact.
- Added: verbosity tiers for that stream, on both runners and on both `start` and `resume`. The default keeps the operations that change your repository and hides file reads and searches, which do not. `-v` shows reads with the line range they read and searches with their hit count. `-vv` also shows diff hunks and the diagnostics a tool reported. `--quiet` and `--raw` are unchanged and take precedence, since they choose which renderer runs at all. The tier is recorded with the run, so a resume keeps the setting unless you pass a new one.
- Fixed: an error event from the child agent produced no output at all. The stream dropped the event silently, so a failure that the agent did report could look like nothing happening. Error events now render at every verbosity level, including the quietest.
- Fixed: `-v` was reserved as an abbreviation of a `--version` flag that neither runner actually had, so it was treated as a command name rather than an option. `aw oc run -v <selector>` failed with a confusing complaint about the selector, and `aw oc run -vv <selector>` failed differently. Both now work, in leading or trailing position. There is still no `--version` on the runners.
- Added: `/askme`, a workflow that puts the open decisions in front of you. It sorts a plan's open questions into what the repository itself can answer, what only you can decide, and what can genuinely wait; asks yours one interactive prompt at a time in plain English, with enough context to decide from the prompt alone and a labelled recommendation; and records each answer in the artifact that raised the question so it survives the session. Under a runner there is nobody to ask, so it records and defers instead of blocking.
- Fixed: `/askme` asked a question its maintainer could not understand, so its instructions now make an unreadable question fail before it is asked. Four changes, each from a measured failure. It must state what is going wrong before it offers you any options, because a question that describes only the proposed repair leaves you nothing to weigh the options against. It may no longer replace an internal name with a placeholder like "a pair of helpers": dropping a name now obliges it to say what the thing actually does, since a named thing can at least be looked up while an anonymous one cannot be pictured at all. It must check whether the plan or spec raising the question already states the rule for deciding it, and if so answer it rather than asking you to repeat yourself. And if you reply that you did not understand, including by typing your own answer instead of picking one, that is now treated as a broken question: it is thrown away and asked again, never recorded as your decision. The final checklist also gains one item that is deliberately not mechanical, asking whether a reader who has never opened the repository could restate the problem, the choice, and what each option would cost. The nine checks it already had were all the kind a machine could make, which is exactly how an unreadable question passed every one of them. That item is a self check, so it is honest about its limit: it cannot prove a question is clear, and only your reply can.
- Changed: an unresolved BLOCKING open question in a plan (`Blocking: yes` with `Status: open`) is now refused by `aw ipd lint` at EVERY checkpoint, not only at `pre-execution`. Previously a default lint reported such a plan as conforming, so a plan could be authored, handed over, and reported complete with a question its own author had marked load-bearing never actually asked. The refusal names the remedy: ask and record the answer, or set `Blocking: no` if it does not really block. Non-blocking open questions are unaffected and remain legal at every checkpoint, since that is the normal state of a plan in progress.
...
- Fixed (BEHAVIOR CHANGE): `aw find research <status>` now returns the research documents that actually carry that status, which is roughly ten times as many as before. A research document records its metadata as YAML between `---` fences, while every other record type uses `- Key: value` bullets, and the selector understood only the bullet form. So for research the id, set, and status rules could never match anything and every research search quietly fell back to matching the FILENAME. That produced a short, plausible answer with nothing to tell you the metadata had never been read: `aw find research reference` reported 5 documents while 64 carried `status: reference`, and `aw find research archive` and `aw find research todo` reported none at all while 32 and 19 existed. Searching by a research document's id or set was filename-shaped in the same way. Expect noticeably larger result sets for these three kinds of research query; that is the correction, not a regression, and the counts above are what the repository held when the fix shipped. Nothing changes for any other record type: plans, specs, backlog, releases, reviews, prompts, walkthroughs, comms, and roadmaps resolve byte for byte as before, verified record by record across all 1642 of them. No command that MOVES or MODIFIES a file changes behavior either, which was checked rather than assumed: `aw archive`, `aw rename`, and `aw group` reach a research document by its id or its set, never by its status, and their output is identical before and after. The bullet form is still tried first everywhere, so no record that matched before matches differently now.
